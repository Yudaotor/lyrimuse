...
private let logger = Logger(subsystem: "com.chenyuhao.applemusic-desktop-lyrics", category: "config-store")

// 推送提醒目前支持的平台——绝大多数都是"群机器人 webhook"这个模子,一个 URL、POST
// 一份 JSON 就能收到消息(Server酱是个例外,走表单编码,见 collector/notify.go 顶部
// 注释),desktop-lyrics 这边只管选平台+填 webhook 地址,不关心具体协议。rawValue
...
// 用户刚敲进去几个字符、还没点保存,状态就会被误判成"已配置/生效中",这是三方设计提案
// 交叉评审时specifically指出的一个真实隐患,这里刻意避开。
@MainActor
public final class ConfigStore: ObservableObject {
    ...
    public func load() {
        ...
        // 字段上的注释,两边保持一致,不单独给 desktop-lyrics 这边改名。
