...
- **Eleven lyrics sources checked automatically** — NetEase Cloud Music, QQ Music, Kugou, Kuwo, Migu, Musixmatch, LRCLIB, LyricFind (via YouTube Music), Deezer, AMLL (a hand-curated, word-by-word lyrics database), and Apple Music's own lyrics (optional — connect your account in Settings; the only official source, with word-by-word timing on many tracks) — always picks the best match, no manual searching required
