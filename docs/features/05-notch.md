# 05. 灵动岛歌词
> 最后核对:2026-09-14 · 基线:d672cd3+工作树(2026-09-14 菜单栏快捷面板的灵动岛那格补进「副行」「字号」、两根宽度滑杆合成一根双滑块,见 06 章「左键面板」与决策 29)
...
- **耳朵模块可配**(2026-08-31,`NotchEarModule` + `notchLeftEar`/`notchRightEar`,入口是编辑台工具栏上**左右各一个**的「左耳」「右耳」浮层)。八选一:**歌名 / 歌手 / 专辑 / 封面 / 播放控制 / 已播时长 / 剩余时长 / 不显示**,默认 `左=封面 右=不显示`(2026-09-07 起,见「默认值的一次整体校准」;此前是 `左=歌名 右=歌手`,那一版跟可配之前那套写死的排布**逐像素一致**)。
  ...
  - **但音浪「独占」这只耳朵时改为居中**(2026-09-15,用户报「音浪在耳朵位置没有居中」):这只耳朵的模块是「不显示」(收起态同理,那时模块固定 `.none`)、音浪是里面唯一的东西时,它不再贴外缘,而是居中于**可视的那一段耳朵**(刘海边沿 → 卡片外沿)。耳朵里还有模块时照旧贴外缘 —— 那时音浪跟模块是一组,而文字跑马灯溢出时必须从外缘起排,居中会把开头几个字挂到容器外面。
    ⚠️ **hover 展开态不居中,一律贴外缘**(同日用户看到展开卡当场否掉:「展开状态也变为居中了，这个不符合预期哈，展开要在最边上；之前只说缩小状态的位置不太美观而已，有点偏右」)。居中修的是**稳态那 2.25pt 的偏心**;展开态耳朵一下子宽出一大截(默认稳态 252 / 展开 482 → 单耳 26.5 → 141.5),同一个公式推出来是 **58.25pt**,那就不是「正正好」而是「飘在中间」。换句话说这条**只对窄耳朵成立** —— 耳朵宽到音浪在里面找不着北时,贴外缘才是那个有位置感的答案。判据用**状态**(`isExpanded`)而不是给宽度设阈值:阈值要挑一个挑不出理由的数,而「展开」本身就是用户心里那条线。`soloEqualizerInset` 因此多收一个 `expanded:`(**必填,不给默认值** —— 每个调用点都得自己表态),selftest 里那组断言同时钉住「不看状态会推 58.25」和「看状态则为 0」,以及「同一组几何、展开与否结果不同」这条判据身份。
    实测(截图逐列亮度,258pt 卡片 / 179pt 刘海):改前音浪落在 839.0~854.0(宽 15.0,= `EqualizerBars.width` = 5×1.8 + 4×1.5),而可视耳朵是 824.5~864.0、中心 844.25 —— 音浪中心 846.5,**偏外 2.25pt**,两侧留白 14.5 / 10.0;改后 836.75~851.75,两侧各 12.25。
    ⚠️ **成因不是 alignment 写错,是布局容器比可视区窄**:耳朵的 `.frame(width: earWidth)` 到内容区边界就结束了,外面还有 `topRow` 那层 `cardHorizontalPadding`(10pt)才到卡片外沿;贴外缘 = 贴内容区边界,于是外侧多留了那一截的一半。所以**不能**图省事把 alignment 换成 `.center` —— 那是居中于 `earWidth`,会偏**内** 5pt,比原来错得更多。要补的是两侧边距之差:`(earWidth − 音浪宽 − cardHorizontalPadding) / 2`,夹 0 给窄卡片兜底(装不下时退回贴外缘,不能变成负 padding 把音浪推出卡片)。公式在 `NotchWidthBounds.soloEqualizerInset`,跟 `NotchExpandedMetrics` 同一个理由下沉到 LyrimuseCore:selftest 只依赖那个 target,而这是要被断言钉死的几何契约(14 条,含"居中于 earWidth ≠ 居中于可视耳朵、两者正好差半个 `cardHorizontalPadding`"这条反例哨兵,以及锁住 10 / 5·1.8·1.5 两组常量的源码契约 —— selftest 够不着 app target 的那两个数,只抄不钉的话常量一改就会变成在测一组不存在的几何)。
...
- **兜底入口**多一根「展开宽度」单滑块(step 10):「全部设置」抽屉 `expandedWidthRow`。(⚠️ **菜单栏快捷面板 2026-09-14 起不在此列**:那边两根单滑块已经合成一根 `RangeSlider` 双滑块,跟编辑台同形态、两只共用 `usableWidthRange`,不再用 `usableExpandedWidthRange`;起因与取舍见 06 章决策 29。)区间 `usableExpandedWidthRange(steadyWidth:)`:下界 = 稳态**真实**宽(向上取整到 step、不越过 `500 − step`,稳态拉满时不给 Slider 零长区间),上界 500;读数 `effectiveExpandedWidth(steadyBase:expandedBase:)`。稳态那根「宽度」的写回也改走 `commitWidths(steady:)`,拖过展开时展开跟着上去。
...
| 歌词显示 → 灵动岛 → 舞台里的双滑块调整条(右滑块);菜单栏面板快捷设置里的双滑块(右滑块,2026-09-14 起) | 展开宽度 | `notchExpandedContentWidth`(默认 482,≥ 稳态,上限 800;2026-09-07 前跟稳态共用一个常量、同为 360) | hover 展开后卡片撑到的宽度,也是窗口常驻宽度;不变量「展开 ≥ 稳态」(`NotchWidthBounds`),两值相等 = 展开不加宽;「重置」不碰;见「宽度 → 展开态可以更宽」 |
