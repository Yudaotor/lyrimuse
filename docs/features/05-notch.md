...
- **任一浮层开着时预览钉在展开态**(`keepsExpandedForPopover = popover != nil`,不分哪个浮层、从哪开的;第二轮只钉「展开态」那个,第三轮用户要求扩成全部):从卡片上点开浮层时卡片本来就是展开的,浮层一开它缩回去就是"选中的东西自己变形了";展开态是稳态的超集,钉着不会让任何浮层要改的东西看不见。浮层一关就放开,指针若还在卡片上下一次 hover 事件会再展开。
...
23. **预览卡可点,但点的是"打开管这块的浮层",不是把真控件放开(2026-09-06,用户提出;同日按用户反馈改过一轮)**。四条纪律:①预览不产生副作用(命中区只写 `popover` / `popoverAnchor`,真视图仍 `allowsHitTesting(false)`);②**一个浮层只算一块区域**(首版按渲染段落切成七块、四块开同一个「展开态」,用户「拆的太密」;现在一块可以由多个矩形组成,被歌词行隔开的头部和展开区算一块);③浮层锚在**被点的区域旁、从侧边弹**(首版锚工具栏按钮,用户要「从我点击的这个位置弹出来」;不从下方弹是因为会压住卡片);④浮层开着期间预览一直钉在展开态(第三轮,任一浮层)。区域边界按用户的语义分、不按渲染树分:「下一句预览」渲染上属于展开区,但归「歌词行」那块(「选中歌词行的时候,应该把下面那个下一行歌词也包进去」)。区域几何**必须**取自跟渲染同一份的度量(`contentTopInset` / `expandedTrackInfoHeaderHeight` / `compactRowHeight` / `cardHeight`),不另写数字 —— 本章反复出现的"两处各写一份必然漂",在这儿的表现会是"高亮框跟内容差半行"。锚点视图放在缩放组**外面**按比例换算位置,因为 SwiftUI 的几何效果不改布局 frame 而 NSPopover 认布局 frame。顺带补了「展开态」浮层开着时预览钉在展开态的洞:那批开关改的全是展开区,指针去点浮层卡片就缩回去,以前从工具栏开也看不见,只是现在这条路成了必经之路才不能再拖。
