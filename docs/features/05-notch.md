...
- **播放指示条**(`EqualizerBars`):4 根纯装饰跳动条,与音频数据无关;0.28s 换一次目标高度、补间时长只取间隔一半(2026-08-19:原来 duration==interval 首尾相接零空档,播放期间把窗口钉死在持续合成态),暂停时静止在 2.5pt 不归零;2026-08-17 起固定开启不可配。
