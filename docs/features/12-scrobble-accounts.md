...
| 账号→Last.fm→设置 | Scrobble 的播放器（逐播放器勾选 + 已信任播放器各一行开关） | `lastfm_excluded_bundles`（features.json，bundle id 列表，缺失/空 = 全部上送；只挡 Last.fm 的 scrobble 与 now-playing，ListenBrainz 不受影响）。见 §1「Scrobble 的播放器」 |
