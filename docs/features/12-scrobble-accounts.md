...
| 账号→Last.fm→设置 | Scrobble 的播放器（内置播放器 + 已信任播放器同一排图标芯片，点图标勾选） | `lastfm_excluded_bundles`（features.json，bundle id 列表，缺失/空 = 全部上送；只挡 Last.fm 的 scrobble 与 now-playing，ListenBrainz 不受影响）。见 §1「Scrobble 的播放器」 |
