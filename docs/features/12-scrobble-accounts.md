...
- **按播放器排除（2026-09-10，借鉴清单 S10）**：设置 → 播放器 →「收听历史」卡按播放器勾选「计入收听历史」，默认全勾。取消的播放器写进 features.json 的 `history_excluded_bundles`（bundle id 列表；内置播放器用各自的 bundle id，信任列表里的 App / 浏览器用它们自己的；Safari 的媒体进程 `com.apple.WebKit.GPU` 经 `mediaProxyOwners` 归到 `com.apple.Safari` 再查），collector 开会话那一拍算一次 `historyExcluded`（historygate.go）存进 `playSession.historyExcluded`，与 `isAd` 同一批出口：`submitSingleAsync`（LB single、Last.fm 镜像、本地收听日志、relay 最近播放全挂在它后面，挡住这里就一起挡住）、`announce`（LB playing_now 与 Last.fm now-playing）、退出兜底那条同步刷写。**不挡**：歌词显示与解析、网页中继的「正在播放」状态卡（那是状态不是历史）、iPhone 桥接。粒度是 bundle：collector 侧不知道浏览器里放的是 YouTube Music 还是 Spotify 网页版，配对的浏览器只能整个开或关。缺失 / 空 = 全部计入，老配置行为不变。
...
| 播放器 tab | 收听历史 · 计入收听历史（逐播放器勾选 + 已信任播放器各一行开关，2026-09-10） | `history_excluded_bundles`（features.json，bundle id 列表，缺失/空 = 全部计入）。见 §1「按播放器排除」；App 侧入口的细节见 02 章设置项表 |
...
10. 按播放器排除做成「会话开头算一次的标记」而不是每个出口现查（2026-09-10，借鉴清单 S10）：一首歌中途改设置不该把同一次收听切成「前半记、后半不记」，也不该让挂着等 scrobble 时点的那条 Last.fm 收听在 settle 时凭空消失；标记跟 `isAd` 同一个位置、同一批出口（`submitSingleAsync` / `announce` / 退出兜底），将来再加一个「这次收听不算」的理由照这个模子放。粒度按 bundle 而非网页平台：collector 没有平台身份（02 章的平台配对只在 App 侧），硬要区分就得再开一条 App→collector 通道，S10 的诉求（把某个 App 整个排除）不值这一条。网页中继的「正在播放」状态卡刻意不挡——它是此刻的状态，不是历史。
