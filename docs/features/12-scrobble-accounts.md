...
- 设置 → 账号：四个目的地卡片——**ListenBrainz**、**Last.fm**、**网页推送**（stateRelay）、**推送提醒**（Bark），各自的连接状态/配置入口（`AccountLinkingTab`；Last.fm 在设置侧栏顶部是身份区 `LastfmIdentityRow`——圆形头像 + 用户名 + 「Last.fm 账户」，2026-09-12 起，见 14 章决策 #24；其余三个在折叠的「实验室功能」区，行尾有状态点）。
...
- **⚠️ `playSession.meta` 是开会话那一拍的快照，凡是「异步才到」的字段都必须显式回填**（2026-09-11 定，被电台打卡全丢这件事逼出来的）。会话建立时执行的是 `p.sess = &playSession{key: key, meta: p.cur, …}`——`meta` 就此定格，`handle()` 里此后只回填过两个字段：专辑（`AlbumHint`，回填晚一拍到的专辑名）和电台真曲长（`needsRadioDurationBackfill`）。**阈值、短曲目闸、Scrobble 时点百分比档全都读 `sess.meta.Duration`**，所以一个晚到的时长不是「精度差一点」，而是整类收听永久丢失：`listenThreshold(0)` 不报错，只是把门槛悄悄抬到 240s，而电台曲目普遍 2~4 分钟——实测电台上 4.5 小时**一条都没记上**。实证与完整链路见 [02 章「电台」](02-playback-source.md#apple-music-电台--直播流2026-09-10--09-11-实测)。
  - 这类缺陷的共同特征是**没有失败信号**：没有一行日志说「这次没记上」，`loopRestart` 每拍触发也只是正常的 INFO。真正把它照出来的是「该有多少 vs 实际有多少」的**比值**（那 4.5 小时里 `loop restart` 2397 次 : `listen recorded` 4 条）。以后往 `meta` 加字段时先问一句：这个值在开会话那一拍**一定**已经就位吗？答不上来就照 `AlbumHint` 那三行的形状加一处回填。
- **Scrobble 的播放器（只管 Last.fm，2026-09-10，借鉴清单 S10）**：`lastfm_excluded_bundles`（features.json，bundle id 列表，缺失 / 空 = 全部上送；设置入口 账号→Last.fm→设置→Scrobble 卡「Scrobble 的播放器」，内置播放器与已信任的 App / 浏览器摆在**同一排**图标芯片里，点图标勾选；副标题按状态说人话——全勾「全部勾选」、全不勾「全部不 scrobble」、部分则只列被排除的那几个。第一版把信任项做成一人一行的开关，四个浏览器就吃掉五行、比整张卡其余部分加起来还高，同日按用户「这一块占用区域太大了…收拢到一起」收成一排）。用户原话（2026-09-10）：「只控制 lastfm 的上送」——与短曲目、Scrobble 时机、合唱歌手三项同一口径。机制：collector 开会话那一拍算一次 `lastfmExcluded`（lastfmexclude.go，与 `isAd` 同一个位置）存进 `playSession.lastfmExcluded`，挡两处 Last.fm 出口——`recordLastfmListen` 直接标 `lastfmSettled` 返回（不挂 pending、不 `mirrorScrobbleTracked`、不写给它兜底的本地收听日志，退出兜底那条同样不挂），`announce` 里 `track.updateNowPlaying` 那次镜像跳过。**不挡**：ListenBrainz 的 single / playing_now（照旧在官方阈值那一刻提交）、relay 网页中继与「正在播放」状态卡、歌词显示与解析、iPhone 桥接。粒度是 bundle：collector 侧没有网页平台身份，配对的浏览器只能整个开或关；Safari 的媒体进程 `com.apple.WebKit.GPU` 经 `mediaProxyOwners` 归到 `com.apple.Safari` 再查。**改这一项不重启 collector**（2026-09-10 第二轮）：这是全仓第一个进 `CollectorRestartPolicy.hotReloadedKeys` 白名单的键，collector 按 features.json 的 mtime 热重读它（`currentLastfmExcludedBundles`，写法照搬 `lyricspins.go` 的 `lyricsPinned`：每次调用只 Stat 一下，mtime 或大小任一变了才重新解析，且只解析这一个键）；App 侧 `save()` 发现这次改动的键全落在白名单里就跳过 `CollectorRestartCoordinator`，那句「正在应用到后台服务…」也就不出现了。生效时机仍是**下一首**（`playSession.lastfmExcluded` 照旧在开会话那一拍取一次），区别只是不再需要等 collector 重启完。
...
`lbClient.submit`（playing_now / single），additional_info 带 media_player（如实按播放器报）、source=mac（桥接 iPhone 覆盖为 iphone）、duration_ms、进度锚（自跟踪 Position+AnchorTS，不用 media-control 的 timestamp——连播时冻结、跨休眠会漂）；**Spotify 原生播放另带三个标准字段**（2026-09-09）：`spotify_id`（LB 官方定义「这条录音的 Spotify 曲目 URL」，LB 拿它做匹配与元数据补全）、`origin_url`、`music_service=spotify.com`，形状跟 LB 自家 Spotify 导入一致。曲目 ID 来自换曲那一拍 collector 本来就要跑的 AppleScript `spotify url`（`spotifyCurrentTrackURI`，广告判据同一次脚本），存进歌词缓存的 `spotify_track_id`；ID 是录音级身份、缓存里跨播放器共用，但「在哪个服务听的」按当次播放的 bundle 算，Apple Music 等别的播放器放同一首歌时这三个键一个都不写（`spotifyListenFields`，纯函数、单测）。原来那个非标准键 `spotify_url` 照旧发，它装的现在也是真曲目链接（没在 Spotify 上放过的歌仍是搜索页链接）。token 在设置里配置并校验（`ListenBrainzTokenCheck`）。未配 token 时 collector 纯本地运行（歌词/封面照常）。
...
- ⚠️ **上送口径：歌手 / 歌名 / 专辑一律原样发播放器报的标签，不做任何替换**（2026-08-31 定，`lbMeta`）。
  ...
  第二个例外（2026-09-08，**补缺不是替换**）：播放器**没报**专辑名时，用 Apple 目录按「署名 + 曲名 + 时长」反查到的专辑**补空**发出去（`snapshot.albumForUpload`，四个出口：relay 网页 / Last.fm `album` / LB `release_name` / 本地收听日志；歌词缓存 key 不动）。报了就一个字不动；查不到就照旧发空。判据与缓存见 02 章「专辑名回填」与决策 27。
...
- **补提交之后最近记录马上刷新**（2026-09-03，用户要求「刚连上补提交之后马上刷新一次最近记录」）。feed 时代最近记录的主来源是 collector 落盘的 feed（15 s/60 s 一拉），而回填是 App 起的**独立子进程**，进程内的 `requestLastfmFeedRefresh` 够不着常驻进程；此前只有 App 侧一发 `refreshBaseline(force:)`，且它紧跟在批量提交之后，Last.fm 多半还没把刚收的 scrobble 并进 recenttracks，于是要等下一个周期才看到。现在两层：① 跨进程信号文件 `lyrimuse-lastfm-feed-nudge`（`lastfmFeedNudgePath`，main.go / backfillcli.go 同一路径）——`runBackfill` 在 `accepted > 0` 时 touch，常驻进程 `bridge()` 每拍 stat 一次，文件在就消费掉（删除）并**排一个 `backfillFeedNudgeDelay`（5 s）之后的拉取**，App 靠 5 s 一次的 mtime 轮询几秒内拿到（`TestLastfmFeedNudgeFile` / `TestBackfillFeedNudgeIsDelayedNotImmediate`）；② App 侧 `ScrobbleBackfillService.runBackfill` 在原有那发强刷之外，8 s 后再补一发**只在 `feedIsFresh == false` 时**发的强刷——⚠️ 这一发实际**只兜 collector 不在**的情况，见下面第 3 条。`accepted == 0` 两层都不动——Last.fm 侧什么都没变。

- **信号文件当场拉 = 拉到旧内容，还会把兜底闸门永久关上**（2026-09-12，用户**第三次**报「补提交之后下面的列表没刷新」）。①那条信号原来是「文件在就**立刻**拉一次 feed」，而 Last.fm 把刚收到的 scrobble 并进 recenttracks 要一两秒——拉回来的是旧内容。真正的代价不在这一次白拉：feed 每次写都会把 `fetchedAt` 刷成此刻，而②那发兜底强刷的判据 `LastfmStatsService.feedIsFresh` 看的正是 `LastfmRecentFeed.isFresh`（`fetchedAt` 落在 180 s 窗口内）。于是「当场拉旧内容 → fetchedAt 变新 → 兜底判定 feed 新鲜 → 不补刷」。更糟的是这条闸门**本来就几乎永远关着**：collector 只要活着就每 `feedHeartbeat`（60 s）重写一次 feed，`fetchedAt` 跟内容有没有变、有没有包含刚补的那几条毫无关系——也就是说②从 2026-09-03 加进来那天起，在 collector 正常运行时**就是死代码**。两头一叠，用户只能干等下一个 15 s/60 s 周期。修法是让①跟进程内那条镜像 scrobble 路径对齐：`requestLastfmFeedRefresh(backfillFeedNudgeDelay)`，`bridge()` 消费掉文件但这一拍不拉。为什么镜像 scrobble 那条一开始就对——它的头注写着「不早于 after：Last.fm 把刚收到的 scrobble 并进 recenttracks 需要一两秒，立刻拉多半还看不到」，跨进程这条照抄路径时把**延迟**这个关键部分丢了。判据是行为不是实现：`TestBackfillFeedNudgeIsDelayedNotImmediate` 构一个「周期没到 + 信号文件在」的 `bridge()` 拍子，断言这一拍 `bridgeFetching` 仍为 false、文件已被消费、且延迟到了之后 `lastfmFeedNudgeDue` 恰好到期一次（倒回旧实现这条会红，已实测）。

- **补提交之后必须有一句话**（2026-09-12，用户报「补提交之后没有反馈」）。结果那句「已补 N 条」原来只长在 `pendingListensRow` 的副标题里，而那一行的出现条件是 `eligible > 0`（`lastfmProfileCard` 里的 `if`）——于是**全部补成功**（`eligible` 归 0）恰恰成了唯一什么都看不到的情况：点一下，整行连同结果一起从界面上消失，跟没点过一样。**补得越干净、反馈越少。**同源的另外两个洞一并堵上：② `Self.run` 返回 nil 时 `lastRun` 也是 nil，跟「还没跑过」撞成同一个状态，界面一声不吭——现在用 `ScrobbleBackfillService.lastRunFailed` 把两者分开（⚠️ 当时把这个 nil 归因成「子进程压根没跑成」，**归因错了**，真正的成因见下一条）；③ `abortedReason` 从加进 `Outcome` 那天起就**没有任何显示面**，而限流(29)、凭据失效、服务端拒收整批全走它。现在 `backfillRunResultText()` 一处产出这句话，两处消费：清单还在时作副标题、清单空了时作独立的 `backfillResultRow`（✓/⚠ 图标 + 关闭叉，`SettingsRawRow` 的 `icon:` 已自带 SF Symbols 锁拉丁）。「已补 N 条」改成**无条件报**（0 也报）——原来要求 `accepted + ignored + quarantined > 0` 才出声，于是「什么都没发生」这种最需要解释的情况反而最安静。结果只属于这一趟访问：重进这一页 `.onAppear` 就清掉，点新的一次补提交也先清（跑的过程中挂着上次的「已补 3 条」会被误读成这次的结果）。

- **Go 的 `omitempty` 撞 Swift 的合成解码器，让每一次真跑都被读成失败**（2026-09-12，用户报「这里为什么显示失败了？但实际上我看好像已经成功了」）。界面报「补提交没能完成，请稍后再试」，而 Last.fm 那边 26 条全补进去了。根因：`backfillOutcome.Items` 带 `json:"items,omitempty"` 且**只在 dry-run 分支填**（`runBackfill` 里真跑那条路径从来不设 `Items`），于是真跑的输出根本没有 `items` 键；而 Swift 侧 `ScrobbleBackfillService.Outcome.items` 是非可选属性——**Swift 自动合成的 `init(from:)` 对非可选属性一律走 `decode(_:forKey:)`，缺 key 直接 throw，属性默认值不参与解码**。`run()` 里那句 `try?` 再把 `DecodingError` 吞成 nil → `lastRunFailed`。链路上每一环都成功了（子进程 exit 0、scrobble 发出去、服务端确认、`markBackfilled` 回执落盘），只有 App 读不懂结果。**这条从 `da7d5d2` 功能上线那天起就坏着**，两侧一个字没改过；`lastRunFailed`（上一条）只是把它从「一声不吭」显影成「报一句失败」，所以它悄悄活过了每一趟真跑。同一个坑 2026-08-25 已经在 `LyricsSearchService.Pick` 上踩过一次并修过（`searchLyricsPick` 那边几乎每个字段都带 `omitempty`，`decidable==false` 这条正常分支不写 `decisionJSON`，整行解不出来、`pick` 恒为 nil）。顺带解释了另一个一直没人报的症状：待补条目**全都超窗**时 dry-run 的 `items` 同样被省略、`pending` 恒为 nil，于是「另有 N 条太旧、Last.fm 不再接受」那句话从来没出现过。修法两件：`Outcome` 手写 `init(from:)`，**所有**字段一律 `decodeIfPresent`（今天只有 `items` 带 `omitempty`，但下一个谁加的不该再炸第三次）；解码失败改成 `do/catch` + `logger.error`——`try?` 把唯一的线索吞光，正是它能活四趟的原因（子进程 exit 0，另外两条 error 日志一条都不会出现，日志里只剩一句 `accepted=-1`）。selftest 加了「omitempty 边界」守卫，从 **Go 的 struct tag** 反推要求而不是抄一份键名清单：`backfillOutcome`/`backfillItem`/`searchLyricsPick` 三处，每个带 `omitempty` 的键在 Swift 侧要么用 `decodeIfPresent` 解、要么属性本身可选。倒回 `HEAD` 旧版这条会红并准确点名 `items`（`abortedReason` 因本就是 `String?` 被正确放行），已实测。
...
- **第①级自带图的第二道纠正：本机「封面归属已核实」的图优先于 Last.fm 实体图**（2026-09-01，
  ...
  行为跟原来一致。
  ⚠️ **这道纠正的凭据被 collector 侧伪造过**（2026-09-07 修，用户报 Michael Jackson《Michael》
  里「Hold My Hand (with Akon)」显示成 QQ 的《The Ultimate Collection》）：collector 把同专辑
  邻居的 QQ 封面借给别的曲目时会盖上 `cover_album = album`，而 QQ 那档从不回传专辑名、本来
  刻意不认领归属 —— 于是一张没核实过的图拿到了「已核实」的资格，反过来把 Last.fm 那张**对的**
  自带图顶掉，正好是这道门槛要防的事情的镜像。App 侧一行没改（门槛本身是对的，被骗的是输入），
  修在 collector：借用分档 + 如实回传归属，加一次性迁移擦掉存量 385 条假戳。详见第 3 章决策 13
  的 ⚠️。判断这类问题时的定位顺序：先看 `defaults`/缓存里这条记录的 `cover_source` 与
  `cover_album`，再看 feed 里 Last.fm 自带图是什么 —— 两张图都下下来看一眼，比读代码快。
...
3. **旧值不撤（真正的 stale-while-revalidate）**：四条次数作废判据命中改成往 `stalePlayCountKeys` 记一笔，
   ...
   - ⚠️ **`userplaycount` 字段缺失 ≠ 那边是 0（2026-09-10，用户报「为什么这三个 lastfm 里面没有播放次数」）**。
     判据原本是 `if let n, n > 0 { 记数 } else if ok, zeroIsFinal { 记进"那边没有" }`，而
     `n = (dig(json,"track","userplaycount") as? String).flatMap { Int($0) }` 对**两件不同的事**都给 nil：
     那边明确回答 `"0"`（一个答案），和响应 200、track 对象正常、**压根没带这个字段**（"这次没答上来"）。
     后者被当成前者 → Last.fm 的一次抖动被写成定论、随快照落盘，而进了名单的行连 `···` 都不画，看起来
     就是"这行天生没有次数"。**现场**：10:56:00 那一批 20 首里 17 首正常，`prince|1999` /
     `prince|little red corvette` / `prince & the revolution|kiss (extended)` 三首同时空白；`log show`
     里那一批**全是 200、一条 `api error` 都没有**（error 6 会打日志，所以不是 notFound 那条路）；
     5 分钟后拿**完全相同的参数**（含 `user=` + `username=` + `autocorrect=1`，按 `LastfmQuery` 的编码）
     直接问，三首分别是 18 / 20 / 1 次；`user.getTrackScrobbles` 显示这些收听最早可追到 2026-07-07
     （周杰倫《晴天》23 次可追到 2022 年、最近一次是 9-03），所以既不是"那边真没有"，也不是上面那条
     "刚 scrobble 完还没并账"。成因是 Last.fm 的按用户计数是**另一次后端查询**，高并发下会静默缺字段，
     而 App 刚重启时一口气打二十几个 getinfo 正好撞上（同一分钟里 6 条被钉死：三首 Prince + 薛之謙
     《演员》+ 盧廣仲《聽見了嗎》+ 周杰倫《晴天》）。**修法**：三态判据下沉到
     `LyrimuseCore/Local/PlayCountOutcome.swift`——`.counted(n)` / `.definitivelyNone` / `.unanswered`，
     字段缺失落 `.unanswered`（什么都不记、下一轮重问，不进名单也不落盘）。⚠️ **error 6 仍是定论**：
     `resolvePlayCounts` 在 `res.notFound` 那条路上显式传 `reportedCount: 0`——"压根没这个实体"跟"0 次"
     是同一个答案，混进"没答上来"会让那 7 首有声书章节每轮重问、永不收敛（那正是 2026-09-03 引入
     `notFound` 要解决的）。同一个混淆在**孪生求和**那条路上还有一个分支：本尊那次没带字段（`count == nil`）时，孪生报回来的 0 不许替它凑出一个 0（`if count == nil && tc == 0 { continue }`），否则又绕回"没答上来被当成那边确实没有"；孪生报正数照收，那是真问到了一本账。selftest「次数记账三态」9 条。
...
| 账号→Last.fm→设置 | Scrobble 的播放器（内置播放器 + 已信任播放器同一排图标芯片，点图标勾选） | `lastfm_excluded_bundles`（features.json，bundle id 列表，缺失/空 = 全部上送；只挡 Last.fm 的 scrobble 与 now-playing，ListenBrainz 不受影响）。**不重启 collector**（热重读，见 §1 与决策 11）。见 §1「Scrobble 的播放器」 |
...
| 回填 | backfill.go `runBackfill`（`Items` **只在 dry-run 分支填**）、backfillcli.go；App 侧 Settings/ScrobbleBackfillService.swift `Outcome.init(from:)`（手写，全字段 `decodeIfPresent`——Go 的 `omitempty` 撞 Swift 合成解码器，见上）`run(dryRun:)` 的 `do/catch` 解码日志 |
| omitempty 跨语言边界守卫 | lyrimuse-selftest/SourceContractTests.swift「Go 的 omitempty 撞 Swift 的合成解码器」（从 Go struct tag 反推，覆盖 `backfillOutcome` / `backfillItem` / `searchLyricsPick`）；已修过的另一处 LyricsManager/LyricsSearchService.swift `Pick.init(from:)` |
| 账号页 | lyrimuse/AccountLinkingTab.swift `AccountDestination` `destinationStatus` |
| Scrobble 的播放器（按播放器排除） | lyrimuse/AccountLinkingTab.swift `lastfmScrobbleSettingsCard` 里那一行 / `lastfmPlayerChoices`；行与芯片 lyrimuse/Settings/PlayerLinkageRow.swift `PlayerBundleChipsRow` `PlayerChip` `PlayerChipFlow`（换行算术 LyrimuseCore/Models/ChipFlowGeometry.swift `rows` `size`）；落盘 lyrimuse/Settings/FeatureSettingsStore.swift `updateLastfmExclusion` `lastfmExcludedBundles` `changedFileKeysSinceLastSave`；跳不跳过重启 LyrimuseCore/Models/CollectorRestartPolicy.swift `hotReloadedKeys` `needsRestart` `changedKeys`；collector lyrimuse-collector/lastfmexclude.go `lastfmExcluded` `resolveLastfmExcludedBundles` `currentLastfmExcludedBundles`（mtime 热重读，路径由 main.go `setLastfmExcludePath` 登记），消费点 poller.go `recordLastfmListen` / `announce` / 退出兜底 |
...
10. 按播放器排除只作用在 Last.fm 那一路，设置也只住在 Last.fm 页（2026-09-10，借鉴清单 S10）：第一版做成了全局的「收听历史」开关（播放器页一张卡，连 ListenBrainz single / playing_now / relay 最近播放一起挡），当天按用户「这个设置给我放在 last.fm 页面吧，只控制 lastfm 的上送」收窄。这跟短曲目（决策见 §1）、Scrobble 时点（决策 9）是同一条线：**Last.fm 页上的设置一律不碰 ListenBrainz**，那是这个项目的主平台。实现上标记做成「会话开头算一次」而不是每个出口现查——一首歌中途改设置不该把同一次收听切成两半，何况 collector 只在启动时读一次 features.json，改设置本来就伴随一次重启；标记跟 `isAd` 同一个位置，将来再加一个「这次不发 Last.fm」的理由照这个模子放。粒度按 bundle 而非网页平台：collector 没有平台身份（02 章的平台配对只在 App 侧），硬要区分就得再开一条 App→collector 通道，S10 的诉求（把某个 App 整个排除）不值这一条。界面上内置播放器与信任的浏览器**摆同一排芯片**而不是一人一行（同日第二次收窄，用户「这一块占用区域太大了…收拢到一起」）：候选个数由用户的信任列表决定、没有上限，所以那排会换行，而换行的算术抽成 `ChipFlowGeometry`（Core，selftest 钉住 13 条）——SwiftUI `Layout` 的 `Subviews` 在测试里造不出来，把判据留在 `Layout` 里就等于没测；`SourceContractTests` 另有一条守卫钉住 `PlayerChipFlow` 必须调这份纯函数，防止有人在 `Layout` 里另写一遍。
11. **能热读的配置就别为它重启 collector——先量代价，再决定（2026-09-10，用户报「点击以后这个转圈会比较久，是在干什么」）**。当时的实现是「写 features.json → `launchctl kickstart -k` 重启 collector」，这是全仓统一约定（01 章决策 1）。量出来的账：App 那句「正在应用到后台服务…」只转 1~3 秒（0.5 s 合并去抖 + kickstart + 最多 3 s 轮询到新 pid），但它把「launchd 报出了一个新 pid」当成完成——新进程还要在 74 MB 歌词缓存之上跑九道迁移、全量导入导出 14307 个歌词文件才真正开始服务。当天四次重启从 SIGTERM 到打出启动横幅分别是 **68 / 37 / 40 / 43 秒**，这段时间歌词与「正在播放」推送整个停摆；期间还能看到 launchd 对间隔太近的两次 kickstart 节流约 10 秒、先后拉起两个实例。为送一个 bundle id 列表付这笔钱不值。修法**不是**放开整份 featureFlags 热重读——别的键（播放器集合、歌词源顺序、`lyrics_dir`）在启动时就被展开进包级变量或决定了读取路径，中途换掉是没人验证过的行为；而是开一条**白名单**：`CollectorRestartPolicy.hotReloadedKeys` 里的键，collector 侧必须真有按 mtime 的热重读，App 侧 `save()` 才跳过重启。白名单为空 = 每次都重启（默认安全方向），改动集合为空也照旧重启（那不是「没必要」，是「不知道改了什么」）。两侧接没接上跨 Core / App / Go 三个 target，只能靠 contracts 组那道源码守卫钉住。**这条决策的可复用部分是方法而不是结论**：先量「用户实际等多久」而不是「那句提示转多久」——两者在这里差了一个数量级，而 UI 上那句话恰恰只覆盖短的那一半。真正的大头（每次启动全量导入导出上万个歌词文件）仍然摆在那里，重启一次就付一次，`build.sh` 装机同理，值得单独做一次「迁移打戳、正常重启直接跳过」。
