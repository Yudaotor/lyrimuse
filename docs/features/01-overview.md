...
| 目的地 | 谁发 | 发什么 | 何时 |
|---|---|---|---|
| 十个歌词源:`music.163.com`、`*.qq.com`、`*.kugou.com`、`*.kuwo.cn`、`*.migu.cn`、`*.musixmatch.com`、`lrclib.net`、`music.youtube.com`(LyricFind)、`*.deezer.com`(Deezer)、`raw.githubusercontent.com`(AMLL) | collector | 歌手、歌名、专辑,部分源带时长;AMLL 只按网易云 / QQ 音乐 ID 取文件 | 每首新歌解析(第 09 章) |
