...
- **第十二轮(2026-08-27):中英混排的角色词、复合词尾字、多段名单里撞停用词、纯日期戳,一次修四个真实案例**(用户报的方大同《起源》、丁世光《瘦子》)。
  ① `matchesEnglishCredit` 原来要求整行**以英文角色词开头**,「编曲 Arrangement by 丁世光 Dean Ting」这类"中文角色词 + 英文角色词 + by"的写法直接卡在锚点上——加一段**可选**的中文角色词前缀(复用 `creditRoleWords`,不新开一张表)。顺带发现前缀本身还不够:`creditRoleWords` 只收了词根「制作」,没收复合词「制作人」,「制作人 Produced by …」的「人」字会把匹配卡断,补了 `\p{Han}{0,4}` 容忍词根后面还有尾字。顺手把 `arranged` 补成 `arranged|arrangement`(名词形态的署名写法,`arranged` 覆盖不到「Arrangement by」)。
  ② `matchesNameListCreditShape` 的整句否决闸(`latinCreditRestLooksLikeSentence`)按单字扫 `nonNameChars`,而真人名偶尔会撞上这类停用词——「项目总监：闫曼嘉/蔡雨燕/庄有豪」里的"庄有豪"含着"有",让整条三人名单被判成"像句子"放过。改成只在**单段**(没有 `/、&,` 名单分隔符)时才用整句否决;多段(≥2)本身就是够强的结构信号(歌词几乎不会写成"甲/乙/丙"这种斜杠并列的短句),交给下面逐段的形状校验把关就够。单段(「他说：我不走」)这道闸还留着。
  ③ 新增 `matchesDateStampLine`:纯日期戳注解行(「July 18, 2012 at 5:25 PM」),没有冒号也不是角色词开头,上面所有规则都够不着——早年手工整理的 LRC/KRC 常见词曲作者在职员表最前面顺手记一句创作日期。判据收得很紧、首尾锚定(月份英文全称/缩写 + 日 + 年,可选「at 时:分 AM/PM」),不需要整份阈值兜底。
  selftest 补了 8 条(3 正 3 反给 `matchesEnglishCredit`、2 条给 `matchesNameListCreditShape` 的多段/单段边界、6 条给 `matchesDateStampLine`),两首歌的真实歌词全文(52 行 / 52 行)端到端跑过一遍,署名/职员表全部消失、真歌词一行没伤。
- **第十三轮(2026-08-27,同日):中间点「·」当分隔符 + 一次关于"永不删空"闸门的、最终没做的改动**(用户报丁世光《背面是我》专辑两首纯配乐 Interlude《Presentness》《Bygone》)。
...
- **第十五轮(2026-09-03):ISRC 行 + 「英文角色名 : 拉丁人名」**(用户报陈绮贞《我亲爱的偏执狂》结尾没过滤干净)。那一份结尾 20 行署名里**只漏了 2 行**:`ISRC TWB870211301` 和 `Publisher : Sam Duann`。
  ...
  **全库前后回归**(逐行 diff `creditLineDropDecisions`):KEEP→DROP **61 行 / 16 种**,全部是真署名(Mixing / Mastering / Publisher / Producers / ISRC),**零误伤**;DROP→KEEP **0 行**。
  ⚠️ **但这份回归当时只覆盖了 77% 的语料**(当晚发现并补审)。那份离线工具切行用了 `split(separator: "\n")`,而 `"\r\n"` 在 Swift 里是**一个**字形簇 —— 酷狗源那 1439 个 CRLF 文件整份被当成一行,少数出 43081 行(23.0%),当时报的"3464 首 / 144439 行"就是这么来的。**这正是本节决策②那个坑,只是这次咬在离线核实工具上**(App 的 LRCParser 自己先归一 CRLF,线上不受影响 —— 也正因如此它躲过了所有既有断言)。改用 `split(whereSeparator: { $0.isNewline })` 重跑完整语料(3480 首 / 187684 行,LRC 入口;走 `load` 合 YRC 的入口是 197236 行,那个更贴近用户实际看到的),对两条新规则做**假阳性专项审计**:`matchesISRCLine` 命中 11 行 / 10 种、`latinRoleColon` 形状命中 60 行 / 11 种,**全部是真署名、全部被删、一行真歌词都没碰**。结论不变,覆盖面的说法按这个改。
  ⚠️ 顺带两个**测量侧**的同源坑,都出在补审脚本自己身上,写在这儿是因为它们比规则本身更容易复发:① 用 `text.contains("\r")` 检测 CRLF 是**假阴性**(同一个字形簇原因,当场报出"CRLF 文件 0 个"),要走 `text.utf8.contains(13)` 按字节判;② 拿"buggy 切法"和"正确切法"比行数时,两侧后处理必须**逐步一致** —— 第一版 buggy 那侧忘了剥 `[时间戳]`,凭空多出两万多行纯时间戳行,把差值算成 8.5%(真值 23.0%)。修正后 buggy 侧数出的 144603 行与原工具的报数**逐位吻合**,诊断才算坐实。
...
- **第十六轮(2026-09-03):「指导 / 总监 / 策划 / 导演」进 `creditRoleWords`**(用户报李佳薇《甲乙丙丁》头部「艺术指导：程楚楚(廊坊师范学院）」没过滤)。
  **先用 App 同一条路径复现**(LRC+YRC 一起喂 `LyricsSyncEngine.load` → `allLines`,而不是单行函数、也不是只喂头部几行):64 行里 16 行署名删了 15 行,**只剩这一行**。它漏的原因是标签「艺术指导」不含表内任何词;右边「程楚楚(廊坊师范学院）」带括注机构、括号还半全角混用,不是干净的人名表,`matchesNameListCreditShape` 也接不住;整份里署名只占 25%,结构性闸不开。
  ⚠️ 第一版探针只喂了 16 行署名 + 4 行正文,这一行**被删了**——那是署名过半触发了结构性整份闸,跟真实文档(64 行)的判定不同。**喂整份、用真实入口**这条教训在第十五轮已经写过一次,这一轮又差点踩:探针的输入必须是缓存里那份完整文本。
  加词前全库语料(3480 首 / 197235 行正文)逐词量过:「指导」2 行(这条 + 周杰伦「进入你梦里 指导你演戏」,真歌词、无冒号、进不了第一道门);「总监」16 行全带冒号全是署名;「策划」9 行 8 带冒号;「导演」5 行 3 带冒号——**带冒号的全是署名、不带的全是真歌词**,冒号那道门恰好把两边分开。刻意**不收**「顾问」:8 行全是真歌词。
...
- **播放到就同步**(`syncPinToOffset`,在 `applyOffsets()` 里调,2026-08-26 前叫 `backfillPinIfNeeded`):双向——校正值非零就补 pin,是 0 就解 pin,不是只补不清的单向操作。原来只会钉不会解:pin 只在 `set()`(nudge/reset/setOffset)被调用那一刻才跟着改,而 `set()` 不是校正值变回 0 的唯一路径(offset key 含内容指纹,内容一换、旧 key 下的非零值就查不到了,这个函数若在内容变化前用旧值钉过一次,之后没人告诉它去解钉)。实测坐实:一台机器 16 条已校准记录里 9 条飘成"校正值早是 0、pin 却一直挂着","仅人工修正"筛选把它们当成用户亲手弄对过,而这首歌其实跟没调过没区别。改成无条件同步之后,这类飘移会在下次播放到时自愈。刻意不做启动时全量扫:offset key 含内容指纹,离开播放上下文算不出对应的 pinKey。
