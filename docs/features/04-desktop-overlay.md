...
23. **不做桌面小组件,也不做「沉在桌面层」的层级选项(2026-09-09,用户看过示意页后拍板「不做」)**。起因:用户问「我们这个软件是否可以做出桌面小组件」。评估把「桌面小组件」拆成两种:① macOS 14 的系统小组件(WidgetKit 扩展)—— 每天只有几十次时间线刷新预算,做不了同步歌词,只能做封面 + 歌名的「正在播放」卡;小组件是独立的沙盒扩展进程,读不到 App 与 collector 之间那层 `~/Library/Application Support` 下的 JSON / 歌词缓存交换面(02 章「进程边界」),要么 App Group(需要 Team ID,本项目 ad-hoc 签名没有)、要么给 collector 加一个 localhost HTTP 接口;仓库是 SwiftPM + build.sh 手工装 bundle,`.appex` 得手工造(PlugIns 目录、扩展 Info.plist、链接 WidgetKit、codesign),ad-hoc 签名的扩展能否被系统加载没验证过,MacPorts / Sparkle 打包都要跟。② Sleeve 式「沉在桌面层」的常驻卡片 —— 只是悬浮歌词窗的层级从写死的 `.floating` 改成三选一(桌面图标之上、窗口之下),同一套逐帧渲染,半天到一天。给用户做了一页可点的假桌面示意(层级三选一 × 卡片样式三种 × 「显示桌面」,`~/Desktop/lyrimuse-桌面卡片示意.html`,不入库),用户看后决定两条都不做。记在这里是为了以后再有人提「桌面小组件」时不用从头评估:两条路的代价与边界如上;若重新考虑,先做 ②,① 只在明确要进系统小组件库时才开一天的验证(ad-hoc `.appex` 能否加载 + localhost 数据通道)。
