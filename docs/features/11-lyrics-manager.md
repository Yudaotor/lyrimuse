...
> 最后核对：2026-09-12 · 基线：5fcf8e6+工作树（2026-09-12 加「当初有几个源应答」的列表标记 / 筛选 / 排序档、空条目的「无源应答」第五档，以及决策弹窗的查询词清单与共识名单，见第 1、6 节与借鉴清单 V1–V4；同日「歌词(LRC)」编辑框改成只显示正文、元信息标签与署名行留底不丢，见第 3 节与决策 23；同日「解析决策」面板整体重设计成判词 + 折叠候选 + 差值分解，见第 6 节与决策 24）
...
- **「证据薄」标记与筛选**（2026-09-12，借鉴清单 V3）：「来源」列的徽章右边多一个橙色小胶囊 **`N/9`**，写的是**这份歌词当初是在几个源应答的情况下定下来的**（分母取 `LyricsSource.allCases.count`，不写死——selftest 的 `SourceContractTests` 对账 Go/Swift 两侧源清单，以后加源自动跟着变；⚠️ 分母是**当前**源数，老条目当年可能没这么多源，tooltip 里说明了）。**点它直接开「解析决策」弹窗**——那里第一行「本轮应答的源：…」正是这个数字的展开；没有决策存档的条目不可点、也不换光标（`Summary.sourcesRespondedCount`，直接读缓存顶层已有的 `lyrics_sources_responded`，没有新增任何字段）。**只在 1～3 时出现**，不常驻——本机 4248 条带该字段的有词条目里 70.6% 有 ≥4 个源应答，给它们每行挂一个「5」是纯噪音；跟搜索弹窗那个轮次前缀「［2］」只从第 2 轮起才显示是同一个取舍，标识恰在「有话要说」的那一刻出现。**0 不显示**：那是老条目压根没有这个字段（不知道），不是「零个源应答」，两者混为一谈会凭空造出一批不存在的可疑条目（判据 `Summary.thinEvidence` = `1...3`）。筛选栏对应加一档「仅证据薄」。
  - ⚠️ **第一版只写了一个孤零零的 `3`、只有 hover tooltip，当天就被用户报「你这里标的数字是什么含义？我点击也没有反应」**（2026-09-12）。两个教训都记下：①一个突出颜色的**孤立数字**天然像可点角标，靠 hover 才发现说明，发现性太差——写成 `3/9` 才自解释；②macOS 上"能点"必须有视觉线索，`.onTapGesture` 要配 `NSCursor.pointingHand` 的 `.onHover`，否则跟纯展示看不出区别。弹窗挂在**列表层**（`decisionTarget: DecisionSheetTarget?` + `.sheet(item:)`），不是详情页那个 `showDecisionSheet`——那个挂在详情视图上，列表行够不到。
  - **为什么值得单独摆出来**：缓存永久保留 + 20 秒总截止，首次解析本来就有运气成分（09 章决策 6）。实测本机 **29.4%（1249 条）**的有词条目当初只有 ≤3 个源应答；而 `needsLyricsRetry` 有一道「已有逐字就不重试」的闸（`enrich.go`），本机 **94.3%** 的有词条目带逐字——交集 **1088 条（25.6%）的升级重试永不触发**，旁证是全库 `lyrics_retry_count` 只出现在 **30** 条上（对照 `lyrics_rescore_count` 有 496 条）。这批条目**有歌词、看起来一切正常**，此前在界面上完全不可见，用户不主动重搜就没人再问。
  - ⚠️ **刻意不改重试策略本身**，也**没有**做「批量重搜证据薄的 N 条」：那道逐字闸有它的理由（逐字是质量的直接证据 +400），改判据要 bump 打分版本、让全库走一遍 rescore——09 章决策 49 已经论证过这类代价（「拿一次全库搅动去换 0 improve 是纯风险」）。而现成的补空通道（`LyricsFillSweep` / `isFillSweepRetryable`）**硬闸要求 `!hasLyrics`**，证据薄的条目有歌词、走不了那条路。所以这一轮只做「可见 + 可筛」，挑出来之后逐条点「重新自动匹配」，要不要做批量入口留给以后。
- **排序**（2026-09-01，用户要求"加一个排序功能"，选的是"筛选栏加一个下拉"而不是点列表头排序）：`LyricsSortOption`，筛选栏「歌手」「专辑」下拉右边新增的「排序」下拉，**十二选一**——「默认排序」（改动前那套写死的 (歌手,专辑,歌名) 固定排序，不是新行为，只是第一次开放成可以主动切走/切回来的选项）+ 歌名/歌手/专辑/来源各自的 A→Z、Z→A + **更新时间 新→旧 / 旧→新**（当天晚些补上，见下）+ **应答源最少**（2026-09-12，借鉴清单 V3：证据最薄的排最前，只给升序一档——「证据最厚的排最前」没有对应用途，而每加一档下拉就长一行；应答源数未知（0）的两个方向都收进尾块、块内按 `resolvedAt` 排，口径与「来源」「更新时间」两档对缺失值的处置逐字同构）。只有点名的那个字段会因方向反转，没点名的次序键恒按升序断平局（跟 Finder"按修改时间降序仍按文件名升序断平局"同一个道理，不会把整条元组一起倒过来）。⚠️ 歌手/专辑排序用的是跟「默认排序」同一套归一化键（`normPrimaryArtist`/`normAlbum`，折简体+小写），不是列表里实际展示的原始写法——理由跟 2026-08-28 那条"筛选/排序按统一名归并、只有展示列如实展示原始写法"一致，同一位歌手因原始标签写法不同（简繁/大小写）被拆成两条记录时，按归一化键排还能让它们挨在一起。来源排序用 `sourceDisplayName(_:)`（跟「来源」列徽章同一份映射），不是原始的 `lyrics_source` 字符串。**「更新时间」两档 2026-09-01 晚补上**（用户点名要）。第一版注释写着"没有这个候选，`Summary` 没有任何时间戳字段，硬凑得先扩缓存模型"——**结论对、理由不全**：缓存里确实没有一个表达「更新时间」的字段（实测本机 3212 条的覆盖率：`lyrics_decision.decided_at` 73%、`translation_ts` 25%、`peripheral_ts` 9%、`lyrics_rescore_ts` 7%，全是偏科的局部时间戳，而 `decided_at` 还只反映「上次自动决策」、手改歌词根本不动它），但**不需要扩缓存格式**。
...
- **工具栏「回到当前播放」有时要点两次**（2026-09-06 用户报，已修）：`List` 的行高是**懒量**的，没被滚到过的行一直按估算高度算。列表几千条、目标又在视口外老远时，一次 `scrollTo` 按估算落点走，停下来差着一截；「点第二次就好了」恰恰是这个机制的自证——第一次已经把目标附近那些行量出了真实高度，第二次才算得准。同一个根因 2026-08-07 就修过一次（`8749350`），但那次的校正只加在**不带动画**那条路（开窗自动定位）上，带动画的工具栏按钮一直是单发。现在两条路都补一发校正：动画那条在 0.35 s 后（比默认动画略长）再定一次位，给 120 ms 缓动而不是硬跳——落点本来只差几行，看起来是"停稳"不是"又跳一下"；已经居中时它是空操作。
  ⚠️ **另一个仍然存在的静默 no-op**（不是上面这条，点几次都没用）：`focusCurrentlyPlaying` 匹配的是 `store.summaries`（全量），而 `List` 渲染的是 `sortedFiltered`。搜索框里有词、或筛选条把当前播放这首排除掉时，选中会设上、滚动却没有目标行，按钮表现为完全没反应。要改的话得让这颗按钮在目标被筛掉时先清掉搜索/筛选。
...
- **没歌词的行分五档，不再一律红色「无歌词」**（2026-09-05 补第四档，2026-09-12 补第五档）：
  - **第五档「无源应答」**（借鉴清单 V4）：最近一轮解析**一个源都没应答**——那一刻网络全挂（09 章决策 48 的形态），不是「这首歌没有词」。判定链里它**排在「源里有歌、无词」之前**：两者常常同时为真，而给用户的行动建议相反（前者「词还不存在、只能等」，后者「重搜也许就有」）。列表里是中性灰的「无源应答」+ 悬停整句说明，详情页是 `antenna.radiowaves.left.and.right.slash` 徽章「这一轮没有源应答」，多选面板对应第三颗计数徽章（顺序与判定链一致，否则面板数字会跟行徽章互相矛盾——那正是 2026-09-05 拎出第四档时立的规矩）。
  - ⚠️ **判据必须用「决策存档在不在」，不能用顶层 `lyrics_sources_responded` 空不空**：那个字段在 collector 侧带 `omitempty`（`enrich.go`），**空数组根本不会被序列化出来**，于是「老条目压根没有这个字段」和「真的零个源应答」在顶层字段上完全不可区分。取 `lyrics_decision`（最近一次评估）那一槽而不是 `lyrics_decision_applied`——空条目从来没「采用」过任何东西，applied 槽恒为空（本机 142 条空条目实测 0/142）。判据本体在 `LyrimuseCore.EnrichSourcePresence.lastRoundHadNoResponder`，selftest 覆盖、做过变异测试（摘掉 `hasDecisionRecord` 那一半当场红）。实现上只做两次字典查找，**不走 `decodedDecision(for:)` 那条强类型解码路径**（那是 2026-08-19 从 rebuild 里优化掉的「JSON 双重编解码」，不能因为这个字段又请回来）。
  - **实测切分**（本机 142 条空条目，改动前 → 改动后）：「源里有歌、无词」**71 → 2**（迁走 69 条），红色「无歌词」**7 → 0**（全部迁走），「无源应答」新档 **76 条**；纯音乐 53、仅纯文本 11 两档不受影响（它们排在新档之前——那两档一个是确证结论、一个有实质内容，不该被盖过）。也就是说改动前那一档 **97% 是误报**，而本机现在**没有一条**够格显示红色「无歌词」（= 九个源真的都答了、都没有）。
「纯音乐」（collector 联网确证，中性灰）→「仅纯文本」（有 `plain_lyrics` 兜底，橙）→**「源里有歌、无词」**（`Summary.knownOnSources`：`netease_url` 带 song id、或 `qq_music_url` 是 `/songDetail/` 页而不是搜索兜底页——平台曲库里**有这首歌**、只是没人挂词，中性灰）→ 真的「无歌词」（红）。第四档的来历：用户问「歌词管理里这些搜不到的怎么办、要不要加源」，把 82 条非纯音乐的空条目逐条核下来，**56 条**是 2026 年 6–8 月新发的网易云独立作品（gamza / jehoda / The Rose / Japanese City Pop），网易云和 QQ 都收录了歌（拿到了 id）、Musixmatch 正常应答也没有、Apple Music 日/美区甚至没上架，外网连歌名都搜不到——它们跟「九个源一条都没搜到、可能是我们匹配失败」是两种不同的"无歌词"，前者不是该修的，只能等。判据是 collector 解析时确实定位到了那首歌，不是猜的（口径在 `LyrimuseCore.EnrichSourcePresence`，selftest 覆盖）。列表行、详情页 infoStrip、多选面板的统计 chip 三处同口径；「仅无歌词」筛选**仍包含**这一档（它们确实没词，重试扫描也仍会碰它们，词哪天被人挂上就能补到）。
...
- **「歌词(LRC)」编辑框只显示正文（2026-09-12，用户圈图「帮我把这些部分在歌词管理里面去掉，不需要显示，只需要显示歌词正文」）**：`[id:]`/`[ar:]`/`[ti:]`…`[offset:0]` 这类元信息标签行，和开头「曲名 - 歌手」/「词:」/「曲:」那几行署名，编辑框里不再出现——判据与「搜索候选歌词」右侧预览框**同源**（`LyricsPreviewText.classify`，署名走 `LyricsSyncEngine.creditLineDropDecisions`，传当前条目的 title / artist 做首行「曲名 - 歌手」识别与对唱豁免）。被摘掉的行由 `LyricsBodyEdit` 按位置**留底**：正文之前的做前缀、之后的接后缀；「保存修改」写回的是 `reassembled`——正文没改动就是原文一字不动（offset 的内容指纹不受影响），改了才是前缀 + 新正文 + 后缀，`[offset:]`（`LRCParser.parseOffsetMs` 消费）与 `[ar:]/[ti:]`（备份归档认的头部）一行不丢。「拷贝」按钮拷的也是正文。译文 / 罗马音两个框不动。
...
- **来源标钉在候选行右上角**（2026-09-08 用户要求「把歌词源的标打在这个位置」，附红框截图，`candidateRow` / `characteristicBadges(showsSource:)`）：来源标原来是封面下方那排标签里的一员，而那排前面站着「无时间戳 / 逐字时间戳 / 译文 / 罗马音」四个可有可无的标签——有几个全看这条候选的成色，于是九条扫下来「这条是谁给的」每行落在不同的 x，窄列时还会被挤到第二行。这跟 2026-08-26「把标签排挪到封面下面、统一一个位置」是同一个诉求的延伸而不是推翻它：那次治的是标签排整体跟着文字长短漂移，这次治的是来源标在标签排**内部**继续漂。分类上也站得住——那几个标签说的是「这条候选**有什么**」（越多越好的加分项），来源说的是「这条**是谁给的**」（身份），身份钉在行右上角、九行右缘对齐最好扫。三处配套：① 胶囊 `.fixedSize()`，列宽能拖到 250pt，不钉住 SwiftUI 会先压它（「网易云音乐」折两行、「Musixmatch」截成「Musixmat…」），来源名截掉半个字等于没标，宁可让上面的歌名先换行——它本来就允许两行 + 悬停看全文；② `characteristicBadges` 加 `showsSource:` 参数，**右侧详情不跟着挪**（传 true）：挪的收益是「多行之间对齐、好扫」，右侧永远只有一条候选、没有可对齐的对象，而那一行的右上角被「采用此候选」这颗主按钮占着；③ 整排一个标签都没有时不渲染（`hasAnyCharacteristicBadge`）——空 `WrapLayout` 高度是 0，但外层 VStack 照样给它算 4pt 间距，来源标挪走之后「有逐行时间戳、没译文没罗马音、既不重复也不是当前使用」的普通候选真会全空，不挡的话那一行比别的行多垫一截。胶囊样式本身没动，仍是 `sourceBadge` 那套身份色（跟歌词管理列表、设置里的来源勾选同一份配色）。
...
**版面分三层**（2026-09-12 重设计，依据与取舍见决策 24）：①**判词卡**一句话回答「为什么是它」；②候选表**一行一条**（源 + 分数 + 相对冠军的差值 + 分数条），折叠态摊的是**差值分解**、展开态才是带解释的绝对明细；③「这一轮的输入与经过」默认折起（查询词 / 曲长 / 应答的源 / 问过哪几组词）。候选表下面一行折掉「所有候选完全相同的项」。
...
- **「这一轮实际问过」摘要**（2026-09-12，借鉴清单 V1）：元信息区那行「查询词」下面多一段，把 collector 记下的 `queries_tried` **分组**摊开。
  - ⚠️ **第一版逐条平铺，当天就被用户报「这部分可读性很差」并截图**。真实案例（宇多田光《Beautiful World (Da Capo Version) [Instrumental]》）一轮问了 9 组词，屏幕上 20 多个视觉行，而其中**曲名重复 9 遍、同一串七个源的名单重复 8 遍**——真正的信息只有「曲名没变，换了 9 个歌手名」，噪音把信号淹了。
  - **两条压缩规则**（都在 `LyrimuseCore/Lyrics/LyricQueryDigest.swift`，纯函数、selftest 覆盖 15 条、做过两次变异测试）：①**全部记录曲名相同**时把曲名提到顶上说一次（`sharedTitle`），组内只列歌手名——曲名变过（标题反查轮会改写曲名）时 `sharedTitle` 为 nil，曲名并回每一条，不丢信息；②按**「来路 + 源名单」**分组，组内歌手名拼成一行，那串源名单于是每组只出现一次。9 条压成 2 组、约 5 行。
  - **两个刻意的取舍**：**总数 `total` 留着**（「实际问过 9 组」）——压缩不该让人以为只问了两次；**分组不排序**，保持首次出现顺序——先问什么、后问什么本身就是要复盘的信息（首轮问的对不对、别名轮被什么触发）。同来路但**源名单不同**要分开成两组：定向重查问了哪几个源是关键差异。
  - 界面与「拷贝」按钮消费**同一份 digest**（`dumpLines` 也走它），拷出去的文本和屏幕上看到的是同一个形状。**只有首轮一组时不显示**——那是绝大多数情况，常驻一行「这一轮实际问过：<跟上面一样>」是纯噪音（同搜索弹窗轮次前缀那个取舍）。来路译名在 `LyricsDecisionSheet.queryReasonLabel`，跟 `pathLabel` 同一套约定：**default 是「原样显示原始值」**，漏补译名就是界面上直接印英文串；Go 侧 `TestLyricQueryReasonsHaveChineseLabels` 双向对账（含空串那档「首轮」——缺了它首轮那行会渲染成「歌手 - 曲名（）」）。机制与实测依据见 09 章第 8 节与决策 54。
- **「这一轮的输入与经过」改成字段表**（2026-09-15，用户截图报「这部分 UI 目前非常垃圾，完全不能看出有效信息，非常乱」）。**同一块地方三轮改可读性，这一轮的病跟前两轮不是一回事**：09-12 那两轮治的是**内容**（压缩查询词分组、给异质字段加标签 +「」），内容其实已经压干净了；这一轮的病在**版面**。
  - 两个具体失效：① 展开态八行清一色 `.caption2` + `.secondary`，**结构（标签）和数据（值）同字号同色**，一眼扫下去没有任何落点；② 「查询词：歌手「A」歌名「B」专辑「C」」是一整句，真实曲目上要折两行，而折下来那半行的左边缘跟**下一个字段的行首**完全重合——"续行"和"新字段"长得一模一样（用户那张截图里，专辑名 `Live from Mexico City, Mexico, Dec 12, 2025 (DJ Mix)` 正是这么断开的）。**「」那一轮的修法在这里反过来帮了倒忙**：它把三个字段焊成了一句更长的话。
  - 改法是**从"一行一句话"换成"一张对齐的表"**：`Grid` + 标签列（压暗、右对齐、`inputFieldLabel`）+ 值列（正常字色、`inputFieldValue`）。歌手 / 歌名 / 专辑 / 时长 / 应答 / 未应答各占一行。**值列有了固定的左边缘，续行永远落在值列里**，不再冒充新字段；标签压暗退到背景，数据才是主角。
  - **源名带各自的品牌色**（跟候选表里那些胶囊同一套 `sourceColor`）——「应答」那行的名字和下面表里的行肉眼一对就对得上；**没应答的那一串统一压暗**：它们本来就不在候选表里，给品牌色会显得也参赛了，而两行一亮一暗，不读标签也分得出谁是谁（老写法两行同色，区别只藏在「本轮应答的源：」和「当前启用的其余源没有应答：」两句开头，得逐字读才发现）。
  - **查询轮次给序号、拆三个层次**：序号（`1` / `2`）不是装饰——`LyricQueryDigestBuilder` 刻意不给分组排序正是为了保住"先问什么、后问什么"，界面得让它看得见；然后来路中等字重（这一轮是什么）、查询词正常字色（问了什么）、范围最暗（问了谁）。**范围单独一行**：那串六七个源的名单跟来路挤在一行时会把组头顶到第二行，于是"哪一轮"和"问了谁"糊成一团。
  - **曲名不再印两遍**：`sharedTitle` 跟字段表那行「歌名」相等时只说「曲名未变」，不同才整句说「曲名始终是「X」」——把一个三四十字符的曲名在同一屏里原样复读，本身就是"乱"的来源之一。
  - **「拷贝」出去的纯文本仍然合成一行**（`dumpLines` 走 `groupHeading` → `groupScopeText`）：纯文本没有字重和缩进可用，拆行反而更难读。
  - ⚠️ 逐段上色走 `AttributedString`，**不要**写成 `Text(a) + Text(b)`：那个 `+` 在 macOS 26 SDK 里已标 deprecated（本仓部署目标 14，所以现在不报警告——也就是说抬部署目标那天会突然冒出一片）。
  - 验证：离屏 `ImageRenderer` 渲了两个真实场景（截图那首 Fred again.. 的 520pt 宽，以及宇多田光那首 9 组词的 428pt 窄窗口），确认窄窗口下「未应答」和「只问」两行都在**值列内部**折行。`LyricsManagerTests` 加了 8 条源码级守卫（字段各占一行 / 界面不准再拼「查询词：…」/ 两行一亮一暗 / 两处上色都用 `AttributedString` / 拷贝那条路仍走 `groupScopeText`）——这些退回去都不会编译报错，只会在某首专辑名长的歌上重新变成一团。
- **候选行的「跟谁是同一份词」**（2026-09-12，借鉴清单 V2）：分数明细那行下面，把 `consensus_peers` 摊成「跟 QQ音乐、酷狗音乐 是同一份词」。原来那行只有「内容获印证 +250」，说了有几家、答不出跟谁——而冠亚军分差中位只有 23 分，分差小的时候真正要问的恰恰是「这两份是不是同一份词」。见 09 章决策 55。
- **纯音乐标记行不按候选渲染**（2026-09-12，用户截图问「它怎么是空的，并且是 -1？」）：候选表里可能出现一行**只有源徽章和一个红色 `-1`**、标题/歌手/专辑/打分明细全空的空壳。那不是候选——collector 判定「这首本来就没词」时，会往 scored 列表里塞一条 `{source, score:-1, instrumental:true}` 的伪候选，把这个信号**搭车**带出来（`enrich.go` 的 `instrumentalMarker`，四个源各一路：LRCLIB / QQ / 网易云 / Musixmatch；当初这么设计是为了不改 `fetchScoredLyricCandidatesStreaming` 的返回值签名——它被手动搜索 CLI 和常驻解析两条路共用，改签名影响面更大）。那个负分是**手段不是评价**：它让 `pickLyricCandidate` 一定跳过这条、不参与打分排序，对用户而言纯属内部实现细节。现在按 `LyricsDecisionRow.isInstrumentalMarker` 分流到 `sidelinedRow`，显示成「LRCLIB · 纯音乐」+「这个源明确说这首是纯音乐，所以它没有参与打分」，**不印那个 -1**；「拷贝」出去的纯文本走同一判据（界面改好了、拷出去还是一行 `LRCLIB · -1` 等于没修——这个面板存在的意义就是贴进 issue 复盘）。
  - ⚠️ **这里刻意不照搬手动搜索的「过滤掉」**：手动搜索 2026-08-03 就把这类标记从候选列表里删了（`searchcli.go` 的 `filterEnabledLyricSources`，注释原话「不该多出一行歌词是空的、点了也没用的候选」），因为那边的判据是**能不能点选采用**；而这个面板的判据是**当时那一刻的证据**——「某个源明确说过这首是纯音乐」恰恰是复盘时最想看到的一句话。用户那次正是这个形状：LRCLIB 说纯音乐（而那首歌本来就叫 `Beautiful World (Da Capo Version|Instrumental)`，它没说错），另两个源却匹配到带词版本、以 1208 分胜出。删掉等于把证据删了。
  - **判据要两个条件**（`instrumental == true && score < 0`）：前者认 collector 塞的那条标记，后者是第二道保险——万一日后某个源既给出了歌词、又把 instrumental 标成 true，那是一条能被选中、甚至可能是胜者的**真候选**，不该被当成空壳换掉说法。老存档没有 `instrumental` 字段（nil）时继续按普通候选显示，同封面那条的理由：存档是当时那一刻的固化，不能事后补。
  - ⚠️ **被拒的候选是同一个病、当时只修了一半**（2026-09-12 重设计时补上）：首项 `kind` 以 `reject` 开头的候选（`rejectPlainTextOnly` / `rejectCreditOnly` / `rejectNotTimed` / `rejectWrongLanguage`）存档里 score 同样恒为 `-1`，面板照印、还是红的——全库 **604 行 / 301 个条目**，比纯音乐标记（148 行）**常见 4 倍**。现在两类共用 `sidelinedRow`，被拒那一支显示 `explanation` 吐的「不可用：仅有纯文本，没有时间戳」+ 那一项的解释。selftest 那条源码级守卫的口径同时扩到两类，并只禁**裸印分数**（`Text("\(c.score)")`）、不禁把 score 传给 `explanation`——后者对被拒的候选吐的正是要显示的那句话。
  - **面板上有这一行 ≠ 这首歌被标成了纯音乐**：`enrich.go` 里真正落地纯音乐结论的那段只在 `picked == nil`（一条真歌词都没选出来）时才看它——选出了歌词还标纯音乐是自相矛盾。用户那次就是：结论在存档里留痕，但因为有源给出了带词版本并胜出，这个结论本轮没被采纳。
...
- **判词卡**（2026-09-12）：`LyrimuseCore/Lyrics/LyricsDecisionAnalysis.swift` 的 `LyricsVerdictBuilder.build` 给结构、`LyricsDecisionSheet.verdictText` 给措辞（Core 不做本地化）。四档：`.sameLyrics`（每条参赛候选都拿到了 `consensus` 项 → 这几份词内容一致，分差比的是包装）、`.decisiveNegative`（亚军吃了冠军没有的否决性负分，且绝对值 ≥ 分差 → 这一项就够定胜负）、`.tooClose`（分差 ≤1 分或 <1%，但内容未必相同）、**nil**（整块不渲染）。全库 4426 份存档实测：`sameLyrics` 3320（75.0%，其中 1534 份同时是 nearTie）、`tooClose` 235（5.3%）、`decisiveNegative` 80（1.8%）、nil 791（17.9%＝504 无判词 + 287 独苗/无对局）。
  - ⚠️ **判词刻意不做归因**。此前实测否掉过一版「用一句自然语言解释胜者凭什么赢」——只有 13.2% 的对局存在单一强势维度能真的解释分差，其余是七八项的合力，硬写一句就是编。判词只说**可以直接量出来**的三件事：是不是同一份词（`consensus` 项）、差多少（相减）、有没有吃否决性负分。冠亚「差在哪」那半句只在 `.single`（**恰好一项**有差）和 `.identical`（打分项完全相同）时才开口，`.multiple` 什么都不说。实测 separator 分布：multiple 2129 · single 1372 · identical 134。
  - `.sameLyrics` 判的是「**有没有** `consensus` 这一项」不是分值多少（2 家以上 250、1 家 150 都算），所以老存档同样吃得到——不依赖 V2 才加的 `consensus_peers`。
- **折叠态 = 差值，展开态 = 绝对明细**（2026-09-12）：落选的候选，用户不关心它得了多少、只关心它**差在哪**。`LyricsVerdictBuilder.deltas` 逐项相减、按绝对值从大到小、为 0 的项不列——「行数 −1」比「+400 / +250 / +164 / +51 / +50 / +30」有用得多，而且相同的项自动消失。⚠️ 总分差用**存档里的总分**相减，不是分项和相减：两者在被夹过分的那 3.74% 上不相等，而用户看到的分数是前者。
- **「所有候选都相同的项」折成一行**（2026-09-12）：`LyricsVerdictBuilder.sharedTerms`，kind 与分值都相同才算，顺序沿用第一条候选里的出现顺序（那是 collector 的打分顺序，本身有含义）。实测打分项**种类**的共有率中位 71%、完全相同的行占全部明细的 32%——这一行是把「它们去哪了」交代清楚，免得让人以为差值分解漏了。
- **分项之和对不上总分时要说出来**（2026-09-12）：collector 在 `match.go` 把负分**统一夹到 1**（重扣表达「差」不是「不能用」），全库 **644 行（3.74%）、443 份存档（10%）** 因此「总分 1」后面跟着一串加起来是 −353 的项。`LyricsScoredCandidate.clampedRawSum` 非 nil 时界面多印一句「分项合计 −353，被夹到最低分 1」——不说的话，谁真去加一遍都会以为界面算错了（重设计之前就是不说，藏得住只是因为没人去加）。
- **「曲名是反查出来的」条件横幅**（2026-09-12）：存档里 `corrected_title` ≠ `query_title` 时，判词卡下面多一条橙色横幅「曲名对不上：本地叫「X」，找到的是《Y》」+「本地那个曲名没搜到，最后靠「标题反查：歌手泛搜」问出真名」。全库只有 **13 份（0.3%）** 命中——罕见，但发生时它是关于这次解析**最重要的一个事实**：这份词是用另一个曲名找回来的。罕见 + 决定性 = 条件横幅，跟「只有首轮一组时不显示查询词摘要」是同一个规矩。`retry_method` 跟查询词摘要的 `reason` 是同一套取值，复用同一张译名表（`queryReasonLabel`）。
- **异质字段一律带标签 + 「」框住**（2026-09-12，用户截图圈了三处：`查询词：米津玄师 & Utada / JANE DOE / JANE DOE - Single`、组内的 `米津玄师 & Utada`、`米津玄师 · 米津玄師、宇多田ヒカル`，说「需要能看出来分别是歌名、歌手、专辑」）。原来是拿 `" / "` / `" · "` 把几个值直接拼起来，读的人只能**靠位置猜**是谁。两层失效叠在一起：① 这几个值经常长得几乎一样（那首歌名 `JANE DOE`、专辑 `JANE DOE - Single`，候选行里歌名和专辑干脆都是 `JANE DOE`）；② **值里本来就有分隔符**（`米津玄師、宇多田ヒカル` 的顿号、`JANE DOE - Single` 的连字符），跟拼接用的分隔符混成一片，连「这是几个值」都读不出来。
  - 改法：`LyricsDecisionSheet.labeledFields` 统一渲染成 `歌手「米津玄师 & Utada」歌名「JANE DOE」专辑「JANE DOE - Single」`。**标签回答「谁是谁」，「」回答「到哪儿为止」，两个都需要**——只加标签时值里的分隔符仍会糊，只加「」时仍得靠位置猜。覆盖四处：查询词、组内查询词、候选匹配到的版本（`matchedText`，参赛与被拒两类共用）、以及「拷贝」出去的纯文本。
  - 组内查询词全组都没带曲名时，「歌手」只说一次、后面一串「」框住的名字（`歌手「米津玄师」「米津玄師、宇多田ヒカル」`）——八个别名时不会把「歌手」重复八遍。
  - ⚠️ 连带把 **`LyricQueryGroup.queries` 从 `[String]` 改成 `[LyricQueryPair]`**：原来 Core 里那句 `r.artist + " - " + r.title` 是**展示决定**，本来就不该由 Core 定，而这次正是它先崩的。Core 只给字段，View 自己加标签。
- **别名轮的组头要说清「为什么触发」和「问了哪些源」**（2026-09-12，用户截图问「这里说的『缺罗马音』是什么意思？」）：
  - 「缺罗马音」像在描述一个**结果状态**，实际说的是**触发原因**，而且省掉了主语（谁缺）。真实判据在 collector `enrich.go` 的 `needsRomanizationRetry`：这一轮拿回来的歌词是**中日韩文字**（`dominantScript` 判 Han/Kana/Hangul），而**没有任何一个源**给出罗马音字段、也没有源标出语种（标了的话本地能自己注音，不必再查）。于是换歌手的其它写法全源再问一轮——截图那首正是首轮用罗马字署名 `Yuuri` 拿回日文假名歌词、没人给注音，别名轮改用日文原名 `優里` 重查。改成 **「别名轮：中日韩歌词但没有源给出罗马音」**。
  - 别名轮三档**问的源范围不一样**，而这件事原来在界面上看不出来：「补缺席的源」只定向问还缺着的那几个，「一个候选都没有」与「没有源给出罗马音」是**全源重查**。实测全库这个对应关系是死的（alias-missing 108 组全部带源名单；alias-rescue 57 组、alias-roma 14 组全部不带），所以后者缀一句「· 全部源重问」，跟前者的「· 只问 X、Y」对称——不然读的人只看到一个歌手名，不知道这一轮的动作范围有多大。
  - ⚠️ 译名改文案不影响 Go 侧 `TestLyricQueryReasonsHaveChineseLabels`：它只钉 `case "<reason>":` 这个记号存在，不钉中文串。
- **「N/9 个源应答」与「当前启用的其余源没有应答」**（2026-09-12）：放在折叠区的摘要行与展开内容里。⚠️ 分母是**当前**启用的源数（`LyricsSource.allCases.count`），存档里没记「当时启用了哪些」——所以措辞只说「当前启用的」、不说「没露面」那种像在陈述历史事实的话，并在 tooltip 里点明「老条目当年可用的源可能更少」。跟列表里那个「3/9」角标是同一个取舍。
...
- UserDefaults：列宽三键。
- `~/.config/lyrimuse/lyrimuse-lyrics-fill-request.txt`（App 写、collector 读完即删）与 `lyrimuse-lyrics-fill-status.json`（collector 写、App 按 mtime 读）：「重试无歌词」的请求/进度通道，见上文与第 09 章「补空扫描」。
...
| 「歌词(LRC)」编辑框只显示正文 | LyrimuseCore/Lyrics/LyricsPreviewText.swift `LyricsBodyEdit`（`body` / `hiddenPrefix` / `hiddenSuffix` / `reassembled`；与 `forPreview` 共用 `classify`）；LyricsManagerView.swift `detailView` 里 `editedLyricsBody` ↔ `editedLyrics` 的两条 `onChange`；selftest `LyricsParsingTests.swift` 「LyricsBodyEdit」段 |
...
| 决策弹窗 | LyricsManager/LyricsDecisionSheet.swift（`pathLabel` / `queryReasonLabel` / `analysis(_:)` / `verdictSection` / `verdictText` / `chipsRow` / `candidateSection` / `candidateRow` / `expandedDetail` / `sidelinedRow` / `sharedTermsLine` / `inputsSection` / `scoreBar` / `dumpLines`）；数据 decision.go |
| 判词 / 差值分解 / 共有项 | 逻辑 LyrimuseCore/Lyrics/LyricsDecisionAnalysis.swift `LyricsScoreTermValue` `LyricsScoredCandidate`（`isInstrumentalMarker` / `isRejected` / `isContender` / `rawTermSum` / `clampedRawSum`）`LyricsVerdict` `LyricsVerdictSeparator` `LyricsVerdictBuilder`（`build` / `ranked` / `champion` / `isNearTie` / `deltas` / `sharedTerms` / `vetoKinds`）`LyricsScoreDelta`；措辞在 LyricsDecisionSheet.swift `verdictText` / `separatorText` / `compactTerms` / `clampNote`；覆盖在 lyrimuse-selftest `lyrics-manager` 组（夹具是两份真实存档：back number《Hanataba》与 Blondie《Call Me (Instrumental)》） |
| 曲名反查横幅 | `LyricsResolutionDecision.retryMethod` / `.correctedTitle`（EnrichCacheStore.swift，走 convertFromSnakeCase）；渲染 LyricsDecisionSheet.swift `titleRewrite(_:)` / `titleRewriteHow(_:)` |
| 查询词分组摘要 | 逻辑 LyrimuseCore/Lyrics/LyricQueryDigest.swift `LyricQueryRound` `LyricQueryPair`（歌手/曲名分字段，**不在 Core 里拼字符串**）`LyricQueryGroup` `LyricQueryDigest` `LyricQueryDigestBuilder.build`（分组/曲名提取/组内去重；`queriesFlatCount` 只给测试核对去重）；渲染 LyricsDecisionSheet.swift `queryDigest(_:)` `groupHeading(_:)` `groupQueriesText(_:)` + inputsSection / dumpLines 两处；覆盖在 lyrimuse-selftest `lyrics-manager` 组 |
| 异质字段带标签 | LyricsDecisionSheet.swift `labeledFields(_:)`（唯一出口）+ `matchedText(_:)` / `queryText(_:)` / `groupQueriesText(_:)` 三个调用方；标签文案复用既有的「歌手」「歌名」「专辑」三个键 |
| 无源应答第五档（V4） | 判据 LyrimuseCore/Local/EnrichSourcePresence.swift `lastRoundHadNoResponder`；`EnrichCacheStore.Summary.lastRoundHadNoResponder` + `EnrichCacheStore.lastRoundHadNoResponder(_:)`（两次字典查找，不走懒解码）；三处显示 + 多选面板计数在 LyricsManagerView.swift（行徽章 / 详情页 infoStrip / `noResponder`·`indexed`·`missing` 三档计数）；覆盖在 lyrimuse-selftest `lyrics-manager` 组 |
| 证据厚薄（V3） | `EnrichCacheStore.Summary.sourcesRespondedCount` + `thinEvidence`（判据 `1...3`，0=未知）；列表 `N/9` 徽章（可点，`LyricsManagerRow.onShowDecision` 回调 → 列表层 `decisionTarget` + `.sheet(item:)` → 同一个 `LyricsDecisionSheet`）与「仅证据薄」筛选在 LyricsManagerView.swift（`thinEvidenceOnly` 串 `hasActiveFilters`/`filterToken`/`filtered`/`resetFilters` 四处）；排序档 LyrimuseCore/Local/LyricsSortOrder.swift `LyricsSortOrder.evidence` + `LyricsSortKey.sourcesRespondedCount`，覆盖在 lyrimuse-selftest `lyrics-manager` 组 |
...
22. **同一份名单抄三处，加源时漏一处（2026-09-06）**：搜索弹窗头部「0/8」vs 空状态「九个源」。根因不是谁忘了，是名单本来就不该有第三份——Go 侧一份是跨进程边界躲不掉的，App 侧 `LyricsSource` 已经是 `CaseIterable`，弹窗直接读它就没有"保持一致"这件事。修法是删掉手抄、加守卫，而不是把 8 改成 9（改成 9 等于下次加源再漏一次）。同类前例：`features` 键镜像守卫（Go json tag vs Swift CodingKeys）、`currentLyricsScoringVersion`（两侧手抄版本号，那个确实躲不掉所以只能守卫）。判据：跨进程的才允许两份 + 守卫；同一个模块里第二份就是 bug。
23. **「歌词(LRC)」编辑框摊出十几行永远不会显示的元信息标签和署名，把正文挤到第二屏（2026-09-12，用户圈图宇多田光《One Last Kiss》QQ 源那条：`[id:$00000000]`…`[offset:0]` 十行 + 「One Last Kiss - 宇多田光」「词:」「曲:」三行，说「不需要显示，只需要显示歌词正文」）**。
    跟 09-04 预览框那条（第 5 节）是同一个毛病换了个框，但这里是**编辑框**，多一层约束：改完要写回缓存，被摘掉的行不能因为没显示就丢——`[offset:]`
    有人消费（丢了整份时间轴偏几百毫秒），`[ar:]/[ti:]` 是备份归档认的头部。**修法**：Core 把 `forPreview` 的判据抽成 `LyricsPreviewText.classify`（blank / hidden / visible
    三态），新增 `LyricsBodyEdit`——`body` 给编辑框显示，摘掉的行按位置留底（正文之前 → `hiddenPrefix`，之后 → `hiddenSuffix`），`reassembled(body:)`
    正文没改就返回原文一字不动、改了才拼前缀 + 新正文 + 后缀。视图侧 `editedLyrics` 仍是完整原文（保存 / 指纹 / 采纳只认它），编辑框绑新的
    `editedLyricsBody`，两条 `onChange` 互拼：`editedLyrics` 变了且**不等于**当前正文拼回去的结果（= 外部写进来的：换曲 / 采纳 / 重新匹配）才重算正文；
    正文变了就拼回 `editedLyrics`。**为什么不能无条件重算**：拼回去会把每行首尾空白、CRLF 归一，用户每敲一个回车就被吃掉一次。**为什么不做成 Binding(get:set:)**：
    get 每次渲染都跑一遍逐行正则，这个窗口曾栽过 20Hz 过度重渲染（决策 5）。**为什么不直接把摘掉的行从存储里删掉**：预览框那条定的边界「只影响显示，不影响落盘」在这里同样成立。
    selftest 用截图那首的真实形状钉了：正文只剩两句、13 行前缀原序、尾部署名进后缀、没改 → 原文、改了 → offset 不丢且尾部换行保留、与 `forPreview` 摘的行完全一致。

24. **「解析决策」面板把数据结构照搬到了屏幕上（2026-09-12 重设计）**。collector 存的是「候选数组，每条带一个打分项数组」，界面就画成「候选块，每块一串打分项」——**那是数据的形状，不是问题的形状**。对本机全库 5200 条缓存做的只读统计：
    - 摊开后的打分明细**中位 27 行**、最长 49 行，其中 **32%** 的行在每条候选上一模一样（同 kind 同分值），打分项**种类**的共有率中位 **71%**；
    - **24%** 的对局冠亚分差 ≤1 分，其中 **789/1011（78%）纯粹差在「行数」**、另有 **134 场所有打分项完全相同**（真平局，先后由稳定排序定）；**43%** 的对局分差不到冠军分的 1%，分差占比中位仅 **2.0%**；
    - **61%** 的对局里所有候选匹配到的是同一个标题（那一行重复五遍是噪音），另 39% 有分歧（那一行是全场最重要的证据）——同一个字号、同一个灰；
    - 九个源中位只有 **4 个**应答；4274 条双槽条目里只有 **2 条**会真的出现第二个页签（0.05%）。

    取两份**需求相反**的真实存档当设计基准：back number《Hanataba》（最常见的形状——五个源给的是同一份词、1 分定胜负，要的是「别废话，说结论」）与 Blondie《Call Me (Instrumental)》（硬骨头——两条吃了版本不符 −600、一条被拒、一条分项合计 −353 被夹到 1，要的是「把证据摆全」）。只在其中一种上成立的版面是错的。落点是三层：判词卡 → 一行一条的候选表（折叠给差值、展开给绝对明细） → 折起的输入区，外加「共有项折一行」。前者把常见款从 **57 个视觉行**压到十来行，后者把硬骨头从 43 行压到十来行，而两边要看的东西一件没少。

    **最容易走偏的一步是判词**：写错比不写更糟。所以规则只认可直接量出来的量（是不是同一份词 / 差多少 / 有没有吃否决性负分），差在多项上时**明确什么都不说**——这正是此前实测否掉「一句话解释凭什么赢」的那条界限（只有 13.2% 的对局有单一强势维度）。判词逻辑整块在 `LyrimuseCore`（纯函数、selftest 覆盖 33 条断言、夹具是上面那两份真实存档），View 只负责措辞；`ranked` / `champion` 抽成公共函数，是因为「界面上戴皇冠的那条」和「判词里说的胜者」必须是同一个，两处各写一份迟早漂开。

    **重设计逼出来的两个既有问题**（都不是新引入的，是原版藏着的）：① 被拒候选还印着红色 `-1`，604 行 / 301 个条目，跟用户当天问过的「它怎么是空的，并且是 -1？」是同一个病、当时只修了纯音乐标记那一半；② `match.go` 把负分夹到 1 导致 644 行（3.74%）分项之和 ≠ 总分，界面照印、不加说明。做差值分解时这两条都绕不过去——**不是新设计制造了问题，是新设计不许再藏**。
