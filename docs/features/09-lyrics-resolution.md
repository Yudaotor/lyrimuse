...
| deezer | 是（身份闸 + 时长闸，保持 Deezer 原序） | **无** | 无 | 无 | **自带**（`album.cover_xl`，1000px） | 跟 lyricfind **数据同源**（都是 LyricFind 供词）但走完全不同的接口：匿名 JWT + `pipe.deezer.com` GraphQL；法语覆盖是现有源里最好的一档，见下 |
...
60. **法语歌覆盖差 → 查出 LyricFind 整条管道在这台机器上不可用 → 接 Deezer 补第二条；过程中把「地区没授权」误判了一版，还顺带挖出并发收集丢结果的老坑（2026-09-13，用户问「有哪些歌词源对法语支持比较好」→「还有没有其他外部源没接入」→「接入 Deezer 管道」→「所以这个是为什么呢」）**。
    ...
    **修法**：给 LyricFind 补第二条管道 —— 接 Deezer（`deezer.go`，第十个源）。实现要点见本章「deezer」小节。附带收益比预期大：Deezer 是法国公司，法语曲库覆盖是现有源里最好的一档，上面那三首"只有 lrclib/netease 给得出低分候选"的小众法语歌它都给出了逐行歌词。
