...
- **解析优先级(App 侧 `MediaControlClient.fetchSnapshot`/`fetchArtwork`,collector 侧 `system.go` 的 `getState`)三级**,跟单选年代的判断树是同一套骨架、只是把"选定播放器"从单值换成集合成员判断:
  1. 选中集合里**包含**「自动识别」(不管是否同时还勾了别的具体播放器,auto 按超集处理)→ 走原有的自动识别路径(`fetchAutoDetectedSnapshot`/`getAutoDetectedState`),行为跟改动前的纯 `.auto` 完全一样,包括"检测到未知播放器"的信任列表机制;
     - ⚠️ **界面必须把这条说出来**(2026-09-11 补,用户原话:「为什么我现在把 qq 音乐这里取消勾选了,但是实际上还是可以识别到我现在 qq 音乐里面播放的歌?」)。"auto 按超集处理"意味着勾着「自动识别」时**取消某个具体播放器根本不改变认哪几个**,而在此之前六张卡片长得一模一样、取消勾选还会如实变灰 —— 用户当然会以为那一下生效了(它确实写进了 `features.players`,只是不参与这条判据)。现在:勾着「自动识别」时,**没单独勾上的那几张卡显示成"由自动识别接管"**(虚线强调色描边 + 右上角 ✨ 角标,指向能读到一句提示;判据 `PlayerChoiceCard.isCoveredByAuto`,设置页 `SettingsView.isCoveredByAuto(_:)`、引导页同一个组件同一份判据),设置页那张卡下面再多一行说明告诉你该去取消哪个开关。**语义一个字没改**(Swift/Go 两侧的三级判断树原样),改的只是界面有没有把它说清楚。⚠️ 角标刻意**不做成"浅一点的实心对号"**:那跟真选中态只差一个透明度,而这张网格 2026-09-03 才因为"纯颜色差异表达不了勾了哪几个"补上对号(见 `PlayerChoiceCard` 头注),再拿透明度区分两种状态等于原地推翻那次的教训;虚线在这个网格里本来就有"不是你勾的选项"这层含义(`MorePlayersComingCard`)。
  2. 选中集合**恰好**是 `{Apple Music}`(没有 auto、没有别的具体播放器)→ 主体走 AppleScript 直问 Music.app(`fetchAppleMusicSnapshot`/`getAppleMusicState`),跟单选年代完全一样,位置精度也更高(实测 289.7659912109375 vs 目录 289.766)。**⚠️ 2026-09-11 起这条路不再完全跳过 media-control**:电台判据 `radioStationHash` 是 MediaRemote 独有的键,AppleScript 拿不到,不补的话电台那一整层在这一种配置下完全不生效(详见下面「电台」一节的「只勾 Apple Music 那条路」)。两侧补法不同,因为要的东西不同——App 只要判据本身,而"是不是电台"在同一个曲目 key 内不会翻转,所以 `radioAwareAppleMusicSnapshot` **按曲目探一次**(换歌才多一次 fork,实测电台上 230~310 秒一次);collector 还要 `catalogDurationSecs`,而目录锚点是异步的、刚换歌那几拍还是 0,按曲目缓存会把那个 0 钉死一整首歌,所以 `getAppleMusicOnlyState` 跟自动识别一样**每拍问一次**(轮询 5 秒一拍,代价与默认配置持平);
...
- **YouTube Music 的破例:改问页面本身是不是在放广告(2026-09-02)**。上面那条守卫把**浏览器里的 YouTube Music 整个挡在门外** —— 用户报「为什么我现在用 chrome 播放的 YouTube music 不能识别到」,根因就是它经 MediaSession 报出来的 album **常常是空的**,正好踩在上一条写明的那个"代价"上。⚠️ 是"常常"不是"总是"(2026-09-02 实测订正,初版把它写成了"恒为空"):同一个 Chrome 里播 YT Music,「死神」「September」的 album 是空的,而「Bad」报的是 `The Essential Michael Jackson` —— 所以这不是"YT Music 一律进不来",是"看运气",报了专辑名的那些本来就能进。**"看运气"这四个字 2026-09-03 有了确切答案:空的是每条播放队列的第一首**(实测两张专辑四个采样,含一条排除"同名去重"的关键反例)——机制、补法和那张实测表见「设计决策与已知坑」第 17 条。Chrome 已在信任列表、media-control 也读得到,卡的纯粹是 album。
  ...
  **artist 是非空的**(广告主的频道名),免检 album 之后这两条会畅通无阻地被当成"KAO Hong Kong 的一首歌"收下 —— 跟第 09 章记的 Spotify 事故同形态(用户曾在「最近播放」看到 `Now Streaming on Hulu.`)。⚠️ **别指望 `minTrackSecs = 30` 兜住**:那条 Liese 广告是 **30.021 秒**,比 30 秒地板高 21 毫秒,照样过闸(打卡阈值 `min(时长/2, 240)` = 15s,播满就超)。YouTube 的标准 30 秒广告位基本都这样贴着线过去。
  ...
  - ⚠️ **音乐视频(MV)的前贴片广告不是独立的 now-playing 条目**(2026-09-08,决策 #25):它在 `#movie_player` 里放,MediaSession 元数据却一直是正片自己的,于是**同一个曲目 key 下判定会先 ad 后 song**。两侧的判定缓存因此按判定分档复用:歌 45 秒(Swift)/ 60 秒(Go),广告只复用 5 秒(`YouTubeMusicAdProbe.refreshInterval(for:)` / `ytmusicAdReuseWindow`)。⚠️ 歌档两边**不再同值**(2026-09-11,决策 32):Swift 的再探间隔必须**严格小于**可读期 `verdictMaxAge=60`,否则判定过期那一拍必然 fail-closed、UI 塌;Go 侧是同步探测(过期就当场现探)没有这一拍,不需要跟。**判据**仍必须逐字一致,分歧只在缓存 / 节流策略;Swift 侧「广告中」标志不再只往 true 棘轮,页面**明确**判成歌时回落(`LocalPlaybackSource.nextAdBreakState`,nil 不动)。**同日第二轮才找到真凶**:Spotify **网页版**的广告判定原来是「浏览器**配对过** spotifyWeb 就套原生那套 album 空 / artist 空 / 标题「—」启发式」,而这台机器上 Safari / Arc 同时配对了 spotifyWeb 与 youtubeMusic,YT Music 里 album 为空的 MV 于是整首被标成「广告中」(探针明明判它是歌)。现在网页版**只认 `SpotifyWebAdProbe` 的正向证据**,判据收在纯函数 `LocalPlaybackSource.adBreakByFields`;原生 Spotify 启发式不变;Go 侧 `isAdBreak` 本来就只认原生 bundle,无此问题。
  - ⚠️ **界面上的「广告中」只认强信号;「裸标题」在换歌边界会单独误报,且 album 非空后没人再去问页面**(2026-09-08,决策 #26):上一首刚结束到下一首 `<video>` 起播之间的两三秒,页面 `document.title` 就是裸的「YouTube Music」(后台 0.6 秒采样抓到的真实边界:`0|0|1 title=[YouTube Music] vt=0.0`),而 YT Music 首次发布新歌元数据常常不带 album、会让 `trustedPlaybackRejected` 踢一次探针 —— 这次探到的弱 `.ad` 于是缓存到了**下一首歌的 key** 下;几百毫秒后元数据重发带上 album,快照被采纳、换曲按当下判定定初值 → 整首真歌「广告中」。之后 album 非空、基础守卫直接放行,探针**再没有人踢**,状态机等的 `.song` 永远不来。修法两层、都在 Swift:① `YouTubeMusicAdProbe.Reading.strongAd`(ad-showing / 徽章命中)+ 纯函数 `badgeVerdict(_:)` / `cachedBadgeVerdict(forKey:)`:**界面口径**下只有强信号的 `.ad` 算广告,只靠裸标题的 `.ad` 当「拿不准」(nil);`gate` / `cachedVerdict`(要不要采纳这条播放)仍按任一命中即广告,collector 判据一字不动。② `LocalPlaybackSource.apply()` 在「广告中」亮着且是浏览器播放时**每拍都踢一次**探针(`kickIfNeeded` 内部分档:广告 5 秒一探),让 `.song` 有机会到达解开棘轮;Spotify 网页广告(`spotifyWebVerdict == .ad`)与原生 Spotify 不踢。
...
- **发现通知(macOS 系统通知,2026-08-22)**:用户报「识别到新的播放器,但我自己不知道要去这里信任,目前没有一个通知机制」—— 发现卡只在设置页、而且只在那个播放器**此刻正在报 Now Playing** 时才出现,不主动打开设置页就永远看不到。用户拍板**只做系统通知,不要菜单栏那部分**;发现卡保留作兜底。
  ...
  - **同一条管道多一个出口:灵动岛(2026-09-11,用户:「那个通知里的逻辑,我是否可以把它加到灵动岛里?这样可能会更明显一点」「如果这个时候用户开启了灵动岛才生效哦」)**。`UnknownPlayerNotifier.tick` 每拍把 `qualifiesForAnnounce`(= 通知门槛 ⑤⑥⑦,不看 ⑧ 次数 / 冷却)的结果喂给 `NotchUnknownPlayerPrompt.shared`(只在 `notchOverlayEnabled` 时喂,关着喂 nil),`announce` 在发系统通知的同一拍调 `prompt.alert()` 让卡片自己撑开 8 秒;**任一条路真的出了声就 `recordAnnounced`**(只按通知记账的话,授权被拒的机器上记录永远空、灵动岛会每 5 秒重提醒一次)。`shouldAnnounce` 因此重写成 `qualifiesForAnnounce && ⑧`,静音名单 / 反查名 / 稳定性三条对两个出口**同源**——播客在灵动岛上同样不提示(用户对播客定了「不处理」)。「正在放:歌手 - 歌名」的拼法也抽成 `UnknownPlayerAlert.nowPlayingDescription` 两处共用。卡片那侧怎么画、控制器怎么撑开 / 叫回隐藏的窗口,见 05 章决策 #37。
...
- **网页版的封面地址也从这条探针带回**(2026-09-09,Safari 实测):MediaSession 交给系统的封面是 640×640(与图床 640 档同一张图,8×8 均值哈希距离 1),够大不进高清替代;页面 `now-playing-widget` 里 `img[data-testid=cover-art-image]` 的 src 是图床 300 档 webp 地址(`ab67616d0000e1a3…`),同一 hash 换 `82c1` 即 2000×2000 原图。JS 输出加第三段 `|<src>`,`parseReading` 解析(第二段非 0 仍整条作废;YouTube Music 两段输出不变),读数被采信时经 `setArtworkSink` → `LocalPlaybackSource.noteSpotifyArtwork` → 与原生同一条 `spotifyArtworkURL` 路,由 `PlaybackCoordinator.refreshSpotifyOriginalCover` 换原图。⚠️ 曲目 ID 页面上拿不到:`context-item-link` 指向 `/album/<id>`、`context-item-info-artist` 指向 `/artist/<id>`,没有 `/track/` 链接,真曲目链接 / LB 字段只覆盖原生客户端。
...
- **兜底轮询按播放态分档**(2026-08-20 性能审计,`PollInterval`):播放中 2s(scrobble 计时/enrich mtime 检查/未解析重试都搭在这个节拍上,不能慢)、暂停 6s、空闲(连曲目都没有)10s——每一拍都要 fork 一个子进程,固定 2s 意味着彻底没在放歌的机器也一天 fork 三万多次。降速安全的前提是三路事件唤醒(AM/Spotify 分布式通知 + media-control stream)把播放/暂停/换歌的感知拉回亚秒,慢节拍只是它们全失效时的兜底;`adjustPollCadence()` 在每拍状态落定后按需重建 Timer(挂 `.common` mode)。⚠️ **刚失去快照的头 3 拍强制留在 2s 档**(`PollInterval.nilGraceTicks`,2026-09-11,决策 32):快照变 nil 会 `clearIfWasPlaying()` 清空 title,而降档判据看的正是 `title.isEmpty` —— 于是浏览器广告闸 fail-closed 丢掉的**一拍**会把自己的恢复从 2 秒推迟到 10 秒(实测真空期 7 秒,菜单栏与灵动岛期间塌成兜底图标)。真停播多跑两次 poll 就降档,代价可忽略。每次 poll 带单调递增世代号,子进程返回后只在"没有更新的轮询已发起"时才生效——防"较早发起的慢轮询用过期快照覆盖新快照"(2026-08-02 实测坐实的乱序 bug)。
...
**开播锚点本身打晚**(同日上午第三个形态):广告结束后开播的歌,MediaRemote 那份 now-playing 信息晚发 ~2.4s 而 elapsedTime 仍是 0(妳和吉他:Spotify 通知 08:12:45.876 报「位置 0」,media-control 08:12:48.253 才出现这首歌、锚点 0@:48),整首歌落后 2.3s、一暂停往前补 2.3s。单看 media-control 认不出来。修法 `SpotifyPositionProbe`:Spotify 原生客户端每首歌开播 2.5s 后用 AppleScript 问一次 `player position`(往返 ~0.1~0.2s,取中点),结果走 `resolvePositionSeconds` 的 `isGroundTruthSeed` 通道(跟浏览器探针同一条,差过 0.30s 才重锚),只在稳定播放中消费(`posWasPlaying` 门),同一首歌只一次。不用通知里的位置当真值是因为通知比 Spotify 的钟真正起步早 ~0.5s(缓冲)。手动切歌两首实测探针读数与锚点一致(差 <0.1s,未重锚)。collector 没有这条探针。**2026-09-09 补:这版只验了"差 <0.1s 不重锚"那一半,差大于门槛时纠偏根本活不过几拍**——用户报「Spotify 进度很多时候偏慢,暂停再播放就准」,真机日志(vampire 15:01):探针 `reanchor delta=+1.956` 之后 35 秒暂停,`pause transition delta=+1.817、errEMA≈0`。原因是 MediaRemote 之后每笔读数都从那个晚打的锚点外推、恒定落后 ~2s,而重锚只改了我们的外推值:差 >2s 下一拍被 seek 分支当"拖动"重锚回去,差 <2s 三拍 EMA(限幅 ±0.75 / 门槛 0.4)拽回去,伺服还认为一切正常。当天下午 7 次暂停量到落后 1.8 / 1.8 / 2.1 / 2.3 / 3.3 / 3.9s(另一次 34s 是拖动),而每首歌开播 2.5s 时探针量的差(+1.91 / +1.96 / +2.14)跟后来暂停量到的落后量一一对得上——**探针每次都量准了,每次都被伺服扔掉**(跟 09-02 浏览器探针那次同一个坑,换了个分支)。修法:探针量到的差**折进 `posReportedBiasSecs`**(允许负值=锚点落后真声),之后每笔流读数先加回这段差再进伺服,伺服看到的误差归零;地面真值分支挪到冻结 / 回绕 / seek 三条守卫**之前**(它不是流样本,三条对它都是误判),那一笔也不记进冻结检测的上一拍。偏置归属改成"量它时对着的那个锚点"(`biasSurvivesAnchor(measuredAgainst:)`):Spotify 开播半秒内会把 0@T 改发成 1.923@T(BIRDS OF A FEATHER),按旧的"elapsed 是不是 0"这首歌的偏置活不过下一拍;暂停 / 拖动重发的锚点仍照旧作废偏置。既然一次读数决定整曲,探针改成**两次采样**(`livenessGapSeconds` 0.5s),第二次减第一次的前进量要落在墙钟间隔的 0.5~1.5 倍内才采信(`clockIsRunning`,挡"钟还在缓冲读到 0"那种;开播那次不过关隔 1.5s 重试一次);开播延迟 2.5s 收到 2.0s、结果落地经 `setResultSink` 立刻 poll 一次消费而不等下一拍轮询 —— 真机量到从锚点到纠偏 5.3s(Still into You,那 5 秒屏上慢 4.4s),收到约 3s,再短就撞上 gapless 开播"钟先超前 0.9s 再停"的窗口;当天第二版撤掉了第一版加的 6s 上限、把偏置的被减数改成这一拍的流读数(`streamRaw`)、并在锚点变化后再问一次(`requestConfirmation`),缘由见决策 29;第三版再把探针值扣掉一段按机器学出来的领先量(`probeLeadSecs`,`player position` 比出声位置恒定领先,蓝牙输出下 ~0.55s,见决策 30)。顺带看到 gapless 歌→歌切换时 `naturalAdvanceCorrection` 其实量出了同一件事(旧曲外推越界 +2.4s → bias 为负)却因"只认正偏置"扔掉了,前 2.5s 仍会慢那一段、等探针来补;是否放开负偏置另议。诊断:`browser probe reanchor: … bias=…`(新增 bias 字段)、`anchor bias dropped: … measuredAgainst=…`、`ground truth seed rejected`、探针的 `clock not advancing normally`;验收看 `pause transition` 的 delta 是否回到 ±0.1s。**collector 侧(网页 / 飞书预览)同日跟进**:那条链路推给状态中继的 progress 走 collector 自己的 media-control 外推,与 App 这套伺服无关,该慢多少还慢多少;用户拍板「collector 复用 App 量出的偏置」而不是再 fork 一个 osascript 各量一次。App 在偏置变化时把 `{artist, title, bundle_id, anchor_elapsed, bias_secs, written_at_ms}` 原子写到 `~/.config/lyrimuse/lyrimuse-position-bias.json`(`PositionBiasFile`,是状态不是信号,collector 不删;换歌 / 清零写 `bias_secs: 0`),collector 每轮读一次,身份(同播放器同歌)、锚点身份(`anchor_elapsed` 与快照原始 elapsedTime 差 ≤1ms,与 `biasSurvivesAnchor` 同判据)、时序(`written_at` ≥ 锚点最初发布时刻 −1s,用 resolvePlayingAnchorTS 解出的原时间戳,陈旧重发不算)、年龄(≤6h)全对得上才 `elapsed -= bias`,放在 `rememberPlayingPosition` 之前让暂停回退值也带上。字段名 / 文件名两边各有一份对称的源码级断言(Swift selftest 读 Go 源,Go 测试解 Swift 形状的 fixture)。collector 5s 轮询,所以网页最多再慢一轮才吃到偏置。2026-09-09 起这次脚本顺带带回 `spotify url` 与 `artwork url`(一次脚本三个值,不多 fork),位置改用整数毫秒回传(避开 AppleScript 实数转文本跟着系统小数点本地化走),封面地址进 `LocalPlaybackSource.spotifyArtworkURL`,消费见 03 章「高清替代」里 Spotify 原生那条。

**偏置不能扣在播放器自己发布的锚点上**(同日上午第四个形态,用上面的诊断行抓到:09:40:50 `bias=1.080 delta=-1.097`,而 shown 152.689 vs frozenRaw 152.673):见「Spotify gapless 自然切歌锚点超前校正」一节里被划掉的那句及其订正。规则落在 apply() 两个分支之前:Spotify 原生、同曲、偏置非零、且快照 `anchorElapsedTime`>0 → 清偏置并记一行 `natural advance bias dropped`(2026-09-09 起这行叫 `anchor bias dropped`、多带 `measuredAgainst=`,判据也改成锚点身份,见上面 09-09 补记)。
...
### Apple Music 电台 / 直播流(2026-09-10 / 09-11 实测)
...
**修法**:系统这一层没有任何单曲级位置可取,唯一能标出曲目边界的就是「元数据换了」这一刻,所以自己起表(`RadioTrackClock` / `radioclock.go`,两边同款:key 一变归零、播放中按墙钟累加、暂停冻结、单拍上限 30s 防休眠跳变)。换值发生在**构造快照那一处**(App 侧 `fetchRawMediaControlSnapshot`、collector 侧 `extract` + `applyRadioClock`),下游的伺服 / 锚点 / 歌词引擎 / 打卡阈值 / 歌词缓存一行都不用改。App 侧还要挡住 `refinedAppleMusicSnapshotIfNeeded` 借 AppleScript 那份位置 —— 它同样是整档节目的。

⚠️ **`duration` 必须原样传下去**,别顺手置成 nil。第一版这么干过,结果整档没有歌词:`LocalPlaybackSource.apply` 建进度锚点那一整支的闸是 `if playing, let duration = snapshot.duration, duration > 0`,duration 一 nil 锚点就建不起来,歌词引擎没有钟可走。分母另走一条路:collector 从 Apple 目录查到真曲长写进歌词缓存(`radioduration.go` 的 hint,实测把 3390.122 纠成 226.283),App 在 `apply` 开头读出来替换 —— 读不到时**保留快照那份**(绝不置 0,`ProgressClock` 会按它夹位置)。

**三轮修正,都是用户听出来的**(时间轴:09-10 晚 → 09-11 凌晨):

| 现象 | 根因 | 修法 |
|---|---|---|
| 「暂停久一点再恢复,进度就不正常」 | 累加与否看的是**这一拍**在不在播,于是「上一拍暂停、这一拍恢复」把整段暂停间隔当成了播放时间。暂停档轮询 6s、空闲 10s,停得越久跳得越多 | 改看**上一拍**。日志里 resume transition 逐条坐实前跳 3.5~5.8 秒 |
| 「本身歌词进度就偏慢一点」 | 表在**轮询到换歌那一拍**归零,从元数据推出来到那一拍之间的时间被永久吞掉,整首歌恒慢那么多。实测开台那次:锚点说播放头 0.000 是 23:18:22,标题到达事件流 23:18:23.425(迟 1.43s),App 应用新曲目 23:18:23.816(再迟 0.39s) | 起表时刻改用 **stream watcher 观察到换歌的那一刻**(`MediaControlClient.noteTrackChangeObserved` ← `MediaControlStreamWatcher.digest` 的 `trackChangeKey`)。锚点是刚打好的就用锚点时刻(亚秒部分交给既有的 `estimatedAnchorInstant`),锚点陈旧就退回事件到达时刻 —— 电台换歌时系统常常**根本不重打锚点**,实测那两次锚点已经旧了 236s / 487s,那种时刻当成换歌时刻会把位置推走一大截 |
| App 一重启,正在放的那首就从 0 重来 | 表只活在进程内存里。实测 09-10 两次装机都打回 0 —— 后一次《Step Up》已播 12.6s,新进程从 0.284s 起 | 落一份 `lyrimuse-radio-clock.json`(`RadioClockFile`),重启时**曲目 key 一致 + 记录不老于 60s + 落盘那刻在播**三条都成立才接回去,追赶量仍由 `maxAdvancePerTick` 夹住。实测装机后日志打出 `radio clock: restored position=129.248 gap=3.665` |

**主持人说话那一段**:这个台两首歌之间还有一到两分钟不是歌(实测 Fact Check 之后多出 66.3s、Blingy 之后 109.9s,拿 Apple 目录曲长比出来的),那段时间元数据还停在上一首、表也还在走。越过真曲长 + 5 秒余量就把歌词收掉(`RadioTrackClock.passedTrackEnd` → `LocalPlaybackSource.radioTrackFinished`,20Hz 那条 tick 上生效),曲目信息与封面照旧留着 —— 那一段确实还是同一档节目。时长拿不到时判定恒 false(fail-closed):刚换曲、缓存还没落地时快照里仍是整档节目那个大数,误判会把整首歌的歌词都收掉。

**口白期间界面显示什么**(2026-09-11,用户:「口白期间,可以恢复到原本电台的封面以及名字。对应歌词的位置可以暂定为口白」):

- **歌词那一格 → 「口白」**。三个展示面各加一条分支,语义与既有的 `isCurrentTrackAdBreak` 平行(都是"这一刻在放的不是歌"),顺序同样必须排在「搜索歌词中…」之前 —— 口白期间元数据还停在上一首,不拦就会一直显示搜索中。
- **曲名与封面 → 换成台名与台标**,歌手/专辑留空(同广告那条口径:口白没有"歌手",画上去是假信息)。

台名台标只有**开台那一刻**拿得到:电台起播时系统会先推一张台卡 —— **`artist` 为空、`title` 就是台名**,持续几十秒(实测开台到第一首歌之间 33.4 秒)。三次实测都是这个形态:`|NCT 127`、`|YEONJUN`、`|petal radio`(enrich key 是 `歌手|歌名|专辑`,第一段空 = 没有歌手)。判据仍取「两者恰好一个为空」——真歌不可能缺其中任何一个,这样两种形态都认。台标跟着取图那条路在几百毫秒后补进来(实测台卡那一拍 `hasArtwork=false`,105ms 后 `artwork attached bytes=78866`,解出来正是 Apple Music 那张「petal radio」600×600 电台封面)。按 `radioStationHash` 分台、落 `lyrimuse-radio-station.json`,换台整张作废。

⚠️ **抓不到台卡就一切照旧**(还显示上一首),绝不编一个台名出来 —— App 在电台已经开播之后才启动就是这种情况。

**台卡期间同样算「不是歌」**(2026-09-11 用户报「第一次开始播放一个电台时……还是会被当成一首歌去搜索,然后显示"暂无歌词"」):那几十秒收进跟口白同一个标记,三个展示面一起显示「口白」+ 台名台标。collector 那侧有同判据的对称守卫(`radiostationcard.go` 的 `radioStationCard`,Go 单测钉住),**不把台卡写进歌词缓存** —— 不拦的话每开一次台就多一条 `|台名|` 空壳,发现时缓存里已经攒了 6 条(`|petal radio|`、`|NCT 127|`、`|YEONJUN|`、`|ADÉLA|`、`|King & Prince|`、`|La Reezy|`),无一例外没搜到歌词。判据只用「电台 + 没有歌手」:真曲目两样俱全,而缺歌手时歌词搜索本来也几乎不可能命中;只对电台生效,本地文件缺标签那条路不受影响。

**这条链路整条都是「断了也不报错」**,所以判据钉在 `SourceContractTests`(源码守卫)+ `PlaybackPositionTests`(纯函数),起表与恢复各打一行日志(`radio clock: start … seed=… observed=…` / `radio clock: restored …`),否则退化成"安静地从 0 起"只有用户的耳朵能发现。
...
**「只勾 Apple Music」那条路**(2026-09-11 补,用户问「当前这个模式是不是仅限于 Apple Music」时查出来的):

判据本身**一处 bundle id 都不认** —— `isRadio` 只看载荷里 `radioStationHash` 非空(`MediaControlClient.swift` 的 `let isRadio = !(raw.radioStationHash ?? "").isEmpty`,collector 侧 `snapshot.go` 的 `radio := str("radioStationHash") != ""`),谁报这个字段就算谁。实际上只有 Apple Music 会命中:那是 MediaRemote 独有的键,Spotify / QQ音乐 / 网易云 / 酷狗通过 `MPNowPlayingInfoCenter` 报的是普通曲目信息,不带它(Spotify 的广告插播检测是另一条独立的路,判据是曲长与标题特征,不共用)。**所以是机制上自然收敛到 Apple Music,不是写死的。**

讽刺的是,**唯一不生效的配置恰恰是「只勾了 Apple Music」**:那条路走纯 JXA,AppleScript 问 Music.app 要不到这个键 → `isRadio` 恒 nil → 单曲表 / 台卡 / 口白 / 按台校准全不启动;就算判据补上了,collector 少了 `catalogDurationSecs` 也只能拿整档节目那个数当曲长,分母和口白判据跟着一起废。默认勾着「自动识别」,走 media-control,一直是好的。

现在两侧都补上了(接法与"为什么一边按曲目探一次、一边每拍问"见上面「多选」一节的判断树第 2 步):
- App:`MediaControlClient.radioAwareAppleMusicSnapshot` —— AppleScript 那份快照整份留着(位置精度更高),只按曲目探一次 media-control 取判据,再 `withRadio(position:)` 把 elapsed 与锚点换成单曲表。探测失败(media-control 不在 / 超时 / 系统 Now Playing 焦点不是 Apple Music)**也记进缓存按"不是电台"处理**:最坏是这首歌整首退回改动前的行为、换歌自愈,而不是每 2 秒白 fork 一个必然失败的子进程。
- collector:`getAppleMusicOnlyState` —— 同样以 AppleScript 那份 state 为主体,只把 `radioStationHash` + `catalogDurationSecs` 两个键补进来;合并动作与 `refineAppleMusicState`(auto / 多选那条路)共用 `mergeRadioKeys` 一份实现,免得两边漂移。焦点不是 Apple Music 时不合 —— 那份 hash 属于别人。

**台卡在这条路上同样认得出来**(2026-09-11 用户实测):值得单记一笔,因为这里的 `title`/`artist` 来自 **AppleScript**(`Music.currentTrack`)而不是 media-control —— 之前三次台卡形态实测全是在走 media-control 的配置下抓的,AppleScript 在开台那几十秒报什么形态属于未知。用户把设置切成只勾 Apple Music 开了一次台,**开台那段确实显示了「口白」+ 台名台标**,也就是说 AppleScript 那份同样留了 `title`/`artist` 其中之一为空。判据取「两者恰好一个为空」而不是钉死哪一列,正好覆盖住了这个当时没量过的差异 —— 这次是那个选择第一次真的兑现。

**已知边界**:
- ~~**电台仍然不打卡**。`listenThreshold` 对未知时长退回 240s 上限,而电台曲目普遍不到 4 分钟,所以一条都记不上。~~ **⚠️ 2026-09-11 订正:这条归因是错的,已修。** 真曲长早就从 Apple 目录拿到了(日志里每拍都在写 `catalog 185.000s`),`listenThreshold` 拿到的是曲长的一半、根本不是 240s 上限。真正的原因是 collector 少了一道 App 侧 09-10 就有的闸:

  `poll()` 里 `applyRadioClock` 刚把位置换成单曲口径,紧接着那段「Apple Music 用 AppleScript 精确定位」又把**整档节目**的位置(`Music.playerPosition()`,实测同一档 4005.696s)直接写回 `p.trackPos`。下一拍 `prevTrackPos` 是几千秒、曲长却是一百多秒,「上一拍已过 90% + 这一拍越过曲尾」的单曲循环判定必然成立 → `handle()` 走 `loopRestart` 分支 finalize 当前会话、新建一个 `playSession` → `playedSecs` 归零。**每一拍都这样**,所以已播时长永远涨不过一拍,到不了阈值。

  实测(用户 2026-09-10 那 4.5 小时电台):`loop restart` **2397 次**,单曲最高 145 次、每 5 秒一次贯穿整首;同期 `listen recorded` 只有 **4 条**。副作用还有 relay 每拍都当 reanchor 写一次(日志里 30 秒涨 8 个),按 5 秒一拍约 3200 次写,正是 `pushRelayState` 注释里担心的「烧穿 1000 写/天」。

  修法是把那个条件抽成纯函数 `borrowAppleScriptPosition(applePlayerSelected, bundle, playing, tracked, radio)`、末尾加 `&& !radio`,跟 Swift 侧 `refinedAppleMusicSnapshotIfNeeded` 里的 `guard snapshot.isRadio != true` 逐字同义;Go 单测 + selftest 跨语言守卫各钉一道。

  **⚠️ 修完这道闸实测仍然一条都不打卡 —— 后面还有第二道。**`sess.meta` 是会话**创建那一刻**的快照,而电台的真曲长由 Apple 目录**异步**给出:实测 Dolly Parton《Dumb Blonde》会话 20:15:45.030 建立、目录 20:15:49.740 才给出 150.447s,晚 4.7 秒。于是 `sess.meta.Duration` 一整首歌都是 0,`listenThreshold(0)` 退回 240s 上限,而电台曲目普遍 2~4 分钟 —— 够不着。**这正是原文那条错归因描述的现象,只是它把「会话里没有曲长」当成了「系统给不出曲长」。**补法照抄同一个 `handle()` 里既有的专辑回填(`AlbumHint`)那三行的形状,抽成 `needsRadioDurationBackfill(sameTrack, radio, sessionDuration, currentDuration)`:只填空缺、不覆盖,且**只对电台生效** —— 非电台时 `p.cur.Duration` 来自播放器自己,而 media-control 在换曲预载窗口里会把下一首的时长拼进当前曲目(见 `enrich.go` 的 `observeWrongDuration`),拿那种脏值回填等于把错的分母钉死一整首歌;电台这一路的 duration 只可能来自过了自校验的目录锚点。由此定下的通则(「`playSession.meta` 是开会话那一拍的快照,凡是异步才到的字段都必须显式回填」)写在 [12 章 §1](12-scrobble-accounts.md#1-一次算数的收听collector-poller) —— 那才是 `playSession` 的主场,以后往 meta 加字段的人先看得到那条。**根子是 537 行警告过的「改一边只修一半」**——App 侧 09-10 加闸时 collector 没跟过来,上次踩这个坑方向恰好相反(只改了采集器、悬浮窗还是慢)。至于「够不够格算一次电台收听」本身是不是还要单独定口径,仍是未定的产品决策。
- **元数据切换比声音晚多少(δ),系统里量不出来 —— 靠用户校一次**(2026-09-11 定案)。我们这一段已经压到观察时刻(≈ 元数据推出来那一刻),剩下的全在苹果那一侧。实测:δ **每首歌不一样**(用户实测同一个台不同的歌也不同,所以钉常数没用),但**同一首歌可复现**(同一档节目重放两次,kiss me / Touch It 的边界位置只差 0.50s / 0.71s)。
  - 为什么量不出来:`NowPlayingInfo` 的 18 个字段里没有"当前曲目在流里的起点";`startTime` 键 Music.app 在电台上不填;ShazamKit 要 `com.apple.developer.shazamkit` 授权,ad-hoc 签名拿不到(实测 202 Missing entitlements)。纯靠元数据时间也不可辨识:`间隔 = 曲长 + 口白长度 + δ 的变化量`,而口白长度同样观察不到(口白期间系统一个字段都不变),一个方程两个未知数。
  - 落地:`LyricsOffsetStore` 的第四层「按台标哈希 + 曲目」校正,只在放电台时生效,用现有的「歌词提前/延后」快捷键调。详见 [08 章](08-lyrics-engine.md#时间轴偏移基准全部--按播放器二选一--单曲微调--电台校正lyricsoffsetstore)。**绝不能落进单曲那层** —— 用户实测同一首歌正常播放是准的。
  - 同行现状(2026-09-11 查):没有任何同类软件处理电台。`radioStationHash` 在 GitHub 上的全部命中都是协议定义/头文件转储,没有一个 App 的业务逻辑用它;翻过的几个同类实现全部零处理(它们对 Apple Music 走 AppleScript `player position`,在电台上直接错几百秒);没有歌词软件用 ShazamKit 做同步;**Apple Music 自己在电台上也不给逐行歌词**(用户 2026-09-11 实测)。
- **collector 那块表不落盘**。`radioclock.go` 与 App 侧同款算法,但**不共用** `lyrimuse-radio-clock.json` —— 两个进程各写各的会互相覆盖,而 collector 的位置只喂状态中继那条路,重启从 0 起的代价远小于引入跨进程写冲突。真要接上,得先定谁是唯一写方。
...
### 专辑名回填:播放器没报专辑名时问 Apple 目录(2026-09-08)

播放器报上来的 `album` 为空时(最典型:YouTube Music 的**音乐视频**,MediaSession 不带专辑、页面 byline 也只有频道链接,`ytmusicAlbumPatch` 无从补起 —— MV 在 YT Music 里是独立的「视频」实体),collector 按「署名 + 曲名 + 时长」问 iTunes Search(`itunes.apple.com/search` + 一次多 id `lookup`,公开 HTTPS 接口,不依赖本机装 iTunes / Apple Music),把这首歌出自哪张专辑补成 `snapshot.AlbumHint`。用户 2026-09-08 拍板作为**通用逻辑**:任何播放器、只要专辑名为空就试。实现在 `lyrimuse-collector/albumhint.go`。

- **只补呈现 / 上送,不进歌词缓存 key**(`snapshot.albumForUpload()`):relay 推给网页的 `album`、Last.fm 的 now-playing / scrobble `album`、ListenBrainz `release_name`、本地收听日志四个出口用它;`trackEnrichment` 的 key、`isAdBreak`、专辑预取、会话 key 仍看播放器原始 `Album`。理由:App 侧 `EnrichCacheReader` 按播放器报的 `artist|title|album` 查歌词,这边 key 若带上回填值两边就对不上、App 拿不到词;会话 key 若带上它,一首歌中途回填出专辑就被当成换歌。
- **两段式:取候选一次、挑每拍**。取候选(`fetchAppleAlbumHintCandidates`,后台):「署名 曲名」+ 裸曲名 × CN / US,留曲名归一相等、时长在 max(4s, 3%) 内、专辑名非空的结果(本地 < 75s 不问,与 `appleTitleSearchIdentities` 同一组守卫),再用**一次**多 id `lookup` 补**专辑级发行日期** —— Search 给的 `releaseDate` 是**歌**的首发日期(精选集里的老歌也标 1979),分不出原专辑和精选(实测 Prince「Prince」1979-10-19 vs「The Hits/The B-Sides」1993-09-13)。候选按 key 落盘(`~/.config/lyrimuse/lyrimuse-apple-album-hint-cache.json`,key = 署名|曲名|整秒时长),查空只记内存、同 key 最多试 2 次(网络不通那一轮的空不算查空 —— 2026-09-09 补,`fetchAppleAlbumHintCandidatesTracked` 复用 networkobs 的 `lyricsRoundConfirmsNoResult` 判据,否则 DNS 抖两下这首的回填就在进程内永久关闭)。**搬运频道形态的兜底(2026-09-11)**:主查询零候选时,把 `normEnrichTitle` 剥掉尾括号后的曲名在第一个「 - 」处拆成「署名 - 曲名」(`albumHintTitleSplit`),只发「前段 后段」一个查询 × CN / US,候选除三道门外还须 Apple 署名与前段 0 档相符(`albumHintCandidatesFromTitleSplit`),记 `title_artist` 落盘、挑的时候当 0 档 —— 「音樂頑童 / Musiq Soulchild - Buddy (Official Video)」这种署名位放频道名的视频才有得问;时长容差不放宽(该 MV 231.4s 对录音室版 223.8s 仍会被挡,是否放宽另议)。挑(`pickAppleAlbumHint`,纯函数,每拍从缓存重挑、零网络):署名 0 档 = 归一相等或 credit 子集(「Prince」↔「Prince & The Revolution」,含繁简折叠);1 档 = **歌词链路核实过的署名**(`lyricResolvedArtists`:enrich 条目的 `CanonicalArtist`,或已采纳歌词决策里胜出候选所报的 artist —— 王子那首 kugou 候选报「Prince」);两档都不中一律不采。同档内「非群星合辑 > 非 Single/EP > 非豪华/重制版 > 专辑发行日期最早 > Apple 顺序」。
- **为什么不"跨文字系统就认"**(09 章决策 45 那条对歌词可以,对专辑不行):真机探针里 周杰伦《七里香》在 US 商店只回别人的同名歌和一位拉丁名艺人的「Jay - Piano Cover (Piano Version)」(时长差 4s、在容差内),裸按文字系统判就会把钢琴翻唱专辑安到周杰伦头上;歌词同词无所谓,专辑写错就是错。所以 1 档必须有歌词链路的旁证 —— 旁证晚几秒到也没事,每拍重挑,那一拍自然出现。
- **异步与补推**:poll 主循环只读缓存(跟 Apple 目录锚点同一条纪律),回填通常比换歌**晚一到几拍**;relay 的去重 key 为此多了 `|a` 标记(同封面的 `|c`)补推一次,不用等 4 分钟心跳;会话 `meta.AlbumHint` 在同一首歌期间跟着补(`poller.handle`),到点提交的 scrobble 也带专辑。
- **实测**(装机前用真实 iTunes 跑 `fetchAppleAlbumHintCandidates` + 缓存条目旁证):王子《Why You Wanna…》→「Prince」;Seal《Kiss from a Rose》→「Seal II」(豪华版 / 精选 / Hits 都排后);Patrice Rushen《Forget Me Nots》→「Straight from the Heart」;陶喆《Doxology》47s → 不问;「Alice Nowhere」同名 → 空。⚠️ CN 商店当天对所有查询返回 0 结果,候选全部来自 US 商店。
- **也充当封面解析用的专辑名(2026-09-08 晚追加)**:collector 选封面 / 复查 / 换封面判定在播放器没报专辑时用回填名做打分依据(`coverAlbumForTrack` / `appleAlbumHintSync`,后台解析路径同步等一次 Apple),但回填名**只进挑选过程、绝不落盘成 `cover_album`**(那是「归属已核实」的凭据);缓存 key 照旧不变。来历与边界见 03 章决策 16。
- **App 侧暂未跟**:灵动岛 / 悬浮窗的专辑行仍显示播放器报的(MV 为空)。Swift 侧 `playback.album` 同时被十几处当歌词缓存 key 用(歌词管理、歌词窗口、Last.fm 统计),不能直接换成回填值;要跟就得另开一个纯展示用的属性,另案。
...
| Apple 专辑名回填缓存 | `~/.config/lyrimuse/lyrimuse-apple-album-hint-cache.json`(署名\|曲名\|整秒时长 → 候选专辑列表,只存查到了的;挑选每拍重做) | collector `albumhint.go` 读写 |
...
| 主题 | 文件 + 符号 |
|---|---|
| 数据源主状态机(轮询/apply/清理/seek/偏移) | `lyrimuse/Sources/LyrimuseCore/Local/LocalPlaybackSource.swift` · `LocalPlaybackSource`(`poll`/`apply`/`clearIfWasPlaying`/`seek(toMs:)`);轮询档位 `PollInterval`(`playing 2s`/`paused 6s`/`idle 10s`/`nilGraceTicks 3`)+ `desiredPollInterval`/`adjustPollCadence` |
...
| App → collector 位置偏置文件 | `LyrimuseCore/Local/PositionBiasFile.swift` · `PositionBiasRecord` / `PositionBiasFile.encode/write`,写入点 `LocalPlaybackSource.publishPositionBiasIfChanged`;Go 侧 `lyrimuse-collector/positionbias.go`(`positionBiasApplies` / `currentPositionBias`),扣减点 `system.go` fetchRawMediaControlState 播放分支;文件 `~/.config/lyrimuse/lyrimuse-position-bias.json` |
...
| 浏览器广告闸探针(YT Music / Spotify 网页版) | `LyrimuseCore/Local/YouTubeMusicAdProbe.swift` · `YouTubeMusicAdProbe`(`probeJS`/`parse`/`gate`/`badgeVerdict`/`albumPatch`/`AdSlot`/`kickIfNeeded`/`cachedReading`)。⚠️ **时间常数之间有硬不变量**(决策 32,selftest 对每一档钉着):`adRefreshInterval(5)` 与 `songRefreshInterval(45)` 都必须**严格小于**可读期 `verdictMaxAge(60)`,相等就等于没有重叠窗、判定过期那一拍必然 fail-closed 把快照整条丢掉。`SpotifyWebAdProbe.swift` 同构(`refreshInterval(45) < verdictMaxAge(60)`,不分档)。两条探针的 `setResultSink` 挂在 `LocalPlaybackSource.startObservingPlayerInfoNotification`(结果落地立刻补查一次);闸本身在 `MediaControlClient.trustedPlaybackRejected` / `spotifyWebAdAccepted`;Go 侧 `lyrimuse-collector/ytmusicad.go` 判据逐字一致,但**同步探测**、缓存节流策略不跟 |
| 电台曲内时钟 | `LyrimuseCore/Local/RadioTrackClock.swift` · `advance`(纯算术,含 `seedPosition` 起表播种、`passedTrackEnd` 放完判定,selftest 钉住);落盘 `RadioClockFile.swift` · `restorable`/`shouldWrite`;起表时刻 `MediaControlStreamWatcher.digest` · `changedTrackKey`/`trackChangeInstant` → `MediaControlClient.noteTrackChangeObserved`;接线 `MediaControlClient.fetchRawMediaControlSnapshot`(判 `radioStationHash`、换 elapsed/anchor,**duration 原样传**)与 `refinedAppleMusicSnapshotIfNeeded`(电台不借 AppleScript 位置);「只勾 Apple Music」那条纯 JXA 路走 `MediaControlClient.radioAwareAppleMusicSnapshot` · `radioProbeNeeded`(纯函数,selftest 钉住)/`probedRadioStationHash`/`probeRadioStationHash` + `MediaControlSnapshot.withRadio`,collector 侧对应 `system.go` · `getAppleMusicOnlyState`/`mergeRadioKeys`(Go 单测钉住);分母替换与收歌词在 `LocalPlaybackSource.apply`/`fastTick`;台卡 `RadioStationCard.swift` · `stationName`/`card`,抓取在 `LocalPlaybackSource.noteRadioStationCard`/`noteRadioStationArtwork`,三个展示面经 `PlaybackCoordinator.isRadioTalkBreak`/`radioStationName`/`radioStationImage`;collector `lyrimuse-collector/radioclock.go` · `advanceRadioClock`/`applyRadioClock`,判据在 `snapshot.go` · `extract`,真曲长 hint 在 `radioduration.go` |
...
| 专辑名回填(播放器没报专辑) | `lyrimuse-collector/albumhint.go` · `appleAlbumHint`/`appleAlbumHintSync`/`albumHintTitleSplit`/`albumHintCandidatesFromTitleSplit`/`coverAlbumForTrack`/`coverNeedsHintCheck`/`fetchAppleAlbumHintCandidates`/`albumHintCandidatesFromResults`/`pickAppleAlbumHint`/`lyricResolvedArtists`;`snapshot.go` · `snapshot.AlbumHint`/`albumForUpload`;消费点 `poller.go` · `albumHintFor`/`relayAlbumHintSuffix`,`lb.go` · `lbMeta` |
...
17. **YouTube Music 每条队列的第一首没有专辑名,靠同一次探针从页面上补**(2026-09-03)
    ...
    - **补法:搭 `YouTubeMusicAdProbe` / `ytmusicAdProbe` 现有那次 JS 往返的顺风车**,零额外
      ...
      ⚠️ **2026-09-09 又插了一段**:现在是 `a|b|c|slot|album` —— 第四段是**广告徽章上的计数**
      (形如 `1/2`,给灵动岛显示「广告中 · 1/2 · 还剩 0:20」用,见 05 章「广告态」⑧),专辑名挪到
      最后一段。**计数插在专辑名前面而不是追加在末尾**:专辑名是自由文本、明确支持自带 `|`
      (`0|0|0||A|B` 那条守卫钉着),追加在它后面解析必然拆坏。同理 `parse` / `parseYTMusicAdProbe`
      **只认 5 段、不给 4 段旧形状留兼容分支** —— 专辑名含 `|` 时旧形状切出来也是 5 段、跟新形状
      逐字同形,分不开(这一点是先按「兼容」写、被那条守卫当场打红才定下来的)。两侧 JS 逐字相同,
      selftest 跨语言守卫盯着;计数在 **JS 里**就归一成 `1/2` 这种受限形状,抓法是**语言无关的
      两步**(用户追问「YT Music 不是中文的应该也可以吧」后改的第二版):先把时间样式的数字
      整段剔掉(`[0-9]+:[0-9]+` —— 徽章上常挂着「· 0:20」,不剔的话 `20 · 1` 会被当成一对),
      再抓「整数 + 1~12 个非数字 + 整数」(`([0-9]+)[^0-9]{1,12}([0-9]+)`)。中文「赞助商广告 1/2 ·」、
      英文 `Ad 1 of 2`、德语 `Anzeige 1 von 2` 一网打尽,不用为每种语言列词表;日韩「2件中1件目」
      这种总数在前的写法会解成 `2/1`,被 `AdSlot` 的「总数不小于序号」挡掉 → 不显示,宁缺毋错。
      (这个文件的 JS 里双引号和反斜杠都不许出现,写不了 `\d`,只能 `new RegExp('…')`。)
      ⚠️ **计数形状不对不 fail-closed**:前三段是判定、坏了必须不猜,计数只是装饰、坏了当没有,
      不能连坐掉「广告中」。collector 侧不消费这个计数,但必须正确跳过它。
...
25. **YouTube Music 里没有专辑名的音乐视频被整首标成「广告中」 —— 真凶是「配对过 Spotify 网页版的浏览器一律套 Spotify 字段启发式」,前贴片广告的缓存/棘轮是次因(2026-09-08,用户报「有视频的歌好像识别错了,变成广告了,比如现在播放的这首」;第一轮只修了次因、用户回「现在怎么还是不行」后第二轮才抓到真凶)**。
    **第二轮(真凶)**:第一轮装机后用户重放同一首,悬浮窗仍「广告中」,而此刻页面探针读数是歌、collector 在 08:58:29 一秒内就 `now playing`、之后 60 秒一次的复核也没再拒 —— 也就是说页面上根本没有广告,Swift 侧却自己判成了广告。把 `adByFields` 的输入逐个对:`isSpotify = isSpotifyNative || isSpotifyWeb`,而 `isSpotifyWeb = isPaired(浏览器, "spotifyWeb")` —— 这台机器 `np:browserPlatformPairsJSON` 里 Safari / Arc **同时**配了 spotifyWeb 和 youtubeMusic,于是任何 Safari 播放都被当成 Spotify,再撞上原生那条「album 空即广告」:王子《Why You Wanna Treat Me So Bad?》album 空 → 广告;同专辑带专辑名的《Sexy Dancer》正常(后台每 5 秒采样 70 秒,悬浮窗全程歌词、页面全程无广告标志)。差别只在 album 空不空,跟有没有前贴片无关。修法:网页版**只认 `SpotifyWebAdProbe` 的正向证据**(`.ad`;nil / `.song` 都不算 —— 跟 `MediaControlClient.spotifyWebAdAccepted` 那道闸同口径,网页广告形状 artist 空、本来就只有探针判成广告才进得来),原生 Spotify 的启发式 + AppleScript 复核原样保留,其它播放器一律不是广告;判据收成纯函数 `LocalPlaybackSource.adBreakByFields(isSpotifyNative:title:artist:album:youTubeMusicVerdict:spotifyWebVerdict:)`,selftest 十条(含用户那首的原样输入)。`nextAdBreakState` 的 `pageVerdict` 也只在**原生** Spotify 上置 nil,不再按 isSpotifyWeb 置 nil —— 否则配对了两个平台的浏览器里 MV 的前贴片回落照样被挡。Go 侧 `isAdBreak` 只认原生 bundle,没有这个越界;`spotifyWebAdAccepted` 注释里那句「放行之后由 adByFields 那套形状自然标成广告」随之失效,现在是探针缓存两处同读。**教训**:第一轮拿到 collector 日志里三轮 `rejected` 就认定根因,没有先核「页面此刻真没广告、UI 却说有」这条最直接的矛盾 —— 那条矛盾一比就能排除前贴片。**第一轮(次因,仍成立、仍保留)**:现场核实(Safari 播王子《Why You Wanna Treat Me So Bad?》,album 空、走 YT Music 广告复核):此刻用探针同款 JS 读页面 —— `#movie_player` class 只有 `ad-created` 没有 `ad-showing`、四个广告徽章选择器一个不中(只有隐藏的 `.ytp-ad-module`)、`document.title` 带歌名 —— 判定明明是**歌**,UI 却挂着「广告中」。collector 日志给出了时间线:08:30:22 / 08:30:27 / 08:30:37 三轮 `ytmusic: rejected as advertisement (王子 - Why You Wanna…)`,08:31:27 才 `now playing` —— **正好是 60 秒缓存到期后的第一轮**。机制:MV 的前贴片广告在 `#movie_player` 里放(`ad-showing` 为真),但 MediaSession 元数据一直是正片自己的 title/artist,不像音频歌曲的广告那样是一条独立的 now-playing 条目;于是前贴片期间探针把「王子 - Why You Wanna…」这个 key 判成 ad、缓存 60 秒,两侧都在这条前提上出了问题:① Go 侧 60 秒内每轮都按缓存拒掉,正片开头白丢一分钟(打卡本身没丢,只是 now playing 晚到);② Swift 侧 `isCurrentTrackAdBreak` 同曲期间**只往 true 棘轮、不回落**(那条棘轮是给 Spotify 广告字段闪变设计的),前贴片一过判定翻回 song 也回不来,整首 MV 挂着「广告中」。**修法**:(a) 两侧的判定复用窗口按判定分档 —— 歌仍 60 秒(稳态播放不白烧 AppleEvent),**广告只复用 5 秒**(`YouTubeMusicAdProbe.adRefreshInterval` / `refreshInterval(for:)`,Go `ytmusicAdRefreshWhenAd` / `ytmusicAdReuseWindow`),广告一条只有 5～30 秒,前贴片一过最多 5 秒被看到;Swift 侧**可读有效期仍 60 秒**、只缩再探间隔 —— 窄窗里 `cachedReading` 返回旧的 ad 判定而不是 nil,nil 会让 `gate` fail-closed 把快照整条丢掉、广告中途 UI 塌成"没有在播放"。(b) Swift 侧「广告中」的状态机收成纯函数 `LocalPlaybackSource.nextAdBreakState(previous:isNewTrack:adByFields:pageVerdict:)`:换曲按当下定初值;同曲判成广告往 true 棘轮;同曲**页面明确说是歌(`.song`)才回落**;nil(探针超时 / 还没探到)一律保持 —— 既不许抹掉已判定的广告,也不许把歌变成广告;Spotify 走这里时 pageVerdict 恒为 nil,它的字段启发式 + AppleScript 复核语义一字不变。**没做的**:不把 `ad-created` 当信号(它只说明这次播放曾创建过广告,正片期间也在);不改三条判据本身(当场读数证明它们对 MV 正片判得对);不给 MV 单独走别的路 —— 问题只在"多久再问一次"和"能不能回落"两处。验证:`swift build` 通过、selftest 3479 ALL PASS(新增 11 条:再探间隔分档 4 条 + 状态机 7 条);Go `go test ./...` 全绿(新增 `TestYTMusicAdReuseWindow`);跨语言 JS 逐字比那条守卫不受影响(JS 没动)。真机效果要等下一首带前贴片的 MV:预期前贴片期间显示「广告中」、广告结束后 ≤5 秒翻回歌词。
26. **YouTube Music 一首真歌整首显示「广告中」(第三次) —— 换歌边界的「裸标题」弱信号被缓存到下一首的 key 下,而 album 非空之后再没有任何地方去问页面,棘轮永远解不开(2026-09-08,用户报「帮我看看这首歌为什么还是显示广告中」:Safari 播 Prince《It's Gonna Be Lonely》,截图 17:53:13、歌进行到 1:15 仍「广告中」,歌词却正常滚动)**。**现场**:页面探针此刻判歌(`0|0|0`)、collector 09:51:56Z 一秒内 `now playing`、这首全程没有一条 `rejected` —— 跟决策 25 第二轮一样是"页面没广告、Swift 自己判成广告"。App 侧探针不打日志,时间线只能从统一日志的旁证拼:17:51:53.0 上一首《I Feel for You》曲末锚点归零 → 17:51:53.5 新歌首次发布(anchor 0.000)→ **17:51:53.8 `snapshot failed`**(这一拍快照被拒:YT Music 首次发布常常不带 album,`notASong` 命中、踢了一次页面探针,而正片 ~17:51:56 才起播,此前页面标题是裸的「YouTube Music」)→ 17:51:54.0 元数据**重发**(带上 album)→ 17:51:54.4 快照被采纳、换曲(`isNewTrack` 按当下判定定初值 → 「广告中」)→ 17:51:56.2 位置探针读到 `<video>` 还停在 0.0 → 17:51:56.5 菜单栏塌回图标(广告中不显示)。**后台采样坐实边界形状**(0.6 秒一拍同时读 media-control 与页面,18:07:19 抓到一次真实边界):上一首刚结束那一拍页面 `0|0|1 title=[YouTube Music] vt=0.0` —— **裸标题单独命中**;紧接着一条 6 秒真广告(YTM 自家 promo)全程 `1|1|1`、然后下一首 `0|0|0`。两个洞叠在一起:① 边界那次探针把"裸标题"这条**弱**证据判成 `.ad`,缓存到了《It's Gonna Be Lonely》的 key 下;② 探针只在 `MediaControlClient.trustedPlaybackRejected`"album 为空"那条路上被踢,重发后 album 非空、基础守卫直接放行,**再没有任何地方去问页面**,`nextAdBreakState` 等的 `.song` 永远不会来 —— 60 秒可读期一过判定变 nil、按「保持」走,整首 5 分半挂着「广告中」直到换歌;决策 25 修的"广告 5 秒再探"在这条路上根本没人触发。**修法(两层,都在 Swift;Go 不动)**:(a) `YouTubeMusicAdProbe.Reading` 记下判定的**强弱**(`strongAd` = ad-showing 或徽章命中;歌恒 false),新增纯函数 `badgeVerdict(_:)` 与 `cachedBadgeVerdict(forKey:)`:**界面口径**下只有强信号的 `.ad` 才算广告,只靠裸标题的 `.ad` 当「拿不准」(nil);`gate`(要不要采纳这条播放)与 `cachedVerdict` 不变、仍按任一命中即广告,collector 同款判据与 JS 一字不动(跨语言逐字比守卫不受影响)—— 那边误判只是这一轮不采纳、下一轮自愈。真广告期间三个信号 22/22 同时命中(头注样本 + 今天这条 6 秒真广告全程 `1|1|1`),裸标题从没单独撑过一条真广告,降它不丢召回。`LocalPlaybackSource.apply()` 的 `youTubeMusicVerdict` 改读 badge 口径(同时喂 `adBreakByFields` 与 `nextAdBreakState` 的 pageVerdict:弱 ad → nil → 既不点亮也不抹掉)。(b) `apply()` 在「广告中」亮着且是浏览器播放时**每拍都踢一次**探针(`kickIfNeeded` 内部按判定分档:广告 5 秒一探、歌 60 秒不动),让 `.song` 有机会到达、解开棘轮;`spotifyWebVerdict == .ad` 时不踢(那是 Spotify 网页广告,去问 YT Music 标签页只会 NOTFOUND 白烧往返),原生 Spotify 不走这里,非浏览器 bundle 在 kickIfNeeded 里自然 no-op。**没做的**:不改 JS、不改 `parse` 的 verdict 映射;不把"首次发布没 album 就丢一拍"那条路改掉 —— 那是 fail-closed 守卫的既定代价(每次边界 UI 会空一拍),另案。**教训**:决策 25 加的棘轮回落条件(`.song` 才回落)默认了"有人会一直问页面",但问页面的唯一入口挂在"album 为空"这个**跟广告无关的条件**上 —— 一个状态机的退出条件必须由**它自己**保证有人去产生,不能寄望别处顺路。**验证**:selftest 新增 14 条(badge 口径 6 + gate 不变 1 + 读数强弱 5 + 状态机/判据 2),`swift build` 通过、20 组 3503 条 ALL PASS;真机效果看装机后下一次 YT Music 自动切歌:边界弱 ad 不再点亮,即便强信号误报也会在 ≤5 秒内被 `.song` 解开。
27. **播放器没报专辑名时,按「署名 + 曲名 + 时长」问 Apple 目录补一个 —— 但只补呈现 / 上送,绝不进歌词缓存 key;跨文字系统的署名必须有歌词链路的旁证(2026-09-08,用户问「为什么现在这歌没有专辑上送」,随后拍板「可以加,作为一个通用逻辑」)**。**现场**:Safari 播 YT Music 里王子《Why You Wanna Treat Me So Bad?》的 MV,media-control 快照 `album=""`,页面 byline「王子 • 501万次观看 • 5万 人赞」只有频道链接、没有 `browse/MPREb` 专辑链接,`ytmusicAlbumPatch` 无从补起;relay key 是 `mac|Why You Wanna Treat Me So Bad?|王子||c`,网页 / 上送都没专辑。同专辑音频版(《Sexy Dancer》《I Wanna Be Your Lover》)都带 album「Prince」—— 差别只在 MV 是独立的「视频」实体。**为什么不用现成的 `cover_album`**:那是"封面匹配到哪张专辑"的记录,这首的条目里是「The Hits/The B-Sides」(Apple 精选),不是原专辑。**为什么不改 `raw["album"]`**(像 YT 页面补专辑那样):那条能改是因为 App 侧也从同一个页面 byline 读到同一个值、两边 key 一致;iTunes 反查只有 collector 做,改了 key 之后 App 按播放器原标签 `王子|Why You Wanna…|` 查歌词就落空 —— 这首歌会从"有词"变"没词"。所以回填挂在新字段 `snapshot.AlbumHint` 上,只从 `albumForUpload()` 流向四个出口(relay / Last.fm / LB / 本地收听日志);`trackEnrichment`、`isAdBreak`、专辑预取、会话 key 一律仍看 `Album`。**第一版被真机探针打回来两次**:① 按 Search 结果的 `releaseDate` 取最早 —— 结果精选「The Hits/The B-Sides」和原专辑「Prince」都标 1979-10-19(那是歌的首发日期,不是专辑的),挑成了精选;改成多 id `lookup` 一次补专辑级日期(31 个 id 一次请求),1979 vs 1993 才分得开。② 署名"跨文字系统就认"(照搬 09 章决策 45 给歌词用的规则)—— 周杰伦《七里香》在 US 商店只回别人的同名歌和「Choiyl」的「Jay - Piano Cover (Piano Version)」(时长差 4s 在容差内),于是把钢琴翻唱专辑安到了周杰伦头上;歌词同词无所谓,专辑写错就是错。改成 1 档必须有旁证:`lyricResolvedArtists` 读 enrich 条目的 `CanonicalArtist` 和已采纳歌词决策里胜出候选的 artist(王子那首 kugou 候选报「Prince」),两档都不中一律不采。由此顺带定下**两段式**:候选取一次落盘,挑每拍从缓存重做 —— 歌词晚几秒解析出来,旁证到位那一拍回填自然出现,不用为"歌词还没好"另设重试。**判据**收在纯函数 `pickAppleAlbumHint` / `albumHintCandidatesFromResults`(albumhint.go):曲名归一相等 + 时长 max(4s,3%) + ≥75s;署名 0 档(归一相等 / credit 子集,含繁简)、1 档(旁证);同档内「非群星合辑 > 非 Single/EP > 非豪华/重制版 > 专辑发行日期最早 > Apple 顺序」。`itunesResult` 为此多解码 `releaseDate` / `collectionArtistName`。**异步**同 Apple 目录锚点(poll 只读缓存、后台补取);relay 去重 key 加 `|a` 补推,会话 meta 同曲期间跟着补,`appendListen` / `mirrorScrobble*` / `updateNowPlaying` / `lbMeta.ReleaseName` 四处改读 `albumForUpload()`。**没做的**:App 侧灵动岛 / 悬浮窗专辑行不跟(`playback.album` 在 Swift 侧同时是十几处歌词缓存查询的 key,另开展示属性另案);不给 `www.youtube.com` 视频补(守卫处已拒);不持久化查空(Apple 目录会变)。**观察**:CN 商店当天对所有 Search 查询返回 0 结果,候选全部来自 US 商店 —— 这条也影响 09 章那几路 Apple 身份检索,先记一笔。**验证**:`go test ./...` 全绿(新增 albumhint_test.go 四组:候选过滤 / 挑选 / 排序 / 工具函数),`gofmt -l` 干净,`go vet` 干净;真机探针(装机前跑真实 iTunes):王子→「Prince」、Seal→「Seal II」、Patrice Rushen→「Straight from the Heart」、陶喆 Doxology 47s 不问、同名的「Alice Nowhere」→ 空;装机后看这首 MV 的 relay key 是否带上 `|a`、payload album 是否为「Prince」。**2026-09-11 追加**:用户问「为什么当前这首 YT Music 的歌没有专辑上送」—— Safari 播「音樂頑童」频道上传的《Musiq Soulchild - Buddy (Official Video)》,署名位是频道名、歌手在曲名里,主查询曲名永远对不上、署名两档也都过不去;加了「主查询零候选时按第一个破折号拆『署名 - 曲名』再问一次、候选须 Apple 署名与前段相符」的兜底(`albumHintTitleSplit`,候选带 `title_artist` 落盘)。同一现场还坐实两件事:开播那 6 秒内 iTunes 搜索 28 次有 11 次 403(限流),按现口径算「Apple 说没有」;这首 MV 比录音室版长 7.6s、超出 max(4s, 3%) 容差 —— 这两条都没改,单靠拆曲名这首仍不会有专辑。
28. **一次性真值样本喂给以连续流为准的伺服,赢不了那条流 —— 校正要落在"流读数的解释"上,不能落在"当前位置"上(2026-09-09,用户报「Spotify 进度很多时候偏慢,暂停一下再播放就准」)**。09-07 的 Spotify 探针每首歌问一次 Spotify 的钟、差过 0.3s 就重锚,验收时只碰到"差 <0.1s"的歌,于是没发现:重锚之后 MediaRemote 的每一笔读数仍从晚打 ~2s 的锚点外推、恒定落后,伺服把重锚当成偶然偏差在几拍内"纠正"回去(差 >2s 走 seek 分支当拖动,差 <2s 三拍 EMA),暂停时 `errEMA≈0` —— 它自认为一切正常。这跟 09-02 浏览器探针"每次都量准、每次都被扔掉"是同一类错,只是换了个吞掉它的分支。真机日志一一对上:开播 2.5s 探针量的差(+1.91 / +1.96 / +2.14)= 几十秒后暂停量到的落后量(+1.81 / +1.82 / +2.08)。修法:把差折进 `posReportedBiasSecs`(与 08-20 的自然切歌偏置同一个变量,允许负值),让之后每一笔流读数先加回它 —— 这才是"流的解释变了",伺服就没有东西可纠;地面真值分支挪到冻结 / 回绕 / seek 之前。配套:偏置归属改成"量它时对着的那个锚点"(决策 24 的精确化:基准换了作废,而"基准"就是那一份 anchorElapsedTime,不是"是否为 0");既然一次读数决定整曲,探针改两次采样验钟在走(第一版还加了 6s 上限,当天被真机两例 60~110s 的假锚点推翻,见决策 29)。为什么 Spotify 的开播锚点会晚 ~2s(歌是,广告不是,自然切歌与广告后开播都是)不知道,只能校正不能消除;暂停 / 拖动后 Spotify 重发的锚点是准的,所以用户的"暂停一下就准"是真的,这也是偏置在那一刻作废的依据。**同一个量,只测一次、两处用**:collector 那条链路(网页 / 飞书预览)不再自己问 Spotify,而是读 App 写的偏置文件——两个进程各测一遍不但重复 fork,还可能差几十毫秒各说各话;文件里带着偏置对着的锚点与量的时刻,collector 用与 App 同一条判据决定它还成不成立。
29. **MediaRemote 会在歌播到一半时把开播那份 now-playing 带着新时间戳晚发(elapsed 0.367 / 2.458),单看流跟"用户拖回开头"一模一样;Spotify 原生的锚点一变就再问一次它自己的钟(2026-09-09 当天第二轮,用户装完第一版后报「刚才还是不准,暂停重新播放才好」)**。日志(That's What I Like):16:34:02 开播锚点 0@34:02,16:34:07 探针纠 +2.216 之后一切正常;**16:35:01 流里冒出一个 `0.367@35:01` 的新锚点**,seek 分支当成真拖动重锚到 0.4s、偏置随锚点作废,歌词回到开头;16:35:20 用户暂停,Spotify 冻结值 79.905 对屏上 19.209,差 60.7s。同一进程里 16:12:30 的 `Perfect` 也来过一次(2.458,差 110s,那次探针量对了却被第一版加的 6s 上限当"离谱"拒掉)。08-20 记的 `isStaleAnchorRepublish` 只认"elapsed 逐 ms 相等、时间戳变了",这两次 elapsed 都变了(0 → 0.367),认不出;触发源仍没查到。修法不再猜锚点真假,**问有真值的人**:seek 分支重锚之后、以及偏置随重发锚点作废(播放中)之后,`SpotifyPositionProbe.requestConfirmation` 0.4s 后再做一次两采样探针,走同一条地面真值分支——真拖动时探针与新锚点一致(Δ<0.3s)什么都不改;假锚点时按探针重锚、差折进偏置,之后这个假锚点的每一笔读数都被加回去,直到 Spotify 再发一个真锚点(暂停 / 拖动)。同一首歌只允许一次确认在飞;6s 上限撤掉(两采样验钟在走已经够保真,而流那边倒是会差出 60~110s)。**同一轮顺手修了偏置的被减数**:`greedy` 开播半秒内 Spotify 把锚点从 0@47 改发成 2.434@48,流读数已跟着新锚点、我们自己的外推还在旧锚点上(差 1.45s),第一版按 predicted − 探针值折出 −2.025,流读数再扣它就超前真声 1.5s(暂停 delta=+0.523、恢复回退 −1.100);偏置的定义是"流读数相对真声偏多少",被减数必须是**这一拍的流读数**(`streamRaw` = snapshot.elapsedTime),我们的外推只决定"这一下要不要重锚"。教训:一条"重锚"能不能活下来、一份"偏置"该对着什么量,都得回到"这个量的定义是什么"——伺服以谁为准,校正就得落在谁身上。
30. **AppleScript `player position` 不是耳朵里的位置:它比 Spotify 暂停 / 恢复时发布的冻结值恒定领先一段,量级跟音频输出路径走(内建 0.06~0.14s,蓝牙 AirPods 0.51~0.65s);探针值要先扣掉这段领先量,领先量在每次暂停时学(2026-09-09 当天第三轮,用户报「感觉有一点点偏快」)**。真机对账(把探针量到的差与后来暂停量到的落后量相减 = 探针钟相对冻结值的领先):vampire +0.14、Adore You +0.10、RAYE +0.06、IDGAF +0.06(下午,内建输出);Rihanna +0.51、Kiss It Better 二播 +0.60、London Boy +0.65(17:26 起,`system_profiler` 此刻默认输出是蓝牙 AirPods)。先排除了两个假说:①"开播瞬态、领先量随时间衰减" —— 只读探针对 Capital Letters 从换歌起每 0.35s 同时读 media-control `elapsedTimeNow` 与 `player position` 连读 26s,差是常数 2.2s(±0.05),没有任何衰减;②"MediaRemote 外推漂了" —— 暂停时我们按事件时刻外推的 frozenRaw 与 Spotify 冻结值只差 +0.055。剩下的解释只能是 `player position` 报的是播放头(解码)位置、冻结 / 恢复锚点是出声位置,两者差一个输出链路延迟,蓝牙把它放大到半秒。用户对"准"的判据就是暂停再播之后的位置(= 冻结 / 恢复那套钟),探针把 App 拉到播放头位置就是整首偏快一个蓝牙延迟。修法(`probeLeadSecs`):偏置随暂停锚点作废那一拍,「我们停在的位置 − Spotify 冻结值」= 领先量样本(只在偏置是探针量的时候学;|残差|>1.5s 不学 —— 暂停中拖过进度条),没先验时第一份直接采信、之后 EMA α=0.5(换输出设备是阶跃,要跟得快),按机器持久化 `np:spotifyProbeLeadSecs`(机器本地键,不随配置搬家 —— 换机器就是另一条音频路径);消费探针值时先 `probed − probeLeadSecs`。诊断行 `probe lead learned: residual=… lead a -> b`。**同日第四版改成按输出设备分别记**(用户追问「不用蓝牙了依旧准确吧,逻辑必须通用」):单个 EMA 在蓝牙 ⇄ 内建每次切换后都要靠两三次暂停慢慢收敛、中间那段反着偏。改为 Core 侧 `AudioOutputRoute`(公开 CoreAudio HAL:默认输出设备的 UID / 名字 / 传输类型 + `kAudioHardwarePropertyDefaultOutputDevice` 监听)+ 一张按设备 UID 的表 `np:spotifyProbeLeadByDevice`(机器本地;单值键 `np:spotifyProbeLeadSecs` 启动时迁进表并删除):取值看此刻默认输出,学过用学过的,没学过按传输类型给先验(`probeLeadPrior`:蓝牙 0.5、内建 0.1、AirPlay / USB / 显示器 / 其它 0 = 不假设),第一份残差直接采信、之后 EMA α=0.5;默认输出一换立刻换值,并且正在放 Spotify、偏置又是探针量的,就 `requestConfirmation` 再问一次探针让位置在 ~1s 内按新领先量重折,不等用户暂停。诊断行 `output route changed: … probe lead a -> b`。**同日晚订正学习公式**:暂停量到的残差是**扣过当前领先量之后**剩的偏差,真值 = current + residual,第四版把残差当真值直接采信,真机 19:43 先验 0.5 下残差 0.07(真值 0.57)被写成 0.07、19:49 残差 0.50 又折成 0.285 —— 越学越离谱;改成没学过的设备 current + residual、学过的 current + 0.5·residual,并把表里 AirPods 那条按两次样本对出的真值 0.57 手工订正。Spotify 自己不会按输出延迟校正 AppleScript 位置,这一层只能校正不能消除。
31. **Spotify 的「单曲循环」不做;随机键改为按 `shuffling enabled` 能力位隐藏(2026-09-09,免登录白捡清单第 4 条 / 第 6d 条,用户拍板)**。三条路都实测够不到:① MediaRemote 的 repeat / shuffle 指令(media-control 的 `repeat` / `shuffle` / `toggle-repeat` / `toggle-shuffle`)Spotify **一概不理**——真机连发 repeat one / all / off 与 shuffle on / off,AppleScript `repeating` / `shuffling` 读回全部不变,二进制字符串表里也找不到 MediaRemote changeRepeatMode / changeShuffleMode 命令的处理器;② AppleScript `set repeating to true` 被接受(退出码 0)、值却不变,`repeating enabled` / `shuffling enabled` 在用户这个 Free 账号的播放上下文里均为 false——这两个 `… enabled` 是 Spotify 告诉脚本"允不允许改"的能力位,false 时写入就是 no-op,不是脚本写法的问题;③ 字典里 `repeating` 只是布尔,即便可写也映射不到「单曲循环」这一档(2026-08-21 起循环键在 Spotify 上就整颗不显示)。结论:循环键维持不显示;随机键顺着同一个能力位收口——`shuffling enabled` 为 false 时 `spotifyPlaybackMode(fromModePart:)` 读成 nil、整颗隐藏,不再摆一个点了没反应却会乐观亮起来的开关;能力位为 true 的账号 / 上下文(预期 Premium 或允许随机的播放上下文)随机键照旧显示、照旧可点。测试期间对 Spotify 的 repeating / shuffling 写入都已恢复原值。同一批(第 6a 条)把「在 Spotify 中显示」从"只激活 App"改成 `spotify:track:<id>` 深链跳曲目页,机制在 07 章「⋯」菜单一节。**免登录这条路到此收口(2026-09-10 用户拍板「就这样吧」)**:清单剩下的 ⑥b(停播页「最近听过」按 `spotify_track_id` 跳 Spotify——材料已在缓存里,但那一列统一跳 Apple 目录页,混两种去处不见得更好)、⑥c(简介面板多展示 album artist / popularity / played count 等)、以及网页版曲目 ID 的反查(页面只有专辑 / 歌手链接,要靠抓 open.spotify.com 的 embed 页按曲名时长对,格式无承诺)都**不做**。登录集成(Web API)2026-09-09 评估后同样不做:开发者模式上限 5 个用户且需 Premium、扩展配额 2025-05 起只给组织(250k MAU 门槛)、没有歌词接口、播放控制类接口仅 Premium 生效,对 Free 账号连单曲循环都换不来;Developer Policy 对第三方展示 / 缓存 Spotify 数据另有限制。位置精度(探针双采样 / 锚点确认 / 输出路径领先量,决策 28~30)是独立的一条线,不在这张清单里。
32. **广告闸探针的「可读有效期」与「再探间隔」取了同一个值(两个 60),于是判定过期那一拍必然 fail-closed —— album 为空的歌每 60 秒塌一次 UI,而一次 fail-closed 又把轮询自己降到 10 秒档,把 2 秒的抖动放大成 7 秒真空期(2026-09-11,用户报「YouTube Music 经过一个广告之后,下一首歌开始的时候有几秒真空期没有歌曲信息,灵动岛走到兜底的 lyrimuse 图标」)**。
    **这一条就是决策 26 末尾留的那个「另案」**(原话:「不把"首次发布没 album 就丢一拍"那条路改掉 —— 那是 fail-closed 守卫的既定代价(每次边界 UI 会空一拍),另案」)。真机日志显示代价远不止"边界空一拍"。
    **现场**(Safari 播 YT Music,`音樂頑童 - teachme`,253 秒,全程正常播放):`local` 类别的 `snapshot failed` 出现在 15:57:45 / 15:58:17 / 15:59:19 / 16:00:20 / 16:01:22 / 16:02:24 —— 间隔 **60.2 / 62.1 / 62.2 / 62.2 秒**,与「60 秒可读期 + 一个 2 秒轮询拍」严丝合缝;16:02:31 的锚点还在报 253.694s,这首歌一秒都没停过。也就是说**真空期不是只在广告边界,是稳态播放期间每分钟一次**,用户注意到的只是广告后那一次最显眼(广告期间 UI 有内容,塌下去落差最大)。
    **两个独立成因**:
    - **(a) 主因 —— 两个量取了同一个值,没有重叠窗**。`cachedReading` 的可读期是 `verdictMaxAge=60`,`kickIfNeeded` 的跳过条件是 `refreshInterval(.song)`,而后者当时**就是 `verdictMaxAge` 本身**。于是 age 跨过 60 的那一拍必然同时满足:① 读缓存刚过期返回 nil、② 这一拍才开始**异步**重探、结果拿不到 → `gate(verdict: nil)` = `.reject` → 快照整条丢掉 → `clearIfWasPlaying()` → 三个展示面一起塌。⚠️ **决策 25 当时就写明了这个机制**(「nil 会让 `gate` fail-closed 把快照整条丢掉、广告中途 UI 塌成"没有在播放"」),并为此给广告档留了 5 秒的窄窗 —— 但歌档没留,同一份注释里那句"两者之间的窄窗里 `cachedReading` 仍返回旧判定,不会出现 nil"对歌档**根本不成立**。原理识别对了,只应用到了一档。
    - **(b) 放大器 —— 一次拿不到快照,就把下次去拿的时间推迟 5 倍**。nil 快照走 `clearIfWasPlaying()` 清空 title(那是对的),紧接着 `adjustPollCadence()` 按 `title.isEmpty` 判成"空闲"→ 轮询从 2s 降到 **10s**。而探针 ~187ms 就把结果放进缓存了,没人去读。实测 16:02:24.464 `snapshot failed` → 16:02:31.486 `snapshot recovered` = **7.0 秒**,期间 16:02:27.651 `slot rebuild: fixed(223.5) -> icon(38.5)`(菜单栏塌成图标,灵动岛同时退到兜底图标)。这 7 秒还是侥幸 —— 靠 16:02:31 一条 media-control 事件唤醒补查才提前救回,没有那条事件就是满 10 秒。
    **修法(三层,全在 Swift;Go 不动)**:① `YouTubeMusicAdProbe.songRefreshInterval = 45`(新常量),`refreshInterval(.song)` 返回它 —— 留 15 秒重叠窗,稳态播放期间判定**永不为 nil**;代价是一首 4 分钟的歌从 4 次 AppleEvent 变 5 次。② `desiredPollInterval` 在 `consecutiveNilSnapshots ≤ PollInterval.nilGraceTicks`(3 拍 = 6 秒)内**留在 2s 档**,不让一次 fail-closed 自己延长自己的恢复;真停播时多跑两次 poll 就降档。③ 两条广告闸探针加 `setResultSink`(照 `SpotifyPositionProbe` 那条成熟先例),结果一落地就 `poll()` 一次,把**换曲 / 广告边界那一拍**(新 key 下缓存必然为空,fail-closed 是设计如此)的等待从"一整个轮询周期"压成探针往返本身。三条各治一个独立成因,缺一条问题就还在。
    **`SpotifyWebAdProbe` 同构缺陷同日一起修**:它的 `kickIfNeeded` 此前直接拿 `verdictMaxAge` 当跳过条件,同一个洞;改为独立的 `refreshInterval = 45`。它不按判定分档 —— 网页版广告是独立的 now-playing 条目(artist 空),判定翻转必然伴随 key 变化,没有 MV 前贴片那种同 key 翻转。
    **Go 侧为什么不跟**:`ytmusicad.go` 的 `ytmusicAdProbe` 是**同步**的,缓存过期就当场 `runYTMusicAdProbe` 阻塞探一次再返回,压根不存在"过期了但结果还没到"的那一拍。这是 Swift 为了不卡住 UI 轮询而选择异步换来的副作用 —— 两边在**判据**上仍逐字一致(那才是必须同步改的),缓存 / 节流策略本来就各按各的执行模型走。这一条现在写进了 `ytmusicAdMaxAge` 上方那段注释里("两边同时改"原话需要这个限定)。
