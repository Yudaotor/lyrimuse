...
| 分类 | 图标/色 | 内容 |
|---|---|---|
| 歌词（indigo） | text.quote | 四段：**获取**（**两张卡**，2026-09-05 拆开——此前一张「歌词来源」卡装五样东西，卡名只对第一样成立。卡一：歌词来源十源勾选[含每源独立「测试」按钮＋卡片右上角「测试」（原「全部测试」，2026-08-31 去掉"全部"两字），2026-08-30]；卡二不带卡名（同「译文」「效果」卡）：匹配算法智能/顺序优先[**纵向原生 radio 组**（`.pickerStyle(.radioGroup)`），「智能算法」标签带「（推荐）」后缀（格式键 `%@（推荐）`，沿用引导页「Apple Music 自动化权限（推荐）」写法）；副标题随选中项变（智能→「给每个来源打分，取分最高的」，顺序→「不打分，按下面的顺序取第一个有结果的来源」），原「?」气泡撤掉。⚠️ 2026-09-03 版是分段控件＋叠在 Picker 左上角的 8pt 橙星，2026-09-05 用户原话「星星好丑啊，违和感很强」，根因是 `NSSegmentedControl` 没法按格放东西、只能往控件外叠装饰——**以后要强调某一档就改文字，别再叠图标**]＋顺序列表[2026-09-05 每行左侧 `line.3.horizontal` 把手拖拽排序，算法在 Core `ReorderDrag`（滞回 6pt / 让位按槛距 / 写回完整排列保禁用槽位）；上下箭头保留作键盘 / VoiceOver 通路并补 accessibilityLabel；见设计决策第 20 条]＋**跟进算法升级**[2026-09-03，`lyrics_auto_upgrade`，默认开；关掉后已选定的歌词不再被后台重打分/升级重搜换掉，闸门在 collector 的 `needsLyricsRescore`/`needsLyricsRetry`，见第 09 章]＋提前解析同专辑）/**译文**（显示译文→译文语言→系统兜底翻译→翻译语言包[2026-09-05 从下拉菜单改成「已下载 6 / 18 ＋ 管理…」，点开在卡里展开 3 列语言网格：✓ 已下载不可点、⭳ 未下载点击触发系统下载 sheet、转轴下载中，网格下一句说明＋「打开系统设置」去删包；读数按**当前译文语言**统计语言对，译文语言自己与同语系语言被过滤，所以比系统设置的「已下载语言数」少是正常的——用户曾因此觉得"有 bug"，进程外探针核过系统真值一致]）/**效果**（繁简转换[按语言条件显示；2026-09-06 起是卡里第一行，分隔线随条件走]、显示罗马音＋日韩中分语言勾选、时间轴偏移±5s步长0.05＋作用于哪个播放器的下拉框。⚠️ **「卡拉OK效果」2026-09-06 从这里撤掉**——用户原话"这个开关是否没有必要放在这里，要么放到单独的某一种类型里面"：它讲的是某个面怎么画、其余三项改的是歌词内容本身；拆成悬浮歌词「文字」浮层 / 灵动岛「歌词行」浮层 / 菜单栏「配色」（既有的「卡拉OK染色」改名）各一颗，歌词窗口始终逐字，见 04/05/06/08 章）/**管理**（**两张卡**，2026-09-06 重排——此前一张无名卡装三样异质内容、按钮一处行尾一处另起行。卡一「歌词库」：卡名行右侧「打开歌词管理」小按钮［原独立一行「歌词管理 / 打开…」］＋统计块［「共 N 首」＋行尾占用空间 → 6pt 分段比例条（`SettingsProportionBar`，accent 三档明度＋灰＋橙一处）→ 四桶图例 → 「暂无」独占一行：橙色数字＋ⓘ＋行尾「重新扫描（N 首）」，原六格等宽数字撤掉］＋两条从属行「译文」「已缓存罗马音」［原竖线拼接的一句密文］；卡二不带卡名、单行「歌词文件夹」：路径 `~` 缩写＋「在访达中显示」「更改…」同一行行尾［原「选择文件夹…」「打开歌词文件夹」另起一行］，自定义过路径才多出从属行「已改用自定义位置 / 恢复默认位置」。见第 11 章 §7、§8）。分段选择器记忆上次停留段（`@AppStorage settings:lyricsSection`） |
| 播放器（mint） | play.circle | 播放器选择（Apple Music/QQ音乐/网易云/Spotify/自动；⚠️ **勾着「自动识别」时，没单独勾上的那几张卡显示成"由自动识别接管"**——虚线强调色描边＋右上角 ✨ 角标，卡片下方多一行说明，2026-09-11 加。auto 按超集处理，取消某个具体播放器此时不改变"认哪几个"，而在此之前六张卡长得一模一样、取消勾选还会如实变灰，用户会以为生效了；语义没动，只是界面把它说出来，完整记录见第 02 章解析优先级第 1 条）、Apple Music 自动化权限状态、后台采集服务状态。**侧栏项尾部带警告徽标**（2026-09-03 加，橙色感叹三角；2026-09-12 改成红色计数徽标 `SidebarCountBadge`、数字 = 警告条数，悬停看原因）：已选 Apple Music 且自动化权限被拒，或后台采集服务开着却没在跑；判定 `PlayerHealth`（Core，纯函数），数据源 `PlayerHealthMonitor`（设置窗口开着期间每 2s + App 激活时刷新，AE 权限查询与 collector 状态查询都在后台线程）。平时不亮，见设计决策 #17。页面订阅经窄代理 `PlayerTabStores`；浏览器卡片的实时状态由 `refreshBrowserLiveStatus` 后台查进 `browserLiveStatus` 缓存，body 只读缓存（见设计决策 #18） |
| 歌词显示（yellow） | rectangle.3.group | 四段按展示面分。前两段是**编辑台**版式（内容区里一块 1:1 的实时画布＋工具栏浮层＋舞台内的宽度调整条，页顶不钉预览）：**悬浮歌词**（`OverlayEditorStage`：**两行工具栏**共五个浮层，第一行「主题」[跟随封面/配色主题/我的配色主题；「跟随封面」2026-09-07 从「文字」搬来——它跟「配色主题」回答的是同一个问题，跟悬浮窗右键「更改配色」子菜单的排法取齐]／「文字」[字体/粗细/字号/卡拉OK效果＋文字色（跟随时这一行显示灰字「跟随封面」）/文字描边/描边色]／「背景」[背景色/毛玻璃背景]＋「重置▾」，第二行「排版」[双行显示/对齐方式]／「行为」[锁定位置/长按拖动/悬浮淡化＋截屏/录屏时隐藏、暂停/无播放时隐藏；浮层与抽屉「行为」组 2026-09-07 起调同一份 `OverlayBehaviorSettingsRows`，抽屉分组顺序标题跟工具栏一一对应、原「窗口」组拆成组外「宽度」一行 + 「行为」组，见 04 章第十九步]（⚠️ 2026-09-02 按"改的是哪一层"把原来的「文字」＋「配色」重新分成 **主题 / 文字 / 背景** 三组，组件 `OverlayThemePopover` / `OverlayTextPopover` / `OverlayBackgroundPopover`；工具栏按钮已接上，两个过渡壳 `OverlayColorPopover` / `OverlayStyleSummary.color` 已删、全仓 0 引用，见 04 章第十七步）；舞台内宽度条；下面只剩开关＋「全部设置」折叠抽屉）/**灵动岛**（`NotchEditorStage`：两行工具栏共八个浮层[风格/屏幕/左耳/右耳＋歌词行/字体/展开态/行为；「字体」2026-09-09 加：字体族选择器＋粗细六档下拉＋字号滑杆 11～17pt，控件照搬悬浮歌词「文字」浮层前三行，只管歌词文字（主行/副行/展开预览/广告态那一格）、行高不变、副行与展开预览固定 11pt，抽屉「字体」组同一份，见 05 章「歌词行 → 字体」与决策 #34；2026-09-07 按卡片解剖重排——「音浪」在左右耳浮层顶部是一行开关（抽屉末尾那张独立音浪卡撤掉）、「展开时预览下一句」从「展开态」搬进「歌词行」挂在「副行」下、「展开态」= 控制区两颗＋「快捷操作」（同日晚些加：展开态头部右侧四颗图标键 搜索歌词 · 显示歌词 │ 设置 · 关闭，见 05 章决策 #28）＋「曲目信息」标题行带封面/歌名/歌手/专辑四个子行，浮层与抽屉调同一份分组视图，见 05 章决策 #27]＋「重置▾」＋舞台内宽度条；下面是开关＋「全部设置」折叠抽屉[2026-08-31 补齐，跟悬浮歌词那个同一个定位；2026-09-07 起分组顺序与标题跟工具栏一一对应]）/**菜单栏**（`MenuBarEditorStage`，2026-09-01 也改成了编辑台：两行工具栏[第一行 布局（宽度模式/对齐方式/副行/歌词旁的图标）／配色／字体（2026-09-03 加：粗细六档下拉 + 字号滑杆 10~16pt，字体族仍跟系统；「粗细」英文词条为此从 Font Weight 缩成 Weight，横向预算见 `MenuBarEditorStage.toolbar` 头注）＋「重置▾」；第二行 行为（悬停显示播放控制/无歌词时显示歌名）——2026-09-07 按"不要这一个那一个"重排：两个行为开关此前因第一行预算用尽寄住在「布局」里，拆成第二行的「行为」浮层；「副行」从「字体」挪到「布局」（排几行是版面）、「字号」在副行开着时显示灰字「由副行决定」而不再整行隐藏；抽屉从十一项平铺改成按工具栏镜像分组 布局/配色/字体/最大宽度/行为，见 06 章「设置项」节末尾]＋一块**舞台**——壁纸铺满、仿菜单栏条贴上边缘、宽度胶囊浮在舞台内的通道里，caption 在舞台外；下面是总开关＋「全部设置」折叠抽屉。⚠️ 「页顶仍钉一条仿菜单栏预览」这个旧说法从那天起就不成立了，预览已经搬进可滚动内容区；壁纸底 2026-09-02 又从"只垫菜单栏那一条"扩展成"包住整块舞台"，见 06 章）。⚠️ **原来还有第四段「其它」，只装着一张跨形态的「自动隐藏」卡，2026-09-01 整个撤掉**（用户原话：「不要单独开一个其他页面出来了」）——那两个开关同时被**拆成两份独立的值**（`hideDuringScreenCapture`/`hideWhenNotPlaying` 归悬浮歌词，`notchHide*` 归灵动岛），各自摆进对应形态那一段（排在「全部设置」抽屉上面）。⚠️ **2026-09-02 再进一步**：那两张独立卡也撤掉了（用户原话：「不要单独放在外面，要遵循设计理念，放到行为卡片里面去」），两个开关分别并进悬浮歌词的「行为」栏（＋抽屉「窗口」组）和灵动岛的「行为」浮层（＋抽屉「行为」组），渲染真源合成一份 `UI/AutoHideSettingsRows.swift`——**共用的是视图与文案，值仍是两份**。这一页因此不再有任何一张只装隐藏开关的卡。⚠️ **同日第三次**：用户看过成品后要求「和灵动岛设置页一样处理，放到上面的小按钮里面，点了出现下拉框」，于是悬浮歌词那条**「行为」栏本身**也删了（`OverlayBehaviorBar`），五项进了编辑台工具栏新起的第二行「行为」浮层（`OverlayBehaviorPopover`，宽 420，跟灵动岛那个同宽）——两个形态的「行为」入口至此完全同构。这一段因此只剩编辑台＋开关＋抽屉三块，页面内容高度 605→449pt。详见 04 章第十六步。⚠️ **这两个数按当时口径少算了约 86pt**（一直拿第一步的画布 246 在加，而画布第九步起就是 332），真实的逐块账见 04 章「高度账」那张表。⚠️ **2026-09-03 又压了一次**（用户原话：「这个预览窗口给我高度搞小一点，把下面的开关完整漏出来」）：编辑台 425→**382**（`maxCardHeight` 288→266，＋删掉舞台底下那行常态为空的 caption，那句「两端已裁切」搬进舞台里跟宽度调整条并排），总开关卡的底边从 551.5pt 落到 508.5pt，552pt 高的窗口（可视内容 519pt）里从"只露 13.5pt"变成完整露出；抽屉头仍在折线以下。详见 04 章第十八步。删掉一个分段 rawValue 是安全的：`section` 计算属性带 `?? .overlay` 兜底，老用户上次停在「其它」段的话下次打开会落回悬浮歌词。⚠️ 页顶那层固定头部**收不到点击事件**，凡是可交互的预览都只能待在滚动区。歌词窗口刻意不在此页配置（按需打开的窗口，非常驻展示面） |
| 快捷键（teal） | keyboard | 全部 16 个全局快捷键的录制行（四张卡：显示 / 窗口 / 歌词时间轴 / 播放控制）+ 步长 |
| 通用（gray） | gearshape | 菜单栏与 Dock（菜单栏图标：12 款 2×6 的 `MenuBarIconPicker`，行尾常驻所选款名；随播放律动；在 Dock 中显示）、语言与启动（语言：跟随系统 / 简体中文 / 繁體中文 / English，引导页欢迎页同一组选项；开机启动）、备份与迁移（iCloud/自选文件夹备份、设置文件导出/导入；配置文件夹 2026-09-01 搬去「关于」）、**封面**（**动态封面**，2026-09-09：有 Apple Music motion artwork 的专辑让封面动起来。⚠️ 2026-09-10 起**只管歌词窗口那张封面卡**——落地当天灵动岛展开态那枚小图也叠了一层，用户看过实机后拍板撤掉（「灵动岛上的效果不是很好」），那一格最大 32pt 量级、motion artwork 缩到这个尺寸只剩蠕动色块；见 03 章第 6 节）、清除所有设置。⚠️ **「封面」这张卡 2026-09-10 从页首挪到了这里**（用户原话：「在通用的下面加吧，不分散了；就是把现在这个位置下移即可」）——落地那天它在页首，占掉了本该留给「菜单栏与 Dock」的位置（那是这一页唯一带画面的一张），而这一页的副标题压根没提封面。**它待在「通用」而不是「歌词显示」**：那一页严格按展示面分段，而**歌词窗口按设计压根不在那页配置**。同一句「不分散了」还定了将来的落点——歌词窗口那批候选配置项（字号/字体/对齐/景深/当前行位置/动态背景/时间行显示…，清单见 07 章）也落这一页、跟「封面」卡为邻，**不**给「歌词显示」加第四段、**也不**在窗口内另开入口（设置搜索的目录只映射设置页，窗内入口搜不到）。压轴仍是「清除所有设置」：本页唯一不可撤销的动作。⚠️ **这一页没有预览**：2026-09-04 曾按「歌词显示」编辑台范式在页顶放过一块仿菜单栏舞台（真壁纸 + 所选图标本体、律动时真的动），用户看完明确不要（「我不想这块」），次日撤掉——菜单栏就在屏幕顶上，选哪款抬头就看得见，别再加回来 |
| 关于（blue） | info.circle | **2026-09-03 重排**：页头 = 图标 + 名字 + 可点击拷贝的版本胶囊（版本 · 芯片架构）+ tagline + 两颗胶囊按钮（请作者喝杯咖啡 / GitHub 带 star 数）+ 求 star 那句；四张带标题的卡：更新（**2026-09-12 起只剩一行「软件更新」入口 + 副标题「上次检查 / 有新版本」**，检查更新 / 自动检查 / 自动下载并安装 / 测试版更新[2026-09-05，机器专属键 `np:receiveBetaUpdates`，见 15 章决策 11]四个控件全搬去了「软件更新」页，见下文与决策 #25）、反馈与社区（反馈问题、想法与建议）、许可与版权（版权说明、第三方许可、开源许可证）、诊断与数据（导出诊断、配置文件夹）；页脚「© · GPL-3.0」。

**每页都有页内大标题；窗口尺寸跟着最高的那一页定（2026-09-13）**：六页全部走 `SettingsPage` 的页头三件套（22pt 粗体标题 + 13pt 次要色副标题，「关于」另有 hero 图），`showsHeader` 这个开关现在没有任何调用点关掉它。「歌词显示」曾经是唯一的例外——2026-08-30 用户嫌"太占位置"关掉过，2026-09-13 用户把六个分类逐页截图并排看，指出只有这一页没有大标题、"会有点不一致"，要求加回来。窗口 `.frame` 同一次从 `minHeight 520 / idealHeight 640` 抬到 **`690 / 720`**：页头多占 68pt，而这一页的悬浮歌词段本来就是六页里最高的一段。实机量过（900×652 的窗口截图按 2x 采样）：标题栏 32pt，页内从上留白 26 起算，「桌面悬浮歌词」总开关卡的底边落在 605pt、「全部设置」抽屉头的底边在 658pt，加 28pt 下留白 = 686pt——690 是"这一段整段不用滚"的下限。⚠️ **两个数里只有 `minHeight` 改得到已经存在的窗口**：这扇窗是 SwiftUI 的 `Settings` 场景，尺寸由 macOS 自动存档，`idealHeight` 只在没有存档时（首次打开 / 重置）说了算；老用户手里那扇 652pt 高的窗口是被 `minHeight` 顶上来的。所以这一档不能按"别比头部还矮"那种下限定，得卡在"总开关卡整张露出来"之上。三段（悬浮歌词 / 灵动岛 / 菜单栏）都在 722pt 的实机窗口里核过，全部一屏放得下。逐块的高度账见 [04 章](04-desktop-overlay.md)「高度账」那张表。
...
**侧栏顶部的设置搜索（2026-09-09，借鉴清单 S8）**：侧栏 `List` 顶上钉一个搜索框（`SettingsSearchField`，`.safeAreaInset(edge: .top)`，不随列表滚），有字时整张侧栏换成结果列表（每条 = 标题 + 面包屑「分类 › 分段 › 组」），清空即恢复——系统设置就是这个做法（调研过的同类 App 也一样）。结果来自 Core 的静态目录 `SettingsSearchCatalog`（每条设置行 / 卡登记：目的地、分段键与取值、所在抽屉、标题 L10n 键、可选副标题键、关键词、面包屑），App 侧 `SettingsSearchIndex` 把它本地化后按 `SettingsSearchMatcher` 排序：查询按空白拆词、每个词都得命中，标题前缀 > 标题包含 > 只在副标题 / 关键词 / 面包屑；三种语言的译文一起索引（从 bundle 的三份 `.lproj/Localizable.strings` 读），所以英文界面打中文、中文界面打英文都搜得到。命中一条做四件事：`selection` 切到目的地（账号页若在折叠的「实验室功能」区先展开）；往那一页分段的 `@AppStorage` 键写取值（`settings:lyricsSection` / `settings:appearanceSection` / `np:lastfmDetailSection`，那边立刻翻）；`SettingsSearchRouter` 发两路信号——「要高亮的标题集合」与「要展开的抽屉」，由 `SettingsView` 根部塞进 Environment（`settingsSearchHighlightedTitles` / `settingsSearchPendingDrawer`），三个「全部设置」抽屉看到自己被点名就 `withAnimation(.settingsCardReveal)` 展开并 `consumeDrawer`，`SettingsRow` / `SettingsSubRow` / `SettingsCardHeader` 看到自己的 `title` 在集合里就画一层 accent 高亮（1.8s 后淡出）并用 AppKit `scrollToVisible` 把自己滚进可见区；最后清空搜索框。键盘：⌘F 聚焦搜索框、回车打开第一条、Esc 先清空再让出焦点。**焦点纪律**（同日用户实测提出）：窗口刚打开时搜索框不抢焦点（它是键视图环第一个文本框，AppKit 默认会给它第一响应者，`onAppear` 下一运行环让掉，跟系统设置一致）；选了侧栏别的分类、或打开了一条结果后焦点让出，光标不再闪——`@FocusState` 因此放在 `SettingsView` 而不是搜索框里，以 `FocusState<Bool>.Binding` 传下去。搜索框文字不持久化，也没有任何新 UserDefaults 键。**目录会漂，靠 selftest `settings-search` 组守着**：源码里每一处 `SettingsRow(` / `SettingsSubRow(` / `SettingsCardHeader(` 的字面量标题、三族"标题在枚举里"的行（`OverlayBehaviorItem` / `AutoHideItem` / `NotchBehaviorItem`）都必须登记在目录里（纯分组卡头与只读状态行写在那边的白名单，白名单项反过来必须真存在于源码），目录里每个键都必须在 `Localizable.xcstrings` 里，目的地 / 分段取值必须跟 `SettingsTab` / `AccountDestination` / 两个 `Section` 枚举对得上。**新加一行设置 = 到 Core `SettingsSearchCatalog` 登记一条**，不登记 selftest 会红。理由见设计决策 #23。

**侧栏按系统「设置」重排（2026-09-12，用户拍板）**：用户把我们的侧栏和系统设置的侧栏并排截图，要求「全面向苹果原生这种设计靠拢」。对照落地（新组件在 `Settings/SettingsSidebarChrome.swift`，头注有一张对照表）：① **身份区**——侧栏顶部一行 `LastfmIdentityRow`（tag 仍是 `.account(.lastfm)`，点了进 Last.fm 页）：36pt 圆形头像 + 用户名（15pt semibold）+ 「Last.fm 账户」；头像来自 `user.getinfo` 的 image（`LastfmStatsService.fetchUserAvatarURL`，占位星过滤成 nil → 退回品牌图裁圆），`LastfmAvatarStore` 只在内存缓存一张、失败 10 分钟内不重试、两跳请求都记 `NetworkAuditLog`；未连接时是灰底白人像 + 「连接 Last.fm」+ 「同步收听记录」（首版沿用 Last.fm 页头那句「把你播放的歌记录到 Last.fm」，130pt 宽的文字列里截成省略号，用户当天看了说丑，换成六个字）；授权失效 / 连接失败在头像右上角挂红色「!」，悬停看原因。原「账号」Section 与 `AccountSidebarRow(.lastfm)` 撤掉。② **「有软件更新可用 ①」**——`SparkleUpdaterManager.availableUpdate` 非 nil 时身份区下面多一行 `SoftwareUpdateSidebarRow`，是按钮不是分类：切到「关于」并调 `checkForUpdates()`，已查到的版本这一步直接弹 Sparkle 的安装对话框；没有新版本时整行不存在。③ **分组去标题**——「核心设置」撤掉，六个分类是一段无标题 `Section`，分组之间只留空白（「核心设置」「账号」两个键留在 catalog 里不删）。④ **「实验室功能」改原生折叠**——`Section(isExpanded:content:header:)`，header 里保留标题 + 「?」悬浮提示；此前手搭 Button + 转 chevron 的理由（"原生放不下 ? 图标"）只对 `Section(_:isExpanded:)` 便捷初始化成立。⑤ **图标**——`IconBadge` 默认 22/6 → 20/5（系统设置侧栏图标尺寸），连续圆角 + 上亮下暗的竖向渐变（白 0.20 → 黑 0.07）+ 0.5pt 半透明白描边；账号页头等所有 `iconBadge` / `accountIconBadge` 调用点同一份，深色亮度封顶不变。⑥ **红色计数徽标**——「播放器」健康警告从橙色三角改成 `SidebarCountBadge`（红胶囊白数字，数字 = 警告条数），跟「有软件更新可用」同一套。⑦ **搜索框**——占位词「搜索设置」→「搜索」，26pt 高、圆角 8；材质在 macOS 26+ 是液态玻璃（`.glassEffect(.regular)`，同日用户追加要求「像系统设置那样」），旧系统退回 quaternary 圆角底，`#available` 门控、不模拟，跟悬浮窗胶囊 / 设置卡同一取舍；两档都带 0.5pt 发丝描边。⑧ 侧栏列宽 170/190/220 → 180/205/240。四个新 L10n 键：「搜索」「Last.fm 账户」「有软件更新可用」「同步收听记录」。理由见设计决策 #24。

**「软件更新」页（2026-09-12，用户提议「做成苹果这种更新页面」，两个方案里拍板了 B：自定义 Sparkle 界面驱动）**：新增侧栏目的地 `SettingsSidebarItem.softwareUpdate`（不是 `SettingsTab`：不进六分类、不记上次停留），页面 `Settings/SoftwareUpdatePage.swift`，结构照系统设置那页：顶上一张状态卡（App 图标 + 「Lyrimuse 1.7.0」+ 大小 · 发布日期，或检查 / 下载 / 解包进度 + 右侧动作），卡里往下是发版日志（appcast `<description>` 的 HTML，Sparkle 已按系统语言从两份 `xml:lang` 里挑好；`NSAttributedString(html:)` 解析后字体统一压成 13pt 系统字、标题与加粗 semibold、颜色换 labelColor、链接保留，`ReleaseNotesRenderer`）；然后「已安装」一行、「自动更新」卡（自动检查 / 自动下载并安装）、「测试版更新」；页脚「上次检查：…」。**三处入口**：侧栏「有软件更新可用」那行（有新版本时才存在，现在是 tag 为 `.softwareUpdate` 的可选行、选中会亮）；「关于 › 更新」卡只剩一行「软件更新」入口 + 副标题（原来的四个控件全搬走）；任何「检查更新」动作（菜单栏右键菜单、面板底栏、页面按钮）都先把窗口翻到这一页再查——检查中 / 已是最新 / 找到了都显示在页上，跟系统设置从菜单点「检查更新」落到软件更新面板一个意思。另有深链 `lyrimuse://settings/software-update`（仿 `x-apple.systempreferences:`，AppDelegate 按 host 分流，Last.fm 授权回跳不受影响；带 `?check=1` 顺手发起一次检查，支持回复里「点这个链接检查更新」就靠它，本机核对「检查中 → 已是最新」那条回调链也是靠它）。**Sparkle 这边**：`SPUStandardUpdaterController` 换成 `SPUUpdater(hostBundle:applicationBundle:userDriver:delegate:)` + 自己的 `SoftwareUpdateDriver`（`Settings/SoftwareUpdateDriver.swift`，`SPUUserDriver` 全部回调转发成事件），一个 Sparkle 弹窗都不弹；管理器新增 `flow` 状态机（idle / checking / downloading / extracting / readyToInstall / installing / failed）与 `pendingItem`（版本、大小、日期、说明、Release 链接、是否已下好），页面只画不算。**谁在等**决定「找到了」怎么答：周期检查（没人在页面上等）→ 记下来给侧栏 / 页面显示、立刻 dismiss 让 Sparkle 收工（自动下载开着时 stage 已是 downloaded，dismiss 即「退出时安装」，页面据 `installOnQuit` 说明并把按钮换成「立即重启」）；页面上点「检查更新」→ 找到后攥着 reply，「立即更新」才答 install，窗口关掉答 dismiss（`settingsWindowClosed`）；点「立即更新 / 立即重启」而手里没有 reply → 再发一次检查、found 里按 `.install` 意图自动答，已下好的包 Sparkle 直接进安装。「下完待装」这一步「立即重启」答 install、「退出时安装」答 dismiss（`SPUUserDriver.h` 的语义）；首次运行的「要不要自动检查」问询不弹窗，按开关当前值答。菜单栏面板底栏那格与 `availableUpdate` 照旧（点了翻到这一页而不是弹 Sparkle 窗）。搜索目录里原「关于 › 更新」下的四条改指 `.softwareUpdate`（目的地枚举新加 `case softwareUpdate`，selftest 的目的地核对 / 扫描文件列表 / 白名单同步）。预览钩子 `settings:previewUpdateVersion` 改成造一整条假 `SoftwareUpdateItem`（说明正文明说是预览）。新词条 20 个。理由见设计决策 #25。
...
- **这个永久偏好关着时，辅助窗口自己借一个 Dock 图标**（`AuxiliaryWindowActivation.swift`，2026-08-04）：「设置」/「歌词管理」/「歌词窗口」/「欢迎使用」四扇窗各自的根视图在 `.onAppear`/`.onDisappear` 里报到，用一个开关计数器 `openCount`——只要还有任意一扇开着就借 `.regular`，全部关掉才还原成 `.accessory`，不跟上面那条永久偏好打架（用户手动开了永久显示的话，这边全程不用管）。目的是这几扇窗打开期间能进 Cmd-Tab、能靠 Dock 图标切回来，不用非得先回菜单栏点。
  - ⚠️ **最小化的窗也算"开着"**（`.onDisappear` 只在真正关闭时触发），所以图标会继续借着——这是设计，不是 bug。但 2026-09-15 之前判据里的 `canBecomeMain` 在窗口最小化时是 false，把最小化的窗整个排除在外，于是那扇窗点 Dock 叫不回来、图标又一直赖着；用户开着「最小化窗口到应用程序图标」时 Dock 上连缩略图都没有，表现就是"窗口明明都关了，图标不走"。见 07 章决策 23。
  - **计数器只是代理值，另有一道对账**（`reconcile`，2026-09-15）：真实窗口列表说一扇都没开着，就把 `openCount` 清零并还原 `.accessory`。没有这道闸时，任何一次只加不减都会让计数器永久停在 >0，而"还原"整个挂在归零上，用户开一扇关一扇也回不去（+1−1），只能重启 App。挡 `NSApp.isHidden`（Cmd+H 时窗口没关但 `isVisible=false`）、延后一拍执行（关窗那一拍窗口还在列表里），两条都有实测支撑。
...
- **首启引导**（OnboardingView）：分步向导，每步直接绑定 AppSettings/FeatureSettingsStore **立即生效**（不做最后统一确认）；步数按所选播放器动态算（选 QQ 音乐则跳过 Apple Music 自动化权限步）；关窗=稍后再说（下次启动再问）。
  ...
  - **欢迎页底部一句不阻断的告知**（2026-09-03）：「继续即表示你已了解 版权说明」，链接与关于页那一行同一处（`LegalNotices.openUsageNotice`）。**刻意不做阻断式「接受」页**：引导原则是介绍性内容不锁下一步（跟 `.lastfm`/`.lyricsExtras` 同档），GPL 个人工具没有需要用户「接受」的条款，「接受版本号存偏好」那套是分发渠道场景的产物；一句告知 + 链接就是这件事在个人工具上的正确尺度。
  ...
  - **最后一页会撒花**（2026-09-11 用户要求「加入 Mac 很经典的那种撒花效果」）：走到 `.done` 就有一阵小纸片从窗口上沿之外落下来，左右飘摆、边落边翻面（正对镜头时最宽、侧过来收成一条线），约 4.1s 洒完。形态照 macOS / iMessage「庆祝」那种**下落式**，不是从中心炸开的礼花——后者在 480×420 这扇窗里要么盖住这一页那几行字，要么得再塞一套爆炸＋重力参数。几何在 `LyrimuseCore/Util/ConfettiField.swift`（确定性 SplitMix64 播种：同一个 seed 同一场雪），View 侧只有一层 `Canvas`（`UI/ConfettiOverlay.swift`，90 多片纸屑各一个 SwiftUI 视图会让每帧布局重算翻两个量级；也不用 `drawLayer` 包每一片，路径先在原点画好再整体仿射搬到落点）。
    - **盖在整扇窗上**（叠在 `.frame(480×420)` 之后）：纸片从进度点和「开始使用」上面落过去，而不是只落在上面那块内容区里（后者看着像"纸片撞在一条看不见的线上"）。`.allowsHitTesting(false)`，按钮照常能按。
    - **触发挂 `currentStep`、不挂 `step`**：最后一页的**下标**会因为设置窗口同时改播放器集合而变（见上面那条越界）。`onChange` 只在值真变时触发，所以停在这一页不会反复重放；退回去再翻回来会再撒一阵（seed 掺了 burst 计数，不是同一场雪）。
    - ⚠️ **到点把整个 `TimelineView` 摘掉**（`run = nil`），不靠 `paused:` 兜——`.animation` 只要还在视图树里就按屏幕刷新率一直重算 body，而撒花是一次性的四秒（同第 07 章那条"离屏探针实测五种不可见状态都还是 ~63 次/秒"）。帧率上限钉 60Hz：ProMotion 上多出来的那一半帧，对匀速平移看不出差别。另一处坑：`.task(id:)` 换 id 时是"先取消旧任务、再起新任务"，被取消的 `Task.sleep` 只是抛错、`try?` 吞掉之后**收尾那行照样会跑**，把新任务刚设上的 `run` 清掉（表现成"翻回来撒花闪一下就没了"）——收尾前必须 `guard !Task.isCancelled`。
    - **`reduceMotion` 下整个不画**：它是纯装饰、不承载任何状态（该说的话由「一切就绪」和体检清单说），跟灵动岛那批"补间可以省、变色不能省"（那是"点到了"的功能反馈）不是一档。
    - **参数是一组一起调的**（出场窗口 1.5s／落速 1.5–2.6s／130 片），离屏把落点画成 PNG 逐帧看过才定：第一版 0.9s／90 片／1.6–2.9s 画出来是**一条纸带扫过去**（最后一片 0.9s 就出场，到 1.8s 上沿已经全空），拉长出场窗口之后前后叠上，t=1.4 / 2.2 两帧都是满窗的持续洒落；片数同比例补到 130 是为了保住"同时在画面里多少片"。
    - selftest 新增 **`onboarding` 组**（18 条）：同 seed 同一场雪、**到 `duration` 那一刻每片都已整片落出下沿**（否则收尾会被整块抹掉）、`elapsed=0` 时每片都还在上沿之外（不是凭空出现在画面中间）、行程中点必须真的在画面里、纵轴只往下不回弹、翻面只让纸片变窄且不会缩到看不见、空画布／0 片／负数片不崩。
...
- **工作流**：改 `Localization/Localizable.xcstrings`（唯一真源）→ 跑 `python3 Localization/generate-strings.py` → 三份 `.strings` 生成物一起入库。selftest 有双向一致性守卫。生成物入库是因为 xcstringstool 只在完整 Xcode 里。
  - ⚠️ **用脚本改这份 catalog 时，序列化风格必须跟 Xcode 一致，否则整份文件都会进 diff**（2026-09-10 实测：一次 Python `json.dump` 写回让它相对 HEAD 变成 **19570+/19412−**，1345 个键几乎每行都不同，而真实改动只有 9 个新键；两个并发会话都被这份假 diff 卡住——挑 hunk 时无从分辨谁的词条，还差点把整份格式和别人未提交的键一起提交上去）。Xcode 的写法是 `indent=2` + **`separators=(",", " : ")`**（冒号两边都有空格）+ 结尾一个换行。⚠️ **不要**顺手 `sort_keys=True`：这份文件不是按 Python 的码点序排的（实测 `清除搜索` 排在 `没有找到匹配的设置` 前面），重排会白添两百多行 churn。**动手前先做一次 round-trip 断言**——把 `git show HEAD:…xcstrings` 读出来、按你打算用的参数写回、跟原文件 `filecmp.cmp(shallow=False)` 比一次，相同了再改工作树那份；改完逐键比对确认只动了自己那几条。
- UI 文案约定：不加句尾句号。
...
| 引导页最后一页撒花（2026-09-11） | 几何 LyrimuseCore/Util/ConfettiField.swift（`Piece` / `state(of:elapsed:in:)` / `duration` / `spawnWindow`，确定性 SplitMix64）；一层 Canvas 的 UI/ConfettiOverlay.swift（`burst` 计数触发、`play()` 里 `run = nil` 摘掉 TimelineView、调色板与 `reduceMotion` 门控）；接线 OnboardingView.swift（`confettiBurst` / `.overlay { ConfettiOverlay(burst:) }` / `.onChange(of: currentStep)`）；selftest `onboarding` 组 |
...
15. **「歌词来源」测试按钮(2026-08-30,用户要求"可以测试这些歌词源的可用性")没有另起一套
   ...
   ⚠️ **探测范围没有按 `-source` 收窄**:两首探测曲各自内部仍然会把全部
   已启用的源一起并发打一遍(跟 healthcheck/search-lyrics 完全一样),`-source` 只决定
   "只报哪一行"——AMLL 需要先从网易云/QQ 拿到平台 ID 才能测,拆"只探测它自己"的
   独立调用反而更复杂。但**要测的源全部有了结论之后不再继续跑**(2026-09-13,
   见下面第 17 条):最后一个目标源报完就 `cancel()` 掉这一轮探测、进程正常退出。设置页侧
...
17. ⚠️ **已修的真 bug：单独测一个源，结果都出来了，标题行那颗「测试」还在显示"测试中…"**
    （2026-09-13，用户圈图反馈"我手动点击了其中一个歌词源的测试，但结束之后，右边的测试状态
    一直没有变更"）。根因**不在** UI 侧的 `isTestingLyricSources`/`lyricSourceTestGeneration`
    那套机制——它忠实地反映着"collector 子进程还活着"；问题是 `test-lyric-sources`
    **该报的行都报完了还在继续跑**：两首探测曲雷打不动各跑到底，`-source deezer` 这种"只要
    一行"的调用在 deezer 那行打完之后还要再跑一整首英文探测曲。改前实测 `-source deezer`：
    **5.8s 出结果、12.0s 才退出**，中间这 6.2 秒里那一格已经是绿色 ✓、右上角还在转圈；而单轮
    20 秒总截止（`lyricSearchDeadline`）意味着最坏形态能干等三十多秒。

    修法在 collector 侧（`testlyricsourcescli.go`）：探测改用可取消的 `context.WithCancel`，
    `scanForPositives` 每报完一批就问一次 `allReported()`（`targets` 里每个源都已经有结论），
    是就 `cancel()`；第一首探测曲跑完已经全部有结论就不跑第二首。取消走的是
    `fetchScoredLyricCandidatesStreaming` 收集循环里**早就存在**的
    `case <-ctx.Done(): break collect`（"用户主动取消搜索"那条路），不是新开一套；进程随后
    正常走完尾部循环并退出，**退出码仍然是 0**，Swift 侧不会把它当成测试失败。

    **为什么不在 Swift 侧修**（"目标源的结果一到就把 `isTestingLyricSources` 置回 false"）：
    那样"这一轮结束了"会变成 UI 和 collector 两边各判一次，而且子进程还在后台空跑、白占网络
    请求；collector 是唯一知道"要报的行报完了没有"的那一侧，闸开在它这里，命令行直接跑
    （本章和第 09 章里那几条 `collector test-lyric-sources -source xxx` 验证命令）也一起受益。
    ⚠️ **探测范围仍然没有收窄**——第 15 条那段⚠️照旧成立，这次改的只是"报完就收工"。

    **验证**：`-source deezer` 的结果行与进程退出相隔 **16ms**（改前 6.2s），退出码 0；
    `-source lyricfind`（这台机器上常年 `warn`）照旧把两首探测曲跑满 10.4s 才报出
    `lyricfind_region_restricted`——"还没有结论的源不许提前收工"这条边界没被打破；不带
    `-source` 跑全部十个源，8 个源在 2s 内陆续到达、`lyricfind` 拖到 11.6s 收尾，逐行流式
    到达的行为不变。`go test ./...` 全绿（28.1s）。
...
22. **设置页文案「简约克制」专项：47 条标题 / 副标题 / ? 气泡整体精简（2026-09-07，用户发起、逐条拍板）**：
    用户原话「设置里面有很多文案，其实都太过于繁琐复杂了，不够简约，不够克制」。先抽全部结构化文案量分布：标题 126 条中位 4 字（本就克制），
    **副标题 20 条中位 26 字、? 气泡 62 条中位 40 字**——繁琐集中在后两类：副标题在解释"为什么"而不是"是什么"，气泡写成了文档。
    四条原则：①标题只说是什么，解释进气泡或删；②副标题只在标题不自明时出现，一句话、无分号括号；③不在标题里列举范围、同类项写法统一；
    ④平台惯例优先——macOS 自己写「显示/隐藏程序坞」，「显示/隐藏悬浮歌词」就不算繁琐，不动。
    **A 档 17 条标题**：三个编辑台的「恢复默认…」各写一套范围→统一「恢复默认」，范围由菜单第二行 / 抽屉行副标题交代；「播放器（可多选）」去括号；
    「配置备份与搬家」→「备份与迁移」；「提醒开关」→「提醒」等。**B 档 18 条副标题**：5 页只有 3 页有副标题→全删；「快捷键」页 46 字规则→19 字
    （录制器早就在拦无修饰键组合）；许可三行的长副标题只留「GPL-3.0」「开源组件与词典」等名词；「歌词提前」「每日听歌报告」的副标题删，跟同卡没副标题的兄弟行对齐。
    **C 档 12 条 ? 气泡**：1176 → 630 字——去贯穿三段的「Khalil Fong & Fiona Sit」例子、去实现细节、去标题已说的话，分档说明每档一句；
    压缩不到 25% 的 19 条不动（「开：… / 关：…」两行结构本身就是最清楚的写法）。
    落地时撤一条：「歌词偏移归零」不改，它在 `GlobalHotkeys` 那组「歌词提前 / 歌词延后 / 歌词偏移归零」里是一致的。顺带修三处：「新播放器提醒」行的
    ? 气泡是从上一行复制的（说"信任之后同权"），删；「配置文件夹」help 正文引用的卡名「配置备份与搬家」已不存在，跟改；C9 英文 help 里的按钮名
    "Get an API account" 跟按钮实际的 "Get One" 对不上，对齐。
...
23. **设置搜索用静态目录 + selftest 守卫，命中靠"本地化标题相等"高亮，滚动走 AppKit（2026-09-09，借鉴清单 S8）**：
    ...
    - **为什么结果放侧栏而不是内容区**：系统设置就是在侧栏出结果（调研过的同类 App 也一样），且内容区保留当前页、看着结果还能对照。侧栏 170～220pt 放"标题 + 面包屑"两行够用，长英文标题尾截、面包屑中截。
...
24. **侧栏整体向系统「设置」的侧栏靠拢：身份区 + 更新提示行 + 无标题分组 + 原生折叠 + 渐变图标 + 红色计数（2026-09-12，用户拍板）**：
    - **为什么照搬系统设置而不是自己设计一套**：用户原话「全面借鉴苹果这一套……全面向苹果原生这种设计靠拢」。这扇窗本来就是 `NavigationSplitView` + `.listStyle(.sidebar)`，选中高亮、行高、字号已经是系统件；差的是系统设置在侧栏上层叠的那几样：顶部账户块、红色计数、无标题分组、图标的体积感。补齐这几样就够，不另起一套视觉语言。
    - **为什么身份区是 Last.fm**：系统设置顶上是「Apple 账户」——这个 App 里唯一有"人"的账号就是 Last.fm（有用户名、有头像、有"连没连上"），ListenBrainz / 网页推送 / 推送提醒都是凭据表单，不配做身份。它原本就是常驻可见的「账号」Section 里唯一一行（2026-07-29 从折叠区提出来），换成身份区只是换了画法，tag 不变，引导页信箱 / 搜索目录 / 跨账号跳转全都不用改。
    - **为什么取真头像、又只缓存在内存**：没有头像的账户块就是一张品牌图，"身份"的感觉出不来。头像来自 `user.getinfo`（用户在 Last.fm 设过头像的话 image 就是真图，没设是占位星，沿用 `filteredImageURL` 那条过滤规则）。App 常驻、换账号才会变，一张 `NSImage` 在内存里够用；不落盘、不加 UserDefaults 键，就没有配置导出 / 迁移 / 清理要跟着改的地方。失败 10 分钟内不重试——反复开关设置窗口不该反复打接口。两跳都记 `NetworkAuditLog`（用户定的"所有对外请求都要记日志"）。
    - **为什么「有软件更新可用」是按钮不是分类**：系统设置那一行落在「通用 › 软件更新」面板；我们的更新控件在「关于」，而「关于」已经是一个分类、同一个 tag 不能有两行（都会高亮）。按钮切到「关于」并直接调 Sparkle 的 `checkForUpdates()`——已经查到版本的那一刻，这就是弹「安装 / 重新启动安装」对话框，比让用户再去找「检查更新」那一行少一步。
    - **为什么去掉分组小标题**：系统设置的侧栏一个小标题都没有，分组靠空白；「核心设置」四个字压在只有六项的列表上是自我介绍。「实验室功能」保留标题——它是可折叠的，折叠区必须有个能点的头，Finder / 邮件的侧栏就是这种"有标题的可折叠组 + 无标题的平铺组"混排。
    - **为什么红色计数替代橙色三角**：系统里"这里有事"的侧栏语言是红色数字（软件更新、Dock、App 徽标），橙色三角是表单校验的语言。数字 = 警告条数，两条硬故障同时在就是「2」，比一个三角多一点信息；悬停文案不变。
    - **图标渐变的分寸**：系统设置的图标不是纯平色块，有一点上亮下暗。这里只叠一层很浅的竖向渐变（顶部白 0.20、底部黑 0.07）+ 0.5pt 半透明白描边，色相不动，`SettingsIconTint` 的深色亮度封顶照旧作用于底色；所有 `iconBadge` 调用点（账号页头 36～52pt 的大徽标）一并生效，侧栏和页头才是一家的。
    - **「有软件更新可用」只在真有新版本时出现**：跟系统设置一致，Sparkle `didFindValidUpdate` 置上 `availableUpdate` 才出现，`notFound` / 跳过 / 开始安装都清掉。用户追问「这个也加进来」时装的 1.6.0 正是最新 release、且「自动检查」关着——它没露面不是没做，是没有可报的更新；开着自动检查或点一次「检查更新…」、且有新版本，它就在身份区下面。想不等新版本先看效果：`defaults write me.yudaotor.lyrimuse settings:previewUpdateVersion 1.7.0` 后重开设置窗口（`SparkleUpdaterManager.shownItem` = 预览键优先、否则真值；侧栏行、「软件更新」页与「关于」页副标题都读它，菜单栏面板底栏只认真值），`defaults delete` 同名键即恢复。
    - **没做的**：系统设置的「家人共享」行没有对应物，不硬凑；detail 面板（卡片式 `SettingsDesignSystem`）不在这次范围内，用户只点了侧栏。

25. **更新界面从 Sparkle 标准弹窗改成设置窗口里的「软件更新」页，Sparkle 用自定义 `SPUUserDriver` 全接管（2026-09-12，用户提议、两案选 B）**：
    - **为什么不是「页面 + 标准弹窗」的折中（方案 A）**：页面、说明、版本、大小都自己画了，「立即更新」一点下去 Sparkle 自己的下载进度窗和「安装并重启」对话框照样弹出来，两套界面叠着，跟系统设置"就在这一页里完成"差一口气；用户看过对照后直接选了 B。
    - **接管界面的代价与兜底**：2026-09-03 曾决定不接管（「接管之后若自家提示没亮，用户到下一版都收不到提醒」）。现在的兜底是三层：周期检查节拍一点没动（Info.plist 的 SUEnableAutomaticChecks / 开关照旧）；发现更新时侧栏「有软件更新可用」与菜单栏面板底栏两处会亮；页面上每一步都有状态与按钮，失败有文案与「重试」。
    - **为什么"谁在等"这套意图机制**：Sparkle 的每个 reply 闭包必须且只能答一次，不答会话就一直挂着、周期检查进不来。周期检查发现更新时没人在看页面，攥着 reply 等用户开窗不现实，所以立刻 dismiss、只留信息；用户真要装时再发一次检查（一次 appcast 拉取，很快）并自动答 install——比让页面和 Sparkle 会话生命期耦合简单得多，也天然覆盖「已在后台下好」的情形（found 的 stage 是 downloaded，install 直接进安装）。
    - **窗口关掉时的收尾**：攥着的「找到了」答 dismiss（信息留着，下次开窗照样显示）；「下完待装」答 dismiss = Sparkle 在退出时装，页面记 `installOnQuit`；检查中的取消；下载 / 解包 / 安装中的不动，回来照样看进度。
    - **发版日志走 `NSAttributedString(html:)` 而不是 WebView**：Sparkle 标准窗用 WKWebView 渲染说明；这里要嵌在滚动的卡片列里、跟随深浅外观、可选中复制，Text + AttributedString 更合适。代价是 SwiftUI Text 不认段落样式（列表缩进 / 段距），靠转换时生成的「•⇥」字符仍能看出层次；字体、加粗、链接、颜色都重新归一（HTML 默认黑字在深色模式下看不见）。解析只能在主线程，按原文缓存、同一份说明只解一次。
    - **深链 `lyrimuse://settings/software-update`**：系统设置每个面板都有 `x-apple.systempreferences:` 地址；这里照样给一个，发版说明 / 支持回复能直接指到这一页，也是本机不点按就核对这一页外观的唯一入口（GUI 自动化不做）。
    - **没做的**：系统那页的「今晚更新」（定时安装）Sparkle 没有对应能力，只有「退出时安装」；「其他可用更新」没有对应物。

26. **「歌词显示」页把撤掉的页内大标题加回来，代价由窗口高度吃下（2026-09-13，用户拍板）**：
    - **为什么推翻 2026-08-30 那次移除**：当时的理由是"标题栏已经写着「设置 – 歌词显示」，页内再来一遍 22pt 大标题是同一句话说两遍"，而那一页顶着一块接近真实尺寸的编辑台、纵向每一分都金贵。这次用户是**把六个分类逐页截图并排**看的——一致性问题只有在并排时才成立：其余五页都有页内大标题，唯独这一页没有，翻页时页首会"跳"一下。"省 68pt"和"六页长得一样"这两件事没法同时要，用户选了后者，并且同一句话里直接给了补偿方案（"顺便帮我把整个设置窗口的高度搞大一点"）。
    - **为什么补偿落在 `minHeight` 而不是 `idealHeight`**：`Settings` 场景的窗口尺寸由 macOS 自动存档，`idealHeight` 只对没有存档的窗口（首次打开 / 重置）生效。用户手里那扇窗已经存档在 652pt，只改 `idealHeight` 对他**一点效果都没有**——这正是"改了没反应"最容易发生的地方。真正把窗口顶上去的是 `minHeight`，所以它从 520 抬到 690，落在"「桌面悬浮歌词」总开关卡整张露出来"之上（用户的原话就是"确保桌面悬浮歌词的开关可以展示在页面上"），而不是按"别比页面头部还矮"那种历史下限。
    - **副标题留着**：「三种展示方式可以同时开启」这句只在页头出现，撤掉页头等于这句话没地方说。「通用」「快捷键」两页本来也是标题 + 副标题，「歌词」「播放器」是只有标题——两种写法并存，不是这次要统一的东西。
    - **核实方式**：真机三段各截一张（`check-windows.swift` 拿窗口 ID + `screencapture -l`，不碰 GUI 自动化）。窗口被顶到 722pt 后，悬浮歌词 / 灵动岛 / 菜单栏三段的「编辑台 + 总开关卡 + 「全部设置」抽屉头」全部一屏可见，悬浮歌词这一段第一次不用滚——04 章那句"仍然没有做到严格意义上的一屏不用滚"从此只对 652pt 及以下的窗口成立。
