...
- Dock 图标点击(reopen)**只在设置/歌词管理/歌词窗口/引导四扇辅助窗口一扇都没开着时**才打开这扇歌词窗口(2026-08-21 先定的规则是"点 Dock 只弹歌词窗口、任何情况下都不弹设置",2026-08-25 用户进一步收窄:只要还有任意一扇辅助窗口开着——哪怕被最小化——就把开着的窗口带回来,不再额外抢开歌词窗口;**2026-09-09 起"带回来"由 App 自己做**——`AuxiliaryWindowActivation.bringOpenWindowsForward`:最小化的全部还原、可见的取最前那扇 makeKeyAndOrderFront——而不是 `return true` 交给 AppKit,AppKit 默认 reopen 只在没有任何普通窗口可见时还原**一扇**,有一扇可见就什么都不做,见决策 20;App 本来就在前台(不是这一下点击切回来的)、开着的窗口也本来就全在最前面——这一下什么都没变——时,兜底落回开歌词窗口,保证每一下点击都有可见结果)。判据是 `AuxiliaryWindowActivation.hasAnyOpen`(那个计数器本来就在追踪"这几扇窗口有没有任意一扇还开着",给 reopen 用不需要另起逻辑)——不用 `hasVisibleWindows`,那个系统参数只反映"当前有没有窗口**可见**",窗口被最小化时它也是 false,分不出"关掉了"和"最小化了"。实现在 `AppDelegate.applicationShouldHandleReopen`,几处刻意的写法都是为了"不要冒出设置窗"以及"不误判已关闭窗口为开着":
  ...
  - **一律返回 `false`,不给 AppKit 默认行为**(2026-09-09 之前是 `hasAnyOpen` 时返回 true):返回 true = 让 AppKit 执行默认的「恢复/带回本 App 窗口」行为,而 SwiftUI 的 Settings 场景关掉之后 NSWindow 对象仍然活着(只是 orderOut,`check-windows` 能看到它 `onscreen=false` 挂着)——这正是 2026-08-21 那版要避开的坑(默认行为会把用户明确关掉的设置窗一起端出来);而且探针实测那个默认行为在本 App 里几乎等于不做事(决策 20)。`hasAnyOpen` 靠的是 `.onDisappear`(只在窗口真正**关闭**时触发、最小化不触发——2026-09-09 隔离探针对 Window 与 Settings 两种场景各验一遍),所以窗口真关掉时 `hasAnyOpen` 仍是 false,不会重新踩这个坑;只有真的还有窗口开着(minimized 也算)才走"带回来"那一支,且带回来的对象按窗口形态枚举(有标题栏、非 NSPanel、能当主窗口),关掉的 NSWindow 既不可见也没最小化,枚举不会碰到它。
...
- **对唱歌词**:行带演唱者标记(`LyricDuet.Side`)时按左/右/中分栏对齐,并叠加**两侧内缩**(`LyricDuetLayout`:远侧 15%、4 字宽封顶,近侧是远侧的一半、2 字宽封顶,2026-08-26 前近侧恒为 0) —— 这一列按比例算只放得下约 12 个汉字,顶满整宽的行光靠对齐是分不出左右的;近侧留白是为了不让短句贴着这一列的边缘,跟悬浮窗共用同一份计算(见第 04 章)。没有标记的歌 side 为 nil,本窗口兜底 `.leading`(悬浮窗兜底是 `.center`,两边不同),且 nil 行**不吃内缩**,普通歌排版逐像素不变。
...
- **本窗口的逐字填色始终开着、不给开关**(2026-09-06 用户拍板):「卡拉OK效果」从「歌词 → 效果」那颗全局开关拆成悬浮歌词/灵动岛/菜单栏各一颗时,这扇窗刻意不参与——它是 Apple Music 风格的大窗,逐字填色就是它的核心,没有理由关。它读的 `allLines` 直接来自协调器,不经过任何展示面的 `SyncedLyricLine.lineLevel` 压平。此前全局那颗关着时这扇窗也会退成整行——那是被引擎层的开关连带的,不是设计。
- **驱动方式定稿 = 逐帧重算 TimelineView @ 60Hz**(2026-08-21 五轮拉锯后,勿再翻烧饼):填色几何与**悬浮歌词逐像素同款**(渐变中心=人声位置、软边=±8%词宽同一个 `KaraokeFill.wordEdgeSoftenBand`,用户点名的观感基准),唯一差别是刷新档位 `WordKaraokeGradient.windowRefreshInterval = 1/60`(悬浮窗 30Hz;这台面板 60Hz,窗口字号大,30Hz 步进可感知)。同日第三轮曾整体改成**排程式**(fillFraction 对时间线性→一次性排 .linear 显式动画交渲染管线插值,这类歌词渲染的常规架构)——CPU 是零逐帧代码,但 **SCK 逐帧探针实测 macOS 只以 ~20Hz 提交这些动画**(系统对长时程慢动画自动降档、无 API 干预;对照组悬浮窗 TimelineView 30Hz 准点),20Hz×14px 步进正是"卡顿感"本体,故回退。逐帧的开销结构由既有三件套压住:字级叶子时钟+4Hz 粗时钟只养活"正在扫的字"+WrapLayout contentKey 缓存+Palette 纯色跨帧复用。
...
- 视觉参数弯路记录(二/三轮按同类实现的逆向参数"改良"、全被用户否掉):②羽化放大到 0.55×字号→光晕横跨大半个字,**字内的人声位置信号被洗掉**("全都是一个速度");③尖端对齐→相邻字光晕不跨界搭接,每字边界"熄灭再重生"("更机械了")。结论:**软边必须骑在人声位置上**,字内位置信号(边缘贴人声爬行)才是"流动感",不是宽光晕。
...
- 罗马音逐词标注:开着「显示罗马音」且该行有 `wordGroups` 时,读音**逐词**标在正文每组字底下(组内左对齐、读音按整组起止时间的伪词填色、不跟着抬升),这时不再单独渲染整行罗马音;拿不到词组时退回正文下方一整行罗马音。**不论活跃与否都这样排**(2026-09-10 起,决策 #21):非活跃行的读音跟正文一样定格全填色、由行级 lineOpacity 压暗;此前只给当前行逐词、非当前行退回整行,同一句唱完往上一滚读音就换排法。
...
- 罗马音行:「显示罗马音」开 + 该行有 `romanization` + 没走逐词标注(=这一行没分出词组,跟活跃与否无关)时,显示在主行**下方**(与 Apple Music 一致)。
- 译文行:「显示译文」开 + 该行有 `translation` 时显示在最下,次级颜色。
- 两个开关的帮助文案明说"只影响「桌面悬浮歌词」和「歌词窗口」"。

### 左列:封面卡与曲目信息

- ⚠️ **广告期间这张卡换成广告标识**(2026-09-09,用户:「只要识别到是广告的话,封面部分都用这个替代」):`isCurrentTrackAdBreak` 时不画封面,改画跟灵动岛/菜单栏同一枚 `megaphone.fill`,底沿用下面那个「没有封面」占位的同一块。播放器在广告时给的是广告物料的缩略图,把它当"正在听的这张专辑"摆在这张最大 460pt 的卡上最误导。全 App 四个封面位的完整清单与理由见 05 章「广告态」⑦。
- 封面卡 1:1 方形、圆角 12、投影;图片优先用**高清替代** `highResArtworkImage`(只在系统 Now Playing 那份实在太小、或者不是方形时才有值——网易云客户端只给 100×100、QQ 音乐客户端给 300×300,YouTube Music 的 MV 条目给 320×180 的视频缩略图(2026-09-08,见 03 章决策 15),替代图来自 collector 缓存里解析歌词时顺手记下的 cover_url,取用前过一遍 `EnrichCacheReader.nativeSizedCoverURL` 顶档,见下面「高清封面替代」),否则用系统那份 `artworkImage`;都没有时显示灰底音符占位。换图交叉淡入 0.5s,动画收在 overlay 内容上而不是整卡最外层(否则会把别的布局变化 animate 成"进度条从上面飘下来")。**有 Apple Music 动态封面的专辑,会在静态图之上叠一层无声循环视频**(2026-09-09;⚠️ 2026-09-10 起这张卡是全 App **唯一**的落点——灵动岛那份用户拍板撤了,理由是那一格太小,见 03 章第 6 节;静态图始终铺在底下 —— 没有、还没下好、或被总闸拦掉时这张卡就是它,所以"动态封面到货"表现成一次淡入而不是从占位符跳到视频)。暂停时视频跟着停(它描述的是"这首歌在放",判据同下面封面缩放那条用的 `isPlayingSmoothed`);整条链路与三道省电闸门见 03 章第 6 节。
...
- 菜单结构(自绘玻璃面板,机制见已知坑 #12;2026-08-22 二批定稿):Apple Music 播放时 = 添加到资料库 / 减少推荐 / ─ / 前往专辑 / 前往艺人 / 在 Music 中显示 / ─ / 显示简介 / 搜索歌词…(有曲目才显示) / 歌词时间轴(内联控件行);其它播放器只有后半段,「在 %@ 中显示」标题随 `resolvedPlayerDisplayName` 变;**Spotify 原生客户端点它会跳到当前曲目页**(2026-09-09,`SpotifyReveal`:后台读 `spotify url of current track` → `spotify:track:<id>` 深链交给正在跑的 Spotify;广告 / 本地文件 / 读不到时退回只激活 App),其余播放器仍只把 App 带到前台(网页版 Spotify 的页面没有曲目链接,「在 Safari 中显示」也只激活浏览器)。「歌词管理…」「拷贝歌词」按用户要求移除(管理入口保留在菜单栏菜单)。
...
- **AppleScript 动作**(`MusicPlaybackController`,全部 2026-08-22 实机验证):添加到资料库=`duplicate current track to source 1`(流媒体曲目唯一可行路;不返回引用;fallback `library playlist 1`——不能用名字 "Library",中文系统叫「资料库」;共享播放列表源的 `shared track` 走 source 1 报 -10006、靠 fallback 接住);减少推荐=`disliked` 布尔(UI 的 Suggest Less 就是老 Dislike);在 Music 中显示=`reveal current track`+activate。统一走 `runAppleMusicMenuAction` 外壳(权限确认+后台线程,失败静默)。Spotify 的「在 Spotify 中显示」不走这个外壳(权限确认是 Music.app 专用的):`SpotifyReveal.revealCurrentTrack` 后台读 `MusicPlaybackController.spotifyCurrentTrackURI()`(Spotify 字典没有 `reveal`,只有会从头重播的 `play track`),`SpotifyURI.deepLink` 只放行 `spotify:track:` / `spotify:episode:` + 恰好 22 位字母数字 ID(广告 `spotify:ad:`、本地文件 `spotify:local:`、带 `:play` 后缀的自动播放形态一概 nil),经 `NSWorkspace.open(_:withApplicationAt:)` 交给**正在跑的那份** Spotify.app(lsregister 里 `spotify:` scheme 有两份注册——/Applications 与 PersistentCache/Update 里自更新留下的那份,按默认处理器开可能拉起另一份);nil / 失败退回 `openResolvedPlayerApp`。⚠️待核对:深链应落到曲目页而不重播(Spotify URI 语义即定位资源页),2026-09-09 实施时用户在网页版播放、原生客户端没在跑,未真机点过。
...
- **搜索歌词…**(2026-08-22 二批):歌词管理的联网搜索面板(`LyricsSearchSheet`,自包含)独立调起。点击瞬间快照曲目字段(sheet(item:) 的身份即快照,弹窗期间换歌不串)、后台解析**写回 key**(`EnrichCacheReader.resolvedKey` 新只读口,精确→宽松两级——播放器报法与缓存写法有空格/繁简出入时写回必须落在读取路径命中的同一条上,否则读写分家改了不生效;缓存无条目退 normalizedKey 新建)+当前来源。onApply 写回三步:`EnrichCacheStore.reload(onlyIfChanged: true)` 兜「store 未加载」(空 raw 上 saveEdit 会丢条目其它字段如 cover_url)→ `saveEdit` → `refreshLyricsForCurrentTrack()` 即时刷新播放侧(不等 2s mtime 轮询)。
  ...
  - **采纳后关不关窗,三个入口不一样(2026-09-04)**:本窗口这条 sheet 与歌词管理采纳后照旧关窗;悬浮窗 ⚙ 小窗传 `keepsOpenAfterApply: true` 留着不关、点即切、「当前使用」徽标跟着挪(理由与机制见 04 章「小窗采纳后不关窗」)。配套 `onApply` 改成 `async -> Bool`:本窗口这处的三步不变,只是不再自己套 `Task {}`,改成等 `saveEdit` / `savePlainTextEdit` 返回落盘成败再回报,面板据此决定关窗。
...
- 只在 `poller.soundVolume` 非 nil **且 `!isIdle`** 时显示(同样是 Apple Music + Spotify;Apple Music 还要求自动化权限已授权)。调的是**播放器自己的输出音量**(0~100),不是系统音量。
  - `!isIdle` 是 2026-08-25 补的(用户报"停播页不该有这个音量胶囊")。这里原来漏了这道判断,跟紧邻的翻译键/[歌词|播放记录]胶囊(`if !isIdle`)不是同一套判据。根因不只是"忘了判":`soundVolume` 读的是 `LocalPlaybackSource.lastResolvedBundleID`,而它读的是 `lastSnapshot?.bundleIdentifier`——**`lastSnapshot` 真正停播后也从不清空**(`SettingsView.offsetScope` 那条注释也踩过同一个坑),所以哪怕歌词窗已经判定 `isIdle=true` 切进了「停播页」,`soundVolume` 依然吐着 Apple Music 最后一次播放时的音量值,胶囊因此照常渲染。没有改底层那个"从不清空"的字段(它服务于诊断导出,另有存在理由),只在这一个消费点上补上判断。
...
### 播放记录取代播放队列(2026-08-27,右栏内容切换)

「[歌词|队列]」双钮胶囊右边那颗钮原来打开一个悬浮的「待播清单」小面板,2026-08-27 整个换成了「[歌词|播放记录]」——右边那颗钮不再弹小面板,改成把**整个右栏内容**从歌词换成 Last.fm 播放记录(跟歌词栏是同一块地皮,不是弹窗;点击机制、图标激活态样式跟左边「隐藏歌词」钮完全对称)。

**为什么把「待播清单」整个拆掉,而不是修**:那个面板靠 AppleScript 读 `current playlist` 模拟队列,用户 2026-08-25 报"点了没反应"、2026-08-26 又追问"我明明在放专辑不是电台,为什么还是识别不到"——两轮实机复测把能力边界摸清楚了,结论是**没法修**:
- `current playlist` 只在这首歌是从**本地资料库对象**播放时才可靠取到。直接在 Apple Music 目录页面点开一张专辑/播放列表播放(没先加进资料库)同样会让它失效,判据不是"是不是电台",范围比"电台/自动播放"这个最初的猜测宽得多。
- 就算触发条件相同,失败的**报法也不稳定**:同一首歌先后两次读,一次是"不能获得 current playlist"(-1728),另一次是"能取到对象、但 name 转 Unicode text 失败"——同一个属性在这类上下文里压根没有一个固定的失败态。
- Apple Music 自己原生窗口能显示待播清单,是因为它读的是**自己内部状态**,跟 AppleScript 暴露给外部脚本的那套对象模型是两码事,不受这条限制,没有办法照抄。
- 查过 MediaRemote(非 AM 播放器走的那条路)有没有替代数据:`media-control get` 确实带 `queueIndex`/`totalQueueCount` 两个字段,但①不含曲目名,建不出一份能点的清单;②实测在这类上下文里数值本身也对不上(专辑内已经放到第 8 首,`queueIndex` 却报 1),同样信不过。**没有已知的可靠替代路径**,用户由此决定把这个位置换成一个完全不依赖这个能力边界的功能。

**「播放记录」按 `LastfmStatsService.isConnected` 二选一**:
- **已连接** → `RecentListensPanel`(按天分组、封面 + 「第 N 次听」+ 时刻、底部翻页)。这个组件 2026-08-27 从 `IdleStandbyView.swift` 的 private `IdleRecentPanel` 搬出来改成 internal、去掉 `Idle` 前缀——停播页和这里现在共用同一份实现,不是各写一份;「第 N 次听」的两把尺子(`playCountKey` vs `PlayCountFold.familyKey`)复制一份就是 2026-08-21 那个「第 15 次听下面紧跟第 21 次听」的成因重演,只该有一份能改。`recentPage` 仍是**服务层共享状态**,退场同样要拨回第 1 页(理由与停播页那条完全一致,道理不再重复)。
- **未连接** → `PendingListensPanel`(新写):本地静默记的、还没提交给 Last.fm 的收听,数据源是 `ScrobbleBackfillService`——跟设置页「账号 → Last.fm」标签的 `pendingListensRow` 共用同一份数据(collector 一直在往本地记,不管连没连账号),只读,不提供补提交/删除(补提交需要连账号,删除留给设置页那边已有的完整管理入口),给一个「连接 Last.fm…」按钮直接深链到设置(`requestSettings(.account(.lastfm))` + `NSApp.activate` + `openSettings?()` 三连,跟这扇窗口里"收听次数"那行同一套跳转机制)。自己管自己的 5 秒 mtime 轮询(同 `pendingListensRow` 的理由:按秒轮询 collector 子进程太重,stat 一个文件几乎免费),没有更外层的卡可以像设置页那样把轮询挂上去。
- 两个分支的日期分隔行(「今天/昨天/M月D日」)与 HH:mm 时间戳格式化共用一份 `RelativeDayFormat`(`RecentListensPanel.swift`),不各写一份让两个列表的日期分组标准跑偏。
- **下拉刷新手势**(2026-08-26 用户要求,只加在 `RecentListensPanel` 这一侧,`PendingListensPanel` 未跟进):`ScrollView` 挂 SwiftUI 原生 `.refreshable`,动作是 `LastfmStatsService.refreshBaselineAndWait(force:)`——跟表头那颗刷新按钮调的 `refreshBaseline(force:)` 是**同一件事**,只是多包了一层可以 `await` 的等待:`refreshBaseline` 本身即发即忘(内部自己起 `Task` 就返回),直接塞进 `.refreshable` 的闭包会让系统下拉转圈立刻收起、实际请求其实还在飞,手感是"松手就没反应"。等待用 `recentUpdatedAt`(成功路径,`applyRecent` 里设)/`baselineFailed`(失败路径)的变化当完成信号,轮询上限 12 秒(比 `request()` 单次请求的 10 秒超时留了余量,避免慢网络下转圈比真实请求先收起)。刷新的是当前正在看的这一页,不强制跳回第 1 页——跟按钮的既有行为保持一致,不是"下拉=回到最新"这套在别处常见但这里并不适用的语义。

**实现要点**:
- `showsListenHistory` 是新按钮的状态;⚠️ `lyricsPaneVisible` 原来只看 `showsLyricsPane || !showPlayerPane`,漏了这一项——「隐藏歌词」之后再点「播放记录」,按钮显示已激活,右栏却因为整块可见性的闸门没跟着一起判断而继续塌成空白,补成 `showsLyricsPane || showsListenHistory || !showPlayerPane` 才对(实现时自己测出来的,不是用户报的)。
- 播放记录钮**不挑播放器**(数据来自 collector 本地记录/Last.fm,不靠 AppleScript),原队列钮只在 Apple Music 才显示这条限制一并解除。
- 单列窗口没有「隐藏歌词」钮(右栏本来就是整窗唯一内容,不能收起来),但播放记录钮仍然有意义,单独摆一颗圆钮(36×36 的 `Capsule()` 在等宽高时会退化成正圆,跟"翻译"钮未激活态同一个写法)。
- 图标定了又改了一轮:先用 `clock.arrow.circlepath`(语义上更贴"历史记录"),用户 2026-08-27 截图反馈"这里图标依然用之前那个吧",改回旧队列钮用的 `list.bullet`——两处(双列胶囊/单列独立钮)都要改,新旧两版图标只在这一个字符串上有差异。
- **上下留白必须躲开两个窗级 overlay,内容本身不知道它们在哪**(2026-08-27 用户截图报"顶部卡头跟音量胶囊挤在一起、底部行跟翻译/播放记录圆钮撞了")。`ListenHistoryPane` 原来只有 `.padding(.vertical, 18)`,不够。两个 overlay 的footprint 都能从代码精确推出(本窗口全程不 `ignoresSafeArea` 内容层,只有背景那层 `ignoresSafeArea`,所以 overlay 和内容共用同一个 safe-area 原点):上方音量/AirPlay 胶囊贴 `topTrailing`、`.offset(y: 8)`、自身高 36pt → 下缘 = 8+36 = 44pt;下方翻译/播放记录圆钮贴 `bottomTrailing`、`.padding(.bottom, 11)`、自身高 36pt → 上缘 = 11+36 = 47pt。改成上下各 52pt(留 5~8pt 视觉间隙),不是凭手感调的数字。
- **列表不该带卡片背景**(2026-08-27 用户追问"是不是改成直接替换歌词比较好,而不是这种覆盖的模式")——同意:歌词文字本身从来没有圆角磨砂卡片背景,直接铺在封面模糊背景上,这个列表带一张独立卡片,视觉上自然像"盖了一层"而不是"歌词真的换了内容"。`RecentListensPanel`/`PendingListensPanel` 因此加了两个外观参数(声明处注释详细写了理由):`showsCard`(默认 true,决定要不要画那圈圆角磨砂底+描边)和 `onArtwork`(默认 false,决定文字色走系统语义色还是"有封面就固定白色系"那套,三档 primary/secondary/tertiary 分别对应 `.white` / `.white.opacity(0.6)` / `.white.opacity(0.4)`,数值抄的是这扇窗口里 `WindowVolumeCapsule.capsuleIconColor`/`primaryTextColor` 已经在用的同一套档位)。`ListenHistoryPane` 传 `showsCard: false, onArtwork: onArtwork`;停播页(`IdleStandbyView`)两个参数都不传,保持默认——那边"最近听过"是仪表盘式多张卡片里的一张,卡片背景在那个语境下是对的,不该跟着一起摘掉。⚠️ `Color` 没有 `.tertiary` 这个静态成员(`.tertiary` 只存在于 `ShapeStyle` 协议、给 `foregroundStyle(.tertiary)` 用,类型是 `some ShapeStyle` 不是具体的 `Color`)——`tertiaryTextColor` 的返回类型定成 `Color` 之后系统语义色那一档得写 `Color(nsColor: .tertiaryLabelColor)`,不能直接写 `.tertiary`,会编译报错。
- **[歌词|播放记录] 两颗钮必须互斥**(2026-08-27 用户反馈"应该是互斥的,一个选了另一个就灰掉")。原来是两个独立布尔量各自 `toggle()`,会产生脱节状态——歌词已隐藏时点了播放记录、再点歌词切回去,`showsLyricsPane` 因为从没被这条路径碰过仍是 `false`,跟上面"⚠️ `lyricsPaneVisible` 原来只看 `showsLyricsPane || !showPlayerPane`"那条是**同一族坑**、只是换了个触发路径。改成显式二选一:歌词钮点击时,当前在播放记录就切回歌词(`showsListenHistory = false`,不碰 `showsLyricsPane`、不隐藏整栏);当前已经是歌词就走原有的隐藏/显示切换(2026-08-22 就有的既有功能,原样保留)。播放记录钮点击是**单向选中**、不是 toggle——总是 `showsListenHistory = true; showsLyricsPane = true`,再点一次不会切回歌词,要切回去只能点歌词钮,跟 Apple Music 自己的 Lyrics/Queue 分段控件同一个交互(点已选中的那段不取消选中,换靠点另一段)。歌词钮的"激活"判据也要跟着改:不能单看 `showsLyricsPane`(播放记录钮点击后会强制把它设回 true),得是 `showsLyricsPane && !showsListenHistory`——这样两颗钮的高亮状态永远互补,不会同时亮或同时灭。单列窗口那颗独立圆钮没有配对的歌词钮可以"切回去",所以仍然是普通 toggle,不受这条影响。
...
- **「收听次数」徽标**(#1,2026-08-22 改版):挪到时间行正中央 —— AM 在这个位置放的是「高解析度无损」这类音质标签,这里复用同一份 AM 染色(见 `WindowProgressSection.secondaryTextColor` 注释),纯文字、无底色胶囊。`nowPlayingCount`(track.getinfo userplaycount + **这一次还没落库时才 +1**,孪生合并/防刚 scrobble 查回 0/落库判定都在服务侧;2026-09-13 前是无条件 +1,第一次听的歌必然显示成 2,见第 12 章那条);取数由徽标容器的 onAppear/onChange(键=「歌手|歌名」)触发,容器永远在场防"没数字→不渲染→永不取数"死锁。⚠️ 首发版本(#1)是歌手行下的独立小胶囊,另配一套进度条计次刻度 + 时间行 checkmark 对勾(#6,`ScrobbleRule.thresholdFraction`,复刻 collector 的 min(时长/2, 240s)、<30s 不计);用户反馈刻度点/对勾太打扰,改成只留这一枚居中徽标。`ScrobbleRule` 本身连同 selftest 断言原样保留,只是眼下没有 UI 消费方。
...
- **「显示简介」面板加一行「网页」**(`InfoPanelLinksRow`):首版三个平台全列;**2026-09-10 起改成只给当前播放器自己那个平台的歌曲页**(用户定的规则,见决策 22),该平台没链接整行不出现。唯一的例外是 Apple Music 那项会改写成 `music://` **进 App**,其余落浏览器。
...
`idleWelcomeView`(与播放态那条 640pt 双列断点同一个思路,硬塞两列会挤成一团)。
断点变量 `idleWide` 布局与背景共用一个,不许两处各写一遍 860。

**未连 Last.fm** 时的退化(2026-08-27 改,与歌词窗口「播放记录」按钮同一套逻辑):
左上「收听总览」没有本地替代来源(三个数字全靠 Last.fm 每日归档),直接省略——不画空卡、
不画破折号,沿用「没连账号也不难看」那条契约。但右列**不再跟着一起消失**:改成
`PendingListensPanel`(本地静默记的、还没提交的收听,详见下面「播放记录取代播放队列」
一节),两列布局因此恒定,只是左上那张卡有没有各自独立判断。旧版本这里是「未连接时
`IdleStandbyView` 整段退化成只有唱片 hero 居中」,这个版式已经不再是当前行为。

- **左上 · 收听总览**:今天 / 近 7 天 / 累计 三个大数字 + 近 30 天走势 + 一行说明。口径三条必须记住:
  ①**「今天」只有一个来源**(大数字和走势末点都取 `overview.today`,天粒度桶那边也有「今天」但没有 TTL、可能滞后近一小时,两处各取一个就自相矛盾);②**「近 7 天」是「今天往回 7 个自然日」**——⚠️ 2026-08-30 改过口径:原来数值取 API 的 `overview.week`(从此刻往回滚动 168 小时),而紧挨着的环比百分比是本地桶按自然日算的,**两个窗口定义不同、永远对不上**(实测:API 给 781、桶给 669,差 112 次约 17%,用户看到「781 · 近 7 天 · 较上周 -47%」却核对不出来)。现在数值也走 `IdleListeningStats.lastSevenDays`,跟环比**同源同窗口**。代价知情:这个数会比 Last.fm 网页的「last 7 days」略低(那边是滚动 168 小时),宁可块内自洽、用户能自己核对。设置页 `LastfmStatsSection` 那个「近 7 天」用**同一个函数**——同名数字两处口径必须一致。桶没同步完(`dailySyncing`)时两处都退回 API 值;③**环比与日均只在桶里算**,且 `dailySyncing` 期间整体缺席(桶没同步完算出来是假日均)。环比**会把今天那一格用 `overview.today` 补上**(2026-08-30 加):桶同步到昨天为止,不补的话今天算 0、把环比系统性拉低(实测 -48% vs -47%)——走势图早就在补这一格,这里以前没补,同一个问题两处处理不一致。派生算术全在 `IdleListeningStats`(Core 纯函数 + selftest)。
  ...
  2026-08-29 修交互(用户反馈「移动鼠标到某一天,左下角这个文案数字提示很不明显…我有时候还在想
  怎么移动上去没反应」):不叠气泡这个决定本身没问题,问题在**换过去之后没有任何"我响应了"的信号**
  ——悬停前后字号、颜色、位置全不变,只有文字内容变,而那行是 11pt 的 `.tertiary`(最淡一档)、
  固定钉在左下角;鼠标在图右侧时视线要横跨整条图去找它。三处一起改:①悬停时整行换**强调色 +
  semibold**;②读数**水平跟到游标正下方**(`trendCaptionOffsetX`:文字中心对准那一天的点,再
  钳进图宽,贴边时不越界;宽度靠 `TrendCaptionWidthKey` 实测而不是估固定值——中英文与位数变化
  会钳错边);③游标本身画清楚(竖线 0.22→0.35,点 7→8pt 并补一圈底色描边,同「今天」端点的做法)。
  ⚠️ 位置跟随**不做动画**(与内容变化同处一个 `.animation(nil, value: hoverIndex)`):加了就滞后
  于鼠标,反而更像"没跟上"。`hoverX` 存的是**吸附后那一天的点位**、不是鼠标裸坐标,读数才跟竖线对齐。
  同日修布局抖动(用户反馈「首次进入这个页面的时候上面这个折线图模块区域会加载一下,加载完窗口会
  抖动一下」):走势区原来只有"有数据"那一档才渲染 52pt 的图,同步中只渲染一行 11pt 文字(≈13pt)、
  连数据都还没拉到时更是**一个 View 都不渲染**(高度 0),数据一到高度直接跳到 `52 + 13(spacing)
  + 13(说明)` ≈ 78pt,整个窗口当场往下弹一格。修法不是给容器写死总高,而是让三态**共用同一套
  骨架**:`sparkHeight` 的图位 + 一行 11pt 说明,加载态图位留白并补一条跟图内同款的基线
  (`primary 0.10`、1pt),数据到了在**原地**换成真图。三档都用同一个 `.font(.system(size: 11))`
  的单行 Text,行高必然相等,不靠估数字对齐。⚠️ 三个细节:①`sparkHeight` 必须是常量,占位和
  `trendChart` 分开写两个 52 迟早对不上;②说明行要 `.lineLimit(1)`,换行会让占位高度重新对不上;
  ③没数据那一档的占位文字用**一个空格**,空串 `Text("")` 的高度是 0。这张卡本身只在
  `stats.isConnected` 时渲染,所以"进到这里就意味着迟早会有数据",没数据也该先占住位置。
- **左下 · 上次那首 + 一句歌词**:216pt 真封面(本机 enrich 缓存直出,零网络)+ 曲名歌手 + 继续播放/前往专辑/⚙︎设置,下面挂一句从**这首歌**歌词里挑出来的话。⚙︎ 那颗是 2026-09-05 补的(用户要求「在红框这里加一个按钮打开设置」):停播页把这扇窗口平时常驻的那排圆钮（星形 / 「…」 / 齿轮）整个藏掉了，于是**恰恰是最容易停在这一页的时候**（没在放歌）反而够不着设置、只能去菜单栏图标右键。排在这一排最后（前两颗是"对这首歌做什么"，它是"对 App 做什么"），只给图标不给文字（这排已有两颗文字按钮，没有「继续播放」的播放器还会有第三颗「打开 X」，再加一句「设置…」整排会比封面还宽）。动作与文案跟窗口那颗齿轮、菜单栏右键「设置…」同一条路径、同一个本地化键（`AppActions.shared.openSettings?()`，自带 `NSApp.activate`；不经 `requestSettings` 指定分类，沿用上次停留的设置页）。⚠️ 空态（「没有在播放」，没有上次那首）那一版**没有**这颗，只有「打开 X」。三级空态:有封面→唱片;有曲目无封面→`stableColor` 色块 + 首字母(封面存的是**远端** URL,断网/冷启动拿不到图是**常态**不是意外);连 UserDefaults 都空→退回呼吸音符。2026-08-24 新增 `np:lastTrackAlbum` 键(此前只存曲名/歌手/播放器,所以「歌手 · 专辑」那行拿不到专辑;**旧安装第一次打开时它是空的**,要等下一次停播才写上,空就只显示歌手)。
...
**「搜索歌词…」按钮(2026-09-03,用户拍板)**:只在「暂无歌词」「网络连接失败」两档在图标文案下追加一颗按钮(`emptyStateSpec` 第三个字段 `offersSearch`)——前者是自动解析的明确结论,用户不服就手动搜;后者是此刻查不了,网好了想立刻再查也在这里。**「纯音乐」刻意不给**(用户原话「纯音乐的就不给了」);「没有在播放」「广告中」没有可搜对象,「搜索歌词中…」正在搜。系统版走 `ContentUnavailableView(label:description:actions:)` 的 actions 插槽;封面背景版自绘白色玻璃胶囊(`clearGlassCapsule`,跟本窗口别的浮层控件同款)。动作与「⋯」菜单那一项同一条 `openLyricsSearch()`,弹同一个面板、不另开小窗;此前这个入口只藏在「⋯」菜单里,看到「暂无歌词」的人得先知道那里有。文案复用已有键,无新文案。
...
| 播放控制/喜欢/模式 | `LyricsWindowView.playbackControls` / `favoriteButton` / `shuffleButton` / `repeatButton` / `modeToggleButton`(2026-08-19 起两颗互斥按钮取代原 `playbackModeButton`) |
| 「在 %@ 中显示」 | `LyricsWindowView.moreMenuPanel` 那一行:AM `MusicPlaybackController.revealCurrentTrack`;Spotify `lyrimuse/Sources/lyrimuse/SpotifyReveal.swift` `SpotifyReveal.revealCurrentTrack`(取数 `MusicPlaybackController.spotifyCurrentTrackURI`,深链 `LyrimuseCore/Local/SpotifyArtworkURL.swift` `SpotifyURI.deepLink`);其余 `PlaybackCoordinator.openResolvedPlayerApp` |
...
| [歌词|播放记录] 双钮胶囊 | `LyricsWindowView.lyricsQueuePill` / `pillSlotButton`;状态 `showsListenHistory` |
| 播放记录面板(取代原播放队列) | 外壳 `LyricsWindowView.ListenHistoryPane`;已连接分支 `lyrimuse/Sources/lyrimuse/UI/RecentListensPanel.swift` `RecentListensPanel`(与停播页共用,下拉刷新手势见 `.refreshable`);未连接分支 `LyricsWindowView.PendingListensPanel`(数据源 `lyrimuse/Sources/lyrimuse/Settings/ScrobbleBackfillService.swift`);两分支共用的日期格式化 `RecentListensPanel.swift` `RelativeDayFormat`;下拉刷新的可等待封装 `Settings/LastfmStatsService.swift` `refreshBaselineAndWait` |
| 占位态 | `LyricsWindowView.emptyStateSpec` / `emptyState` |
| 停播页三区版 | `lyrimuse/Sources/lyrimuse/UI/IdleStandbyView.swift` `IdleStandbyView` / `IdleOverviewCard` / `IdleLastTrackHero`;右列「最近听过」2026-08-27 搬进 `RecentListensPanel.swift`,与本窗口的播放记录面板共用 |
| 停播页背景 | 同文件 `IdleStandbyBackground`;接入点 `LyricsWindowView.body` 的 `.background(...)` |
| 停播页派生算术 | `LyrimuseCore/Local/IdleListeningStats.swift`(走势序列 / **近 7 天** / 环比 / 日均 / 日期对齐;`lastSevenDays` 与 `weekOverWeekDelta` 共用同一窗口与 `todayCount` 补格,两处都配 selftest) |
...
11. **行激活不能切换渲染结构**(2026-08-21):原来非当前行渲染单个 Text、变成当前行时整棵子树替换成 WrapLayout+逐词 KaraokeLineText——SwiftUI 对结构替换只能淡出淡入,叠上行级 blur/opacity 动画,表现为"新行有一个虚化重新构建的过程"(只有带逐字时间轴的歌触发,纯行级歌词两态都是 Text——"有时候"的来源)。修法=统一结构:有 words 的行恒走 KaraokeLineText,`isActive` 参数化,非活跃行定格全填色、不排动画、字不上浮。逐词读音(groups)当天留作例外只在活跃时挂(理由"占位会撑行高"),2026-09-10 也统一了——见决策 #21。
...
18. **「减少动态效果」下滚动不改瞬移(2026-09-04,用户拍板)**。用户原话「清单里面关于动态减少的都列为不做」——清单里两条挂着系统「减少动态效果」开关的条目(灵动岛音浪 / 菜单栏图标音条那条 2026-09-02 已标不做,本条是歌词窗口滚动)一律不做,是用户的产品决定,不是技术判断。现状备查:本窗口已把 `accessibilityReduceMotion` 接到行模糊、呼吸、弹簧、伪全屏动画上;`scrollToActiveLine` 与间奏点两处(`withAnimation(lineTransition) { scrollTo }`、`.animation(lineTransition, value: currentGapIndex)`)仍无条件 `.smooth(0.45)`。被参考的做法是开关开着时滚动改瞬移。若将来推翻这条:只需给那三处加 reduceMotion 条件、只关位移,行 opacity 的淡入淡出保留——那是内容跟随的必要反馈,不是装饰动效。

19. **不做「手动滚动即暂停跟随 + 按需出现的回到当前歌词」(2026-09-04,用户拍板)**。被参考的做法是滚轮/按下即暂停跟随、只在暂停时渲染一颗「回到当前歌词」、换歌自动恢复。lyrimuse 现状:没有手动滚动暂停机制(见「自动滚动与当前行」那节——用户往回翻歌词后下一次换行会被拉回),常驻的「回到当前播放」按钮此前已按用户拍板删除(理由:常驻冗余)。这次评估把它改成「只在暂停跟随时才出现」重新提了一次,用户仍然不做——歌词窗口的定位是跟着播放走的展示面,不是可浏览的歌词阅读器;往回翻两眼被下一句拉回是可接受的代价,再加一套 following 状态 + 滚轮监听 + 触控板惯性忽略窗 + 按需胶囊,复杂度配不上这个场景。若将来推翻:`@State isFollowing`,滚轮侦测复用 `LyricsWindowController` 已有的 local monitor(Esc 那套)再挂 `.scrollWheel`、只认 `event.window === self.window`,两处 `.onChange(of: scrollLineIndex / currentGapIndex)` 加 `guard isFollowing`,换歌置真,恢复后留一小段忽略窗吃掉触控板惯性尾巴;**别走** `onScrollPhaseChange`(macOS 15+,部署目标 14)。

20. **点 Dock 图标"有时候没反应"=把还原窗口交给了 AppKit 默认 reopen,而它几乎什么都不做(2026-09-09)**。用户报「有时候点击 Dock 栏的图标没有任何反应」。unified log 里 `reopen` 分类 7 天 23 次点击**每一条**都是 `hasVisibleWindows=true` + 走「辅助窗口开着 → 交给 AppKit」那一支,没有一次进过开歌词窗口的分支;02:25 那段连点 12 下,同时刻 `spacediag` 快照是「设置 vis=1、歌词窗口 vis=0」——设置窗可见、歌词窗口最小化或关着,AppKit 什么都没做。为了不瞎猜 AppKit 的规则,写了一个只动自己窗口的隔离 .app 探针(Window×2 + Settings 场景各挂 onAppear/onDisappear 计数、一扇 .floating 的 nonactivating NSPanel、一个 NSStatusItem,靠 `open -g` 自己给自己发 reopen),三条结论:①`hasVisibleWindows` 只要有 NSPanel/状态栏窗口可见就是 true——对这个 App 恒为 true,不能拿它判断任何事(旧注释"最小化会让它变 false"只对没有面板的 App 成立);②AppKit 默认 reopen:**没有任何普通窗口可见时还原一扇(且只一扇)最小化窗口;有一扇可见就什么都不做**——三扇全最小化只回来一扇,"A 最小化 + B 可见"点了不动;③SwiftUI `.onDisappear` 关窗(performClose)必触发、再 openWindow 时 onAppear 再触发、最小化不触发,Window 与 Settings 场景一致,所以 `AuxiliaryWindowActivation.openCount` 这个"开着(含最小化)"口径是可靠的,不是计数器漏了。修法:`bringOpenWindowsForward` 自己做——最小化的**全部**还原(日志里用户本来就是先后手动把两扇都点回来的)、可见的取 orderedWindows 最前那扇 makeKeyAndOrderFront(在别的 Space 上时随 key 切 Space,这一点未单独实测);"辅助窗口"按形态判(有标题栏、非 NSPanel、`canBecomeMain`),不对标题,顺带把不参与 openCount 的「搜索歌词…」也带回;再加一条兜底:App **本来就在前台**、开着的窗口也本来就全在最前面(那一下什么都没变)时落回开歌词窗口——08-25 那条是"不要替代还原动作去抢开歌词窗口",不是"设置开着就永远不准开";从别的 App 点 Dock 切回来的那一下不算,激活本身把窗口带到前台就是可见结果。"本来就在前台"不能用 `NSApp.isActive`(reopen 时已是 true),也不能只看 didBecomeActive 到没到(日志里它多数早 reopen ~100ms、02:25 那次晚 1ms,两种顺序都有),所以 AppDelegate 两头记:didResignActive 清标记、didBecomeActive 设标记并记时间,「标记没设」或「设上不到 1 秒」都算切回来的点击。验证只能靠日志:重装后 `log stream --predicate 'subsystem == "me.yudaotor.lyrimuse" AND category == "reopen"'`,新格式是 `restored=N fronted=N alreadyFront=…`。

21. **逐词罗马音不分活跃行:非当前行也标在每组字底下(2026-09-10,用户圈图:「当前播放这句的罗马音标识在对应的字下面没问题,但是放到这句滚到上面之后位置就重置了,没有对应起来了」)**。现场是一首日文歌(「ラララ歌わせて」):当前行 `rarara / utawa / se / te` 各自钉在 ラララ / 歌わ / せ / て 底下,同一句唱完滚到上面之后变成一整行 `ra ra ra u ta wa se te` 平铺——两种排法在同一屏里上下叠着,一眼就是"位置重置了"。**成因**是 `LyricsLineRow.usesPerWordRomanization` 从 2026-08-10 逐词标注落地那天起就带着 `isActive &&`:当时非当前行还是单个 Text,只有当前行有 WrapLayout+逐词结构可挂读音;08-21 统一结构(已知坑 #11)时非活跃行也改走 KaraokeLineText,却把 groups 留作例外,理由写的是"读音出现是内容变化、非活跃行渲染读音占位会撑高行高"。这个理由站不住:开着罗马音的非活跃行本来就在正文下面另画一整行 `Text(roma)`,占位早就在,留例外只换来两样坏处——排法跳变(用户报的),以及激活瞬间一次**真正的结构替换**(WrapLayout+整行 Text ↔ 带读音列的 WrapLayout),而这正是 #11 想消灭的东西(当前行比非当前行还矮 6pt 的 VStack 间距,换行时整列跟着挪一下)。**修法**:`usesPerWordRomanization` 去掉 `isActive`;`KaraokeLineText.lineContent` 的词组分支给正文字和读音伪词都补上 `forceFilled: !isActive`(原来只有无词组分支有,词组分支此前只在活跃时走到、从没需要过),非活跃行的读音于是跟正文一样定格全色、不上浮、时钟全停(`isLive` 本来就 `guard isActive`),压暗交给行级 lineOpacity。`WindowLineKey.hasGroups` 对同一行前后恒定,WrapLayout 的测宽缓存在激活前后不再失效。**代价**:每个非活跃词组多一个挂着停表 TimelineView 的 `KaraokeWordText`(替掉原来那一个 `Text(roma)`),静态成本,跟 #11 里非活跃字的量级一样;读音颜色从整行那条的 `secondaryTextColor`(白 0.6)变成逐词那套 `base.opacity(0.75)`——跟当前行一致本来就是这次的目的。`swift build` 通过、`./build.sh` 已装;真机对照待用户下次放同一类日文歌看。

22. **简介面板的「网页」行只给当前播放器自己那个平台的歌曲页(2026-09-10,用户截图问「这里的网页为啥只显示 QQ 音乐和 Apple Music」,随后定规则「这里只显示对应播放器的」)**。现场:酷狗在放周杰伦《蒲公英的约定》,面板「网页」行摆着 QQ 音乐 ↗ / Apple Music ↗ 两个别家的链接。查下来两件事叠在一起:①**网易云缺席是故意的**——周杰伦在 collector 的 `neteaseImpersonatorRiddenArtists`(match.go,版权整体下架、库里只剩仿冒号)名单上,`withholdImpersonatorRiddenIdentity` 只留歌词族字段参与打分,歌曲页链接 / 封面 / 署名全扣下,不然这里点过去就是仿冒号的上传页;核缓存 218 条周杰伦 `netease_url` 全空,其他艺人覆盖 3762/5025,与设计一致。②**酷狗从来没有链接**——collector 只拿酷狗的 `hash` 去查 KRC 歌词,不存歌曲页;而且酷狗网页版实测(无头 Chromium)`kugou.com/song/#hash=…` 已经是登录墙(播放器壳 + 扫码弹窗,`wwwapi.kugou.com/play/songinfo` 回 status 0),能开的只有 `kugou.com/mixsong/<EMixSongID>.html`(实测 `j6jua75` 正常出歌曲页),那个编码 ID = base36(mixsongid) + 2 位未知校验(11 组样本试过 md5/sha/crc/取模全不中),只有带签名的网页版搜索接口 `complexsearch.kugou.com/v2/search/song` 才直接给 `EMixSongID`;collector 现用的 `mobilecdn.kugou.com/api/v3/search/song` 没有这个字段。**改法**:规则下沉成纯函数 `PlatformLinks.songLink(forPlayerBundleID:webPlatformID:)`(Apple Music → `music://` 曲目页,QQ → 歌曲页,网易云 → 歌曲页,Spotify 原生或浏览器配对成 `spotifyWeb` → `open.spotify.com/track/<id>`),`InfoPanelLinksRow` 从"三个 chips"收成"一个",该平台没链接(酷狗 / YouTube Music 没存;周杰伦这类没有网易云链接;QQ 只有搜索兜底;Spotify 只有搜索页没有真 ID)整行不出现、**不拿别的平台顶上**——这样这行永远在回答"这首歌在你正用的这个平台上的页面",跟「⋯」菜单的平台入口同一口径。顺带 Swift 侧第一次解码 `spotify_track_id`(`PlatformLinks.spotifySong`,22 位 base62 形状闸与 collector 的 `spotifyTrackIDFromURI` 同源;缓存里 `spotify_url` 那个本地拼的搜索页兜底刻意不读),`PlaybackCoordinator.resolvedWebPlatformID` 从 private 放开。**collector 侧顺手修的**:`needsPeripheralBackfill` 的 `missingNeteaseURL` 没排除仿冒号名单,周杰伦的歌哪怕别的外围字段全齐也因"缺网易云链接"被拉去重补最多 5 轮、每轮所有源重查一遍(缓存里 42 条只缺这一项,5 条已打满),加 `&& !isNeteaseImpersonatorRidden(artist)`,两条 Go 用例钉住。**没做、待用户拍板**:给酷狗补链接——要 collector 多打一次带签名的 complexsearch(签名盐是网页 JS 里的常量,跟 QQ 的 DES3 同一类逆向依赖)、落 `kugou_url`、Swift 解码、`songLink` 多一支;做了之后酷狗播放时这行才会重新出现。selftest 21 条断言钉住 `songLink` / `spotifyTrackURL`;这次由 Amy 的 build.sh 顺带装上(App 23:29、collector 23:28,`nm` 核过 `songLink` / `spotifyTrackURL` 符号在)。
23. **「在 Dock 中显示」关掉了图标还赖着 = `canBecomeMain` 在窗口最小化时为 false,把决策 20 的"最小化全部还原"变成了死代码(2026-09-15)**。用户报「我在设置里面关掉了在 dock 栏展示,怎么又出现了这个」,且随后「这两个窗口都已经关掉了,为什么还是显示」。现场证据:`np:showInDock = 0`(偏好没被改掉)、`lsappinfo` 报 `type="Foreground"`(策略确实是 `.regular`)、`reopen` 日志里 17:37–17:42 每一次点击都是 `restored=0 fronted=0` + `counter says open but no window found`。**用隔离探针(自建 .app、独立 bundle、全程脚本驱动不点界面)测了六件事**:①**`canBecomeMain` 只要窗口一最小化就是 false**(同一扇窗 `isVisible=false isMiniaturized=true canBecomeMain=false`)——`isAuxiliaryRegularWindow` 把它写进判据,于是 `bringOpenWindowsForward` 里 `($0.isVisible || $0.isMiniaturized)` 的后半截是**死代码**,决策 20 花了一整轮探针做的"最小化的全部还原"**从来没跑起来过**,`restored=0` 是必然而不是巧合;最小化的辅助窗口既点 Dock 叫不回来、又一直占着借来的 Dock 图标。②用户机器上 `com.apple.dock minimize-to-application = 1`(最小化折进应用图标),Dock 上连缩略图都没有,所以"还有一扇窗最小化着"这件事用户**根本看不见**,只会认为窗口都关了。③—⑥ 四条"会不会多加一笔"的路径全部收支平衡:同一扇窗重复 `openWindow`、两个方向切 `setActivationPolicy`、App.body 观察的 `ObservableObject` 变化导致 Scene 树重建(`App.swift` 那个 `@ObservedObject languageSettings`,拨任意一个设置开关都会触发)、`Window(标题串)` 变化、`NSApp.hide` + `unhide`——`onAppear`/`onDisappear` 一一配对,没有一条漏加。另外两条修法必需的时序:**`NSApp.hide` 让开着的窗 `isVisible=false` 却不触发 `onDisappear`**(所以拿真实窗口列表对账必须先挡 `NSApp.isHidden`,否则会把"隐藏着的开着的窗"读成"一扇都没开");**`.onDisappear` 触发的同一拍里正在关的那扇窗 `isVisible` 仍是 true**,下一轮 runloop 才从列表消失(所以对账必须 `DispatchQueue.main.async` 延后一拍)。**修法三件**:①判据改成 `(w.canBecomeMain || w.isMiniaturized)`——探针并排跑新旧两版,只在"最小化"这一格不同,正常开着 / 还原之后 / 真的关掉三格逐字相同;②`windowDidAppear/Disappear` 带窗口名字进日志(新分类 `dockicon`),加减两头都记,下次对不上账能一眼看出是哪扇窗;③新增 `reconcile(reason:)`——真实窗口列表说一扇都没开就把计数器清零并还原 `.accessory`,挂在两处:关窗后延后一拍,以及 reopen 发现 `foundNone` 时(那正是"计数器跟现实对不上"的现场)。**没能定位的**:这次**没有**证据证明计数器真的漏加过——排查中途 App 被另一次重装冲掉(pid 23583 → 37331、策略回到 `.accessory`),旧现场取不到了,所以"窗口本来就最小化着(计数器是对的、枚举错了)"与"确实漏了一笔"两种可能分不出。①是已证实的真 bug,③是不管哪种都能自愈的兜底,②让下次能直接判。**顺带发现、并且刻意没改的一处**:`AppDelegate.applicationDidFinishLaunching` 里那句 `NSApp.setActivationPolicy(settings.showInDock ? .regular : .accessory)` 跑在窗口 `.onAppear` **之后**,所以"启动时被系统状态恢复带回来的辅助窗"拿不到借来的图标(既不进 Cmd-Tab、也没有 Dock 图标),跟其余路径不一致 —— 实测装完新版那一次:设置窗被恢复出来、`openCount=1`、`showInDock=0`,策略却是 `.accessory`。补 `|| AuxiliaryWindowActivation.hasAnyOpen` 一行就能对齐,**2026-09-15 用户拍板不补**:对齐意味着"上次留着设置窗没关,下次启动 Dock 图标就会冒出来",而他关掉这个偏好要的就是少看见图标;不一致只在启动这一格,且不会让计数器与现实脱节(reconcile 不受影响)。别当 bug 顺手"修"掉。


## 专项:背景取色逼近 Apple Music(进行中,2026-08-23 开新会话延续)

第八轮(见上)把 `bakeWindowBackgroundLayers` 的饱和度系数从"方向反了的 1.5 倍放大"扳正成"跟真机数据拟合的 0.35 倍收缩",5 首真机样本 4 首落进 AM 范围——但这仍是**单变量线性模型在 5 个样本上的粗拟合**,不是逆向出 AM 真实算法。第九轮(见下)用 23 组样本推翻了"固定倍率"本身,换成幂函数。第十轮(见下)又用追加的 13 组样本 refit 了幂函数系数,并**验证并排除**了一个"暖色调额外加成"的假说。以下是继续这项工作的素材和方法,新会话可以直接从这里接着干,不需要重新摸索采样方法。
...
**第十轮(2026-08-27,追加 13 组,refit 系数 + 排除"暖色加成"假说)**:第九轮上线后用户陆续又发来独立验证样本(《爱我吧》《公园》,方大同/未来专辑)——《爱我吧》(中饱和度暖色人像,src=0.504)明显偏灰(AM 0.727 vs 我们 0.462),《公园》(高饱和度)对得很好,由此起了疑心:是不是暖色调(琥珀色相 15°~55°)会被 AM 额外加成?此后用户又连续甩来 The Weeknd/Janet Jackson/Weezer/NEXZ/薛凯琪等 13 组新截图,其中好几张暖色调封面(20 Y.O. Intro/Say Yes/自由的夜/Twisted Elegance)都比公式预测浓,像是坐实了假说。但把这 13 组 + 原 23 组合并成 36 组全量数据,认真做了一次 `AM_p75 = a×satP75^b×(1+c×warmScore)` 的二变量回归——**结果推翻假说**:加色相项后 R² 只从 0.754 爬到 0.762(噪声量级的提升),且逐条看是拆东墙补西墙——改善了 20 Y.O./Say Yes/Twisted Elegance,却让原本拟合很好的 P.Control(误差 0.018→0.098)、黑夜、Get on the Boat、NEXZLoco 全部变差;反例也很扎眼:玩乐(绿色封面)残差 2.16、Controversy(品红封面)残差 1.68,比任何暖色案例都离谱。**结论:色相不是这批"暖色发灰"案例背后的真正变量**,不采纳色相项。改为只用全量 36 组重新拟合单变量幂函数(系数从 0.94/1.45 微调到 **1.029/1.433**,R²=0.754 基本不变,但拟合更稳)。验证:13 张新增封面全部裁出跑 `bakebg-repro.swift`,`satMul` 无一顶到 1.6 上限、不需要跟着抬,MAE 从 0.135 降到 0.125。**Say Yes**(Weezer,金属压纹烫金封面)是全数据集里最突出的残余反例——AM 给到源图 1.51 倍,refit 后我们仍有 0.365 的误差,比第二名(20 Y.O. Intro 0.248)大得多——真正原因还没查,候选方向不是色相,可能是"金属反光/高光质感"这类构图特征在 AM 的取色管线里被特殊处理,留给下一轮。此轮改完后 `./build.sh` 编译通过(此前第九轮那份"跟本专项无关的未完成改动"导致的编译错误,在这轮已不存在,大概率被用户自己的其它工作顺带修掉了)并重启了正在跑的 App,已过真机验证起点(用户后续截图与新公式离屏复现数值吻合,证实改动确实生效)。

**第十一轮(2026-08-27,按"材质假说"补样,定位并修复一个独立的近黑放大 bug)**:延续第十轮"Say Yes 可能是材质/反光"的猜测,按建议方向补了 8 组"金属铬面/镜面反光/金箔烫金/丝绸光泽"封面(Random Access Memories、One More Time/Discovery、Renaissance、Chromatica、Silk Sonic、Bad、Diamonds and Pearls、Golden)——结果**材质假说没有统一验证成立,反而分裂成两种方向相反、互不相关的问题**:
- **Silk Sonic** 延续 Say Yes 的方向(AM 比我们浓,src≈0.49 → AM 0.710 / 我们 0.381),跟"暖色调欠修正"那批(爱我吧/20 Y.O.)同类,材质本身(其实是哑光牛皮纸插画,并不反光)跟这条无关。
- **Random Access Memories / One More Time / Renaissance / Diamonds and Pearls** 这四张是全新的、方向相反的问题:**我们比 AM 艳得多**(RAM 我们 0.853 vs AM 0.195,One More Time 我们 0.900 vs AM 0.071,Renaissance 我们 0.818 vs AM 0.171)。这四张封面的共性是"大面积纯黑背景 + 一小块铬合金/反光高光",不是"反光"本身导致,而是背景太黑导致的一个独立 bug。

用 `--verbose` 逐格拆解 Random Access Memories 定位到根因:6×6 网格里有 8/36 格(22%)原始亮度 ≈0.047(肉眼看是纯黑),却读出 `s=0.36`——纯黑区域的原始 RGB 不是精确 (0,0,0),带着一丝几乎不可见的偏色(摄影黑位/JPEG 压缩常见现象),这个偏色的**比值**(HSV 饱和度)在亮度极小时天然被放大到跟中高亮度区域一样高的数字。这片伪色不仅在亮度归一化后变得肉眼可见,还因为占比够大(22%)直接把整张图的 `hueDom` 判定拖偏(算成蓝色),并顶进了 `satP75` 的排序位置,让 `satTarget` 算得远高于封面实际观感——AM 的对应区域测出来就是老实的低饱和度暗灰,没有这个问题。数学上验证过:亮度归一化(`homog` 那步)是等比缩放三个通道,不改变饱和度比值(缩放前后 S 几乎不变,只是从"暗到看不见的假信号"变成"暗但看得见的假信号")——所以问题根子不在"要不要做亮度归一化"(那是 round 6 就验证过的、AM 确实会抹平明暗布局的正确设计),而在于**从来没有一道"多暗就不该信这格颜色"的判据**,导致近黑噪声跟真实色块被同等看待。

修法:用**归一化前**的原始格亮度(`cellLuma`,光斑锚点已经在用同一份数据)设一道下限 `darkLumaFloor = 0.08`(取自本轮实测:伪影格 ≈0.047、其余正常暗色内容普遍 >0.1,不是精确刻度)——低于这道下限的格子不参与 `hueDom`/`hueCoherenceScale`/`offHueFraction` 的色相统计,不参与 `satP75` 的排序统计(亮格不够 9 个时退回不筛选,这类封面本来就该给低 satTarget),也不参与把欠饱和格子拉向 `satTarget` 的那次修正——四处判据是同一道下限,`PlaybackCoordinator.swift`/`bakebg-repro.swift` 保持同步。

验证(离屏复现,同一批 21 张封面新旧两版对比):目标封面 Random Access Memories 0.528→0.000(目标 0.195,误差 0.333→0.195)、One More Time 0.385→0.235(目标 0.071,误差 0.314→0.164)、Renaissance 0.522→0.222(目标 0.171,误差 0.351→0.051,收得最干净);其余 16 张(含 Diamonds and Pearls/Chromatica/Silk Sonic/Bad/Golden 这五张同批"材质"样本、以及第十轮的 13 张)里有 14 张**逐位元字节不变**——这道下限只在真的有大片近黑区域时才生效,没有误伤其它封面。两张有轻微副作用:Starboy(0.886→1.000,目标 0.911,误差 0.025→0.089,变差但绝对值不大)、Alone Again(0.660→0.694,目标 0.491,误差 0.169→0.203,变化很小)——两张都有一定比例的暗部,被下限筛掉部分格子后统计样本变少、稳定性略降,是这次改动可预期的代价。全 21 张 MAE 从 0.152 降到 0.129。`./build.sh` 编译通过,已重启在跑的 App。

**这轮同时明确了一个方法论问题(用户主动提出)**:`our_p75`(我们自己的渲染结果)和"差别大/还好"这类主观判断,只反映**截图那一刻在跑的代码版本**,每次改完 `satTarget`/`hueCoherenceScale` 这类公式,历史截图里"我们"那一半就全部过期,不能拿旧截图判断当前代码对不对——`src_p75`(封面本身)和 `AM_p75`(AM 真机渲染)这两个数字才是永久有效的校准目标,不随我们的代码变化。日常判断新公式好不好用新样本的量化结果就够,不需要每次改完都重新截一遍全部历史样本;只有想确认"某张历史封面现在具体变成什么样"时,才需要重新截图(或者封面原图还在的话用 `bakebg-repro.swift` 离屏复现)。
...
**第九轮 23 组数据集**(2026-08-23,同一次批量截图对拍;`src_p75`=源封面网格 p75,`AM_p75`/`our_p75`=对应背景网格 p75,`verdict`=用户主观判断):P. Control 0.939/0.922/0.410·差别大;The Beautiful Ones 0.421/0.234/0.148·还好可以接受;2 Bad 0.592/0.500/0.404·还好可以接受;I Wanna Be Your Lover 0.620/0.371/0.556·还好可以接受;Babygirl 0.789/0.928/0.360·差别大;Deadman 0.258/0.100/0.077·可以接受;月亮代表我的心 0.163/0.090/0.179·可以接受;你瞒我瞒 0.277/0.142/0.077·可以接受;2 Bad(复测)0.600/0.471/0.404·可以接受;Blood on the Dance Floor 0.707/0.640/0.306·稍微差点不过还好;黑夜 0.625/0.343/0.385·差别大(色相问题,见发现 2);Get on the Boat 0.751/0.438/0.384·差别大(色相问题);玩乐 0.466/0.671/0.118·还好;All for Joy 0.429/0.076/0.045·还好;张永成 ≈0(近灰阶)·还好;公转自转 0.764/0.363/0.423·还好;早上好 0.133/0.044/0.123·还好;谁稀罕 0.671/0.760/0.281·差别大;明明就 0.359/0.253/0.253·还好;Controversy 0.318/0.300/0.122·还好;Sign O' The Times 0.649/0.780/0.240·差别大;小镇姑娘 0.961/1.000/0.236·差别大;鬼-Overture 0.595/0.447/0.500·还好。

**第十轮追加数据集**(2026-08-27,13 组,格式同上 `src_p75/AM_p75/AM_hue`,均无独立主观 verdict——用户直接批量甩图核对,详见上面"第十轮"叙述):爱我吧(方大同)0.504/0.727/37°(数据仅存于本节文字,原截图临时文件已过期无法重新裁封面复现);公园(方大同)0.885/1.000/188°;Wake Me Up(The Weeknd)0.810/0.569/24°;Alone Again(The Weeknd)0.650/0.491/55°;Starboy(The Weeknd)0.960/0.911/337°;20 Y.O. Intro(Janet Jackson)0.620/0.814/31°;Twisted Elegance(Janet Jackson)0.510/0.600/14°;Intro·All For You(Janet Jackson)0.560/0.544/20°;Don't Stand Another Chance(Janet Jackson)0.540/0.292/322°;Morning(Janet Jackson)0.224/0.184/340°;Control(Janet Jackson)0.867/0.894/351°;Say Yes(Weezer)0.647/0.980/39°;LOCO(NEXZ)0.996/0.947/13°;自由的夜(薛凯琪)0.745/0.836/19°。(Call Out My Name/The Weeknd 因封面本身太暗、取样点原始 RGB 落进 8-bit 量化噪声区被排除;Dawn FM/The Weeknd 同理排除——**背景太暗时单点网格采样不可靠,这是个持续存在的方法论坑,量化不出可信数字就该直接排除而不是硬凑**。)

当前上线公式:`AM_p75 ≈ 1.029 × satP75^1.433`(第十轮 36 组全量数据 refit,取代第九轮的 0.94/1.45)。

**已知未解决问题**(新会话的重点候选方向,按优先级):
1. **Say Yes(Weezer)/Silk Sonic 式的"暖色欠修正"大残差(优先级最高)**:refit 后全数据集里误差最大(Say Yes 0.365,Silk Sonic 0.273),AM 给到源图 1.3~1.5 倍。**已排除色相**(第十轮核心结论)和**已排除"金属反光/材质"这个笼统假说**(第十一轮:8 组材质样本里只有 Silk Sonic 延续这个方向,其余 4 组是完全独立的近黑放大 bug——见下面"已修复"条目——Silk Sonic 本身其实是哑光牛皮纸插画,并不反光,材质假说站不住)。目前唯一收窄的线索是"暖色调/中高饱和度"这个粗糙相关(第十轮已验证过不是简单的色相区间开关),真正的自变量还没找到,建议下一轮换角度——比如构图是否以人像/大面积肤色为主、是否有真实材质光影(哑光 vs 反光可能本身就不是关键变量)。
2. **色相偏差(第九轮的独立坑)**:黑夜/Get on the Boat 这类"AM 与我们饱和度接近但色相偏了 20°+"的案例,`hueCoherenceScale`/`offHueFraction` 都测出算法认为色相没问题,说明偏差另有病灶,还没定位。建议用 `--verbose` 逐格拆解这两张封面。
3. **RGB 空间高斯模糊的过渡色伪影**:两个色相相差很大的区域(如你瞒我瞒的蓝天 ~250° 与暖褐 ~20°)在模糊过渡带会生成一个跟两端都无关的诡异色相(偏洋红),这是结构性问题——换更大/更小的系数都治不了,需要换混色方式(例如在 HSV/Lab 空间做混合,或者过渡带单独限幅色相跳变范围)。第九/十轮里你瞒我瞒被判"可以接受",不代表这个 bug 已经消失,只是取样的帧没暴露它。
4. **`satMul` 闭环的面积均值/网格 p75 口径错位**:见上面"发现 3"。高对比度(两大色系反差大、模糊后 `fieldS` 崩得凶)的封面会顶到 `satMul` 上限却依然够不到 `satTarget`,抬上限只会让它过冲。需要的是把闭环本身换成网格采样校准(而不是 `CIAreaAverage`),但那需要在 CIImage 管线里做网格采样,渲染期开销未评估,留给下一轮设计。
5. **样本仍偏单一变量**:幂函数拟合只用了源图饱和度这一个变量,R²≈0.754——`satTarget = f(satP75)` 之外还有残余误差,但**色相、材质反光都已验证不是那个缺失变量**(第十/十一轮);明暗/人像肤色占比/构图复杂度仍是候选,继续扩样本才能分清到底是哪个。
6. **`darkLumaFloor` 下限带来的轻微副作用(第十一轮新引入,优先级低)**:Starboy(误差 0.025→0.089)、Alone Again(0.169→0.203)两张有一定暗部占比的封面,排掉近黑格后统计样本变少、`satP75`/`hueDom` 的稳定性略降,导致误差小幅上升——目前判断是可接受的代价(换来 3 张近黑封面误差腰斩以上),但如果后续样本里这类"暗部占比中等、不到大面积纯黑"的封面持续出现类似退化,需要重新评估 `darkLumaFloor = 0.08` 这个经验值是否需要按暗部占比做连续过渡而不是硬阈值。
- **已修复(第十一轮)**:近黑放大 bug——大面积纯黑背景 + 一小块铬合金反光高光(如 Random Access Memories/One More Time/Renaissance)会把近黑区域的隐性偏色在亮度归一化时放大成显眼假色,进而拖偏 `hueDom`/推高 `satTarget`。已用 `darkLumaFloor` 下限修复,见上"第十一轮"记录,不要重新排查这类"我们比 AM 艳"的近黑封面案例——已知根因、已验证修复。
...
- `scripts/bakebg-repro.swift`——逐行对应现网 `PlaybackCoordinator.swift` 的 `bakeWindowBackgroundLayers`(2026-08-27 第十一轮同步更新到 `darkLumaFloor` 近黑下限),加了 `--verbose` 打印每格 hue/sat 与 hDom/offHueFraction/hueCoherenceScale/satMul 中间量,`--out` 导出 360×360 烘焙结果。改了正式代码的算法记得同步改这份脚本(两边不共享源码,靠人工保持一致)——第十一轮定位近黑放大 bug 正是靠这份脚本的 `--verbose` 逐格打印才抓到 8/36 格同值的异常。
...
- ⚠️ **粘贴板截图的临时文件会过期**(第十轮踩坑):用户直接粘贴发来的截图存在 `/var/folders/.../T/otty-paste/` 这类系统临时目录,实测 4 天后旧文件被清掉(《爱我吧》的原图到第十轮想回头重新裁封面复现时已经不存在)——凡是要跨会话/跨天复现用的截图,当场就要把源文件(或至少裁好的封面/背景采样值)拷到仓库外的持久位置(比如整理进本节文字记录,像上面"第十轮追加数据集"那样把数字留下来),不要指望临时文件还在。

**当前构建状态(2026-08-27)**:第十一轮改完后 `./build.sh` 编译通过、重启了正在跑的 App(第九轮记录过的那份"跟本专项无关的未完成改动导致编译不过"的问题,到第十轮已经不存在了,大概率是用户在别的会话里把它处理掉了——**这类无关编译错误具体是怎么消失的没有细查,新会话如果又遇到编译不过,不要假设"上次的坑还在",先重新跑一遍 `git stash` 隔离法确认是不是新坑**)。真机验证:第九轮上线后用户反馈《爱我吧》"感觉有点灰",经核实其截图实测饱和度(0.441)与当时用第九轮公式(0.94/1.45)离屏复现算出的预测值(0.462)基本吻合,证实第九轮改动确实生效在运行中的 App 上、不是停留在离屏复现层面——这也是"爱我吧发灰"反馈的真正含义:不是改坏了,是改得不够(AM 真实值 0.727,第九轮只追到 0.462),第十轮的 refit 系数已经把这类案例的目标误差进一步收窄。第十/十一轮的改动尚未等到类似的真机对拍反馈闭环(第十一轮的 `darkLumaFloor` 修复只验证到离屏复现这一层),新会话如果收到用户反馈,记得按这个方法核对是"改动没生效"还是"改动生效了但幅度不够"——**另外用户已经明确过一条方法论**:历史截图里"我们"那一半只反映截图当时的代码版本,每次改完公式就过期,不能拿来判断当前代码对不对,只有 `src_p75`/`AM_p75` 这两个数字永久有效(详见上"第十一轮"段落末尾)。
