...
**跟着 Apple Music、QQ 音乐、网易云音乐、酷狗音乐、汽水音乐、Spotify，或浏览器里的网页版 YouTube Music / Spotify 播放，在 Mac 桌面上实时显示逐字同步歌词——外加 Last.fm 听歌档案和端上机器翻译。**
...
Lyrimuse 常驻在菜单栏里，跟着当前播放弹出一个悬浮歌词窗口——Apple Music、QQ 音乐、网易云音乐、酷狗音乐、汽水音乐、Spotify，以及浏览器里的网页版 YouTube Music / Spotify，任选几个组合（也可以交给自动识别）——逐字同步、常驻置顶、跨 Space 显示。就是网易云音乐桌面客户端那种"桌面歌词"体验，只不过是原生 macOS 版本。
...
### 歌词，做到位
- **逐字同步高亮**，跟随播放进度实时显示
- **自动查十二个歌词源**——网易云音乐、QQ 音乐、酷狗、酷我、咪咕、Musixmatch、LRCLIB、LyricFind（经 YouTube Music）、Deezer、AMLL（人工校对过的逐字歌词库）、汽水音乐（官方逐字）、Apple Music 官方歌词（可选，需在设置里连接账号；唯一的官方源，不少曲目还带逐字时间轴）——自动挑出最合适的一份，不用自己动手搜
...
- **完整的「歌词管理」窗口**——浏览、手改、删除、重新搜索任意一首歌的歌词，支持多选批量删除、列宽随手拖，遇到不同步还能单独调整这首歌的时间轴偏移；没搜到词的存量歌曲还能一键全部重试，整个曲库也能用现在的匹配规则重新挑选一遍（手动改过的歌词不会被动）
...
- **什么时候才算听过一次，你来定**——按 Last.fm 官方规则（50%）记，或者压到 75%、90%、真正播完才记；ListenBrainz 的提交不受影响。还可以把某个播放器整个排除在 Last.fm 之外，不想记录的那个播放器放的歌就不会进你的收听历史
...
- **菜单栏文字模式**——不想要悬浮窗，直接在状态栏看当前这一行歌词；太长的句子会横向滚动播完，而不是截成半句（想要截断也留着开关）；还可以加开一行副行，把下一句、译文或罗马音直接排在当前句下面——灵动岛胶囊上也有同一套副行
...
- **外观完全自定义**：字体（也可以跟随系统，或者导入自己的 .ttf / .otf 字体文件）、字号、文字/背景/阴影颜色（可以存成配色主题反复用，也可以让文字颜色跟着当前专辑封面走）、悬浮窗宽度
...
- **自动检查更新**（也可以随时在菜单栏里手动检查）——不用自己老回 Releases 页面看；更新在设置窗口里的「软件更新」页完成，更新说明和进度都在同一扇窗里；想尝鲜的话还有一个「接收测试版更新」开关
...
LyricsX（最后一版发布于 2022 年 4 月，支持 macOS 10.11+）覆盖 Apple Music、Spotify 等几个经典播放器；Lyric Fever 专注 Spotify + Apple Music，要求 macOS 15+。Lyrimuse（macOS 14+）额外原生支持 QQ 音乐 / 网易云音乐 / 酷狗 / 汽水音乐，支持浏览器网页播放器，逐行判定的拼音 / 粤拼 / 注音假名，以及 Last.fm / ListenBrainz 打卡加本地听歌统计。逐项核实过的对照表见[对比页](docs/lyrics-apps-comparison.zh-CN.md)。
