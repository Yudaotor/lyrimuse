...
///
/// 参考 QQ 音乐悬浮歌词加的四个:`expandToLyricsWindow`(展开到歌词窗口)、
...
public enum OverlayControlHitTest {
    ...
    /// 结果随机,是最难查的一类 bug。
    /// SwiftUI 内容坐标(左上原点、y 向下)到 AppKit 窗口本地坐标(左下原点、y 向上)。
    ///
    /// 抽出来是因为这套换算在三处(按钮矩形/控制热区/歌词热区)都要用同一套规则。
    ///
    /// 换算结果**不能存成屏幕坐标**——窗口一移动,SwiftUI 布局没变、PreferenceKey 不
    /// 重发,存的屏幕坐标就还停在旧位置,按钮和热区会失效。只存**窗口本地坐标**、判定时
    /// 把鼠标点转进来——窗口本地坐标不随窗口移动改变。
    ///
    /// `contentTopInset`(位置预设「底部居中」引入):内容块顶边离窗口顶边多远。
    ...
    /// 指针此刻**该高亮哪一颗**按钮(「悬浮歌词这上面的按钮帮我开一个鼠标
    ...
    ///
    /// 「为什么我桌面悬浮歌词,鼠标放在红框部分就会出菜单栏选项了?不应该
    /// 只有字的范围才会吗」——「暂无歌词」这四个字左右两侧的一大片空白照样把整排按钮叫出来。
    ///
    /// 病根不是 `chromeHoverZone` 算错了,是**只有一档判据**:控制排槽位常驻、矩形无条件
    /// 上报(见 `LyricsOverlayView.controlsSlot` 那条 提醒 —— 让它兼表可见性会被 preference
    /// 归约冲掉),于是包围盒恒被那排按钮的胶囊撑到胶囊自己的宽度。歌词越短,白送进命中区的
    /// 空白越多;「暂无歌词」这种占位文案下包围盒几乎就是整扇窗 —— 那次"从整扇窗
    /// 收紧成包围盒"在短歌词上等于没收。
    ///
    /// 两档回答的是两个不同的问题,不能共用一个区域:
    ///  - **还没露出来**时问"用户是不是在看这行字" 到 只认歌词文字矩形。指针从窗口空白处
    ///    扫过不再叫出按钮,这正是用户要的。
    ///  - **已经露出来**时问"用户是不是还在跟这排按钮打交道" 到 用包围盒。按钮在歌词**外面**
    ...
    /// `hoverControlsEnabled` 这个开关——开关关掉时,锁定态在悬浮窗上什么控制类 UI 都不露。
    ///
    /// 解锁在悬浮窗**之外**还有三条路,都不经过这个开关:菜单栏「控制中心风」面板
    /// (`MenuBarPanelQuickSettings`)、菜单栏右键完整菜单(`MenuBarStatusMenu`)、以及全局
    /// 热键(`GlobalHotkeys`),三处都直接读写 `AppSettings.lockPosition`。这颗图标不需要
    /// 单独兜底,靠上面那三条路。
