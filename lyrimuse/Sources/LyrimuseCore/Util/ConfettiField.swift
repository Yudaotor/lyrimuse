...
/// 引导页最后一页那阵**撒花**的纯几何(「帮我在引导页的最后一页加入
/// Mac 很经典的那种撒花效果」)。
///
...
public struct ConfettiField {
    ...
    ///
    /// 这个数和 `fall`(1.5...2.6s)、`pieceCount`(130)是**一起调的一组**,离屏预演过才定的
    /// (把落点画成 PNG 逐帧看,scratchpad 里那个一次性 renderer)。 别调成 0.9s / 90 片 /
    /// 1.6...2.9s:那样画出来是**一条纸带扫过去**,最后一片 0.9s 就出场了,到 1.8s 上沿已经全空,
