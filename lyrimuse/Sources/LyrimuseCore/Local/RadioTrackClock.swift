...
/// 非歌曲内容**(英雄到Fact Check 多 63.3s、Fact Check到Blingy 多 66.3s,都是拿 Apple 目录曲长比的),
...
public enum RadioTrackClock {
    ...
    /// 播种初始位置的上限。换歌时刻只可能比"现在"早一点点(事件到达 到 去抖动 250ms 到 子进程往返),
    ...
    ///
    /// - 换歌(key 变了)或第一次见 到 重新起表,位置 = 从 `startedAt` 到现在这段(见 seedPosition);
    ...
    /// resume transition 逐条坐实:0.000到5.841、26.416到29.953、13.801到17.545,跳了 3.5~5.8 秒。
    ...
    ///
    /// 三个边界各挡一种错配:没有观察时刻 到 0(跟没有这个参数时逐字相同);观察时刻在未来(时钟毛刺)
    /// 到 0,不要负位置;超过 maxStartSeed 到 夹住,见那个常量的注释。
