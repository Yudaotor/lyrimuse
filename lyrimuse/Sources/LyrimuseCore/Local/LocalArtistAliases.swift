...
/// 歌手写法归并(罗马字艺名 与 中文本名、乐队别名)的**通用推断**,取代此前手写的
/// `romanizedArtistAliases` 静态表 —— 「尽可能去掉手工表,一切由通用逻辑覆盖,不要特殊化」。
///
/// 证据全部来自本机、零网络:
///  1. collector 的 MusicBrainz 缓存(它解析歌词时已经替每个新歌手查过一次 MusicBrainz):
///     - `lyrimuse-artist-alias-cache.json`:原始标签 到 中文别名(`Crowd Lu 到 卢广仲`);
///     - `lyrimuse-artist-identity-cache.json`:名字 到 {mbid, zh}(`Soft Lipa 到 蛋堡`);
///     - `lyrimuse-artist-primary-cache.json`:名字 到 MusicBrainz 上的全部别名
///       (`周杰伦 到 [Jay Chou, Jay, Zhou Jie Lun, ジェイ・チョウ, …]`、`Will Pan 到 [潘瑋柏, Wilber Pan]`)。
///  2. enrich 缓存里的共现:两种歌手写法名下的曲目被歌词解析落到了**同一批歌曲 id**
///     (`Khalil Fong` 与 `方大同` 共享 13 个网易云 id、`David Tao` 与 `陶喆` 3 个)。要求 ≥ 2 个
///     不同 id —— 单个共享 id 实测全是噪音(`方大同 与 王诗安` 是合唱、`K 与 英雄联盟` 是配错)。
///
/// 把这些边丢进并查集,每个连通块选一个代表:优先含汉字/假名的写法,同样含的取本机曲目数最多的
/// (那才是用户自己历史里的主流写法),再同样取字典序最小 —— 结果确定、不依赖字典遍历顺序。
/// 输出:`stripSpaces(normalized(mergeArtist(写法))) 到 代表写法`,只给 `PlayCountFold.canonicalArtist`
/// 用(查族键 / 明细 / 别名推断的歌手分桶),不碰整本账的身份键 `PlayCountFold.key`。
///
/// 保守闸:
///  - MusicBrainz 别名里去掉空格后不足 3 个字符且不含汉字的不用(`K` 这种撞上别的艺人的概率太高;
///    匹配本身是整串相等,`asi`(A Si 到 阿肆)这种 3 字符键只会撞上同名艺人,留下);
///  - 合唱串先归首位(`Khalil Fong & Fiona Sit 到 Khalil Fong`),不让一次合唱把两位歌手并成一块。
public enum LocalArtistAliases {
    ...
    public static func derive(caches: MusicBrainzCaches, entries: [EnrichTitleAliases.Entry]) -> [String: String] {
        ...
        /// 碎片去连边就是乱连(实测推出 `earth 到 アース`)。碎片不当边的端点;共现那条路不受影响
