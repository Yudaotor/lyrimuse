...
/// 滞后可达几十分钟(实测:陳綺貞《慢歌 3》16:36 落库,16:36/16:38/16:40/16:46/
/// 16:50 五次查都还是 0,过了 15 分钟那一轮把它钉死;Last.fm 网页那边当时已显示 1 次)。
/// 一次滞后 = 永久空白,而且不显示「···」占位(占位只给"还在解析"的行),用户看到的就是
/// 一行没有次数、也没有任何解释。
///
/// 现在"没有"是一个**带时间戳、可过期**的结论:第 1 次判 0 之后 1 小时允许重探,仍是 0 到
/// 6 小时,再 到 24 小时封顶。重探只发生在 `resolvePlayCounts` 本来就会看的行上(当前页 /
