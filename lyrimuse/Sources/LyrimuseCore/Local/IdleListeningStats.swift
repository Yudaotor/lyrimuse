...
/// 那份桶**只存非零天**(实测 316 个键、没有一个 0 值),所以 `dailyCounts.count` 就是
...
public enum IdleListeningStats {
    ...
    ///
    /// 存在的理由:界面上那个「近 7 天」数字块,**数值**原来取的是
    ...
    ///
    /// 代价必须知道:这个数会比 Last.fm 网页上的「last 7 days」略低,因为那边是滚动
    ...
    /// 两个窗口都在同一份桶里算,所以这个比值自洽; 但它**跟界面上那个「近 7 天」大数字不同源**
    /// (那个取的是 API 的 @attr.total),别把它当成「大数字 ÷ 上周大数字」的结果去核对。
    ///
    /// `todayCount`:今天那一格的实时值。**桶是同步到昨天为止的**,今天
