...
/// 「歌词管理」列表排序的**纯逻辑**部分。
///
/// 为什么放在 LyrimuseCore 而不是留在 `LyricsManagerView.swift`:`lyrimuse-selftest` 只依赖
/// LyrimuseCore,排序规则(哪一档优先、平局怎么断、缺失值排哪儿)留在 View 层就够不到它、
/// 拿不到任何自动化覆盖——"逻辑对但在某些数据形状下退化成静默无操作"这类问题靠肉眼看
...
public enum LyricsSortOrder: Sendable, Equatable {
    ...
    public func less(_ a: LyricsSortKey, _ b: LyricsSortKey) -> Bool {
        ...
        case .source(let ascending):
            // 无来源的行**两个方向都排最后**,不是按「无来源」这个展示名参与字母序。
