...
/// # 系统在口白期间什么都不给
///
/// 抓了一整段口白(01:06:41 歌放完 到 01:07:42 下一首,共 61 秒):事件流里**一条都没有**,
...
/// | 23:18:15 | 空 | `NCT 127` |
/// | 早先 | 空 | `YEONJUN` |
/// | 00:26:50 | 空 | `petal radio` |
///
/// 订正:这张表原先把两列写反了(记成 title 空、artist 是台名)。判据是
...
public struct RadioStationCard: Codable, Equatable, Sendable {
    ...
    /// 封面往往在几百毫秒后单独发一次(实测 artworkData 是独立一行)。
