...
/// 一键开启)。
///
/// **只检测,不代改**。曾经有过一条"一键开启":读浏览器 profile 里的 `Preferences`(JSON)、
...
///
/// 两套完全不同的机制(逐个用 `strings` 翻二进制 + 实际读文件验证过,不是凭直觉猜的):
///
/// - **Chromium 系**(Arc / Chrome / Edge / Brave,同源于 Chromium 本体——四家的真实引擎二进制里都
...
public enum BrowserAutomationPermission {
    ...
    ///
    /// 认码不认名字:命令的显示名会随浏览器本地化/改版变,四字码是 AppleScript 的 ABI,
    ...
    public enum Status: Equatable {
        ...
        /// 说成"确定没开"。
        ///
        /// 之前 Safari 分支把 nil 判成 `disabled`(那时这里还写着"Chromium 系
    ...
    public static func safariStatus(fromPrefValue value: CFPropertyList?) -> Status {
        guard let value else {
                // **读不到 ≠ 关着**(修)。原来这里 `return .disabled`,理由写的是
    ...
    ///
    /// 读不到就返回 nil,`status` 据此报 `.unknown`(不是 `.disabled`)。读不到最常见的原因
