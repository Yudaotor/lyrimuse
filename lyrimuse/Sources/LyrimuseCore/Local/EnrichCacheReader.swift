...
public struct EnrichCacheEntry: Decodable, Sendable {
    ...
    // 播放器报的时长(秒,collector/enrich.go 的 DurationSecs)。解码,给「英文歌名 到
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case motionCoverIdentityVerified = "motion_cover_identity_verified"
...
public enum EnrichCacheReader {
    ...
    private struct PlayingEntryFile: Decodable {
        let key: String
        let entry: EnrichCacheEntry
    }
    /// 按 mtime 缓存解出来的那一份(轮询每拍都会问)。
    private static var cachedPlayingEntry: (mtime: Date, key: String, entry: EnrichCacheEntry)?
    ...
    // 宽松匹配索引(looseKey 到 组内字典序最小的原 key),跟 cachedEntries 同寿命、惰性
    ...
    /// 字段一直解码着但没往外传,这里补一个只读口;沿用 lookup 同款的 精确 key 到 宽松
    ...
    /// 沿用 lookup/sourceInfo 同款的 精确 key 到 宽松 key 两级匹配。
    public static func platformLinks(artist: String, title: String, album: String) -> PlatformLinks? {
        ...
        let links = PlatformLinks(
            // https://music.apple.com/… 到 music://(进 App)。非 AM 链接会被这个函数拒掉。
    ...
    /// 同款的 精确 key 到 宽松 key 两级匹配;查不到或值非正返回 nil。
    ///
    ...
    private nonisolated static func mtime(of url: URL) -> Date? {
    ...
    /// 不同专辑版本下封面经常是真的不一样(现象是「方大同 - 放不过自己」
    /// 就是坐实 —— `JTW 西游记 (Gold) [Explicit]` 是金底半脸特写,更早听过的
    ...
    ///
    /// 查法跟 `albumMatchedCoverURL` 逐字一致(精确 key 到 仍然认专辑的 looseMatch),连
    ...
    /// (scrobble 自带图 到 track.getInfo 到 同专辑兄弟),而 Last.fm 对中文曲库缺图非常
    /// 常见 —— 现象是「陶喆 - 聖誕之吻」就是三级全空(Last.fm 只给它那张所有
    ...
    ///
    /// 这一级"忽略专辑"的兜底只适合"专辑名本来就不可信/拿不到精确对照"的消费方
    ...
    /// 纯函数,selftest 直接覆盖。
    ///
    /// 为什么要退这一步(现象是「最近记录里这几首没封面」):同一次收听会以两种
    ...
    /// 一档。实测两个不同的 album mid:300/500/800 都 200,1000 与 2000 都
    /// **404** —— 所以 `qqCoverMaxEdge = 800` 是这个图床的天花板,不是随手挑的数。不带
    /// Referer 也照给(实测 200),而 App 这边发图片请求没有 Referer,所以能直接用。
    ///
    /// ③ **Apple**:thumb 管线要多大给多大(实测同一张图 600/1000/1200/2000/
    ...
    private nonisolated static func appleUpscaledCoverURL(_ url: URL) -> URL? {
        ...
        // `600x600bb.jpg`(清点)。别的变体(`-999`、`sr`、裁切后缀)没实测过,
        ...
        // 后者对带查询串的 URL 行为没实测过。提醒 .backwards 不能跟 .regularExpression
    ...
    /// 从「缓存 key 到 封面 URL」建出忽略专辑的封面索引。纯函数,selftest 直接覆盖。
    ///
    ...
    /// MusicBrainz 缓存 + 共享歌曲 id)与「英文歌名 到 中文歌名」(EnrichTitleAliases.derive,证据 = 同歌曲
    ...
    // reload 并命中新缓存。 不能拿文件 mtime 当触发键:stale 返回窗口里 mtime 已变而
    ...
    public static func refreshIfNeeded() {
        ...
        if cachedEntries == nil && !releasedUnderMemoryPressure {
            decodeSynchronously()
        } else {
    ...
    /// 上一次清空是不是内存压力让出的。
    ///
    /// 冷启动那次必须同步解(没有缓存就什么都查不到,而那一刻屏幕上还没有东西在动);
    /// 压力让出不一样 —— 它发生在**播放中**,同步解就是当场冻住主线程。文件已经涨到
    /// 上百 MB,`Data(contentsOf:)` 加 `JSONDecoder` 实测 0.5 秒起步,而压力事件在内存
    /// 紧张的机器上并不罕见,于是表现为"播着播着毫无征兆地卡死半秒"。
    private static var releasedUnderMemoryPressure = false
    ...
    /// 原样灌回,极端时序(清空到冷路径同步解到新版到更旧的在飞结果后到)还会把内容
    /// 倒退一版(对抗核实抓出的竞态)。
    public static func installMemoryPressureRelief() {
        ...
        source.setEventHandler {
            MainActor.assumeIsolated {
                decodeGeneration += 1
                inFlightGeneration = nil
                releasedUnderMemoryPressure = true
                cachedMTime = nil
                cachedEntries = nil
                cachedCoverIndex = nil
                cachedLooseIndex = nil
