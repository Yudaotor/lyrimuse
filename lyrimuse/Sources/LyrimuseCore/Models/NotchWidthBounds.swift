...
public enum NotchWidthBounds {
    ...
    ///    252 / 展开 482 到 单耳 26.5 到 141.5,同一个居中公式推出来是 58.25pt),那就不是
    ...
    /// 负 padding 把音浪推出卡片。
    ///
    /// 居中**不能**图省事把 alignment 换成 `.center`:那是居中于 `earWidth`,会偏**内**
    /// `cardPadding / 2`,比原来错得更多。要补的是两侧边距之差,不是重新选一个对齐方式。
    /// 实测(截图逐列亮度,258pt 卡片 / 179pt 刘海):音浪落在 839.0~854.0(宽 15.0,与彼时的
    /// `EqualizerBars.width` 逐位吻合),而耳朵在屏幕上可见的那一段是 824.5~864.0(刘海右沿 到
    /// 卡片右沿)、中心 844.25 —— 音浪中心 846.5,偏外 2.25pt;两侧留白 14.5 / 10.0。这组数
    /// 是在音浪宽 15 时量的,现在条宽对齐了整物理像素、音浪宽是 16,同一组几何推入 1.75pt ——
    /// 公式没变,变的是代进去的宽度。成因不是
    ...
    /// 的话 `max(420, 360) = 420` —— hover 时一个像素都不多长,观感跟改动前逐字相同。提醒
