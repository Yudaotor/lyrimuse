...
/// 核心契约(尤其"三样齐必须仍然等于 76"这条),值得被断言钉住,所以把**纯
...
public enum NotchExpandedMetrics {
    ...
    ///
    /// **不再是"恒有"**(之前是):这排键从右耳搬进展开卡以来
    /// 一直无条件显示,后来加了"控制键是否展示"这个开关,于是它从 `height(...)`
    ...
    ///
    /// 头部占用 `trackInfoHeight + trackInfoTopSpacing + trackInfoSpacing`——**两份**
    /// 间距,不是一份,而且 **两份不再同值**:
    /// 一份贴在上边(`trackInfoTopSpacing`,离 topRow;现象是"标题首行贴到上面
    ...
    /// 头部跟**上面** topRow 之间的间距。
    ///
    /// 从跟 `trackInfoSpacing` 共用一个值(4)拆出来单独放大到 12——
    ...
    /// 头部右侧那排「快捷操作」按钮的高度(用户圈出头部右边那块空地:「塞进一些
    ...
    /// 后来又要在展开态里单独加一个显示封面,于是重新给头部配上**自己**的一枚,
    /// 这次没有位置四选一——固定贴文字块左边,所以高度算术只需要"封面和文字块谁更高"的
    /// `max`,不需要"并排/堆叠"两种模式的复杂度。
    ///
    /// 每一项的高度只看"这个开关开没开",不看"这首歌这个字段是不是空字符串/有没有
    /// 封面数据"——跟 `height(hasLyricPreview:hasScrubber:)` 那条"曲目级信号,不是此刻
    /// 有没有内容"的纪律同源:开关是稳定的会话级配置,按它留出的高度不该因为某一首歌
    /// 恰好没有专辑名/封面就抽一下。
    ///
    /// `showsActions`:头部右侧那排快捷操作按钮。跟封面一样是**并排**的第三块,
    ...
    /// 顶一下。
    ///
    /// 40 到 72 到 76:展开区从「预览 + 进度条」加了一排完整三键(设计评审把
    ...
    ///
    /// 之前这是一个写死常量,因为两个输入(`hasLyricPreview`/`hasScrubber`)
    ...
    ///   - `hasControlsPossible`(= `notchExpandedShowsControls` 这个开关):
