...
///
/// 为什么要有这个判据:原来只按**页内出现次数变多**判作废,而连播同一首歌时最近记录
/// 那一页很快被它占满——新的挤进来、旧的挤出去,页内次数**不再增长**,于是缓存的总
/// 次数永久冻结,而真实次数一路往上爬(简繁被折成同一实体时尤其明显)。
///
...
public enum PlayCountRecency {
    ...
    /// 判据④:不看"页内次数",只问"距离上次真验证过去了多久"——超过
    /// `maxAge` 就无条件判定过期,不管页内出没出现矛盾。
    ///
    /// 补的是判据③的一个盲点:`contradicted` 的第一道闸是 `onPage > cachedTotal`,只能
    /// 抓"缓存明显偏小"的情况(比如缓存冻结在 1、服务端真实是 31 这种量级差)。这首歌
    /// 很久没被主动播放/浏览到,①②(依赖上一轮内存基线)从来没机会比对,这次它只是又
    /// 被听了一次重新出现在页面上,`onPage=1` 恰好没有超过冻住的旧值 `1`,`contradicted`
    /// 直接判"没问题"、永远不会触发重新验证。这类"好久没被翻到、这次只是随手又听一次"
    /// 的老歌都会踩中同一个盲区,不是罕见的边界情况。
    ///
    /// - Parameters:
    ...
    /// 实测(《Controversy》):换歌那一刻取到 16(显示 17),同一时刻历史行经
    ...
    ///
    /// 之前这里无条件 +1,代价是**第一次听的歌必然多算一次**:正是这次 scrobble
    /// 让它第一次出现在最近记录里,新 key 没有 playCountVerifiedAt、`resolvePlayCounts` 对它
    /// 无条件取数,于是必定撞在"已入账、还在播"这个窗口里 —— 用户实测
    /// (《Cow-girl moderne》,第一次听):06:02:06 开播、约 06:03:30 越过 scrobble 门槛、
    /// 06:03:45 取到 userplaycount=1,徽标当场从 1 跳到 2。老歌同理,只是 N到N+1 不显眼。
    ...
    /// 当前这次播放是不是**已经**被 Last.fm 记进 userplaycount 了。
    ///
    ...
    /// 同一个容差 —— 那边就用它顶替过这个多算的数,只是没下沉到这里,于是
    /// 歌词窗口的徽标一直吃着裸值。
    ///
