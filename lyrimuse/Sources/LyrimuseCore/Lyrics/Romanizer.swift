...
public enum ChineseVariant: String, CaseIterable, Sendable {
    ...
    /// 只对**中文**歌词生效,日文歌一律原样返回。
    ///
    /// 日文汉字里有大量新字体(学/国/条…),简繁转换会把它们一起改掉(学到學、国到國),
    /// 那不是"转成繁体",是把日文写坏。判据用"这段文字里有没有假名"——有假名就是日文,
    /// 中文歌不会有假名。
    ///
    /// 用 ICU 的 Simplified-Traditional。实测它不是无脑逐字:头发到頭髮、干净到乾淨、
    /// 后来到後來 都对,「只有你」也没被误转成「隻」;日文/拉丁字符完全不动。
    ///
    ...
    ///
    /// 抽成单独一个判据,是为了让**所有**依赖"这段文字算不算中文"的地方共用
    /// 同一份逻辑,不可能各自漂开:
    ...
    ///     据它决定显不显示「简繁转换」)。
    ///
    /// 判据是"有汉字、且没有假名",**不是**"整首歌是不是中文歌"。这两者不等价,而且
    /// 用后者会出真 bug:
...
public enum Romanizer {
    ...
    ///                    到 形态分析: "kayou hi no asa ha"
    ///     受話器を取った君 到 Any-Latin: "shòu huà qìwo qǔtta jūn"
    ///                    到 形态分析: "juwa ki wo totta kimi"
    ///
    ...
    /// 留着 到 あしたの,再整体音译成罗马字。纯假名走 ICU 的 toLatin 是确定的、无歧义的
    /// (汉字才有拼音那个坑,见 romanize 的注释),所以这一步安全。
    /// `unitsHint`:调用方已物化好的整行 UTF-16 数组。japaneseSegments 对一行的每个 token
    /// 都要调进来,原来每个 token 各自 `Array(text.utf16)` 重新物化整行(性能
    ...
    /// 的整行读音和逐词分组共用**同一次** CFStringTokenizer,不再各分一遍(
    ...
    ///
    /// 两个条件是**或**的关系,不是"整首歌像日文"再判行:
    ///   · 行内有假名 到 这一行确证是日文,不管整首歌是什么(中文歌里引用的日文行);
    ///   · 纯汉字行 到 中日读音歧义,只有这种行才看整首歌的标记。
    ...
    /// 且各自的读音都跟自己那段原文一模一样。
    ///
    /// 只作用在**整行拼接**这一步,不动 `segments` 本身 —— 逐词分组(`buildWordGroups`)
    ...
    /// "나는 너를 사랑해"(3 词)到 ICU 给的 "naneun neoleul salanghae" 恰好也是 3 个空格
    /// 分隔的词、顺序不变;而单字块内部(比如"안녕"到"annyeong")ICU 不插空格,所以这里
    ...
    // MARK: - 行内中日无缝拼接
    //
    // 这类歌词的梗是日文词直接焊进中文句子、中间往往连空格都没有("三更半夜 さびし的我"
    ...
    //   ·"さびし的我"里的"的我"右边是行尾 到 独立 到 改判中文(de wo)。
    //   ·"4時半です"里的"時半"左边挨着数字"4"、右边挨着假名"です",两侧都不是硬边界
    //     到 不独立,继续当日语处理(ji han)—— 这条时间表达本来就该保留日语读音,
    //     不能被这次修法误伤。
    ...
    ///
    /// 这是"这一行里的中文部分该不该出读音"的唯一表达方式。行的语言标签由
    ...
    /// 赫本式的写法是把后一个辅音双写:取った 到 "to~tsu" + "ta" 到 "totta"。
    ...
    // 实测排查坐实的真实 bug:汉字本身是中文/日文共用的文字系统,
    ...
    // 信号)也被这套兜底逻辑摆上了一行拼音,而现实世界的歌词 App(几个主流播放器自带的
    // 歌词面和几个同类工具都看过)都不会这么做。
    //
...
/// 听中文歌却完全不需要拼音(对中文读者是纯噪声,就是因为这个才把中文歌的
...
public struct RomanizationScripts: OptionSet, Sendable, Codable {
    ...
    /// 粤语(粤拼/Jyutping)。加——此前粤拼罗马音只在服务端无条件生成,
    /// 客户端完全没有开关能控制显不显示。
...
extension Romanizer {
    ...
    // MARK: - 整首歌 vs 一行:两个不同粒度的判断
    //
    // 现象是「怎么中文也注音了」:《这样吧》(群星/动力火车翻唱)是一首中文歌,
    ...
    //
    // 现在分成两层:
    //   · 一行里有假名/谚文 到 按这一行自己的文字算(确证,不看整首歌);
