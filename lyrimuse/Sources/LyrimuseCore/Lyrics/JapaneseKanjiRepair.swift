...
/// 本机缓存实测(69 首日文歌 / 3334 行含假名的正文):66 行受害、涉及 24 首,重灾区
/// 神山羊《journey》《青い棘》31 行;其余多是署名行「作词」「编曲」(引擎随后就过滤掉)和 `[ti:]`
/// 里源加的中文译名。
///
/// ═══ 不维护任何表,全部是通用规则(「不允许维护人工表,必须是通用规则」)═══
///
...
/// 这一条不能换成「是不是简体字」:「国」既是简体也是日文新字体,按后者判就会把它改成「國」。
///   2. **不是的话,它的繁体是什么** —— ICU `Simplified-Traditional` 单字转换。转出来的字再过一次
///      规则 1,能编码才换(饮到飲、闻到聞、优到優),不能编码就原样留着(「步」ICU 不动 到 留;「你」不是
///      简体 到 留)。
///
...
///
/// 已知边界(不引表就到此为止,刻意接受):
///   - 繁体与日文新字体不同的字落在**旧字体**上:颜到顏(现行写法 顔)、丝到絲(糸)。旧字体是 JIS X 0208
///     里真实存在、日本读者认得的字形,比留着一个日本根本不存在的简体字好得多;再往新字体走需要一张
///     旧到新对照表,而那正是用户不要的东西。本机实测 31 种被修的字里只有这 2 种落在旧字体上。
///   - 简体与繁体同形、日文却另有写法的字(步 / 歩)无从判断是不是被写坏的,原样留着(实测 6 处)。
///   - 含假名的行里夹着的中文括注(`[ti:クランベリーとパンケーキ (蔓越莓和煎饼)]`)会跟着变繁(饼到餅);
///     那是标签/抬头行,引擎不显示它们,接受。
