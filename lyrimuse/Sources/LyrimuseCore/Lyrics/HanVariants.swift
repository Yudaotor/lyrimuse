...
/// 繁简转换之外的**异体字规范化**。
///
/// 为什么需要它:表现是「明明开了简体,歌词里还是看到繁体」,实例是周杰伦
/// 《开不了口 (Live)》——ICU 把那首歌 37 种字符全转对了(沒到没、煩到烦、開到开、讓到让…),
...
/// ═══ 表从哪来(换掉了原来的手工表)═══
///
/// 原来这里是一张**手写的 6 条对照表**,每个字都要人肉核过词频。
...
///
/// 换完之后原来那 6 个字里有 4 个是**推出来的**(妳到你 / 牠到它 / 濛到蒙 / 痺到痹),只有
/// 祂到他、痲到麻 仍然要补丁 —— 那两个字上游真的没有可用关系,理由写在补丁文件里。
///
/// 只在**转简体**方向生效,绝不做反向。简体只有「你」,转繁体时无从判断该写「你」还是
/// 「妳」——那要猜被称呼者的性别,猜错就是改写歌词。同理「他/她/它」也不反推。
///
/// 原来那张手工表的第三条收录标准是"在中文歌词语境里无歧义",据此**刻意不收**「祇」
/// (神祇 vs 只)、「乾」(乾坤/乾隆,ICU 已按上下文正确保留)和粤语字「嘅咁哋冇」。换成推导
/// 之后这一条不是靠人记着了:那些字**本身就在大陆通用字集(G)里**,规则第一步就把它们排除
/// 在外(逐个核过产物,15 个候选字一个都没进表)。真出现误折,往补丁文件里加一行
/// `字<TAB>-<TAB>理由` 否决即可,不用改代码。
public enum HanVariants {
    /// 异体 到 大陆规范字。数据在生成的 `HanVariantsTable` 里,见上面的说明。
