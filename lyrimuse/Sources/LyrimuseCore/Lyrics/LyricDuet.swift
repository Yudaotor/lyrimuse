...
//
// 逐字时间轴里剥前缀**不能按词删**:标记可能只占某个词的前半截(`男：周`),那个词扛着
...
public enum LyricDuet {
    ...
    /// 「男 到 女 到 男声」会排成 左 到 右 到 左(第 3 位 %2==0),看着对,纯属运气;
    /// 「男 到 男声 到 女」就排成 左 到 右 到 左,同一个人被拆到两边。
    ...
    /// 两件事必须分开:合在一个词表里判断,想支持人名就只能把词表整个放开,一放开就把
    /// 「词：方文山」这类署名行也吞进来。
    ///
    ...
    ///
    /// 为什么非补不可(实测抓到的真误判):**串烧 Live 里署名行会重复出现** ——
    ...
    ///
    /// 这道形状闸必须够狠,因为下面那道整份闸是**全份一起过**的:一旦某个真演唱者把
    ...
    /// 这份歌词里,哪些行首标签是演唱者。
    ///
    /// 必须拿**署名行过滤之前**的原始行来算,而且算出来的结果要回头喂给署名行过滤当
    ...
    public static func speakers(in lineTexts: [String]) -> Set<String> {
        ...
        // 这里**不加**"至少两个身份"的门槛。认出是说话人标记(到 剥前缀、丢掉独占行、
    ...
    /// 只认出一个身份时整首退回"没有对唱信息":
    /// - 只有「合」的歌:若判成"第一个合唱标记之后全程居中",而悬浮窗的 nil 兜底本来就是
    ///   居中 —— 等于白做,还顺带把歌词窗口从左对齐改成了居中,凭空动了排版。
    /// - 只有一位歌手的歌(TTML 规范要求**单人歌也要标** `ttm:agent="v1"`,所以这是常态):
    ///   若判成全程 `.leading`,歌词窗口兜底恰好也是 leading 看不出来,但悬浮窗兜底是
    ...
    /// 逐字版的 plan。
    ///
    /// 判定必须在**整行拼起来**的文本上做,不能只看第一个词 —— 这正是重写前那个让 11 首歌
