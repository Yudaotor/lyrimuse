...
// 菜单栏歌词的跑马灯取窗算法(加,点子来自 FlowX/Kadxy/FlowX——它的菜单栏
// 歌词超宽时会横向滚动,而不是像我们原来那样直接截断成"前 N 个字…")。
//
// 历史(~ 08-16):这里原来只有 scrollOffset 一个函数,因为那时菜单栏还是
...
//
// 菜单栏改成自建 NSStatusItem 之后,滚动交给 Core Animation 在渲染层插值,
...
public enum MenuBarMarquee {
    ...
    // 按真实文字宽度做 —— 不能按字符数固定取窗,字符宽度差着一倍(中文 128pt vs
    // 英文 65pt 同为 10 个字),菜单栏项会跟着内容忽宽忽窄。

    // ---- 像素级滚动 ----
    //
    // 上面那个按字符取窗的版本实测"显得很卡顿",原因不是性能,是两件
    // 事叠在一起:
    ...
    // 把菜单栏从 MenuBarExtra 换成原生 NSStatusItem 之后,滚动交给 Core Animation
    ...
    // ---- 配速:让这一句在换到下一句**之前**滚完 ----
    //
    // 现象是:"一句歌词比较长的时候滚动太慢,还没滚到底就切到下一行了"。
    ...
    //      时间不够时先让路。
    // ( 句尾价值**不能**判成 0、让滚动恰好在换句那一刻走完 ——
    //      那样"一到最后一个字就马上到下一句了",末尾几个字根本
    //      来不及读。句尾按 tailReadSeconds 预留一段读完时间,滚动相应加速。)
    //   4. **开唱之前不滚**。单行展示面会在上一句唱完、本句还没开唱
    ...
    //      那段时间从行程里去掉了,不会重蹈那个"按偏大的 dwell 配速、
    //      长句只滚出开头一小截"的坑。
    ...
    /// 句尾那一停:滚完之后至少让末尾**在换句之前**可见这么久(现象是
    ...
    // ---- 逐字染色:填色边界的声明式运动(加,"像酷狗菜单栏歌词") ----
    //
    // 跟上面滚动完全同一套哲学:把"填色边界此刻在哪个 x"编成一条 CA 关键帧,装好之后
    // 主线程一帧都不碰 —— 逐帧驱动那条路(20/30Hz 改遮罩宽度)正是被实测
    ...
    public static func pacing(
        ...
        // 句尾也预留一段读完时间:原来 travel 直接吃满 dwell - head,滚动
    ...
    // 甩成残影、开唱前别动"这些窟窿。可缓存里 93% 的歌带逐字时间轴(数的:3522 首
    ...
    // "仿照酷狗菜单栏歌词:可以选择在最左侧或者最右侧展示软件图标,
    // 图标上会逐渐染色代表当前歌曲进度条"。
    //
    // 跟上面那套逐字染色**不是**一回事,别合并:
    ...
    //
    // 喂进来的位置**不含歌词时间轴偏移**:那个偏移把歌词往前/往后挪,歌本身放到哪儿
