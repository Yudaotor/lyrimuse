...
/// # 规则
///
/// 1. 总宽连「每栏都给到下限」都不够(窗口被拖到极窄)到 平均分。这时谁也读不全,
///    平均分至少还是可预测的。
/// 2. 全部想要的加起来放得下 到 每栏先拿满,**多出来的平均分**给三栏(而不是全塞给最长
///    的那栏):输入框留一点余量,继续往里打字时不会一个字就顶到边。
/// 3. 放不下 到 按「想要多少」的比例分,但谁都不能低于下限;被下限托住的先钉死,剩下的
