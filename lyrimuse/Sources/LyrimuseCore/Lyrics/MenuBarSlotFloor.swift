...
///     之间跳的歌里几乎每一次回缩都越过容差、照缩,实测回到每 2.8s 一次重建 —— 用户 2026-09-17
///     在"不空"和"不晃"之间选了**不空**(见 MenuBarSlotPolicy.shortLineDriftCeiling 的代价那段)。
///   · 暂停 / 间隙 / 假暂停期间目标宽度 ≤ 已达最大值,`needsRebuild` 恒假 → 零重建。
///
/// ⚠️ **2026-09-16 当天就被实际使用打回来一次,加了收缩容差。** 第一版是"同曲内一律不缩",
/// 上线当晚用户截图问「为什么占了那么大的空间啊?」:双排模式下槽宽取 `max(当前句, 下一句)`
/// (2026-09-06 的设计),所以**下一句长**的时候当前句再短也占满;一旦某一句把槽顶到「最大宽度」
