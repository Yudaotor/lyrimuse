...
/// 到点触发,必然落在句中),其中 **11 次是加宽**。这个 36% 跟加 `skipsShrink`
...
public enum MenuBarSlotPolicy {
    ...
    /// 判据于是退化成**永不改槽宽**。实测(英文快歌,逐句 dwell 0.9~2.5s):
    ///
    ...
    ///
    /// ⚠️ **从 30 提到 120,并跟 `MenuBarSlotFloor.shrinkTolerance` 解绑。**
    /// 这条原来写着「上限取 shrinkTolerance,不另给一个数……两个数必须相等,selftest 有哨兵
    /// 钉住」。绑定的理由是"地板判了该缩、执行层却跳过,那容差就形同虚设"。**推翻它的是
    /// 闪烁的实测代价**:
    ///
    /// 现象是「每次重建菜单栏都闪一下,时间还有点长」。逐帧截图(91ms 一帧)量到一次重建的
    ...
    /// - ⚠️ 时长判据**有偏差上限**:短命行只在偏差小于 `shortLineDriftCeiling`
    ...
    /// 「这一刻菜单栏该显示什么文字」的兜底:有歌词句就显示歌词句(包括间奏的 ♪);
