...
///
/// 抖动就出在这条推迟上,实测抓到的现行(菜单栏 debug 日志):
///
/// ```
...
/// 在换行那一刻当场落地,不会再拖到句中。
///
/// **两个方向都要受这条时长判据约束,不能只挡收缩**:只判收缩会让加宽完全豁免,一句
/// 只活 1.5s 的短句照样能靠加宽烧掉重建配额,下一句(往往活很久)的几何被推到句中才落地,
/// 表现是「换行时先按上一句的长度渲染,然后同一行又变一次」:
///
/// ```
...
/// ```
///
/// 复采 40 分钟 / 39 次重建:36%(14 次)的相邻间隔精确落在 3.00–3.25s(节流定时器自己
/// 到点触发,必然落在句中),其中 11 次是加宽。
///
/// 两个方向都过时长判据之后,不变式才真的成立。**代价说清楚**:一句活不过静默窗、又装不下的
/// 短句,会在偏窄的槽里滚完它那 ≤3 秒,而不是加宽。这是刻意的取舍 —— 它滚 3 秒,换下一句
/// (往往活十几秒)不在用户眼前跳一下。
///
/// 仍然**只在两个歌词槽之间**生效(判断在调用点的 `lyricSlotClasses`)。图标与歌词那次重建
...
public enum MenuBarSlotPolicy {
    /// 值得为它重建一次的最小收缩量。
    ///
    /// 上面那条规则上线后复采日志,抓到另一种同源的浪费:`text(250.749512) 到 text(250.438477)`
    ...
    /// 值得为它重建一次的最小**加宽**量。
    ///
    /// 这条**推翻了**本文件原来那句「只对收缩设死区:加宽方向哪怕只差几 pt 也得给 —— 差一点点
    ...
    ///
    /// 实测原样:`text(222.909180) 到 fixed(223.500000)`,**0.59pt** 的加宽也在重建;
    ...
    /// 短命行"不值得为它改槽宽"的**偏差上限**。
    ///
    /// 这条**收窄了**下面 `skipsResize` 里那个无条件的时长判据:一首句子**普遍**短于
    /// 静默窗的歌,"短句是少数、下一句活得久"这个假设整首不成立,判据会退化成**永不改槽宽**。
    /// 实测(英文快歌,逐句 dwell 0.9~2.5s):
    ///
    ...
    ///
    /// 18.5 秒里 7 句都在一个不属于自己的槽里左对齐画,最短那句右边空 76pt。另一轮实测更
    /// 极端:129 秒不变,期间目标在 68~207pt 之间漂。
    ///
    /// **120,跟 `MenuBarSlotFloor.shrinkTolerance` 解绑**(不能取同一个数):由一次重建
    /// 的内容空窗有多长定的。逐帧截图(91ms 一帧)量到一次重建的
    /// 现行:`19.410` 前景宽 190 到 `19.450` rebuild 到 `19.473`(24ms)歌词没了、邻居在挪 到
    /// `19.700`(250ms)新内容才出现。**一次重建 = 约 250ms 的内容空窗**。而这 250ms 里
    /// 我们自己只占 4~12ms(removeCreate 3.4~10.4ms + render 0.5~1.4ms,实测 5 次),
    /// **其余约 240ms 全是系统 MenuBarAgent 在重排** —— 优化我们这侧对闪烁毫无帮助,
    /// 唯一能减少闪烁总量的办法就是**少重建**。
    ///
    /// 拿真实抓到的 37 拍决策序列(92 秒)离线模拟不同上限:
    ///
    ///     上限     重建              闪烁占比   留白(最大/均)   欠宽(最大/均)
    ///      30    17 次(每 5.4s)     4.6%      124 / 58 pt    109 / 39 pt
    ///      80    10 次(每 9.2s)     2.7%      111 / 58 pt    109 / 38 pt
    ///     120     6 次(每 15.4s)    1.6%      111 / 62 pt    109 / 42 pt
    ///
    /// **闪烁砍掉 65%,而留白和欠宽只动了 ±4pt。** 反直觉但有道理:留白/欠宽的根源是句长
    /// 本身在 82~208pt 之间大幅波动,不是"跳过了多少次改动" —— 跳过更多只是让槽停在某个值
    /// 更久,gap 的分布几乎不变。120 之后再大也没收益(剩下那 6 次是 dwell ≥ 静默窗的长句
    /// 触发的,本来就不归这个闸管)。
    ///
    /// **那条绑定的论证错在哪**:两个常量回答的根本不是同一个问题。地板的容差决定
    /// **目标宽度**该不该缩(管留白);这条上限决定**一次改动值不值一次重建**(管闪烁)。
    /// 目标缩了、执行暂缓,等到真正值得重建时一起落地 —— 这是分工,不是互相否决。
    /// 上面那张表就是证据:上限提到 120,留白一点没变差。
    ///
    /// **代价**:短命行欠宽最多到 120pt 时才会为它重建,那之前它在偏窄的槽里滚完自己那
    /// ≤3 秒(欠多少就滚多少,见 `MenuBarMarquee.pacing` 的 maxOffset)。实测这一档的
    ...
    /// - 时长判据**有偏差上限**:短命行只在偏差小于 `shortLineDriftCeiling`
    ///   时才跳过。没有这条上限,句子普遍短于静默窗的歌整首都不改槽宽。
    /// - 时长判据用"小于"而不是"小于等于":恰好等于静默窗的行留给"照旧改"那一侧,因为它
    ///   换到下一行时静默窗刚好走完,不会拖累下一行。
    /// - **长度完全没变时返回 false**(不是 true)。那种情况是"形态翻转、槽宽不动"
    ///   (`text(223.5) 与 fixed(223.5)`),该交给调用点的 `needsRebuild` 判 —— 那条会走
    ...
    /// 占位态的槽宽该给多少。
    ///
    /// 占位文字(「♪ 歌名」兜底、或前奏/间奏那三颗点)的宽度跟**即将到来的那一句**毫无关系,所以从占位
    /// 切到歌词时槽宽必然要改一次。改在哪一侧是唯一的选择:
    ///
    /// - 改在歌词出现**之后** = 那一句先按占位槽画一版、槽宽跟上再画一版 到 **同一句变两次**
    ///   (表现就是这个,实测原样
    ///   `icon(38.5) -> text(112.423461)` 紧跟 `text(112.42) -> fixed(223.500000)`);
    /// - 改在歌词出现**之前** = 占位期间就把槽撑到下一句要的宽度 到 歌词一出现几何已经对了,
    ///   只渲染一次。这个函数选的是这一侧。
    ///
    /// 这是本文件那条不变式管不到的一半:`skipsResize` 只在两个**歌词槽**之间生效,
    /// 图标与占位与歌词这几跳不受它管。
    ///
    /// **代价**:占位期间这一项比它自己需要的宽,「♪ 歌名」居中浮在按下一句尺寸开出来的
    /// 槽里。跟自适应"省空间"的初衷有点冲突,换来的是邻居少挪一次(icon 到 宽槽,而不是
    /// icon 到 窄槽 到 宽槽)+ 歌词只渲染一次。选的是后者。
    ///
    ...
    /// 「这一刻菜单栏该显示什么文字」的兜底:有歌词句就显示歌词句(包括前奏/间奏那个三点记号串);
    ...
    /// (`♪≡ ♪ 歌名`)纯属重复。前奏/间奏的占位不受这里管辖 —— 它是三颗呼吸圆点,以一个
    /// 记号串(`MenuBarMarqueeRenderer.gapDotsToken`)的身份走 `lyricText` 那条路
    /// (上面第二行就返回了),而且它必须是非空文本才能把槽留住,见 06 章已知坑 11。
    ///
    /// **这里这个音符跟那三颗点是两件事**,别一起换:这个说的是"这首歌没词",
    /// 那个说的是"有词、只是此刻在间奏";写成「••• 歌名」读起来会变成在报间奏。
