...
//
// 对照关系(系统设置 到 我们):
//   Apple 账户块(头像 + 名字 + 「Apple 账户」)      到 Last.fm 身份区(头像 + 用户名 + 「Last.fm 账户」)
//   「有软件更新可用 ①」                            到 Sparkle 查到新版本时的同名一行,点了进「软件更新」页
//   分组之间只留空白、没有小标题                     到 「核心设置」「账号」两个 Section 标题撤掉
//   行尾红色数字徽标                                 到 「播放器」健康警告从橙色三角改成红色计数
//   彩色圆角方块图标带一点上亮下暗的渐变              到 IconBadge 统一加渐变与高光(见 SettingsView.swift)
