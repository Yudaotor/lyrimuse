...
// 设置窗口的一套卡片式组件。参考 Dropover 的设置界面重做——它跟
...
// 各自显式接管。
//
// 分隔线为什么要调用方自己插(CardDivider),不由 SettingsCard 自动加:SwiftUI 的
...
// 模糊+渐变糊出来通常比干净的纯色更糟。
//
// 实测核实过这台机器(macOS 27.0 / SDK 26.4.1 / Swift 6.3.1)上这几个 API 在
...
extension View {
    ...
    ///
    /// 玻璃之外**必须**再补一条发丝描边。拿两页的实机截图逐像素采样量过:
    ...
    /// 设置侧栏搜索框的材质("像系统设置那样的液态玻璃"):macOS 26+ 用
    ...
    func clearGlassCapsule(rim: Color) -> some View {
        if #available(macOS 26.0, *) {
            ...
            // 的内容透过来 —— Apple Music 那个音量胶囊正是这种场景。用户连着
...
//
// 抖动的机制:展开一个 DisclosureGroup 让内容超过一屏,滚动条出现,
// **占掉约 15pt 布局宽度**,于是这一列卡片可用的宽度变窄、居中点左移 —— 整页看着往左挤
// 了一下。收起来时反向再抖一次。
//
// 别被 `defaults read -g AppleShowScrollBars` 读不到值骗了:
...
//
// 这个"量"必须**不改变视图层级**。把整个
// ScrollView 包进 GeometryReader(GeometryReader 在外、ScrollView 在内)确实能治抖动,
...
//
// 这里刻意**不用** .safeAreaInset。那个修饰符是"悬浮"语义:被它修饰的 ScrollView 仍然
...
//
// 它跟卡片里的设置行**必须**长得不一样:同样的字号/图标列/行高会让标题跟它下面的
// 设置行在视觉上完全平级,读者只能靠一条分隔线去猜谁是谁的标题。
//
// 做法是把它往"分组标签"的方向拉开(macOS 系统设置里的分组标题也是这个路子):字号压到 12、
...
struct SettingsCardHeader<Trailing: View>: View {
    let title: String
    // 卡标题下面那一句适用范围说明。 **零调用点**:唯一用过它的是那张「自动隐藏」
    ...
    var subtitle: String?
    var help: String?
    // 标题行右侧的附加操作(「歌词来源」卡的「全部测试」按钮补的)——跟
...
struct SettingsRow<Trailing: View>: View {
    ...
    // 以及歌词来源前面那个代表来源的彩色圆点。
    var iconTint: Color?
    // 用真实 App 图标代替 SF Symbol(设置页"已信任的其它播放器"列表用得到:
    ...
    var body: some View {
        HStack(alignment: .top, spacing: SettingsRowMetrics.iconTextSpacing) {
            ...
            trailing()
                ...
                .labelsHidden()
                // 必须显式指定 .switch:macOS 上 Toggle 的默认样式是**复选框**,只有放在
                // Form/List 里才会自动变成右侧那个开关。这套卡片是自己用 ScrollView+VStack
                // 搭的、不在 Form 里,不写这一句所有开关都会画成 ☑ 复选框(
...
///
/// 单靠 `role: .destructive` 在 macOS 上**不会变红**:那个 role 只在菜单项和弹窗按钮里
...
struct SettingsSubRow<Trailing: View>: View {
    ...
    /// 撑成一堵墙(「宽度模式」那一行就是这么从副标题改过来的)。
    var help: String?
    @ViewBuilder let trailing: () -> Trailing

    // **不用竖线表达从属关系**:分行画的话,行与行之间夹着 `CardDivider()`,连不成
    // 一条连续线,只是一截截断开的短竖条,子行越多越明显。
    //
    // 只靠缩进表达层级:标题落在**主行标题那一列**,主行那一列左边是图标、子行是空的 ——
    // 这个空位本身就是层级信号,macOS 系统设置里的子项正是这么排的。标题左对齐,跟主行
    // "标题在左、控件在右"同一套结构,眼睛才好顺着同一条左边界往下扫。
    var body: some View {
        HStack(spacing: 10) {
            VStack(alignment: .leading, spacing: 2) {
        ...
        // 删竖线时从 `textLeadingInset - 12` 改成 `textLeadingInset`:那 12
...
struct SettingsRawRow<Content: View>: View {
    ...
    // 空洞,看起来就像漏画了(用户指的就是「账户 Token」那一行)。
    var icon: String?
    @ViewBuilder let content: () -> Content
...
struct SettingsPopoverShell<Content: View>: View {
    ...
    ///
    /// 只信这个测量值,不再信"ScrollView 收到高度不限的提议时按内容自身高度返回"
    ...
    /// 新增的行只能挤进原来那块地方、变成"多出一条滚动条"——对拍报的
...
/// **开着的**那几项标题列出来。
///
/// 为什么是共享的而不是各编辑台一份:它产出的是**用户看得见的文案**,而两个
...
/// 一条按数值分段的横向比例条(「歌词 到 管理」的歌词库统计首用)——把「总数由
/// 几个桶组成、各占多少」画成一根 6pt 高的分段条,读法同系统设置「通用 到 储存空间」那条。
///
...
/// 五个桶五种颜色就是「一片彩灯」(用户对统计数字染色否掉过的理由)。
///
...
///     结果。(实测记录:数值对拍时,这里的注释一度写反成"夹到 upperBound、
