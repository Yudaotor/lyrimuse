...
extension View {
    ...
    ///
    /// ⚠️ 玻璃之外**必须**再补一条发丝描边。拿两页的实机截图逐像素采样量过:
...
//
// 现象是的抖动就是这个:展开一个 DisclosureGroup 让内容超过一屏,滚动条出现,
...
struct SettingsCardHeader<Trailing: View>: View {
    let title: String
    // 卡标题下面那一句适用范围说明。⚠️ **零调用点**:唯一用过它的是那张「自动隐藏」
...
struct SettingsSubRow<Trailing: View>: View {
    ...
    // 重排。原来这一行是"Spacer + 标题 + 控件"整体右对齐、左边什么都没有,
    // 跟主行长得几乎一样,读起来像是并列的另一项 —— 现象是的就是"看不出附属关系"。
    ...
    // 顺带一提,标题从右对齐改回左对齐是一起做的:主行"标题在左、控件在右",
    ...
    var body: some View {
        ...
        // ⚠️ 删竖线时从 `textLeadingInset - 12` 改成 `textLeadingInset`:那 12
...
struct SettingsPopoverShell<Content: View>: View {
    ...
    /// (都该更窄)上面。之前灵动岛那几个浮层直接吃了这个默认值,「左耳」「右耳」
    /// 的内容自然宽只有 124pt(中文)/ 136pt(英文),却被硬撑到 380 —— 现象是
