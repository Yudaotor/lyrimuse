...
/// 「歌词来源」卡里那颗橙色锁点开后的说明。
///
/// ⚠️ 必须讲两件事,少一件用户就会卡在半路:①要授权「完全磁盘访问」;②**授权对已经在运行的
...
///
/// ⚠️ 这段说明走 popover 而不是 `.help` 的 tooltip:这颗锁只有 12pt,tooltip 要精确悬停在它上面
...
struct LocalCacheAccessHelp: View {
    ...
    ///
    /// ⚠️ 别按「重启要多久」定这个数 —— `requestRestart()` 返回时 launchd 才刚报出新 pid(1~3 秒),
        ...
        VStack(alignment: .leading, spacing: 9) {
            ...
            case .waiting:
                ...
                HStack(spacing: 6) {
                    ...
                    Text(L10n.t("正在重启后台服务，它要把歌词缓存重新读一遍，通常要一两分钟…"))
                        .fixedSize(horizontal: false, vertical: true)
                }
                .padding(.top, 2)
            case .stillDenied:
    ...
    ///
    /// ⚠️ 别在 `requestRestart()` 返回时就宣布完成:那只代表 launchd 收下了指令并报出新 pid,
