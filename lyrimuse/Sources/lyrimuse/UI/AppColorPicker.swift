...
/// 自制的轻量取色器(2026-09-17),取代系统 `ColorPicker` 弹出的 `NSColorPanel`。
///
/// 用户原话(截图指着系统面板的「蜡笔」tab):「这个地方是不是调用的是系统颜色选取？这样感觉
/// 里面太乱了，都不知道要选什么，功能太多了。我们是否可以考虑自己做一个呢？」——
/// `NSColorPanel` 是全系统共用的那一个(Preview / Keynote / Xcode 点开都是它),5 个 tab
/// (轮盘/滑块/调色板/图像/蜡笔)是 Apple 焊死的,没有公开 API 能只留其中几个、把其余藏起来,
/// 所以"不乱"只能自己做一个替代控件。用户拍板方向:「自己做一个轻量版本，只保留一些会用到的，
/// 用不到的就不要放进去」。
///
    ...
    var body: some View {
        Button {
            ...
            ColorSwatch(color: selection.wrappedValue, cornerRadius: 5)
                ...
                // ⚠️ 必须显式给一个矩形命中区(2026-09-17 用户报「点击不了了」实测坐实):
...
///
/// ⚠️ **刻意不垫棋盘格**(2026-09-17 用户截图指出「太丑了」,当场去掉)。第一版垫过
