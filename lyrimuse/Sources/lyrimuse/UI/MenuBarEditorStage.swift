...
/// 页顶的固定头部里、点不动。这里的做法是"挪位置 + 换外壳"——把它搬进可滚动内容区,外面套
...
/// 这次按"不要这一个那一个"的要求拆出来。分组按卡片解剖:「布局」= 这一格怎么占位、几行、旁边
...
struct MenuBarEditorStage: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            toolbar
            toolbarRow2
            // 宽度调整条浮**在预览框里面**(「把这个配置项也给我
            // 和之前两个页面一样加到这个预览框里面去进行调整」)。前两版都不是他要的:
            // 宽度调整条**要长在舞台里面**,不能摆在预览下面平铺一行、也不能只做 VStack 里的
            // 兄弟节点 —— MenuBarWidthRow 用的是 SettingsRow 的外壳,在舞台那块近白的
            ...
            // 壁纸底扩展到包住整块舞台(含通道里的调整条),caption 挪到舞台
    ...
    private var toolbar: some View {
        HStack(spacing: 8) {
            ...
            // 省下的正是第三个入口「粗细」要用的预算(账见 toolbar 头注)。
            toolbarButton(
            ...
            // 「字体」(用户指着「配色」右边的空位:「把粗细加在这里」;当晚又加字号:
            // 「帮我再加上字体大小吧,也归类到字体这个下拉选项里面去,替换现在粗细这个大标题」)。
            // 「粗细」**不放**在「配色」浮层头一行,要跟另外两个同款、做成独立入口。图标跟
            ...
            // 「简约克制」专项把三个编辑台(悬浮 / 灵动岛 / 菜单栏)的这颗按钮统一成这四个字:
            // 此前这里念的是工具栏四个入口的名字(「恢复默认布局、配色、字体与行为」),
    ...
    /// 四选一不是开关,套不进"只列开着的";默认那一档(是「下一句」,此前是「不显示」)是多数人的状态,无条件报出来是恒定噪声
    ...
    /// "摘要复用现成 item 标题"的纪律。(标题随开关改名「卡拉OK效果」,宽度预算见
...
/// 「歌词旁的图标」行—— 在歌词那一格的最左或最右放一枚菜单栏图标,图标上的
/// 染色从下往上涨,表示当前这首歌的播放进度。"仿照酷狗菜单栏歌词"加的。
///
...
/// 「悬停显示播放控制」行—— 鼠标停在菜单栏歌词上时,歌词收掉、换成
/// 「上一曲 / 播放暂停 / 下一曲」三个键。仿酷狗菜单栏。
///
/// 这是个**行为**开关。~09-07 它寄住在「布局」浮层里(第一行工具栏的横向预算已经用尽,
/// 不值得为一个开关新开第四个入口;当时写着"真要再来第二个行为开关,那时候一起拆一个「行为」
/// 浮层出来")—— 第二个(「无歌词时显示歌名」)09-04 就来了,按"不要这一个那一个"
/// 的要求拆出了工具栏第二行的「行为」浮层(`MenuBarBehaviorPopover`),两项都搬了过去。
///
/// 在「重置 ▾」范围内("这部分所有配置都改为默认,除了宽度"),见
...
/// 「无歌词时显示歌名」行。跟上面「悬停显示播放控制」同住「行为」浮层
/// 与「全部设置」抽屉「行为」组(前两项都在「布局」里):它管的也是"这一格在什么状态下
/// 显示什么",不是样式。判据在 Core
/// (`MenuBarSlotPolicy.displayText`),help 里把三条边界(暂停仍缩回 / 广告不显示 / 歌词一到就换)说清。
/// 进「重置 ▾」——漏过两项被用户当场看出来,见 `MenuBarStyleDefaults` 头注。
...
///
/// **只排除两样**("这里的重置需要把这部分所有配置都改为默认,除了宽度"):
///  - `menuBarLyricsWidth`(最大宽度)—— 留;它也是唯一一项常驻在舞台上、不在
...
/// 「行为」组的行(从「布局」拆出来;浮层与抽屉同一份):悬停显示播放控制 / 无歌词时显示歌名。
...
///      (这条坑两个组件自己的文档注释都写着,「我的配色主题」内联命名行就是
///      同一个坑),表现正是对拍里"宽度模式"标题被压没、只剩一个"?"图标孤零零杵在
...
/// 「配色」浮层:只放三行颜色。「粗细」先放在这里头一行、几分钟后按
...
/// 「粗细」行:菜单栏歌词的字重。字体族 / 字号继续跟系统菜单栏 —— 06 章「字体跟随
    ...
    var body: some View {
        VStack(spacing: 0) {
            SettingsRow(
                ...
                // 本仓面向用户的术语统一是**卡拉OK**,英文侧是 "Karaoke fill" —— 三个展示面
                // (悬浮歌词 / 灵动岛 / 菜单栏)各一颗同名开关,别在某一面另起名字。
                //
...
struct MenuBarAllSettingsDrawer: View {
    ...
    /// 「恢复默认布局、配色、字体与行为」——抽屉里的兜底入口(补;随「行为」入口改名)。
    ///
