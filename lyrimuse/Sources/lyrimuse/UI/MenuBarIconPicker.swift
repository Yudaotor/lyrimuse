...
/// 搬出来,定型)。
///
/// ⚠️ **不带预览**,别再加回仿菜单栏那块编辑台(壁纸底 + 苹果标 + wifi/电池/时钟、
/// 上面摆所选那款的本体、律动开着就挂 `MenuBarLiveIconView` 真的动、下面 caption 常驻款名)。
/// 那套东西装机实拍、两帧 diff 都验过是真在动,但仍然不要:菜单栏就在屏幕顶上,选哪款
/// 抬头就看得见,不需要在设置页里再仿一条。所以这里只剩 12 格。
///
