...
///
/// ⚠️ 下面这段是「跟随封面」曾经住在这一组时的记录 —— **2026-09-16 它又搬回「文字」浮层**、
/// 内联成「文字颜色」那一行的一个取值(跟随封面 / 自定义颜色),这一组从此只剩选配色主题
/// 这一件事。留着这段是因为 2026-09-17 那次改动(见下面第二条 ⚠️)正是冲着它的前提去的。
///
...
///
/// ⚠️ **2026-09-17 起,跟随封面开着时这一行照常报主题名/「自定义」,不再报占位符「—」**。
///
/// 占位符是 2026-09-02 加的(用户原话:「当我跟随封面开着的时候,主题这边摘要和详情都指向一个
/// 占位符」),成立的前提是「跟随封面」跟「配色主题」同处这一组、是同一个问题的两个答案 ——
/// 一个模式占着,另一个就没在生效。**那个前提 2026-09-16 就没了**:跟随封面搬进「文字」浮层、
/// 变成「文字颜色」那一行的一个取值,跟主题解耦。用户 2026-09-17 原话:「跟随封面和主题不挂钩了,
/// 也就是可以被列为自定义,并且可以被正常保存」。
///
/// 占位符留着有两个实际坏处:① 这一组现在只剩"选一套配色"这件事,而那一格「—」的原因在
/// **另一个浮层**里,站在这里看不出来;② 跟随封面开着时点「存为新主题…」,主题确实存下来了、
/// 这一格却仍是「—」,看起来像没存进去 —— 用户这次报的就是这个。
///
/// ⚠️ 悬浮窗右键的「更改配色」子菜单**不跟着改**,两边现在合理地不一样。那份菜单里「跟随封面」
/// 跟主题列表同处一级(见 `OverlayQuickSettingsMenu.colorThemeMenu`),跟随开着时再给某个主题
/// 打勾,就是「跟随封面 ✓」+「黑字描边 ✓」两个互相矛盾的"正在生效" —— 那正是用户报过的那个
...
struct OverlayThemeSettingsRows: View {
    ...
    /// Menu 唯一的选中反馈来源。
    ///
    /// ⚠️ **跟随封面开着时也照常算**(2026-09-17 起)。这一档来回改过三次,理由都记下来:
    ...
    ///  - 2026-09-17 **连占位符一起去掉**,回到 08-14 那一档。跟随封面 2026-09-16 变成
...
enum OverlayStyleSummary {
    ...
    ///  - 2026-09-07:开关搬进了「主题」浮层,这颗按钮管的浮层里就有它,报「跟随封面」= 报这个
    ///    浮层的真实状态;
    ///  - 2026-09-17(现在):开关 2026-09-16 又搬去了「文字」浮层,这个浮层里**没有**它了 ——
