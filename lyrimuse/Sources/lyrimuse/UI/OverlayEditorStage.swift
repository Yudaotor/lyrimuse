...
// 两条路径就会漂,而且漏改不会编译报错)。
//
// 迭代过程(逐步做对宽度调整交互、去掉简化渲染、修正舞台/窗口边界与高度)全部记在
// docs/features/04-desktop-overlay.md「编辑台改造」。
//
// 当前几何:卡片高度由真视图上报的内容高度决定(`onContentHeightChange`,跟真窗口
// `updateHeight` 同一条 `max(120, ceil(h))` 口径),舞台高度按卡片高度加宽度调整条通道
// (`widthBarLaneHeight`)推出来,与 `overlayWidth` 本身无关 —— 同一份内容在不同窗宽下
// 内容高度一致,宽度只决定换不换行。`maxCardHeight`/`totalHeight` 的具体数值和出处见
// 它们各自的注释。
//
// 舞台上只有一张壁纸,按舞台宽度铺满、不随 `overlayWidth` 变化;窗口不自带壁纸
// (`showsDesktopUnderlay: false`),拖宽度调整条时背景纹丝不动。窗口边界由常驻虚线轮廓
// 标出 —— 它是"调整条正在改的是这扇窗的宽度"唯一的视觉锚点,别删。画布上没有任何直接
// 拖拽/点击热区:宽度走调整条,其余一律走工具栏(「文字」「配色」「重置 ▾」)。文字命中带
// 用真视图报上来的文字实际矩形(`onLyricsTextRectChange`)—— 编辑台本身不消费这个
// 回调,但真视图仍要上报,真窗口的「划过让开」靠它当判据,别把上报那头也删了。
//
// 锁标(lockBadge)必须保留:它是「锁定位置」这个开关在编辑台上唯一的视觉产物。

/// 编辑台渲染真 `LyricsOverlayView` 时用的 chrome:凑齐它要的那四个状态,但**不建窗口、
/// 不碰 `LyricsOverlayWindowController.shared`**。
///
/// 不建窗是硬要求,不是优化:`.shared` 是 `static let`,哪怕只是读一个属性都会执行
/// init() 建出窗口并 orderFront —— 悬浮歌词关着的用户一打开设置页,桌面上就会凭空多出
/// 一扇(不可见但已经装好全局鼠标监听器和三个观察者的)窗。灵动岛那边的 `NotchPreviewChrome`
/// 是它的对应版本。
///
...
final class OverlayPreviewChrome: ObservableObject, OverlayChromeSource {
    ...
    /// 恒 nil —— 同上:控制排在编辑台里根本不显示,没有可悬停的按钮。 别为了"让预览更生动"
...
struct OverlayEditorStage: View {
    ...
    ///
    /// (第八步之前,这里是三条 `PlaybackCoordinator` 的窄订阅
    ...
    ///
    /// 这是卡高**唯一**的来源,别再退回按字号估的公式:译文/罗马音/下一句三行、
    ...
    /// 第五步之前它是那条轮廓出不出现的**开关**,那是个错误:轮廓要先被看见,
    ...
    ///
    /// 关键观察:**同一份内容、同一个换行行数下,高度跟 overlayWidth 无关**(420/450/640/
    ...
    /// 高度跟着内容跳要好。
    ///
    /// **从 288 降到 266**(「这个预览窗口给我高度搞小一点,把下面的
    ...
    /// **别改成"有提示时才加那 21pt"**:整块高度必须是常量(见 body 末尾那条
    /// `.frame(height:)` 的注释),而"超宽"恰恰是拖宽度调整条时会来回翻转的状态 —— 那样拖动
    /// 中整页卡片会跟着跳 21pt。搬进舞台正是为了让它出现/消失时整块高度一动不动。
    ///
    /// `* 2` 是**两行**工具栏(加第二行「行为」时改的,同 `NotchEditorStage`)。
    /// 第二行不是为了排版好看:第一行的横向预算早在加第三个入口时就量到了上限
    /// (见 `toolbar` 头上那条提醒——中文常见值 535pt / 可用 600pt,还要留给右边的「重置 ▾」),
    /// 第四颗按钮塞不进去。
    ...
    /// "我调好的宽度自己变了"。
    ///
    /// 从 `420...1000` 扩到 `300...1400`(「配的 420 到 1000 的区间我感觉
    ...
    ///
    /// 第六步之前这里叫 effectiveWidth,要在"拖动中还没落盘的临时值"和设置里的
    /// 真值之间二选一 —— 调整条是实时落盘的(见 widthBinding),不再有第二个真相。
    /// 拖动中的临时宽度(nil = 没在拖)。
    ///
    /// 这是**性能必需**,不是锦上添花。Slider 的 set 若直接写 `settings.overlayWidth`:
    /// step 是 2pt,从 300 拖到 1400 就是 550 次,每一次都要 ①`@Published` 广播一遍(整页所有
    /// 观察 AppSettings 的视图跟着重渲染)、②写一次 UserDefaults、③让真悬浮窗重新布局。而
    /// 编辑台里跑的是**真** LyricsOverlayView,每次还要重算 WrapLayout 换行、重新上报内容
    /// 高度 —— 一帧里塞这么多事,必卡。
    ///
    /// 拖动中只动这个 `@State`:只有编辑台自己重画(它本来就要跟手),不写盘、不广播全局、
    /// 不碰真窗口;`onEditingChanged` 收到 false 时一次性提交。
    ...
    /// 这扇窗已经宽到被编辑台裁掉两端了吗?
    ///
    /// **编辑台按 1:1 真实 pt 画,不缩放**。而 overlayWidth 上限是 1000pt,
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            VStack(spacing: Self.toolbarSpacing) {
                ...
                toolbarRow2
                    .frame(height: Self.toolbarHeight)
                // 舞台底下没有 caption 那一行了(21pt,理由见 totalHeight),
                // 这里直接就是舞台本身。
        ...
        //
        // 卡高(cardHeight)从第八步起是**量出来的**,但整块的高度**仍然是常量** —— 舞台
    ...
    /// 本身才是功能。为此摘要那截给了 `.layoutPriority(-1)`,让它**先**被压(见 `EditorToolbarButtonLabel`)。
    ///
    /// **这一行仍然是三颗,不是四颗** —— 拆分之后总入口从 3 个变成 5 个(主题/文字/背景/
    ...
    ///
    /// 为什么另起一行、而不是当第一行的第四颗:第一行的横向预算是量过的上限(见 `toolbar`
    /// 头上那条提醒:中文常见值 535pt、可用 600pt,右边还有「重置 ▾」),再加一颗必然把摘要压成
    ...
    private var toolbarRow2: some View {
        HStack(spacing: 8) {
            ...
            // 第三颗:「位置」(自由 / 顶部居中 / 底部居中)。摘要就是当前模式名 —— 跟「排版」报
            ...
            Spacer(minLength: 8)
            // 跟第一行「重置 ▾」等宽的隐藏占位:两行的弹性预算一样,三颗入口才逐列对齐。
    ...
    ///
    /// 这里读的是 `binding(...).wrappedValue` 的 **get** 分支,而 `AutoHideItem` 的 get
    ...
    ///
    /// 五项都开时摘要会拼成一长串,交给 `EditorToolbarButtonLabel` 里那 140pt 限宽 + 尾部省略处理 ——
    /// 跟「文字」按钮遇到超长字体名是同一个兜底,不为这里另写一套截断。
    ///
    /// 「位置」三选一上午短暂在这句摘要里挂过前缀(「底部居中 · 全部关闭」),同日搬成
    ...
    private func stage(stageWidth: CGFloat) -> some View {
        ...
        // 舞台自己那条发丝描边挪到裁切**之后**(第五步):壁纸现在铺满整块舞台,
    ...
    /// 什么(见 SettingsDesignSystem 里那条提醒),而这块底板上压着的是真实桌面壁纸 ——
    ...
    ///
    /// **这是整块舞台唯一的一张壁纸**(第七步)。它按**舞台宽度**和舞台高度画,
    /// 跟 overlayWidth 没有半点关系 —— 拖宽度调整条时它一个像素都不动,这正是第七步要修的
    /// 东西(「背景永远不要变」)。窗口那块**不画**壁纸,相当于在这张图上开了个洞,
    ...
    ///
    /// **窗外那圈高斯虚化已经删掉了,别再加回来。** 第五到第七步之间它是必须的:那时舞台
    ...
    /// 舞台正中那扇窗 —— **真窗口那份 LyricsOverlayView 本体**(第八步)。
    ///
    ...
    /// `.frame(minHeight: 0, maxHeight: .infinity, alignment: .top)`,内容永远贴着这个框的顶边 —— 跟真窗口
    ...
    /// 钉住了"这个事实本身。
    /// **它本身在第十步保留了**:行为栏那一格底下原本有一句「锁上后编辑台左下角会出现锁标」
    /// 介绍它,那句按删了,但锁标是这个开关在编辑台上唯一的真实反馈 —— 删掉解释不等于
    /// 删掉被解释的东西。
    ///
    ...
    /// 舞台内部、悬浮歌词窗口正下方那条宽度调整条(第六步,替掉左右两个拖拽握柄)。
    ///
    /// 摆在**舞台里面**而不是舞台底下(第六步当时舞台下面还有一行 caption,删了),
    /// 是因为它得跟它改的那扇窗待在同一块画面里:窗口两侧现在露着桌面(desktopSurround),
    /// 调整条压在窗外那片桌面上、正对窗口下沿,"这根条改的是上面这扇窗的宽度"不用另写一句话
    /// 解释。同一条通道的左端还并排摆着那颗「两端已裁切」提示胶囊
    /// (`overflowHint`,横向余量那笔账见 stage 里那段注释)。
    ///
    /// 配色**固定黑底白字 + 投影,不跟深浅色模式走** —— 同 lockBadge /
    ...
    private var widthBar: some View {
        HStack(spacing: 8) {
            ...
            // **不要**给这根 Slider 传 `step:`(「为什么这里灰色条下面还有
    ...
    ///
    /// 这个数字在整块编辑台上**只出现这一处**:舞台底下那行 caption 第六步起就不再带宽度
    /// (那一行整个删了)—— 同一个数字摆两处,一调两边一起跳,读者会以为那是
    /// 两件事。
    ///
    /// 读的是 `draggingWidth ?? settings.overlayWidth`,不是裸的 `settings.overlayWidth`
    /// (修)。落盘推迟到松手之后(见 draggingWidth),而窗口宽度走的是同一个
    ...
    ///
    /// 提交只有 `onEditingChanged(false)` 这**一条**出口,而这够用 —— 别为"键盘 / VoiceOver
    /// 调节会不会漏掉这条出口"再加一条兜底写入路径(离屏实测排除过):
    ...
    /// 守卫跳过通知之后,**必须**有人在"窗口重新打开"那一刻把新值应用上 —— 那是
    ...
    ///
    /// 这一格长在舞台里、压在真实桌面壁纸上(高度账见 totalHeight / maxCardHeight), 不能用
    /// `.secondary` 灰字 —— 灰字在浅色壁纸上直接看不见。跟调整条那条胶囊共用同一副外壳
    /// (`overlayStagePillChrome`,白字 + 黑底 + 发丝描边 + 投影),自带底色而不是躲开壁纸。
    ///
    /// **不带宽度数字**:数字只长在调整条旁边
