...
// 够得着"。
// ⚠️ 钉条和它那份简化渲染 `OverlayLyricsCanvas` **已于一并删除**:第八步之后
...
//
// 第一步落的是:复用同一份渲染 + 左右两个可拖拽的宽度握柄。
// 第二步落的是:工具栏(带当前值摘要的「文字…」「配色…」「重置 ▾」)和两个浮层。
...
//
// 第四步(现象是原话:「就不应该有什么缩放的概念在这里;只显示实际的大小以及
...
//  照旧要缩" —— 钉条和那份渲染已经整个删掉,这条连同它的对象一起作废。)
//
// 第五步(现象是原话:「我意思是不应该可以拖动的是里面这部分吗外面这个框为什么
...
// (⚠️ 第七步订正:「零改动」到此为止 —— 那一步给共用画布加了一个带默认值的
//  开关 showsDesktopUnderlay,好让编辑台把窗内那份壁纸关掉。钉条的调用点没动、默认值就
//  等于它的现状,见下面第七步那段。)
//
// 第六步(「算了你始终不明白我的意思;既然这样,那么就这样调整,在这个
...
// 悬停虚线框 + 标签,工具栏上还有等价的显式入口兜底)。第十步把命中区也去掉,于是画布
...
// 容易漂,而且漏改不会编译报错。
//
// 第八步(「帮我把这个预览窗口的一切行为都和实际的桌面歌词保持一致;
...
//  随钉条一起解除 —— 钉条零实例化,两个文件都删了。)
//
// 第七步(「还有一点要调整就是**背景永远不要变**,拖动宽度框的时候
...
// 来分,这两样本来就是干这个的。
// 第九步(「现在翻译这些看不到是因为高度不够吗,帮我高度也正常展示
// 出来」)两件事:
...
// ⚠️ 锁标(lockBadge)**保留**:行为栏里介绍它的那句小字按删了,但它仍然是「锁定位置」
...
// 第十八步(「这个预览窗口给我高度搞小一点,把下面的开关完整漏出来」)
...
// 为什么正好压这么多、而不是"随手矮一点":这一段的版面账是量出来的(对拍 1358×1038
...
///   - `isHoveringForControls` 恒 false → 播放控制排不显示。⚠️ 光有这个**不够**:
...
struct OverlayEditorStage: View {
    ...
    /// 真悬浮窗当时的 269 还矮 25pt,卡底连同译文/下一句一起被 clipShape 裁掉 —— 现象是
    /// 「现在翻译这些看不到是因为高度不够吗」)。
    ///
    /// 288 是**实测出来的最坏情况**,不是拍的(第九步,用真 LyricsOverlayView 在
    ...
    /// 高度跟着内容跳要好。
    ///
    /// ⚠️ **从 288 降到 266**(「这个预览窗口给我高度搞小一点,把下面的
    ...
    /// 整块(工具栏两行 + 画布)占的高度。
    ///
    /// ⚠️ **把舞台底下那行 caption 整个删了**(省 `captionSpacing` 6 +
    ...
    ///
    /// ⚠️ `* 2` 是**两行**工具栏(加第二行「行为」时改的,同 `NotchEditorStage`)。
    ...
    /// 拖动中的临时宽度(nil = 没在拖)。
    ///
    /// ⚠️ 这是**性能必需**,不是锦上添花(现象是"拖动这个宽度条的时候卡顿,不流畅")。
    ...
    /// 这扇窗已经宽到被编辑台裁掉两端了吗?
    ///
    /// ⚠️ **编辑台按 1:1 真实 pt 画,不缩放**。而 overlayWidth 上限是 1000pt,
    ...
    ///
    /// ⚠️ 横向够不够(加第三个入口时离屏量的,别凭感觉重排):这一条的可用宽度
    ...
    private var toolbarRow2: some View {
        HStack(spacing: 8) {
            ...
            // 第三颗:「位置」(自由 / 顶部居中 / 底部居中)。上午先当「行为」浮层
    ...
    ///
    /// ⚠️ **这是整块舞台唯一的一张壁纸**(第七步)。它按**舞台宽度**和舞台高度画,
    ...
    private var widthBar: some View {
        HStack(spacing: 8) {
            ...
            // ⚠️ **不要**给这根 Slider 传 `step:`(「为什么这里灰色条下面还有
    ...
    ///      ⚠️ 订正:这里原来还写着"窗口真被打开时 setVisible 会按持久化值再应用
    ...
    ///
    /// ⚠️ **从舞台底下那行 caption 搬进舞台**(那一行整个删了,省 21pt,理由和
    ...
    ///
    /// ⚠️ **不带宽度数字**(第六步的结论,搬家后照旧成立):数字只长在调整条旁边
