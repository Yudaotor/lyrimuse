...
//
// ⚠️ **这里原来写着"没有「全部设置」抽屉,这不是漏做",那条论证已经被推翻。**
...
//
// ⚠️ 舞台**放得下就 1:1、放不下就整体缩小到刚好放下**(`previewScale`)。1:1 是从悬浮歌词
...
final class NotchPreviewChrome: ObservableObject, NotchChromeSource {
    ...
    ///   - 用户在「屏幕」浮层里改了「显示在哪块屏幕」。⚠️ 第二条之前是**漏的**
...
struct NotchEditorStage: View {
    ...
    /// 上界从 500 抬到 **800**(用户拖到 500 后:「最宽还是有点小了,帮我再调大一些」;
    ...
    /// ⚠️ `expandedExtraHeightMax` 不再是无参常量,吃"下一句预览开关"+
    ...
    ///
    /// ⚠️ 工具栏从一行变两行(第二行「歌词行/行为/展开态」三个新入口,见
    ...
    ///
    /// ⚠️ **前这里右边是刻意空着的**——当时的理由是"灵动岛的外观只有「风格」
    ...
    ///
    /// ⚠️ **加了这颗按钮之后,第一行的横向预算从"余量 1pt"变成"确定超支"**(
    ...
    private var toolbar: some View {
        HStack(spacing: 8) {
            ...
            // 跟浮层里那两行是同一对记号。
            //
            // ⚠️ **别再试图给这两颗钉一个固定宽度去凑"等宽"**(试过一次,当天撤掉)。
    ...
    ///
    /// ⚠️ 横向预算:三颗时的账(中文 493 余 6pt / 英文 649 超 150pt)见 05 章「编辑台改造 → 搬过来的
    ...
    ///
    /// ⚠️ **例外:卡片上叠了一层"可点区域"(`hotspotLayer`)** —— 那不是真视图里的控件
    ...
    ///
    /// ⚠️ 「下一句预览」划给**歌词行**那块、不划给展开态(「这个框不应该把这个歌词行也包括
...
///
/// ⚠️ 宽度 240 是**算出来的**(定的 160 是纯列表时代的数,那次现象是
...
struct NotchEarPopover: View {
    ...
    var body: some View {
        // ⚠️ 把这里的帮助气泡整段去掉(那两段话本来是解释"只影响播放中/
