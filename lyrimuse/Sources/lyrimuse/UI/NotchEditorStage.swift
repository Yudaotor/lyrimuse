...
// 「歌词显示 到 灵动岛」的**编辑台**:一小片屏幕顶端(菜单栏 + 刘海 + 桌面),灵动岛卡片
// 1:1 挂在上面,直接在这块画面里改。
//
// 这是把桌面悬浮歌词那一段的编辑台范式(见 OverlayEditorStage 与
// docs/features/04-desktop-overlay.md「编辑台改造」)搬到灵动岛这一段,
...
//      有没有手势无关,复现记录见 SettingsView 里那段的注释),所以凡是
//      "能动手改"的预览都只能待在内容区。
...
//
// **这里原来写着"没有「全部设置」抽屉,这不是漏做",那条论证已经被推翻。**
...
//
// 舞台高度是常量(只随屏幕的刘海让位高度变,那是插拔显示器才会发生的事)。hover 展开
...
///
/// 这个类存在的唯一理由,就是让预览**不去碰 NotchLyricsWindowController.shared**。
...
///
/// (从 SectionPreviewBars.swift 搬来 —— 它唯一的消费方现在是这个文件里的编辑台,
...
final class NotchPreviewChrome: ObservableObject, NotchChromeSource {
    ...
    ///
    /// (NotchLyricsView 自己那个 .onHover 已于删除,真窗口的命中判定在
    ...
    /// 两个赋值**刻意不判等**。这个方法只在屏幕插拔和「屏幕」浮层提交时跑(低频到可以
...
struct NotchEditorStage: View {
    /// 显式写 init 而不是靠合成的逐成员构造器:下面 `settings` 是 `private` 存储属性,
    ...
    /// (从卡片上点开的,「弹出来的那个窗口也不要在原本顶部的那个位置弹出来,
    ...
    /// 拖动中的临时宽度(nil = 没在拖):稳态 / 展开各一份(宽度是一对)。
    ///
    /// 这是**性能必需**,不是锦上添花 —— 同 OverlayEditorStage.draggingWidth 那条(用户
    ...
    /// 数值 —— 同一个窗口里两条控件没有理由长得不一样(这根是双滑块,轨道等长)。
    ...
    /// 上界从 500 抬到 **800**:稳态 360 时展开态只能到 500,+140pt 不够看。两只滑块
    ...
    /// 两端必须是 `widthStep`(2) 的整数倍(`snap()` 先夹后量化,不是倍数会把值顶出界);
    /// 200/800 都满足。
    ///
    /// 这是**存储层**的区间,不是滑杆能拖的区间 —— 后者见下面的 `usableWidthRange`,
    ...
    /// 滑杆**真正能拖**的区间:下界抬到这台机器**当前配置下**的"耳朵下限"(用户
    ...
    ///
    /// **下界是屏幕 _和_ 耳朵配置两者的函数,不是常量**(后半条是同日第二轮改的:用户
    ...
    ///
    /// 现算的前提是调用点会跟着耳朵配置重新求值:编辑台和菜单栏快捷面板都 `@ObservedObject`
    ...
    ///      下界**以下**(例如下限 340.9 到 夹到 340.9 到 量化成 340,比下限还小 0.9pt)。
    ...
    /// `expandedExtraHeightMax` 不再是无参常量,吃"下一句预览开关"+
    ...
    ///
    /// 工具栏从一行变两行(第二行「歌词行/行为/展开态」三个新入口,见
    ...
    ///
    /// 刘海宽度这里现读屏幕(`targetScreen()` / `geometry(for:)` 都是 static、不碰
    ...
    /// (`NotchWindowRoot`)读的是同一份 —— 收成一份,在此之前两处各写一遍,
    ...
    var body: some View {
        ...
        // 高度**在交互路径上是常量**,但它依赖 `chrome.contentTopInset`(刘海让位高度),
        // 而那个值有两个变法:插拔显示器,以及**这一页自己的**「屏幕」浮层换了一块屏(内建刘海
        // 屏 33pt 与 外接屏的菜单栏高度,能差好几个 pt)。后者是页内操作,硬跳的话下面那张
    ...
    ///
    /// **加了这颗按钮之后,第一行的横向预算从"余量 1pt"变成"确定超支"**(
    ...
    private var toolbar: some View {
        HStack(spacing: 8) {
            ...
            // 左右耳各一个入口(从一个「耳朵」拆开:「拆成两个,左右耳,
            // 各一半宽度」)。图标用带方向的 `arrow.left.to.line` / `arrow.right.to.line`,
            // 跟浮层里那两行是同一对记号。
            //
            // **别再试图给这两颗钉一个固定宽度去凑"等宽"**(试过一次,当天撤掉)。
            ...
            //      摊到标题上:英文 @600(= 卡片列上限,窗口拖到多宽都是这个数)标题 41到30.0、
            //      48到37.0,正是对拍里的「Left…」「Righ…」;@499 只剩 9.0pt。中文同样中招,
            //      @530 标题 22到11.0。而摘要列在所有档位恒 60.0、一格没让。
    ...
    /// 工具栏第二行:「歌词行 / 字体 / 展开态 / 行为」四个入口(「字体」加),收纳的是这几轮陆续加的
    /// 一批布尔开关。这些开关最初直接铺在页面上(先是"行为"卡片横排两格,后来陆续并进「显示封面」
    /// 和整张「展开态」卡片的四项),后来改回跟「风格/屏幕/左耳/右耳」一样的
    /// "点开才配置"形态。
    ///
    ...
    /// 卡片从上到下走:歌词行 到 展开态 到 行为(改前是 歌词行 到 行为 到 展开态)。内容(图标 / 标题 /
    ...
    private var toolbarRow2: some View {
        HStack(spacing: 8) {
            ...
            // 「字体」:字体族 / 粗细 / 字号,紧跟「歌词行」—— 它是歌词行文字的属性,排在"这一行显示什么"
    ...
    /// 「字体」按钮摘要:「系统字体 较粗 13pt」,跟悬浮歌词「文字」按钮那截是**同一个函数**
    ...
    /// 「歌词行」按钮摘要:「显示歌词」从「行为」浮层搬过来之后,这里要拼两项
    ...
    private var lyricRowSummary: String {
        ...
        // 「对齐方式」只在**非默认**时才报。三选一不是开关,套不进上面那条
        ...
        // 「副行」同一条规则:四选一、默认「下一句」,只在非默认时报,写成「副行 · 译文」。
        ...
        // 「展开时预览下一句」(从「展开态」搬来):副行选「下一句」时它被顶掉、浮层里也不显示
    ...
    /// 上面那套归约逻辑的本体。从 `behaviorLikeSummary` 里下沉了一层,吃的是
    ...
    /// 「左耳」/「右耳」按钮摘要:这只耳朵的模块名,音浪贴在这一侧时再带上「音浪」。
    ...
    private func stage(stageWidth: CGFloat) -> some View {
        ...
        // 舞台的裁切形状是**顶直角、底圆角**(跟卡片同一个 NotchHangingShape),不是悬浮歌词
    ...
    /// 自动转白,那正是它一眼认得出来的原因(菜单栏预览条实拍确认过,这里直接
    /// 沿用同一条结论)。
    ///
    ...
    ///
    /// **卡片内部一律不接事件**(`.allowsHitTesting(false)`)。改版前它天然点不动 —— 预览
    ...
    private var card: some View {
        NotchLyricsView(controller: chrome)
            ...
            // 卡片按**深色**外观画,不跟设置窗口走:真窗口把 appearance 锁死 `.darkAqua`
            ...
            .onChange(of: popover) { _, newValue in
                // 从卡片上点开的浮层开 到 钉在展开;关 到 放开(指针若还在卡片上,下一次 hover 事件会
    ...
    /// 预览卡上一块可点的区域:点它打开管这块内容的浮层(「类似这些区域帮我
    /// 调整成可以点击来换取相应的配置的逻辑」)。
    ///
    /// **同一个浮层管的内容只算一块**(「现在拆的太密了,你需要把那些一样的都给它整合在
    /// **同一个浮层管的内容只算一块**(别拆太密:展开区拆成下一句 / 进度条 / 播放键三块、
    /// 曲目信息头部又一块、四块都开「展开态」是不对的)。
    /// 进度条与播放键,中间隔着歌词行(含下一句),悬在任一个上两块一起亮、点任一个都开同一个浮层
    /// (锚在被点的那个上)。
    ///
    /// `rects` 是**卡片本地、未缩放**坐标(原点卡片左上角),跟 `NotchLyricsView.body` 那棵 VStack
    /// 的排版逐段对应:顶行两只耳朵 到 曲目信息头部(展开且开着才有)到 歌词行(`showsLyricRow`)
    /// **+ 紧跟着的「下一句预览」**(展开且开着才有)到 展开区剩下的(进度条 / 播放键)。高度全部取自
    ...
    /// 细框 + 一层极淡的白底(告诉"这块能点"),指针换成手形;点一下打开 `spot.target` 那个浮层,
    /// 锚在这个矩形旁边(见 `hotspotPopoverAnchor`)。
    ///
    /// 虚线 + 七成不透明(边框不要太硬;0.9 实线太重)。
    ...
    private var windowEdgeOutline: some View {
        // `.stroke` 而不是 `.strokeBorder`:后者只有 InsettableShape 才有,而
    ...
    /// 舞台内部、卡片正下方那条宽度调整条 —— 是**双滑块**(`RangeSlider`):左边那只
    /// 是稳态宽(下限,没 hover 时卡片多宽),右边那只是展开宽(上限,hover 展开后撑到多宽)。
    /// 「配置宽度的时候可以设置一个上限和一个下限,下限就是正常状态的宽度,上限就是
    ...
    /// 的全部意义。
    ///
    /// 配色**固定黑底白字 + 投影,不跟深浅色模式走**:它底下垫的是用户真实的桌面壁纸。
    ...
    /// 报「稳态–展开」两个。
    ///
    /// 报的是**卡片真实宽度**(`steadyCardWidth` / `expandedCardWidth`,已经过"两只耳朵放得下
    /// 按钮"的下限和「展开 ≥ 稳态」),不是设定值(「这里的宽度帮我改为带上
    /// 耳朵的宽度,这样就没有歧义了」)。此前报设定值、再由 caption 补一句「已被两侧耳朵撑到
    /// NNNpt」—— 同一件事两个数字、还得配一句话解释它们的关系。现在数字跟眼前这张卡逐像素对
    /// 得上,那句 caption 也就删了。
    ///
    /// (稍后:滑杆下界抬到耳朵下限之后,"拖了数字不动"那截死区已经不存在了 ——
    ///  见 usableWidthRange。这条口径仍然留着:它让**历史上落盘过的**、低于下限的旧值也显示得对。)
    ///
    /// 走真实宽而不是 `settings.notch*Width`:后者在拖动期间不更新(落盘推迟到松手),读数会
    ...
    private func captionText(stageWidth: CGFloat) -> String {
        ...
        // 「预览已缩小至 NN%」:展开宽放不进舞台、整组被缩过时出现(随宽度上限 500 到 800)。
        // 上限 500 那会儿这里是一句"正常情况下永远不出现"的「两端已裁切」兜底(500 vs 舞台最窄约 499);
        // 抬到 800 后先按悬浮歌词那套裁切 + 渐隐做过一版,(「太大之后,预览放不下」),改成
...
/// 「屏幕」那颗按钮上的摘要。
///
/// 不能存成 `static let`:`L10n.t` 要在每次取值时现算,存进 static let 等于把首次访问时的
...
/// 「显示歌词」开关(「多一种形态,对于有一些想要显示播放状态、但又不想
/// 有歌词挡住视线的人」)。关掉之后卡片只剩顶行那一条,退化成贴着刘海的状态栏。
///
/// 从「风格」浮层里搬出来,单独放在编辑台下面自己一张卡("不要合并到
...
/// 「暂停时缩到最小」开关(把这个一直以来的默认行为开放成可关的配置项)。
...
///
/// 范围(跟用户确认过,三选一里选的是最宽的一档):风格 + 左右耳 + 屏幕 + 这个
...
enum NotchStyleDefaults {
    static func restoreDefaults() {
        ...
        // 「字体」组三项。它们是字形、不是宽度那类结构性尺寸,按钮自报的排除范围「不含宽度和总开关」
...
/// 一只耳朵显示什么模块。左右各一个浮层,由工具栏上对应那颗按钮打开。
///
/// 用**单选列表**,跟「风格」「屏幕」两个浮层同一个形态。一度写成一个「耳朵」浮层 + 两行
...
///
/// 收起态那一套耳朵(左封面、右音浪)**不在这里配**,理由见 `NotchEarModule` 上方那段。
///
/// 宽度 240 是**算出来的**。列表本身仍只要 124pt(中文)/ 136pt
/// (英文):八行两到四个字的模块名,最长英文 "Remaining" 63.0pt + 行内固定 60pt(左内边距 14 +
/// 勾列 20 + 间距 12 + 右内边距 14)。**抬到 240 的是顶上那一行「音浪」开关**(从勾选行
...
struct NotchEarSettingsRows: View {
    ...
    /// 「音浪」落点第四次(也是最终)拍板处(同一天):风格浮层
    /// 到独立卡到(设计出工具栏第五入口方案,但同事 ls-Rocky 离屏量出中英文都装不下,
    /// 未落地)到**这里**,左右耳浮层顶部各一个独立开关行。"放到左右耳列表里面啊,
    /// 最顶上加一个展示音浪的选项,但是和下面的通过分割线分开,可以和下面的同时选择"。
    ///
    /// **用 `SettingsRow` + `Toggle`,不跟下面 `row(_)` 共用"勾存在/消失"的视觉。**
    /// 这个开关其实是个独立开关(可以跟任意模块共存,不参与那组互斥单选),用同一套视觉、
    /// 只靠一条分割线隔开容易被看成"九行单选里有一行可以跟别的同时勾"。现在两处都是这一行,
    ...
    ///   - 点未选中项 到 target = 该项(照旧);
    ///   - 点已选中项 到 target = `.none`(本次新增的取消语义);
    ///   - 「不显示」已选中时点它自己 到 target 仍是 `.none`、等于当前值,被守卫吃掉(无操作)——
    ///     这一条正是守卫必须算在 target 上的原因,写在入参上会漏掉它。
    ///
    /// 守卫本身不能去掉:@Published 是 willSet 语义,等值赋值照样广播 objectWillChange 打醒
    /// 所有观察 AppSettings 的界面,didSet 还会白写一次 UserDefaults。
    /// 写全 `NotchEarModule.none`,别缩成 `.none`:这个枚举有个成员就叫 `none`,而 `.none`
...
struct NotchScreenSettingsRows: View {
    ...
    /// `if settings.notchOverlayEnabled` 守卫不能省 —— 理由同
