...
let groups: [TestGroup] = [
    ...
    TestGroup(name: "key-parity-sweep", summary: "逐码点跨语言对拍:缓存 key / 宽松 key / 文件名 / 手动选词指纹与 collector 生成的对照文件逐窗比对", run: runKeyParitySweepTests),
    ...
    TestGroup(name: "settings-ui", summary: "设置页交互纯逻辑:顺序优先列表拖拽排序(滞回 / 让位 / 写回)/ 导入字体 / 配置包自报信息", run: runSettingsInteractionTests),
