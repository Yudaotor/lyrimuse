...
func runMenuBarTests() {
    ...
    //
    // 菜单栏从 MenuBarExtra 换成自建 NSStatusItem 之后，滚动不再由计时器逐帧
    ...
    //
    // 加("像酷狗菜单栏歌词")。填色跟滚动同一套哲学:整行进程编成一条
    ...
    //
    // 加("可以根据实际的宽度以及播放逐字进度去滚吗")。有逐字时间轴的句子,滚动不再按
    ...
    do {
        ...
        // 按显示时长配速(没有逐字时间轴的行):开头停一会儿 到 匀速走到末端 到 停在句尾。
        // 字宽 10、溢出 60:基准 4 字/秒 = 40pt/s,上限 12 字/秒 = 120pt/s。
        let paced = MenuBarMarquee.pacedScrollPath(startMs: 10_000, dwellMs: 8_000, maxOffset: 60, averageCharWidth: 10)
        expectEqual(paced.count, 3, "按时长配速: 三个点")
        expectEqual(paced[0].ms, 10_000, "按时长配速: 从这一行开始显示的时刻起算")
        expectEqual(paced[0].x == 0 && paced[1].x == 0, true, "按时长配速: 开头停住不动")
        expectEqual(paced[2].x, 60, "按时长配速: 终点停在 maxOffset")
        expectEqual(paced[2].ms <= 18_000, true, "按时长配速: 显示窗口结束前走完(\(paced[2].ms))")
        expectEqual(MenuBarMarquee.karaokeFillX(atMs: 30_000, path: paced), 60, "按时长配速: 走完之后停在句尾")
        // 窗口很短:加速,但不越过 12 字/秒 —— 60pt 最快 0.5 秒。
        let tight = MenuBarMarquee.pacedScrollPath(startMs: 0, dwellMs: 1_000, maxOffset: 60, averageCharWidth: 10)
        expectEqual(tight[2].ms - tight[1].ms >= 500, true, "按时长配速: 短窗口加速但不超上限(\(tight[2].ms - tight[1].ms)ms)")
        // 不知道显示多久:退回基准速度 40pt/s,60pt 走 1.5 秒。
        let unknown = MenuBarMarquee.pacedScrollPath(startMs: 0, dwellMs: nil, maxOffset: 60, averageCharWidth: 10)
        expectEqual(unknown[2].ms - unknown[1].ms, 1_500, "按时长配速: 时长未知时按基准速度")
        expectEqual(MenuBarMarquee.pacedScrollPath(startMs: 0, dwellMs: 5_000, maxOffset: 0, averageCharWidth: 10).isEmpty,
                    true, "按时长配速: 装得下不滚")
        ...
        // 「唱到最后一定已经滚到最后」:路径末点的偏移必须**逐位等于** maxOffset。
        //
        // 这不是随便挑的边界 —— 单行面「唱完就切」会在唱完那一刻把这一句换走(灵动岛还多挂
        // CompactLyricLead.tailHoldMs),末端差一点点就是"最后几个字永远没露出来过"。
        // 数学上它恒成立(末点阅读位置 = textWidth,锚点 < windowWidth,所以
        // textWidth − 锚点 > textWidth − windowWidth = maxOffset,被上钳咬住),
        // 这里按真实形状的多组参数逐点验,免得将来改锚点比例或钳位时静默破掉。
        for (win, text, frac) in [(40.0, 60.0, 0.45), (40.0, 400.0, 0.45), (286.0, 300.0, 0.45),
                                  (100.0, 101.0, 0.0), (100.0, 5000.0, 1.0)] as [(CGFloat, CGFloat, CGFloat)] {
            let p = MenuBarMarquee.followScrollPath(reading: reading, windowWidth: win,
                                                    textWidth: text, anchorFraction: frac)
            // reading 的末点 x 是 60(上面那三个词的整句末端),按比例缩到这组参数的 textWidth 上。
            let scaled = MenuBarMarquee.followReadingPath(
                words: words, wordEndXs: [text / 6, text / 2, text])
            let q = MenuBarMarquee.followScrollPath(reading: scaled, windowWidth: win,
                                                    textWidth: text, anchorFraction: frac)
            guard let last = q.last else {
                expectEqual(text - win <= 0, true, "跟唱末端: 只有装得下时才允许空路径(win=\(win) text=\(text))")
                continue
            }
            expectEqual(abs(last.x - (text - win)) < 1e-9, true,
                        "跟唱末端: 末点偏移 = maxOffset(win=\(win) text=\(text) frac=\(frac),得到 \(last.x))")
            _ = p
        }

        // 「不要一滚到底就立刻换行,眼睛得来得及看清」:这件事不需要额外的停留机制,锚点
        // 本身就给出了一段静止期 —— 偏移到顶的条件是 `阅读位置 >= 锚点 + maxOffset`,
        // 代入 `maxOffset = T - W`、`锚点 = frac·W` 得 `阅读位置 >= T - (1-frac)·W`。
        // 也就是说**文字还剩 (1-frac)·W 没唱完时就已经滚到底了**,剩下那一段是在文字
        // 完全静止的状态下唱完的。这个量只由槽宽和锚点比例决定,**与句子多长无关** ——
        // 再长的句子,末端静止的都是"最后 (1-frac)·W 宽的文字唱完所需的时间"。
        // frac 取 0.45,所以是 0.55 个槽宽;灵动岛歌词格约 240pt 时 ≈ 132pt ≈ 十个汉字。
        for (win, text) in [(240.0, 400.0), (240.0, 2000.0), (100.0, 101.0), (286.0, 900.0)]
            as [(CGFloat, CGFloat)] {
            let frac = MenuBarMarquee.followAnchorFraction
            let maxOffset = text - win
            let anchorX = win * frac
            // 偏移刚好到顶时的阅读位置。
            let plateauX = anchorX + maxOffset
            expectEqual(abs((text - plateauX) - (1 - frac) * win) < 1e-9, true,
                        "跟唱末端静止: 滚到底时仍剩 (1-frac)·槽宽 的文字没唱(win=\(win) text=\(text))")
            expectEqual(text - plateauX > 0, true,
                        "跟唱末端静止: 剩余量恒为正 —— 不存在'唱到最后一个字才刚好滚到底'")
        }
    ...
    // ---- 菜单栏歌词旁那枚图标:整首歌的播放进度 ----
    //
    // 加("可以选择是否在最左侧或者是最右侧展示软件图标,图标上会逐渐染色
    ...
    // ---- 菜单栏跑马灯:按这一句的停留时长配速 ----
    //
    // 现象是的:"一句歌词比较长的时候滚动太慢，还没滚到底就切换到下一行了"。
    ...
    do {
        ...
        // 句尾预留(现象是"一到最后一个字就马上到下一句了"):时间允许时,
        ...
        // ---- 提前量:开唱之前一步都不许滚 ----
        //
        // 现象是的:「已经到下一行了,但是还没开始染色的时候不需要滚动,现在是会滚」。单行展示面
        ...
        //    head 算的,所以不能滚的那段时间是**自动**从行程里去掉的,不会重蹈
    ...
    //
    // 新增。现象是「灵动岛歌词有时候被封面挡住」——歌词行右边紧挨一枚 32pt 封面、
    ...
    do {
        ...
        if let slot = H.lyricsSlot(buttonWidth: slotW, contentWidth: slotW - lyricsPad,
            ...
            expectEqual(true, false, "歌词格: 129.5pt 的槽应当算得出歌词格")
        }
        // 图标在右:歌词格贴前缘,右边让给图标
        if let slot = H.lyricsSlot(buttonWidth: slotW, contentWidth: slotW - lyricsPad,
                                   reservedIconWidth: iconReserved, iconLeading: false) {
            expectEqual(slot.x, 0, "歌词格: 图标在右(或没图标)时贴按钮前缘")
    ...
    do {
        ...
        expectEqual(P.skipsResize(currentLength: 121.19, targetLength: 121.19 - 20,
                                  dwellSeconds: 1.24, quietSecs: quiet), true,
                    "短命行不改槽: 短命 + 差 20pt(上限内)照旧跳过")
        expectEqual(P.skipsResize(currentLength: 121.19, targetLength: 121.19 + 20,
                                  dwellSeconds: 0.5, quietSecs: quiet), true,
                    "短命行不改槽: 加宽方向同样只在上限内跳过")
        ...
        expectEqual(P.skipsResize(currentLength: 200, targetLength: 220,
                                  dwellSeconds: quiet - 0.001, quietSecs: quiet), true,
        ...
        // ---- 偏差上限:短命行也不许把槽宽甩开太远 ----
        //
        // 治的是"一首歌的句子**普遍**短于静默窗"这一档:原来的时长判据是无条件的,于是整首歌
        // 一次都不改槽宽,每一句都在一个不属于自己的槽里左对齐画,右边空一大片。
        expectEqual(P.skipsResize(currentLength: 200, targetLength: 200 - P.shortLineDriftCeiling + 0.01,
                                  dwellSeconds: 0.5, quietSecs: quiet), true,
                    "偏差上限: 短命 + 差一点点没到上限,还是跳过(收缩)")
        expectEqual(P.skipsResize(currentLength: 200, targetLength: 200 - P.shortLineDriftCeiling,
        ...
        // ---- 死区:太小的变化一律不值一次重建 ----
        // 收缩侧实测原样:text(250.749512) -> text(250.438477),0.31pt 也触发整项重建。
    ...
    // ---- 占位槽按「即将到来的那一句」定宽(现象是"同一行歌词会变化两次")----
    //
    ...
    // 判据在 MenuBarSlotPolicy.displayText,三条边界是刻意保留的:暂停不占宽、
    // 广告不显示广告标题、没歌名不做品牌兜底。
    do {
        ...
        // 前奏/间奏那一档进来的记号串。`MenuBarMarqueeRenderer` 住在 app target、selftest
        // 够不着,所以这里抄一份字面量,并在下面用**源码契约**钉住它没被改。
        let dots = "\u{2022}\u{2022}\u{2022}"
        ...
        // 前奏/间奏那一档进来的是三点的记号串(gapDotsToken),不是音符 —— 这条闸只管
        // "压根没有可显示的行",三点是有内容的一行,必须原样放过去。
        expectEqual(t(dots, "稻香")?.text, dots, "歌名兜底: 间奏三点原样保留,不换成歌名")
        ...
        expectEqual(t(dots, "稻香", icon: true)?.text, dots, "歌名兜底: 间奏三点不归这条管,照旧占住槽位")
        expectEqual(t("", "稻香", playing: false, icon: true) == nil, true,
                    "歌名兜底: 有图标也一样,暂停收回图标")

        // 源码契约:上面抄的那个记号串,以及「♪ 歌名」兜底与间奏三点是**两条**互不相干的路。
        let menuBar = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent().deletingLastPathComponent()
            .appendingPathComponent("lyrimuse/MenuBar")
        let rendererSrc = (try? String(contentsOf: menuBar.appendingPathComponent("MenuBarMarqueeRenderer.swift"),
                                       encoding: .utf8)) ?? ""
        let statusSrc = (try? String(contentsOf: menuBar.appendingPathComponent("MenuBarStatusItem.swift"),
                                     encoding: .utf8)) ?? ""
        expectEqual(rendererSrc.isEmpty, false, "菜单栏三点(契约): 读到 MenuBarMarqueeRenderer.swift")
        expectEqual(statusSrc.isEmpty, false, "菜单栏三点(契约): 读到 MenuBarStatusItem.swift")
        expectEqual(rendererSrc.contains("static let gapDotsToken = \"\\u{2022}\\u{2022}\\u{2022}\""), true,
    ...
    //
    // displayText 返回 nil 的那一刻(歌词间隙 / 暂停 / 广告 / 切歌那一拍 playing 掉 false),
    // 调用方给 present 传的是 collapseDelay = rebuildQuietSecs(3s 收缩观察窗)。观察窗原本
    // **只**省下几何重建,内容那一笔照画 —— 屏幕上歌词照样被图标顶掉、过一两秒又回来,
    // 正是 issue 报的那一幕。现在观察窗内一笔都不画。
    //
    // 这几条钉的是**判据身份**:认 observeRemaining 而不是 delay。delay 还含重建静默节流那
    // 一半,拿它当判据的话,"用户手动关掉菜单栏歌词"(collapseDelay 传 0)会因为撞在节流里
    // 而把歌词也留着不收 —— 那不是间隙,是结论。
    do {
            ...
            if let start = text.range(of: "let delay = max(observeRemaining"),
               let end = text.range(of: "let work = DispatchWorkItem",
                ...
                expectEqual(branch.contains("if cls == \"icon\" {\n                        render(button)"), false,
                            "间隙不闪(反例): 推迟分支里不许再出现无条件的 render(button)")
                expectEqual(branch.contains("if delay <= 0 { render(button) }"), false,
                            "间隙不闪(反例): 判据是 observeRemaining,不是 delay(delay 含节流那一半)")
            } else {
                expectEqual(true, false, "间隙不闪: 找不到推迟分支(改名了?)")
            }
        } else {
            expectEqual(true, false, "间隙不闪: 读不到 MenuBarStatusItem.swift(路径挪了?)")
        }
    }

    // ---- 图标态 / 歌词态的内边距是**两个**数----
    //
    // 实测对账 `requested` / `button.bounds.width` / `item.length`:按钮宽恒等于我们申请的槽宽,
    // **AppKit 一点都没多要** —— 那 18pt 全是我们自己留的。图标态需要它(showIcon 走
    // button.image,AppKit 居中,18 给出左右各 9,跟别的状态栏图标一致);歌词态我们自己摆图层、
    // 又贴了前缘,18pt 会整块堆在尾部(用户圈图:「这右边的也帮我缩一下」),所以只留 4pt 防裁边。
    do {
        let item = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent()
            .appendingPathComponent("Sources/lyrimuse/MenuBar/MenuBarStatusItem.swift")
        if let text = try? String(contentsOfFile: item.path, encoding: .utf8) {
            expectEqual(text.contains("static let fixedSlotPadding: CGFloat = 18"), true,
            ...
            expectEqual(lyricsUses, 4,
                        "内边距: 歌词态四处(text 申请 / fixed 申请 / 两处反推)同源,实际 \(lyricsUses)")
            let iconUses = text.components(separatedBy: "Self.fixedSlotPadding").count - 1
            expectEqual(iconUses, 1, "内边距: 图标态只剩那一处申请,实际 \(iconUses)")
        } else {
            expectEqual(true, false, "内边距: 读不到 MenuBarStatusItem.swift(路径挪了?)")
        }
    }
    ...
    do {
        ...
        expectEqual(pingpong.width(target: 106.1, trackKey: "S"), 113.1,
                    "地板: 实测那个 106→113→106 的来回(差 7),第三步不再改宽")
        // 另一次实测的抖动幅度是 17pt,同样要被吸收。
        var pp2 = F()
        _ = pp2.width(target: 96.0, trackKey: "S2")
        _ = pp2.width(target: 113.0, trackKey: "S2")
        expectEqual(pp2.width(target: 96.0, trackKey: "S2"), 113.0, "地板: 17pt 的来回也吸收")
        expectEqual(F.shrinkTolerance, 30, "地板: 容差 30pt ≈ 两个汉字(取绝对值不取百分比)")
        ...
        expectEqual(r.didResetOnLastCall, false, "地板: 同曲内涨不算重置")
        ...
        // 这一条随容差一起改了口径:目标缩到图标宽(150 → 38.5,差 111)**远超容差**,
        ...
        expectEqual(gap.width(target: 38.5, trackKey: "T"), 38.5,
                    "地板: 目标远小于地板时照缩(间隙期间的保持改由 slotReleaseSecs 负责)")
        ...
        let item = URL(fileURLWithPath: #filePath)
            ...
            .appendingPathComponent("Sources/lyrimuse/MenuBar/MenuBarStatusItem.swift")
        if let text = try? String(contentsOfFile: item.path, encoding: .utf8) {
            expectEqual(text.contains("slotFloor.width("), true, "地板: 自适应分支确实走了它")
            expectEqual(text.contains("trackKey: coordinator.title"), true,
                        "地板: 用歌名+歌手当 trackKey,换歌才重置")
            let hits = text.components(separatedBy: "slotFloor.width(").count - 1
            expectEqual(hits, 1, "地板: 只有一处调用(.fixed 那条不许也套上),实际 \(hits)")
        } else {
            expectEqual(true, false, "地板: 读不到 MenuBarStatusItem.swift(路径挪了?)")
        }
    }

    // ---- 菜单栏双排(副行，用户选 B 方案:主 10pt / 副 9pt)----
    //
