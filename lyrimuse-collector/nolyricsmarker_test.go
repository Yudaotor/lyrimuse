...
// "曲库里有这首歌、但平台上没有歌词文本"这条信号的守卫(2026-09-15)。
//
// 起因是用户报「为什么这首歌搜不到歌词」(Iris / OLORUNNS):网易云和 QQ 都精准命中了
// 曲目(歌名/歌手/专辑/时长四项全中),但两家都没有歌词文本,而弹窗显示的是跟"十个源
// 都没搜到这首歌"一模一样的那句话。完整设计见 scoredLyricCandidateResult.TrackFoundNoLyrics。
//
// 这里钉死的是**判据的边界**——开发时第一版判据写成 `lrc == ""`,对着真实的那首歌跑才
// 发现网易云回的是两行署名占位、一条都认不出来。下面第一个用例就是那次实测的原文。
...
func TestNoLyricsVerdictOnRealWorldBodies(t *testing.T) {
	cases := []struct {
		...
		{
			// 2026-09-15 实测原文(网易云 id=3406605267,Iris / OLORUNNS)。**没有词的歌
