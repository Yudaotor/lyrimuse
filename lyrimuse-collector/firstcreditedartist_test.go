...
func TestFirstCreditedArtistSlashInName(t *testing.T) {
	cases := []struct{ in, want, why string }{
		...
		// 中文"和"夹在两个拉丁字母段之间时当合唱连接词处理(见
		// normalizeArtistCreditHanAnd 头注)。
		...
		// 两侧都是中文、且各自≥2字时也当合唱连接词处理(见
		// normalizeArtistCreditHanAnd 头注)——不拆开时"陶喆和盧廣仲"恒取不出首歌手,
		// 七个源全部搜不到;单独查"陶喆"四个源立刻命中。
