...
// 例如陶喆和盧廣仲《那個女孩》这首本地匹配不到的歌,"换个身份再搜"+
...
func TestNeteaseThrottle(t *testing.T) {
	...
	// 这一条改过语义。**原来断言的是「被标记退避的端点应该等到退避期满」**
...
//
// 25 小时日志的分布(见 neteaseBlockCooldownBase 头注):171 次拒绝有 89 次
...
func TestNeteaseBlockBackoff(t *testing.T) {
	...
	// 这一条守的是一个会让退避**整个失效**的坑:time.Duration 是 int64,base 左移几十位
...
// 这条断言存在的意义是防"顺手改回去":两个常量必须是不同的桶(否则兜底形同虚设),
