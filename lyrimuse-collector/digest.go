...
// 2026-07-22:每周/每日听歌报告最初分别写死用 Last.fm(周榜接口)和 ListenBrainz(逐条
// 收听记录)两套完全独立的实现——后来发现 Last.fm 的 user.getWeeklyTrackChart/
// getWeeklyArtistChart 其实接受任意自定义 from/to(不是只认它自己的官方周边界，实测
// 坐实过)，用户因此要求"每周/每日都能自己选用哪个账号的数据，默认是已经配置好的那个，
// 两个都配了就用 Last.fm，都没配就提示需要先配置"。这个文件是两个 cadence(周/日)
// 共用的部分：统一的统计结果形状、按数据源分派的两条取数路径、统一的推送文案拼装、
// 以及默认数据源的判定逻辑。weekly.go/daily.go 各自只保留"什么时候算一个新周期已经
// 收官、该不该检查"这部分跟周期长度强相关、没法共用的逻辑。
