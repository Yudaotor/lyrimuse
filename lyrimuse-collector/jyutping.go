...
// 多音字坐实这是主读音,如"雅到ngaa5"/"中到zung1"/"和到wo4"/"我到ngo5"/"唔到m4"/
// "係到hai6"/"嘅到ge3");都带权重时取权重最高的那条;再打平就按文件原始出现顺序取
...
// 表现是「离」这类字读错:简体字里有 71 个字,本身**同时**是
...
// 直接返回,根本轮不到下面"转一次繁体再查"那条兜底,"離到lei4"这条正确读音永远用不上。
//
// 排查坐实这不是"离"一个字的偶发情况:系统性比对字表和 OpenCC 简到繁映射,发现有
...
//
// "复/干/并"这 3 个字不进这份覆盖表:这三个字不是"离"那种"只有一个
// 繁体身份、另一个是生僻字"的情况,是简体归并了**两个读音互不相同**的独立繁体字
// (复到復 fuk6/複 fuk1/覆 fuk1;干到幹 gon3/乾 gon1;并到並 bing6/併 ping3),没有
// 客观上更对的默认值(举例:林家谦《隔离》"回复掣 无情被废"里的"复"该读覆/fuk1,
// 选"復"的 fuk6(恢复义)就错了)。判据可编程复核:同一简体字的
...
// 这个函数自己的契约"查不到读音的字符原样保留"(review 坐实:
...
func toJyutpingLine(text string) string {
	...
	// 最后写出去的那一段是不是查出来的读音。
	//
	// 修:这里原来只用一个 needSpace,并在写出"查不到读音的字符"之后按
	// `needSpace = !(unicode.IsLetter(r) || unicode.IsDigit(r))` 决定下一段要不要空格。
	// 两处错:
	//  1. 它只看**当前**字符是不是字母/数字,没看下一个是什么 —— 意图是"连续拉丁串
	//     别被拆成 b a b y",实际把"拉丁到汉字"这个边界也判成不需要空格,输出成
	//     "babyngo5 oi3 nei5"、"OKlaa1"、"Do re midong1"(对拍坐实,
