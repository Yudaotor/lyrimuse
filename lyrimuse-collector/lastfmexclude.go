...
//
// 设置 到 账号 到 Last.fm 到 设置 到「Scrobble 的播放器」取消勾选的写进 features.json 的 lastfm_excluded_bundles
...
// 见 12 章决策 9):挡的是 track.scrobble 的镜像(recordLastfmListen 到 settleLastfmPending)、给它兜底的本地收听日志与
...
// 只热读**这一个键**,不是整份 featureFlags:别的键(播放器集合、歌词源顺序、lyricsDir())在启动时就被展开进
// 包级变量 / 决定了读取路径,中途换掉是没人验证过的行为。
//
// **这个模块如今是冗余的**:features() 已经整体按 mtime 热重读(featuresreload.go),改任何键都不必
// 重启。留着不删的理由同 lyricsourcesreload.go —— 消费点还走 currentLastfmExcludedBundles(),
// 读的是同一个文件、结论一致。
// 判定仍在**开会话那一拍**取一次(playSession.lastfmExcluded 不变):一首歌播到一半改设置不该把同一次收听
// 切成两半。区别只是"下一首就生效"而不再是"等 collector 重启完(约 40 秒)才生效"。
//
// 粒度是 bundle:collector 侧没有网页平台身份(平台 与 浏览器配对只在 App 侧),配对的浏览器只能整个开或关。
...
// resolveLastfmExcludedBundles 把文件里的列表清洗成集合:去首尾空白、丢空串、去重。nil / 空 到 空 map
