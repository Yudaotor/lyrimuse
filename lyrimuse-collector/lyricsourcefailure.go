...
// 分别见 ytmusic.go/musixmatch.go/netease.go 头注),
// 两处消费:设置页"歌词来源"卡片的测试按钮(testlyricsourcescli.go)、"联网搜索候选歌词"
// 弹窗的"歌词源可用情况"明细(searchcli.go 的 lyricSourceFailureReasons)。
//
// 从**自然语言文案**改成**稳定代码**:xxxSetLastFailureReason 原来写的是硬编码
// 中文句子,经共享 JSON 原样传到 Swift 侧直接显示,绕开了这个仓库其它地方统一走的
// L10n.t() 本地化机制。collector 只负责识别"是哪一种
...
const (
	...
	// 这里曾经有过一个 deezer_region_restricted,是**误判**:当时走的 gw-light.php
	...
	//
	// applemusic_not_connected 跟其余所有代码**不是一类**:它不代表"源坏了",而是
...
const (
	...
	// *net.DNSError(秒答的 NXDOMAIN / SERVFAIL)。见 sourcebreaker.go 最后一节的 段。
