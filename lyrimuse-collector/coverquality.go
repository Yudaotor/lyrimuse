...
// ## 要修的是什么
//
// 现象是「这首歌这里的封面这么糊清晰度很低」(Bruno Mars 的《24K Magic》)。查出来的形态
...
// 于是:
//
//	设备图边长 >= deviceCoverTrustedMinEdge          到 直接顶掉(快路径,不取图不比较)
//	没有远程候选                                     到 直接顶掉(没有更好的选择)
//	远程候选不比设备图大                             到 直接顶掉(换了也没好处)
//	两张图**感知上是同一张**                         到 **保留远程那份高清**(既对版又清晰)
//	两张图不一样                                     到 顶掉(身份优先,认这个糊 —— Immortal 那档)
//
...
//
// 这个值是**拿真实数据校准出来的**,不是拍脑袋:
//   - 正例:用户那张 120×120 的设备封面 vs 同专辑邻居的网易云 800×800(同一张图)
//     到 距离 **0**(指纹逐位相同,尽管一张 120px、另一张被 loadCoverImage 降到 64px ——
//     这也正是 coverFingerprint 必须用箱式取平均、不能取最近邻的实证)
//   - 反例:缓存里随机抽的 8 张真实的**其它专辑**封面 到 距离 **17 … 39**,无一误判
//
// 两档之间有巨大的空隙(0 与 17),10 落在空隙里、离正例 10、离最近的反例 7,不是卡在某个
// 样本边上的脆弱阈值。
...
//
// 为什么不直接下载下来量:`loadCoverImage` 是给取色用的,**返回的远程图已经降采样**
...
func deviceCoverDecision(
	...
	if candidateEdge <= 0 {
		// 认不出候选的目标尺寸 到 证不出它更好 到 保守保留设备封面(退回改动前的行为)。
...
//
// 设备封面文件缺失/解不出来时这里返回 true(允许换成远程候选)——那种情况下本地这张
