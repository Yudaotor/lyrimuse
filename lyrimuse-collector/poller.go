...
//     出来。
//
// ⚠️ 应用层错误(lastfmAPIError)**不能**一律当成"拒收":
...
func (p *poller) updatePosition(now time.Time) (reanchor bool, loopRestart bool) {
	...
	if sameTrackAsBefore && !p.snapshotStale && p.posBias != 0 && p.cur.AnchorElapsed > 0.001 {
		// 偏置只属于**开播那个** MediaRemote 锚点(⚠️ 别让它在"暂停⇄恢复"之间继承
	...
	case !p.prevPlaying: // 暂停→恢复(同曲):偏置继承,冻结值扣偏置就是恢复点
		...
		// 且与 App 侧"暂停⇄恢复继承偏置"的语义相反(对抗审查抓出,high)。
...
func (p *poller) announce(now time.Time, why string) {
	...
	go func() {
		...
		// 切换、挂起首条到点补发。实测坐实危害:collector 一重启就把一首
...
func (p *poller) bridge(now time.Time) {
	...
	//
	// ⚠️ 当场拉的代价不止"这一次白拉"(用户**第三次**报「补提交之后下面的列表没
