...
// 本地收听日志 —— **不依赖任何账号**地把每一次"算数的收听"追加到磁盘。
//
...
// 也补不回来 —— 不是"存了没发",是压根没存。这个文件就是补上这一环:先无条件记下来,
// 等账号连上之后再决定要不要补提交(见 §回填,那部分单独实现)。
//
...
type listenLogLine struct {
	...
	UTS int64 `json:"uts"`
	// 艺人名:存**播放器报的原始标签**,不是 canonical_artist、也不是 collapse 折叠后的值。
	// 因为它是 lastfmCollapse.resolve 的**输入**(lastfm.go 里 scrobble 前会先 resolve),
	// 回填时必须喂同一个输入才能得到跟当初一致的输出。
	AR string `json:"ar"`
	TI string `json:"ti"`
	...
	//
	// 取值就是写入那一刻 `p.lfm != nil`。有一个刻意接受的偏差:mirrorScrobbleTracked 不看
	// 凭据是否已被 Last.fm 判死(它会标记 lfmMirrored 却发不出请求),所以凭据死掉那段时间
	// 的收听会被记成 M=1、从此不再回填 —— 漏补一条,而不是重复提交一条。跟 lfmMirrored
	// 自身"记的是尝试过、不是确实到了"的语义保持一致。
...
// appendListen 追加一条收听。**任何账号都没配也要照样写** —— 这是这个文件存在的全部意义。
//
// 失败只记日志、不影响调用方:这条记录是为了将来能补提交,写不进去顶多是少补一首,
// 不该让它影响当下这次播放的其它处理。
// ⚠️ 调用方必须只在**当前没有在往 Last.fm 提交**时才调它(p.lfm == nil)。
//
