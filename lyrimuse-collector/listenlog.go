...
//
// 当前的写入条件(两条,满足其一才写,别照标题想当然):
//
//  1. 没连 Last.fm(`p.lfm == nil`)—— 常规路径,见下面那段收窄。
//  2. 连了 Last.fm 但**这一条确定没写进去**(镜像失败,见 poller.go 的
//     recordFailedMirror)。
//
// 换句话说:一个一直连着账号、且网络一直正常的用户,这份日志会是**空的**,那是对的。
//
// 为什么需要:之前,一次完成的收听只会流向三个地方(applySubmitOutcome):
...
//
// 它记不回**本功能上线之前**的收听。那些数据不存在,任何"补回来"的说法都是假的。
//
...
type listenLogLine struct {
	...
	// 艺人名:存**播放器报的原始标签**。
	//
	// 这条更简单了 —— 上送本身就是原样发原始标签(见 lb.go 的 lbMeta),
	...
	//
	// 靠 dry-run 抓到:少了这个字段,一个**已经连着** Last.fm 的用户,日志里每
	...
	//
	// **现在已经不再写入这个字段了**(appendListen 压根不设 M,
	// 全仓没有任何写点;只有 backfill.go 还在**读**)。盘上所有 m=1 都是老版本留下的。
	//
	// 凭据判死那个窗口现在由 recordFailedMirror 兜住了:判死时照样留痕,
...
//
// 数量级参考:一个每天听 40 首的重度用户一年约 1.5 万行,一行 ~110 字节 到 1.6 MB。
// 上限定在 4 万行(约 4.4 MB、两年半)——远超"先用一阵再连账号"这个场景需要的跨度,
// 而回填本身只关心最近这段没提交的部分。
//
// 压缩是整文件重写(读全部 到 留后 N 行 到 tmp+rename),所以刻意只在**启动时**做一次,
...
// 调用方只有两种情况该调它:
//
//  1. **当前没有在往 Last.fm 提交**(p.lfm == nil)—— 常规路径,见下面那段。
//  2. **提交了但确定没写进去**(recordFailedMirror)—— 镜像失败时补一条,
//     否则这次收听三处都不留痕、永久消失(理由见 recordFailedMirror 的注释)。这一条
//     不破坏下面"不写死数据"的初衷:只有真失败才写,成功路径依旧一行不写。
//
// 并发:第 2 种情况是从 mirrorAsync 的 goroutine 调进来的,所以这个函数(经
