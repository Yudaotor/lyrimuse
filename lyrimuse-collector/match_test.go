...
// 固定样本回归测试——覆盖网易云"纯音乐曲目返回一份看着正常的
// 完整制作人员名单、每行都带真实时间戳"这个坑:isTimedLRC 会认为它是可用的逐行 LRC
// (行数够、时间戳密度够),而指挥/贝斯/中提琴/吉他/大提琴/母带工程师这类角色名不在
// creditLineRe 的词表里就会漏判,导致一份纯制作人员名单被当成真歌词打分、进而可能被
// 选中显示。样本里的角色名/人名全是
...
// 实测排查坐实的真实 bug 的回归测试:同一首歌的不同录音(demo/original
...
func TestTitleVersionTags(t *testing.T) {
	cases := []struct {
		...
		// 假阳性陷阱:限定词只在括号/破折号段里找,不能对整个歌名做子串匹配
		...
		// 补中文版本限定词(举例:「蜗牛 (伴奏)」查 lyricfind,词表原来
		// 只认拉丁词,伴奏版被当成正常版收了)。
		// 中文词折成英文规范键进集合(见 versionTagAliases):「(现场)」跟「(Live)」
		// 是同一个版本声明,不能是两个键。
		...
		{"不插电的夏天", nil},
		// 补裸 "edit"(PRINCE《Diamonds and Pearls (2023 Remaster)》配了《… (Edit)》的词,
		...
		// "edit" 只按整词匹配:edition 不是 edit,Deluxe Edition 这一大类专辑/曲名不能被误伤。
...
func TestVersionTagsMismatchFoldsBilingualSynonyms(t *testing.T) {
	cases := []struct {
		...
		// 折键只合并同义词,不放松判定:本地正式版对候选「(现场)」照罚,「(Live)」对「(伴奏)」照罚。
	...
	// 每个别名都必须指向词表里真实存在的规范键,且规范键自己不再有别名(不许链式)。
	...
	// 打分层:同案的酷狗候选不再吃 versionTags,标题精确同名 120 也保住。
	lyr := "[00:10.00]占位第一句\n[00:12.00]占位第二句\n[00:14.00]占位第三句\n[04:00.00]占位末句"
...
// ---- v17:album version 出词表 / single version 的命名不对称可豁免 ----
//
// 起因是一次面向数据的普查(见 09 章第 51 条):全库 632 条现存 versionTags 罚分按"差在哪个键"
// 聚合、拿候选自报时长当地面真相。album version 一侧独有的 49 条里时长差 >12% 的 0 条(对照组
// edit 是 10/12),说明它没在区分录音;single version 是 12/56,说明它在区分,但沉默的一侧只是
// "这个源没做区分",所以留在词表、靠时长+专辑两道硬证据豁免。
func TestDefaultVersionTagSemantics(t *testing.T) {
	// ---- album version 已不算版本限定词(跟 remaster/deluxe 同类)----
	...
	// 真实数据(Chaka Khan & Rufus《Everybody Has An Aura (Album Version)》对酷狗裸标题,
	// 同专辑、229.2s vs 227s):不再判版本不符,且标题吻合回到精确档 120。
	...
	// 但它不该顺带放过**真的**另一版:album version 对 single version / edit / live 照罚。
	...
	// ---- single version:留在词表,只在时长逐位吻合 + 专辑亲和时豁免 ----
	...
	// 方向不对称是有意的:acoustic 家族只在"候选多出"那一侧豁免。本地标了 acoustic 而
	// 候选没标时,候选可能就是普通录音室版,而不插电版跟原版时长常常差不多 —— 时长替它作保不住。
	...
	// 白名单里的键必须真在词表里(不然是死配置);两张白名单不许把 live/demo 这类放进来。
	inVocab := map[string]bool{}
	for _, tag := range distinctRecordingVersionTags {
	...
	// 打分层端到端:同案 QQ 候选不吃 versionTags(标题仍是剥括号档 60,single version 还在词表)。
...
// 《Will You Be There (Immortal Version)》—— 全库回放里这条是唯一受影响的候选,-600 从此
// 正确落在它头上(它确实是 1992 年那场演唱会的录音,时间轴跟 Immortal 版对不上)。
func TestQualifierDeclaresCJKLive(t *testing.T) {
	segCases := []struct {
		...
		{"爱的初体验 (音乐会)", true},
		{"X - 2004演唱会", true}, // 破折号尾段同样算(口径同 titleQualifierSegments)
		// 中心词不是现场标记 到 不声明。「(演唱会主题曲)」是录音室曲目;第二条是 v9 当年
		// 刻意让专辑推导只看 stripParens 之后的那条真实误伤(韦礼安《女孩》的 lrclib 专辑名)。
		{"晴天 (演唱会主题曲)", false},
		{"女孩 (2015 韦礼安 《放开那女孩》 小巨蛋演唱会求爱主题曲/电视剧插曲)", false},
		// 括号外的字一律不看(限定词只在限定段里找,跟 titleVersionTags 同一条纪律)
		{"我的滑板鞋演唱会", false},
		{"", false},
	}
	...
	// 进 recordingVersionTags 之后的效果:声明了 live,且不影响别的键。
	...
	// 判决:同一场演出的两种写法不再互扣;录音室本地对现场候选照罚。
	...
	// 身份词那条判据的口径**刻意更宽**(拉丁 (Live In Montreal) 的场馆词也要算进身份):
	// 声明 live 是 ±600 的判决、要保守;"这一段里哪些词是场次名"只在已经确定是现场之后才问。
	if got := liveIdentityTokens("Queen", "Save Me (Live In Montreal / November 1981)", ""); !got["montreal"] {
		t.Errorf("拉丁现场段的场馆词必须算进身份词,实际 %v", got)
	}
}

// ---- v7:liveAlbumIdentityConflict(「两场不同演唱会」判据) ----
//
...
func TestLiveAlbumIdentityConflict(t *testing.T) {
	cases := []struct {
		...
		// 真实数据:酷狗对现场版常不给专辑、把场次写进曲名括号。本地是地表最强巡演的现场专辑,
		// 候选是 2018 年 CCTV 音乐频道节目现场 —— 另一场演出,v16 前因候选专辑为空直接放行。
...
// ---- v8:sameRecordingDespiteVersionTags(时长锚定的限定词豁免) ----
//
...
func TestVersionTagsMismatch(t *testing.T) {
	cases := []struct {
		...
		// 举例:本地「蜗牛 (伴奏)」,lyricfind 召回的是正常演唱版
		...
		// PRINCE 的 "1999" 是这类形状的例子:候选歌名干干净净就叫 "1999",Live 只出现在
		...
		// 已知缺口,固化在这里:限定词在专辑名里**裸着**出现(没有括号、也不在
...
// lyricTitleAccepted 是五个源共用的、**唯一**一条曲名判定(由三个函数合并
...
func TestLyricTitleAccepted(t *testing.T) {
	cases := []struct {
		...
		// 丁世光《起源》是这类形状的例子:QQ/酷狗都叫「起源 Origin」,原规则下五源只剩两条候选
...
// ---- v3 维度的单测。分值背书见 lyricsScoringVersion 注释的消融数据 ----
...
func TestTitleMatchTierPoints(t *testing.T) {
	cases := []struct {
		...
		{"候选没报标题", "", "Song", 0},
		// 补:中文限定词在 segmentVersionTags 里没有空格分词,"伴奏版"整段
...
// 独立印证 —— 见 lyricSourceConsensusFamily 的头注。接 deezer 时的真实缺口:
...
func TestContentConsensusPeers(t *testing.T) {
	...
	// 名单不只是计数,得**指名道姓**指向对方 —— 决策留痕要靠它回答
	// "冠亚军这两份是不是同一份词",只有个数答不了。
...
// ---- v5:applyWordTimingTitleOverride ----
//
...
// 方大同《南音》是这类形状的例子:本地标签(Apple Music)罗马化写成
// artist="Khalil Fong" title="Nanyin",两者都不含汉字——不补 candidateArtist 这层信息的话,
// 会把正确候选判成"语言跟这首歌对不上"而拒绝采用。candidateArtist(候选源自己确认匹配到的
// 歌手,已经过前置的歌手身份闸,不是瞎猜)含汉字就说明这首歌本来就该有中文候选,不是
// "上传者把翻译当原文传错了"。
...
// 举例:那英《微笑着离去》本地标签罗马化成 artist="Na Ying" title=
...
// TestAlbumTokensLatinCJKBoundary 钉死 v9 的拉丁与CJK 交界分词(周杰伦《The One》案:
